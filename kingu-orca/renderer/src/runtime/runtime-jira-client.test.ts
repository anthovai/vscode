...
describe('runtime Jira client search bounds', () => {
  ...
  it('routes isolated status and summary reads through local and paired-runtime owners', async () => {
    ...
    jiraLookupIssueSummaryLocal.mockResolvedValue({ key: 'KINGU-1' })
    ...
    await expect(jiraLookupIssueSummary(localContext, 'KINGU-1', 'site-1')).resolves.toMatchObject({
      key: 'KINGU-1'
    })
    await expect(jiraReadStatus(runtimeContext)).resolves.toMatchObject({ connected: true })
    await expect(
      jiraLookupIssueSummary(runtimeContext, 'KINGU-1', 'site-1')
    ).resolves.toMatchObject({ key: 'KINGU-1' })
    ...
    expect(runtimeCall).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        params: { key: 'KINGU-1', siteId: 'site-1' },
        selector: 'env-1'
      })
    )
  })
  ...
  it('cancels a superseded local Jira summary read', async () => {
    ...
    const lookup = jiraLookupIssueSummary(context, 'KINGU-1', 'site-1', controller.signal)
    controller.abort()
  ...
  it('keeps ordinary remote Jira creation compatible with hosts without the user-field capability', async () => {
    runtimeCall.mockImplementation(async (args: { method: string }) => {
      ...
      return {
        id: 'rpc-1',
        ok: true,
        result: { ok: true, id: 'issue-1', key: 'KINGU-1', url: 'https://jira.example/KINGU-1' },
        _meta: { runtimeId: 'remote-runtime' }
      }
    })
    ...
    ).resolves.toMatchObject({ ok: true, key: 'KINGU-1' })
    expect(runtimeCall).toHaveBeenCalledWith(
  ...
  it('streams image-bearing issue and comment payloads from remote runtimes', async () => {
      ...
      async (args: RuntimeSubscribeArgs, callbacks: RuntimeSubscribeCallbacks) => {
        const payload =
          args.method === 'jira.getIssueStream'
            ? { key: 'KINGU-1', description: '![shot](data:image/png;base64,abc)' }
    ...
    await expect(
      jiraGetIssue({ activeRuntimeEnvironmentId: 'env-1' }, 'KINGU-1', 'site-1')
    ).resolves.toMatchObject({ key: 'KINGU-1' })
    await expect(
      jiraIssueComments({ activeRuntimeEnvironmentId: 'env-1' }, 'KINGU-1', 'site-1')
