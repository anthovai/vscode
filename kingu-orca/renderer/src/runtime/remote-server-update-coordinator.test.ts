...
const INSTALL_FAILURE =
  "Could not start the update installer. Kingu remains open. (Command failed: pkexec dpkg -i '/home/u/.cache/kingu-updater/pending/kingu-ide_1.5.0_amd64.deb' pkexec must be setuid root)"
...
describe('remote server update execution', () => {
  ...
  it('fails when no offered update reaches the requested version', async () => {
    ...
    expect(result).toMatchObject({
      phase: 'failed',
      error: 'The server updater did not offer the requested Kingu version.'
    })
  })
