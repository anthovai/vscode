...
describe('useJiraUrlSource', () => {
  ...
  it('requires an explicit account choice and binds the exact account context', async () => {
    ...
    mocks.lookupJiraIssueSummary.mockResolvedValue(issue('KINGU-123', 'site-b'))
    const { result } = renderHook(() =>
      useJiraUrlSource({
        value: 'https://company.atlassian.net/browse/KINGU-123',
        enabled: true,
    ...
    expect(result.current.issue?.key).toBe('KINGU-123')
  ...
  it('reuses an already-loaded connection instead of re-reading status', async () => {
    ...
    mocks.lookupJiraIssueSummary.mockResolvedValue(issue('KINGU-1'))
    ...
    const { result } = renderHook(() =>
      useJiraUrlSource({
        value: 'https://company.atlassian.net/browse/KINGU-1',
        enabled: true,
        sourceContext: context,
        connection: loaded
      })
    ...
    expect(result.current.issue?.key).toBe('KINGU-1')
  ...
  it('discards a late issue response after the URL changes', async () => {
    ...
    mocks.lookupJiraIssueSummary.mockImplementation(
      (
        ...
        return Promise.resolve(issue('KINGU-2'))
      }
    )
    const { result, rerender } = renderHook(
      ...
      { initialProps: { value: 'https://company.atlassian.net/browse/KINGU-1' } }
    )
    await advanceLookup()

    rerender({ value: 'https://company.atlassian.net/browse/KINGU-2' })
    await advanceLookup()
    expect(oldSignal?.aborted).toBe(true)
    expect(result.current.issue?.key).toBe('KINGU-2')

    await act(async () => {
      oldIssue.resolve(issue('KINGU-1'))
  ...
  it('forces Retry past an invalid fulfilled summary result', async () => {
    const context = sourceContext()
    mocks.readJiraStatus.mockResolvedValue(status([site('site-a')], 'site-a'))
    mocks.lookupJiraIssueSummary.mockResolvedValueOnce(null).mockResolvedValueOnce(issue('KINGU-1'))
    const { result } = renderHook(() =>
      useJiraUrlSource({
        value: 'https://company.atlassian.net/browse/KINGU-1',
        enabled: true,
    ...
    expect(result.current.issue?.key).toBe('KINGU-1')
    expect(mocks.lookupJiraIssueSummary).toHaveBeenLastCalledWith(context, 'KINGU-1', 'site-a', {
      force: true,
