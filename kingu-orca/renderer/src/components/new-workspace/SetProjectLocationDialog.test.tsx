...
vi.mock('@/components/sidebar/RemoteFileBrowser', () => ({
  RemoteFileBrowser: ({
    ...
    <div data-testid="remote-file-browser" data-target={targetId ?? ''}>
      <button type="button" onClick={() => onSelect('/remote/kingu')}>
...
function renderDialog(
  ...
  render(
    <SetProjectLocationDialog
      ...
      projectName="kingu"
      projectKind="git"
      defaultCloneUrl="git@github.com:anthovai/kingu-intelligence.git"
      onClose={vi.fn()}
      onReady={vi.fn()}
      {...overrides}
    />
...
describe('SetProjectLocationDialog', () => {
  it('keeps the parent caller in charge of close and saves an existing folder', async () => {
    ...
    expect(screen.getByText('Choose where kingu lives on openclaw.')).toBeTruthy()
  ...
  it('clones onto the selected host', async () => {
    ...
    expect(storeMocks.setupProjectClone).toHaveBeenCalledWith({
      projectId: 'project-kingu',
      hostId: 'ssh:openclaw',
      url: 'git@github.com:anthovai/kingu-intelligence.git',
      destination: '/remote/kingu',
      displayName: 'kingu'
    })
  ...
  it('does not retarget the composer when a slow clone lands after dismissal', async () => {
    ...
    const { rerender } = render(
      <SetProjectLocationDialog
        ...
        projectName="kingu"
        projectKind="git"
        defaultCloneUrl="https://github.com/anthovai/kingu-intelligence.git"
        onClose={onClose}
        onReady={onReady}
      />
    )
    ...
    await user.click(screen.getByRole('button', { name: /Clone from URL/ }))
    await user.type(screen.getByPlaceholderText('/destination/on/host'), '/remote/kingu')
    await user.click(screen.getByRole('button', { name: 'Clone' }))
  ...
  it('still reports success under StrictMode double-invoked effects', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /Browse folder/ }))
    await user.type(screen.getByPlaceholderText('/path/to/project/on/host'), '/remote/kingu')
    await user.click(screen.getByRole('button', { name: 'Set location' }))
