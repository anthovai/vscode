...
const baseSession: AiVaultSession = {
  ...
  resumeCommand: "cd '/Users/ada/kingu' && claude --resume 'session-1'",
  subagent: null
}
...
describe('buildAiVaultProjectContext', () => {
  it('uses durable worktree project ids before repo fallback', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Legacy Repo', path: '/Users/ada/kingu' })
    const project = makeProject({ id: 'project-1', displayName: 'Canonical Kingu' })
    const worktree = makeWorktree({
      id: 'wt-1',
      repoId: repo.id,
      projectId: project.id,
      path: '/Users/ada/kingu'
    })
    ...
    expect(context.sessionProjectById.get(baseSession.id)).toMatchObject({
      ...
      label: 'Canonical Kingu'
    })
  })

  it('normalizes compatibility project ids to repo keys', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Kingu', path: '/Users/ada/kingu' })
    const worktree = makeWorktree({
      id: 'wt-1',
      repoId: repo.id,
    ...
    const context = buildAiVaultProjectContext({
      ...
      worktrees: [worktree],
      projectHostSetupProjection: makeProjection({
        projects: [makeProject({ id: 'repo:repo-1', displayName: 'Compatibility Kingu' })],
    ...
    expect(context.projectLabelByKey.get('repo:repo-1')).toBe('Kingu')
  })
  ...
  it('inherits setup project ids for legacy worktrees without project metadata', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Kingu Repo', path: '/repo/kingu' })
    const worktree = makeWorktree({
      id: 'wt-legacy',
      repoId: repo.id,
      path: '/repo/kingu'
    })
    const session = makeSession({ id: 'claude:legacy-worktree', cwd: '/repo/kingu/src' })
    ...
    const context = buildAiVaultProjectContext({
      ...
      projectHostSetupProjection: makeProjection({
        projects: [
          makeProject({ id: 'github:anthovai/kingu-intelligence', displayName: 'Canonical Kingu' })
        ],
        setups: [
          makeSetup({
            repoId: repo.id,
    ...
    expect(context.activeProjectKey).toBe('project:github:anthovai/kingu-intelligence')
    expect(context.sessionProjectById.get(session.id)).toMatchObject({
      kind: 'repo',
      key: 'project:github:anthovai/kingu-intelligence',
      label: 'Canonical Kingu'
    })
  })
  ...
  it('uses active worktree setup project ids when active repo is unavailable', () => {
    ...
    const session = makeSession({ id: 'claude:restored', cwd: '/repo/kingu/src' })
    ...
    expect(context.sessionProjectById.get(session.id)?.key).toBe(
      'project:github:anthovai/kingu-intelligence'
    )
  })

  it('inherits setup host ids for legacy worktrees without host metadata', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Runtime Repo', path: '/runtime/kingu' })
    const worktree = makeWorktree({
      id: 'wt-runtime',
      repoId: repo.id,
      path: '/runtime/kingu'
  ...
  it('uses the session host when matching overlapping local and SSH project paths', () => {
    const localRepo = makeRepo({ id: 'local', displayName: 'Local', path: '/srv/kingu' })
  ...
  it('falls back to folder when a legacy hostless session matches multiple host buckets', () => {
    const localRepo = makeRepo({ id: 'local', displayName: 'Local', path: '/srv/kingu' })
    const runtimeRepo = makeRepo({ id: 'runtime', displayName: 'Runtime', path: '/srv/kingu' })
    const session = makeSession({
  ...
  it('maps null cwd sessions to unknown', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Kingu', path: '/repo' })
