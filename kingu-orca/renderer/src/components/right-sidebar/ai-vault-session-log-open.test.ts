...
import { openAiVaultSessionLogInKingu } from './ai-vault-session-log-open'
...
describe('openAiVaultSessionLogInKingu', () => {
  it('authorizes the exact path and opens a permanent read-only local tab', async () => {
    const state = makeState()
    getStateMock.mockReturnValue(state)

    await openAiVaultSessionLogInKingu({ filePath: LOG_PATH, executionHostId: 'local' })
  ...
  it('withholds blank, remote, and synthetic paths without authorizing', async () => {
    ...
    await openAiVaultSessionLogInKingu({ filePath: '   ', executionHostId: 'local' })
    await openAiVaultSessionLogInKingu({ filePath: LOG_PATH, executionHostId: 'ssh:dev-box' })
    await openAiVaultSessionLogInKingu({
  ...
  it('shares one in-flight open for concurrent clicks of the same path', async () => {
    ...
    const first = openAiVaultSessionLogInKingu({ filePath: LOG_PATH, executionHostId: 'local' })
    const second = openAiVaultSessionLogInKingu({ filePath: LOG_PATH, executionHostId: 'local' })
    resolveAuth?.()
