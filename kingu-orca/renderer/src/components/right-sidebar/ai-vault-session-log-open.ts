...
import { canOpenAiVaultSessionLogInKingu } from './ai-vault-session-path-actions'
...
/**
 * Open a local AI Vault session log inside Kingu as a permanent, read-only editor
 * tab (or activate an existing tab without reducing its authority). Reuses
 * Kingu's external-file authorize + `openFile` pipeline; it never grants write
 * capability by itself and never redirects the open to a remote host.
 */
export async function openAiVaultSessionLogInKingu(session: AiVaultLogSession): Promise<void> {
  ...
  if (!filePath || !canOpenAiVaultSessionLogInKingu(session)) {
    return
  }
  ...
  try {
    ...
    try {
      ...
      // trust gesture. Reuses Kingu's existing external R/W open grant.
