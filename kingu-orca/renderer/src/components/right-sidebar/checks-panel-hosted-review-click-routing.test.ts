...
describe('checks panel hosted review modifier hint destination', () => {
  it('names the system browser when a plain click already opens in Kingu', () => {
    expect(resolveChecksPanelHostedReviewModifierDestination({ openLinksInApp: true }, true)).toBe(
      'system-browser'
    )
  })

  // Why: inverting is inert while links already open in Kingu — both meanings of the
  // modifier land on the system browser, so checking inverts first would misname it.
  it('names the system browser when inverting is on and links already open in Kingu', () => {
    expect(
      resolveChecksPanelHostedReviewModifierDestination(
  ...
  // and the only way to reach Kingu from this button is undiscoverable.
  it('names Kingu when inverting is on and links open externally', () => {
    expect(
      resolveChecksPanelHostedReviewModifierDestination(
  ...
  // Why: openHttpLink gates routing to Kingu on a worktree id, so without one the
