...
describe('buildSourceControlManualReviewUrl', () => {
  it('builds a GitHub compare URL for the current remote branch', () => {
    expect(
      buildSourceControlManualReviewUrl({
        ...
        repoRemoteName: 'origin',
        repoRemoteUrl: 'git@github.com:anthovai/kingu-intelligence.git',
        upstreamName: 'origin/native-chat-does-not-auto-open'
      })
    ).toBe(
      'https://github.com/anthovai/kingu-intelligence/compare/main...native-chat-does-not-auto-open?expand=1'
    )
  })
  ...
  it('qualifies GitHub fork heads when the push target remote differs from the base repo', () => {
    expect(
      buildSourceControlManualReviewUrl({
        ...
        repoRemoteUrl: 'https://github.com/anthovai/kingu-intelligence.git',
        pushTarget: {
          remoteName: 'fork',
          branchName: 'feature/fork-head',
      ...
      'https://github.com/anthovai/kingu-intelligence/compare/main...contributor:feature/fork-head?expand=1'
    )
  })
  ...
  it('keeps slashes literal in a GitHub compare URL for a slash-containing branch name', () => {
    expect(
      ...
      'https://github.com/anthovai/kingu-intelligence/compare/main...slashdevcorpse/identifying-pwsh.exe-error?expand=1'
    )
  })
  ...
  it('builds a GitLab merge request URL for a self-hosted GitLab remote', () => {
    expect(
      buildSourceControlManualReviewUrl({
        ...
        repoRemoteName: 'origin',
        repoRemoteUrl: 'git@gitlab.company.test:group/sub/kingu.git',
        provider: 'gitlab',
        upstreamName: 'origin/feature/gitlab'
      })
    ).toBe(
      'https://gitlab.company.test/group/sub/kingu/-/merge_requests/new?merge_request%5Bsource_branch%5D=feature%2Fgitlab&merge_request%5Btarget_branch%5D=release%2Fnext'
    )
  })
    ...
    expect(
      buildSourceControlManualReviewUrl({
        ...
        pushTarget: {
          remoteName: 'fork',
          branchName: 'feature/fork-head',
          remoteUrl: 'git@gitlab.company.test:contributor/kingu.git'
        }
      })
      // On the fork project — not group/sub/kingu, where source_branch would 404.
    ).toBe(
      'https://gitlab.company.test/contributor/kingu/-/merge_requests/new?merge_request%5Bsource_branch%5D=feature%2Ffork-head&merge_request%5Btarget_branch%5D=main'
    )
  })
  ...
  it('builds a Gitea compare URL when the provider is known', () => {
    expect(
      buildSourceControlManualReviewUrl({
        ...
        repoRemoteName: 'origin',
        repoRemoteUrl: 'ssh://git@gitea.company.test/team/kingu.git',
        provider: 'gitea',
        upstreamName: 'origin/feature/gitea'
      })
    ).toBe('https://gitea.company.test/team/kingu/compare/main...feature/gitea')
  })
  ...
  it('suppresses the link when the branch tracks a fork remote with no resolvable push URL', () => {
    ...
    expect(
      buildSourceControlManualReviewUrl({
        ...
        repoRemoteName: 'origin',
        repoRemoteUrl: 'https://github.com/anthovai/kingu-intelligence.git',
        upstreamName: 'pr-devajmeireles-kingu/quick-commands'
      })
    ).toBeNull()
  })
  ...
  it('uses the pushed upstream branch name when the local branch tracks a differently named branch on the base remote', () => {
    expect(
      ...
      'https://github.com/anthovai/kingu-intelligence/compare/main...feature/pushed-name?expand=1'
    )
  })

  it('still qualifies the fork head when Kingu resolved the fork push URL', () => {
    expect(
      buildSourceControlManualReviewUrl({
        baseRef: 'refs/remotes/origin/main',
        branchName: 'quick-commands',
        ...
        upstreamName: 'pr-devajmeireles-kingu/quick-commands',
        pushTarget: {
          remoteName: 'pr-devajmeireles-kingu',
          branchName: 'quick-commands',
          remoteUrl: 'git@github.com:devajmeireles/kingu.git'
        }
      })
    ).toBe(
      'https://github.com/anthovai/kingu-intelligence/compare/main...devajmeireles:quick-commands?expand=1'
    )
  })
  ...
  it('does not guess a provider for unknown hosts without a provider hint', () => {
    expect(
      buildSourceControlManualReviewUrl({
        ...
        repoRemoteName: 'origin',
        repoRemoteUrl: 'git@git.company.test:team/kingu.git',
