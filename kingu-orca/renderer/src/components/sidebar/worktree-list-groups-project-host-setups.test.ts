...
describe('buildRows with pinned worktrees', () => {
  ...
  it('keeps the cross-host project header label when another project section renders', () => {
    ...
    expect(buildHeaders([], [])).toMatchObject([
      { key: 'project:github:anthovai/kingu-intelligence', label: 'Kingu' }
    ])
  ...
  it('orders project identity headers by the manual repo order anchor', () => {
    ...
    const analyticsRepo: Repo = {
      ...
      path: '/tmp/analytics',
      displayName: 'analytics',
      upstream: { owner: 'anthovai', repo: 'analytics' }
    }
    ...
    expect(headers.map((row) => row.key)).toEqual([
      ...
      'project:github:anthovai/analytics'
    ])
  ...
  it('splits same-host checkouts of one project into separate per-setup groups', () => {
    const repoB: Repo = { ...repo, id: 'repo-2', path: '/tmp/kingu-2', displayName: 'kingu-2' }
    const worktreeB: Worktree = {
      ...worktree,
      id: 'wt-2',
      repoId: repoB.id,
      path: '/tmp/kingu-2-feature',
    ...
    expect(headers).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          key: 'project:github:anthovai/kingu-intelligence::setup:repo-1',
          label: 'kingu'
        }),
        expect.objectContaining({
          key: 'project:github:anthovai/kingu-intelligence::setup:repo-2',
          label: 'kingu-2'
        })
      ])
    )
  ...
  it('splits only the surface with duplicate checkouts', () => {
    ...
    const headers = rows.filter((row) => row.type === 'header')
    expect(headers.map((row) => row.key)).toEqual([
      'project:github:anthovai/kingu-intelligence::setup:repo-1',
      'project:github:anthovai/kingu-intelligence::setup:repo-local-b',
      'project:github:anthovai/kingu-intelligence'
    ])
  })
  ...
  it('counts projection twins for one directory as one checkout', () => {
    ...
    const runtimeWorktree: Worktree = {
      ...
      id: 'wt-runtime',
      repoId: runtimeRepo.id,
      path: '/home/alice/kingu-runtime-feature'
    }
  ...
  it('keeps Git hosts grouped when folder setups share the project identity', () => {
    ...
    expect(new Set(groupKeys)).toEqual(new Set(['project:github:anthovai/kingu-intelligence']))
  })
  ...
  it('keeps a provisioned runtime copy under the project header alongside a same-host checkout', () => {
    const runtimeRepoB: Repo = {
      ...repo,
      id: 'repo-runtime-b',
      path: '/tmp/kingu-runtime-b',
      displayName: 'kingu-runtime-b'
    }
    const runtimeWorktreeB: Worktree = {
      ...worktree,
      id: 'wt-runtime-b',
      repoId: runtimeRepoB.id,
      path: '/tmp/kingu-runtime-b-feature',
  ...
  it('splits duplicate user checkouts while a provisioned copy nests, on one host', () => {
    ...
    expect(
      headers.some(
        (row) => row.key === 'project:github:anthovai/kingu-intelligence::setup:repo-runtime-b'
      )
    ).toBe(false)
    expect(
      headers.find((row) => row.key === 'project:github:anthovai/kingu-intelligence')
    ).toMatchObject({
      label: 'Kingu',
      count: 1
    })
