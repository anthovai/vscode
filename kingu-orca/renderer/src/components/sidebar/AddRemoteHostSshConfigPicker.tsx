      ...
      <DialogHeader className="text-left">
        ...
        <DialogDescription>
          {translate(
            ...
            'Pick a host to fill the form, or add every new host to Kingu’s host list.'
          )}
        </DialogDescription>
      </DialogHeader>
          ...
          >
            {hosts.map((host) => (
              <li key={host.alias}>
                <button
                  type="button"
                  disabled={picksDisabled || host.alreadyInKingu}
                  className={cn(
                ...
                >
                  ...
                  {resolvingAlias === host.alias ? (
                    ...
                    >
                      {translate(
                        'auto.components.sidebar.AddRemoteHostDialog.sshConfigPickerInKingu',
                        'In Kingu'
                      )}
                    </Badge>
          ...
          {isBulkImporting
            ...
            : newHostCount > 0
              ? translate(
                  ...
                  'Add all {{value0}} to Kingu',
                  { value0: newHostCount }
                )
              : // Why: the remainder can be already-in-Kingu or merely tombstoned, so the
