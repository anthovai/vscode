...
/**
 * Re-runs the setup-script prompt inspection when a shared `kingu.yaml` setup hook
...
export function useSetupScriptPromptRevalidation(input: {
  ...
  // Why: kingu.yaml is edited on disk or the hook runs in a terminal outside React
  // state. Re-inspect on window focus so returning to Kingu detects it (mirrors
  ...
  useEffect(() => {
    ...
    // Why: a runtime reconnect can turn an unreadable kingu.yaml into a readable one,
