  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from Kingu. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from Kingu. Git worktrees will also be removed from git and disk; folder workspaces will only remove the Kingu workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from Kingu. The project folder on disk will not be deleted.'
