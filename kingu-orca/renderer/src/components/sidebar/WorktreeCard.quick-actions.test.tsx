...
describe('WorktreeCard quick actions', () => {
  ...
  it('renders repo identity in the detailed metadata row', () => {
    ...
    expect(markup).not.toContain('aria-label="Project kingu"')
    expect(markup).toContain('>kingu</span>')
  ...
  it('renders detached HEAD identity in detailed card metadata', () => {
    ...
    const markup = renderToStaticMarkup(
      <WorktreeCard
        worktree={makeWorktree({ displayName: 'kingu', branch: '' })}
        repo={makeRepo()}
        isActive={false}
        hideRepoBadge
      />
