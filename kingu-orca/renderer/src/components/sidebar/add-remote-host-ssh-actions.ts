    ...
    if (
      ...
      toast.error(
        translate(
          ...
          'That SSH host is already in Kingu.'
        )
      )
      return 'validation-failed'
    }
  ...
  if (typeof ssh.resolveConfigHost !== 'function') {
    throw new Error(
      translate(
        ...
        'Restart Kingu to finish applying the SSH config picker update.'
      )
    )
  }
...
/** Bulk-load ~/.ssh/config hosts into Kingu’s host list (sidebar targets). */
export async function addAllSshConfigHostsToKingu({
  ssh,
  recordSshRepoReadoptions,
  setSshTargetsMetadata,
  recordFeatureInteraction
  ...
  try {
    ...
    toast.success(
      translate(
        ...
        'Added {{value0}} host{{value1}} to Kingu.',
    ...
    return {
      ...
      newHostCount: value.filter(
        (host): host is SshConfigHostSummary =>
          typeof host === 'object' && host !== null && host.alreadyInKingu === false
      ).length,
