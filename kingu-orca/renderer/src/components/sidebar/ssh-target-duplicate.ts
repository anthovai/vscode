...
/** True when an existing Kingu host already owns this config alias / label. */
export function isDuplicateSshTargetAlias({
  ...
  // Why: the config picker's `alreadyInKingu` flag compares lowercased aliases; match it or the
 ...
 *  otherwise an alias it greys out as "In Kingu" is still savable as a second target. */
