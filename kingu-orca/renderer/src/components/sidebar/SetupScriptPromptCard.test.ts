...
describe('getRenderedSetupScriptPromptState', () => {
  ...
  it('does not reuse a matching repo id from a different host', () => {
    expect(
      getRenderedSetupScriptPromptState({
        promptState: prompt('repo-kingu', 'local'),
        activeRepoId: 'repo-kingu',
        activeRepoHostIdentity: repoIdentity('repo-kingu', 'runtime:windows'),
...
describe('markSetupScriptPromptSaved', () => {
  it('does not apply a completed save to the same repo id on another host', () => {
    const remote = prompt('repo-kingu', 'runtime:windows')

    expect(markSetupScriptPromptSaved(remote, repoIdentity('repo-kingu', 'local'))).toBe(remote)
  })

  it('marks the prompt for the saved host effective', () => {
    expect(
      markSetupScriptPromptSaved(
        prompt('repo-kingu', 'runtime:windows'),
        repoIdentity('repo-kingu', 'runtime:windows')
      )
