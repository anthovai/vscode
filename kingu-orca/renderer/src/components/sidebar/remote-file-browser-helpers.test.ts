...
describe('shouldDeferRemoteFileBrowserPasteResolve', () => {
  it('keeps small path and filter pastes on the immediate resolver path', () => {
    expect(shouldDeferRemoteFileBrowserPasteResolve('/workspaces/kingu/src')).toBe(false)
...
describe('parsePathInput', () => {
  ...
  it('`Documents/kingu` commits `Documents` and filters by `kingu`', () => {
    expect(parsePathInput('Documents/kingu')).toEqual({
      mode: 'path',
      base: 'cwd',
      committedSegments: ['Documents'],
      trailingFilter: 'kingu'
    })
  })
...
describe('resolveSegmentStep', () => {
  const listing: DirEntry[] = [
    ...
    { name: 'kingu-internal', isDirectory: true },
  ...
  it('unique prefix descends', () => {
    expect(resolveSegmentStep('kingu', '/home/neil', listing)).toEqual({
      type: 'descend',
      name: 'kingu-internal'
    })
  })
