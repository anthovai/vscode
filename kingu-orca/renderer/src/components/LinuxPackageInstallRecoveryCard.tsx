...
function copiedNote(packageFileName: string): string {
  return translate(
    ...
    'Command copied. Quit Kingu, run it in a system terminal to install {{value0}}, then reopen Kingu.',
    {
...
export function LinuxPackageInstallRecoveryCard({
  ...
  const EXPLAINER = translate(
    ...
    'Copy the command, quit Kingu, and run it in a system terminal on the computer where Kingu is installed. Reopen Kingu after it finishes.'
  )
  ...
  const TRUST_NOTE = translate(
    ...
    'Kingu checks the downloaded file against the release metadata at the moment it builds this command. The system package itself is not signature-checked, and Kingu cannot vouch for the file after that point.'
  )
