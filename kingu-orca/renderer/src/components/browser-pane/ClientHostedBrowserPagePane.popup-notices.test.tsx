...
type PopupEvent = {
  ...
  origin: string
  action: 'opened-in-kingu' | 'opened-external' | 'blocked'
}
...
describe('ClientHostedBrowserPagePane popup notices', () => {
  it('names the origin whose popup was refused', () => {
    ...
    expect(toastMocks.message).toHaveBeenCalledWith(
      'https://accounts.example.com tried to open a popup Kingu does not support here.',
  ...
  it('silences in-Kingu opens but reports external opens', () => {
    renderPane()

    emitPopup({ action: 'opened-in-kingu' })
