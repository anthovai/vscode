...
function humanizePermission(permission: string): string {
  switch (permission) {
    ...
    case 'openExternal':
      return 'permission to open a link outside Kingu'
    case 'fileSystem':
...
export function formatPermissionNotice(event: BrowserPermissionDeniedEvent): string {
  ...
  return `${target} asked for ${humanizePermission(event.permission)}, and Kingu denied it.`
}
...
export function formatPopupNotice(event: BrowserPopupEvent): string | null {
  ...
  if (event.action === 'opened-in-kingu') {
    return null
  }
  if (event.action === 'opened-external') {
    return `${target} opened a new window in your default browser.`
  }
  return `${target} tried to open a popup Kingu does not support here.`
}
  ...
  if (isChromiumCertificateErrorCode(loadError.code)) {
    ...
    if (loadError.code === -202) {
      return translate(
        ...
        "Kingu doesn't trust the authority that issued the certificate for {{value0}}.",
        { value0: host }
      )
    }
    return translate(
      'browser.loadFailure.certificateVerificationFailed',
      "Kingu couldn't verify the certificate for {{value0}}.",
      { value0: host }
    )
  }
