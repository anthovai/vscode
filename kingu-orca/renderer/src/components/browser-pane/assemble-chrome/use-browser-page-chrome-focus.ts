...
export function useBrowserPageChromeFocus({
  ...
  useEffect(() => {
    ...
    window.addEventListener(KINGU_BROWSER_FOCUS_REQUEST_EVENT, handleBrowserFocusRequest)
    return () => {
      window.removeEventListener(KINGU_BROWSER_FOCUS_REQUEST_EVENT, handleBrowserFocusRequest)
      cancelAddressBarFocusGrab()
    }
  }, [browserTabId, cancelAddressBarFocusGrab, focusGuestNow, isActive, startAddressBarFocusGrab])
