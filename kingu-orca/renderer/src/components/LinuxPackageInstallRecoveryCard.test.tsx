...
const RELEASE_URL = 'https://github.com/anthovai/kingu-intelligence/releases/tag/v1.4.200'
const DIAGNOSTIC = 'pkexec: no polkit authentication agent found'
const INSTALL_COMMAND = 'sudo apt-get install -y /tmp/kingu-updates/kingu_1.4.200_amd64.deb'
const PACKAGE_FILE_NAME = 'kingu_1.4.200_amd64.deb'
const SUMMARY =
  'Kingu downloaded the system package. Quit Kingu before finishing the update from a terminal.'
const COPIED_NOTE =
  `Command copied. Quit Kingu, run it in a system terminal to install ${PACKAGE_FILE_NAME}, ` +
  'then reopen Kingu.'
const INSTRUCTIONS = {
  ok: true as const,
...
describe('LinuxPackageInstallRecoveryCard copy', () => {
  it('leads with the manual-install copy and recovery actions', () => {
    ...
    expect(
      screen.getByText(/a system terminal on the computer where Kingu is installed/)
    ).toBeTruthy()
    expect(screen.getByText(/Copy the command, quit Kingu/)).toBeTruthy()
...
describe('LinuxPackageInstallRecoveryCard details', () => {
  ...
  it('scopes the trust note to the moment the command is built', () => {
    ...
    const detail = screen.getByText(/Kingu checks the downloaded file against the release metadata/)
    ...
    expect(detail.textContent).toContain(
      'The system package itself is not signature-checked, and Kingu cannot vouch for the file ' +
        'after that point.'
    )
  })
...
describe('LinuxPackageInstallRecoveryCard without a usable command', () => {
  it('promotes Show Package and keeps the official-release fallback', async () => {
    ...
    expect(screen.getByText(/Quit Kingu before finishing the update/)).toBeTruthy()
