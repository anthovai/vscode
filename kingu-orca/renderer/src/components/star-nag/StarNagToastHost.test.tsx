...
type StarNagApi = {
  ...
  starKingu: ReturnType<typeof vi.fn>
}
...
describe('StarNagToastHost', () => {
  ...
  beforeEach(() => {
    ...
    starNag = {
      ...
      starKingu: vi.fn().mockResolvedValue(true)
    }
    shell = {
  ...
  it('renders exact onboarding toast copy and confirms only after direct star succeeds', async () => {
    ...
    expect(toastContainer.textContent).toContain(
      'If you’re enjoying Kingu so far, a GitHub star helps other developers discover it.'
    )
    ...
    expect(starNag.starKingu).toHaveBeenCalledTimes(1)
  ...
  it('opens GitHub fallback without calling direct star success path', async () => {
    ...
    expect(shell.openUrl).toHaveBeenCalledWith('https://github.com/anthovai/kingu-intelligence')
    expect(starNag.openWeb).toHaveBeenCalledTimes(1)
    expect(starNag.starKingu).not.toHaveBeenCalled()
  ...
  it('switches to the explicit GitHub fallback when direct star fails', async () => {
    starNag.starKingu.mockResolvedValueOnce(false)
    ;({ root, container } = renderHost())
  ...
  it('does not allow unresolved close while the primary action is busy', async () => {
    ...
    starNag.starKingu.mockReturnValueOnce(pendingStar.promise)
    ;({ root, container } = renderHost())
