...
export function useAutomationsPageDestinationForm({
  ...
  const isKinguForm = createTarget === 'kingu' && local.editingExternalTarget === null
  ...
  useEffect(() => {
    if (!createOpen || editingAutomationId !== null || createTarget !== 'kingu') {
      return
    }
    setDraft((current) =>
