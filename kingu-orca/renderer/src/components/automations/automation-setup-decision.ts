...
import type { KinguHooks, SetupRunPolicy } from '../../../../shared/kingu-yaml-hook-types'
...
function getAutomationSetupSource(
  ...
  yamlHooks: KinguHooks | null | undefined
...
export function getVisibleAutomationSetupDecision(args: {
  ...
  if (args.createTarget !== 'kingu' || args.workspaceMode !== 'new_per_run') {
    return undefined
  }
...
export function resolveAutomationSetupDecisionForSave(args: {
  ...
  if (args.yamlHooks === undefined) {
    // Why: automations cannot pause later for an kingu.yaml trust prompt; when
