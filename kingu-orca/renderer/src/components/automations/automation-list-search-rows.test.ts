...
const repo = { id: REPO_ID, displayName: 'kingu', path: '/src/kingu' } as Repo
...
describe('automation search row index', () => {
  it('builds one index per row and matches in a single pass', () => {
    ...
    expect(matchAutomationListSearchRowKeys(rows, 'kingu')).toEqual([first, second])
