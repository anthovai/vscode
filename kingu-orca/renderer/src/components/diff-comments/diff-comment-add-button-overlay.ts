...
export function installDiffCommentAddButtonOverlay({
  ...
  plus.type = 'button'
  plus.className = 'kingu-diff-comment-add-btn'
  ...
  const rangeDrag = installDiffCommentRangeDrag({
    ...
    onDragChange: ({ dragging: isDragging, focusLine }) => {
      ...
      editorDomNode.classList.toggle('kingu-diff-comment-range-dragging', isDragging)
  ...
  return {
    dispose: () => {
      ...
      rangeDrag.dispose()
      editorDomNode.classList.remove('kingu-diff-comment-range-dragging')
      plus.remove()
    },
