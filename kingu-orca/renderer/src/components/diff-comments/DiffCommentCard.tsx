...
export function DiffCommentCard({
  ...
  return (
    <div ref={cardRef} className="kingu-diff-comment-card">
      <div className="kingu-diff-comment-content-col">
        {/* Header Row */}
        <div className="kingu-diff-comment-header">
          <div className="kingu-diff-comment-meta-group">{metaText}</div>

          {/* Action buttons pill (only shown if not editing) */}
          {!editing && (
            <div
              className="kingu-diff-comment-actions-pill"
              onMouseDown={(ev) => ev.stopPropagation()}
            >
              {headerActions}
              {headerActions && (url || onSubmitEdit || onDelete) && (
                <span className="kingu-diff-comment-pill-divider" />
              )}
              {url && (
                <>
                  <button
                    type="button"
                    className="kingu-diff-comment-pill-btn"
                    title={translate(
              ...
              {onSubmitEdit && (
                <>
                  ...
                  {onDelete && <span className="kingu-diff-comment-pill-divider" />}
                </>
              )}
              {onDelete && (
                <button
                  type="button"
                  className="kingu-diff-comment-pill-btn kingu-diff-comment-pill-btn-danger"
                  title={translate(
        ...
        {/* Quote Block */}
        {quote ? (
          <div className="kingu-diff-comment-quote">
            <div className="kingu-diff-comment-quote-text">{quote}</div>
          </div>
        ) : null}
        ...
        {editing ? (
          <div className="flex flex-col gap-2 mt-1">
            <textarea
              ref={textareaRef}
              className="kingu-diff-comment-popover-textarea"
              value={draft}
              onChange={(e) => {
              ...
              rows={3}
            />
            <div className="kingu-diff-comment-popover-footer">
          ...
          <div className="kingu-diff-comment-body">{body}</div>
        )}
      </div>
    </div>
  )
