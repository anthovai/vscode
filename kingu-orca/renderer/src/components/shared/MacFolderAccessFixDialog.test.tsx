...
describe('MacFolderAccessFixDialog', () => {
  ...
  it('names the denied folder and leads with the cause', () => {
    ...
    expect(
      screen.getByText('macOS is blocking Kingu’s terminal service from this folder.')
    ).toBeTruthy()
  ...
  it('offers only the reset when a fresh daemon is still denied, and says what it does', () => {
    ...
    expect(screen.getByText('Re-allow Kingu for your Documents folder')).toBeTruthy()
  ...
  it('drops the reset explanation when there is no permission to reset', () => {
    ...
    expect(screen.getByText('Allow Kingu under Files and Folders')).toBeTruthy()
