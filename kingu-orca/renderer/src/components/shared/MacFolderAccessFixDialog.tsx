...
function allowStepHelper(mismatch: PtyManagementFolderAccessMismatch): string | undefined {
  ...
  if (canResetPermission(mismatch)) {
    return translate(
      ...
      'Kingu is already allowed, but macOS isn’t applying it to the terminal service. Reset asks macOS for the permission again. Click Allow when it prompts.'
    )
  }
  return undefined
}
      ...
      <ol className="flex flex-col gap-3">
          ...
          label={
            canResetPermission(mismatch)
              ? translate(
                  ...
                  'Re-allow Kingu for your {{folder}}',
                  { folder }
                )
              : translate(
                  'auto.components.shared.MacFolderAccessFixDialog.stepAllow',
                  'Allow Kingu under Files and Folders'
                )
          }
        ...
        <Step
          ...
          label={translate(
            ...
            'Restart Kingu’s terminal service'
          )}
          helper={
            restartState === 'done'
              ? undefined
 ...
 * toast. Two steps, because a restart alone only works once Kingu itself is allowed again — which
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            {translate(
              ...
              'macOS is blocking Kingu’s terminal service from this folder.'
            )}
          </DialogDescription>
        </DialogHeader>
        <FixSteps
