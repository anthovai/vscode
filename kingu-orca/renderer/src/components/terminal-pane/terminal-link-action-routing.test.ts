...
describe('terminal link action routing', () => {
  ...
  it('offers system browser first and Kingu second when system browser is the default', () => {
    const request = vi.fn()
    ...
    expect(
      handleTerminalHttpLink('https://example.com/path', event, {
        ...
        actionDestinations: { primary: 'system', alternate: 'kingu' }
      })
    ).toBe(true)
    ...
    expect(request.mock.calls[0][0].alternate.label).toBe('Kingu Browser')
  ...
  it('offers Kingu first and system browser second when Kingu is the default', () => {
    const request = vi.fn()

    handleTerminalHttpLink('https://example.com/path', plainEvent(), {
      worktreeId: 'wt-1',
      linkActionContext: actionContext(request),
      actionDestinations: { primary: 'kingu', alternate: 'system' }
    })

    expect(request.mock.calls[0][0].primary.label).toBe('Kingu Browser')
  ...
  it('routes an explicit Kingu Browser action to the owning runtime', () => {
    const request = vi.fn()
  ...
  it('routes an explicit Kingu Browser action through the owning SSH workspace', () => {
    const request = vi.fn()
