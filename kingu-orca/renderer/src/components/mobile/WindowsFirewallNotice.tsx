        ...
        if (
          ...
          toast.success(
            translate(
              ...
              'Windows Firewall now allows Kingu Mobile on private networks'
            )
          )
          return
        }
              ...
              {networkIsPublic
                ...
                : blockingRuleDetected
                  ? translate(
                      ...
                      'Windows may be blocking Kingu Mobile'
                    )
                  : translate(
            ...
            <p className="text-xs text-muted-foreground">
              {networkIsPublic
                ? translate(
                    ...
                    'Change this trusted Wi-Fi network to Private before allowing Kingu Mobile connections.'
                  )
                : blockingRuleDetected
                  ? translate(
                      'auto.components.mobile.WindowsFirewallNotice.blocked-description',
                      'An existing inbound Block rule can override the pairing exception. Repair removes conflicting TCP rules for this Kingu app, then allows port {{port}} on Private networks.',
                      { port: firewallStatus.port }
                    )
                  : translate(
                      'auto.components.mobile.WindowsFirewallNotice.missing-description',
                      'Windows may block the pairing server. Add a rule for this Kingu app and TCP port {{port}} on Private networks.',
                      { port: firewallStatus.port }
                    )}
            </p>
            {usingRelay ? (
              <p className="text-xs text-muted-foreground">
                {translate(
                  ...
                  'Pairing still works over Kingu Relay — allowing this only adds the faster local connection.'
                )}
              </p>
            ) : null}
          </div>
