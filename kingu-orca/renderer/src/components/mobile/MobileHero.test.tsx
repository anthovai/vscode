...
describe('HeroFlow height', () => {
  ...
  it('shows Relay mint failure with no QR and the beta note', () => {
    ...
    expect(screen.getByText('Kingu Relay is in beta.')).toBeInTheDocument()
  })
  ...
  it('does not offer a futile retry when Relay is unavailable on the desktop', () => {
    renderFlow(1, {
      relayMintFailure: {
        ...
        message: 'Kingu Relay is not available on this desktop'
      }
    })
    expect(screen.getByRole('alert')).toHaveTextContent(
      'Kingu Relay isn’t available on this desktop'
    )
  ...
  it('lets the user name this computer in the pairing step, ahead of the code', () => {
    const props: React.ComponentProps<typeof MobileHeroPairingStep> = {
      ...
      pairingUrl: 'kingu://pair?code=abc',
      pairingQrError: false,
      relayMintFailure: null,
      onUseLan: vi.fn(),
      onRetryRelay: vi.fn(),
  ...
  it('demotes the network address picker to a disclosure on Kingu Relay', async () => {
    const props: React.ComponentProps<typeof MobileHeroPairingStep> = {
      pairQrDataUrl: null,
      pairingUrl: null,
      pairingQrError: false,
  ...
  it('keeps a custom address visible on Kingu Relay', () => {
