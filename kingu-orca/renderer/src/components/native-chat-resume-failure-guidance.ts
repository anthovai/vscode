...
/**
 * What the user can do about a chat Kingu could not resume, decided from the host's reason.
 *
...
function manualContinuationText(): string {
  return translate(
    ...
    'Kingu couldn’t resume this chat. Open it to continue manually.'
  )
}
  ...
  if (failure.outcome === 'unconfirmed') {
    return {
      text: translate(
        ...
        'Kingu couldn’t confirm the “continue” message was delivered. Open the chat and check before sending another.'
      ),
      primary: 'open',
      secondary: null
    }
  ...
  if (reason === 'agent_session_restart_work_superseded') {
    return {
      text: translate(
        ...
        'Open the chat and reply. New work arrived in it right after the restart, so Kingu didn’t send its “continue” message.'
      ),
      primary: 'open',
      secondary: null
    }
  ...
  if (reason === 'agent_session_conflict') {
    return {
      text: translate(
        ...
        'Another Kingu window or terminal still owns this session. Close it, then retry.'
      ),
      primary: 'retry',
      secondary: 'open'
    }
  ...
  if (RETRYABLE_OWNERSHIP.has(reason)) {
    return {
      text: translate(
        ...
        'Kingu is still working out which process owns this session. Wait a moment, then retry.'
      ),
      primary: 'retry',
      secondary: 'open'
    }
  ...
  if (NOT_RESUMABLE.has(reason)) {
    return {
      text: translate(
        ...
        'This chat can’t be resumed by Kingu. Open it to see where it stopped.'
      ),
      primary: 'open',
      secondary: 'dismiss'
    }
