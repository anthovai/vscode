  ...
  const handlePrimaryAction = async (): Promise<void> => {
    ...
    switch (currentTip.action) {
      case 'learn-cmd-j-palette': {
        ...
        trackCmdJPaletteFeatureTipAcknowledged(
          getKinguCliFeatureTipTelemetrySource(modalData.source)
        )
        closeModal()
        break
      }
      ...
      case 'setup-cli': {
        ...
        const canApplySetupResult = (): boolean =>
          ...
          setupRequestIdRef.current === setupRequestId
        const telemetrySource = getKinguCliFeatureTipTelemetrySource(modalData.source)
        trackKinguCliFeatureTipSetupClicked(telemetrySource)
        setPrimaryBusy(true)
        try {
          const result = await installCliFromFeatureTip(() => window.api.cli.install())
          if (result.kind === 'installed') {
            trackKinguCliFeatureTipSetupResult(telemetrySource, 'installed')
          ...
          trackKinguCliFeatureTipSetupResult(telemetrySource, 'needs_attention')
          if (!canApplySetupResult()) {
            return
          }
          toast.warning(
          ...
          const message = error instanceof Error ? error.message : 'Failed to install Kingu CLI.'
          if (
            import.meta.env.DEV &&
            message.includes('Development mode uses a generated launcher for validation only')
          ) {
            trackKinguCliFeatureTipSetupResult(telemetrySource, 'dev_preview')
            if (!canApplySetupResult()) {
              return
            }
            enableOrchestrationSkillSetup()
          ...
          trackKinguCliFeatureTipSetupResult(telemetrySource, 'failed')
          if (canApplySetupResult()) {
            toast.error(message)
          }
        } finally {
