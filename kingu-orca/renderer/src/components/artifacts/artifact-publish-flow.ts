    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      toast.error(
        translate(
          ...
          'Sign in to Kingu and try again.'
        )
      )
      return null
    }
...
async function ensureArtifactAccountConnected(): Promise<boolean> {
  ...
  if (state.kinguProfileAuthStatus?.state === 'connected') {
    return true
  }
  return (await state.connectCurrentKinguProfile())?.status === 'connected'
}

async function reconnectArtifactAccount(): Promise<boolean> {
  return (await useAppStore.getState().connectCurrentKinguProfile())?.status === 'connected'
}
  ...
  switch (code) {
    ...
    case 'unreadable':
      return translate(
        ...
        "Kingu couldn't read this file. Open it from a workspace and try again."
      )
    case 'unsupported':
      return translate(
