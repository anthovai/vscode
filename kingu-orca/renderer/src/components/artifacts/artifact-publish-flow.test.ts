...
const mocks = vi.hoisted(() => ({
  ...
  toastSuccess: vi.fn(),
  state: {
    kinguProfileAuthStatus: { state: 'connected' } as { state: string } | null,
    connectCurrentKinguProfile: vi.fn()
  }
}))
...
const published = {
  ...
  item: {
    ...
    shareUrl: 'https://share.onkingu.dev/a/artifact-a'
  }
}

beforeEach(() => {
  vi.clearAllMocks()
  mocks.state.kinguProfileAuthStatus = { state: 'connected' }
  mocks.state.connectCurrentKinguProfile = mocks.connect
})

describe('artifact publish flow', () => {
  it('signs in before preparing and publishing the request', async () => {
    mocks.state.kinguProfileAuthStatus = { state: 'local' }
