...
export function artifactAccountIdentity(authStatus: KinguProfileAuthStatus | null): string | null {
...
function artifactRequestIsCurrent(
  ...
  return (
    ...
    artifactAccountIdentity(useAppStore.getState().kinguProfileAuthStatus) === identity
  )
}
      ...
      if (result.status === 'ok') {
        ...
        setError(
          translate(
            ...
            'Sign in to Kingu again to load artifacts.'
          )
        )
      }
    } catch (loadError) {
