...
function storeState(): Record<string, unknown> {
  return {
    ...
    kinguProfileAuthStatus: mocks.authStatus,
    refreshCurrentKinguProfileAuth: mocks.refreshAuth,
...
describe('ArtifactsPage', () => {
  beforeEach(() => {
    ...
    mocks.resolvePartition.mockReset().mockResolvedValue('persist:kingu-default')
        ...
        artifacts: [
          {
            ...
            shareUrl: 'https://share.onkingu.dev/a/report-123'
          }
        ]
      }
    })
  ...
  it('renders the selected artifact in a right drawer with copy link as the primary action', async () => {
    ...
    await waitFor(() => {
      ...
      expect(preview).toHaveAttribute('partition', 'persist:kingu-default')
      expect(preview).toHaveAttribute('src', 'https://share.onkingu.dev/a/report-123?embed=1')
    })

    fireEvent.click(copyButton)
    await waitFor(() =>
      expect(mocks.writeClipboardText).toHaveBeenCalledWith(
        'https://share.onkingu.dev/a/report-123'
      )
    )
    expect(mocks.toastSuccess).toHaveBeenCalledWith('Artifact link copied')

    fireEvent.click(screen.getByRole('button', { name: 'Open in browser' }))
    expect(mocks.openUrl).toHaveBeenCalledWith('https://share.onkingu.dev/a/report-123')
  })
  ...
  it('explains the agent-first sharing workflow', async () => {
    ...
    expect(screen.queryByText(/kingu artifacts share/)).not.toBeInTheDocument()
    expect(
  ...
  it('does not surface an initial auth error after the account changes during refresh', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.queryByText('Sign in to Kingu again to load artifacts.')
      ).not.toBeInTheDocument()
    )
  })
        ...
        artifacts: [
          {
            ...
            shareUrl: 'https://share.onkingu.dev/a/account-a-secret'
          }
        ]
      }
    })
  ...
  it('treats an organization switch as an account identity change', () => {
    const status = {
      ...
      state: 'connected'
    } satisfies KinguProfileAuthStatus
