...
export const EMULATOR_LOCAL_SHUTDOWN_EVENT = 'kingu:emulator-shutdown'
...
export function useEmulatorPaneSessionEvents({
  ...
  useEffect(() => {
    ...
    window.addEventListener('kingu:emulator-auto-attach', onAuto)
    return () => window.removeEventListener('kingu:emulator-auto-attach', onAuto)
