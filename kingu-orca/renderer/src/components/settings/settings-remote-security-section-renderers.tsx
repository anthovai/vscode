...
export function renderServersSettingsSection(context: SettingsRenderContext): React.JSX.Element {
  ...
  return (
    <SettingsSection
      id="servers"
      title={translate('auto.components.settings.Settings.bd0181eeca', 'Remote Kingu Servers')}
      badge="Beta"
      description={
        model.isWebClient
          ? translate(
              'auto.components.settings.Settings.7686cb5c36',
              'Connect this browser to a saved Kingu server.'
            )
          : translate(
              'auto.components.settings.Settings.b5ee17826b',
              'Pair remote Kingu runtimes for persistent sessions, richer remote state, and web or mobile handoff.'
            )
      }
