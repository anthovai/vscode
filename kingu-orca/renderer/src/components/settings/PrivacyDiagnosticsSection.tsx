...
function DiagnosticsDisabledStateNote({
  ...
  const message =
    reason === 'do_not_track'
      ...
      : reason === 'kingu_telemetry_disabled'
        ? translate(
            'auto.components.settings.PrivacyDiagnosticsRows.63d03261d1',
            'KINGU_TELEMETRY_DISABLED=1 is set — creating and sending diagnostic files is disabled.'
          )
        : reason === 'kingu_diagnostics_disabled'
          ? translate(
              'auto.components.settings.PrivacyDiagnosticsRows.d37e92a06b',
              'KINGU_DIAGNOSTICS_DISABLED=1 is set — app diagnostics are off.'
            )
          : reason === 'ci'
            ? translate(
