...
describe('PrivacyPane — isEnvBlocked', () => {
  ...
  it('returns true for KINGU_TELEMETRY_DISABLED', () => {
    expect(isEnvBlocked({ effective: 'disabled', reason: 'kingu_disabled' })).toBe(true)
  })
...
describe('PrivacyPane — envVarNameForReason', () => {
  ...
  it('maps kingu_disabled to KINGU_TELEMETRY_DISABLED', () => {
    expect(envVarNameForReason('kingu_disabled')).toBe('KINGU_TELEMETRY_DISABLED')
  })
...
describe('PrivacyPane — computeBlockedReason', () => {
  ...
  it('names KINGU_TELEMETRY_DISABLED as the env reason when set', () => {
    const result = computeBlockedReason({ effective: 'disabled', reason: 'kingu_disabled' })
    expect(result).toEqual({ kind: 'env', reason: 'kingu_disabled' })
  })
