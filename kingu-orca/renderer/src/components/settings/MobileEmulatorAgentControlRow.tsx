...
import {
  ...
  KINGU_CLI_SKILL_NAME,
  KINGU_CLI_SKILL_UPDATE_COMMAND
} from '@/lib/agent-feature-install-commands'
import {
  AGENT_SKILL_CLI_PREREQUISITE_NOTICE,
  ensureKinguCliAvailableForAgentSkillTerminal
...
export function MobileEmulatorAgentControlRow(): React.JSX.Element {
  ...
  const cliSkillInstallCommand = buildSkillCommandForRuntime(KINGU_CLI_SKILL_INSTALL_COMMAND)
  const cliSkillUpdateCommand = buildSkillCommandForRuntime(KINGU_CLI_SKILL_UPDATE_COMMAND)
        ...
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            {translate(
              ...
              'Let coding agents control the active mobile emulator with Kingu CLI commands.'
            )}
          </p>
        </div>
        <span
          ...
          <div className="min-w-0 flex-1 space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              {translate(
                ...
                'Registers the Kingu CLI command so agents can control the active emulator from their shell.'
              )}
            </p>
        ...
        <div className={cn('py-3', setup.step2Blocked && 'opacity-60')}>
          <AgentSkillSetupPanel
            ...
            description={translate(
              ...
              'Enables agents to use Kingu CLI commands, including mobile emulator control.'
            )}
            command={cliSkillInstallCommand}
            installedCommand={cliSkillUpdateCommand}
            terminalTitle="Kingu CLI skill setup"
            terminalAriaLabel="Kingu CLI skill install terminal"
            terminalWorktreeId="settings-mobile-emulator-kingu-cli-skill-terminal"
            ...
            openingHint={translate(
              ...
              'Checking Kingu CLI before opening skill setup.'
            )}
            onBeforeOpenTerminal={async () => {
              await ensureKinguCliAvailableForAgentSkillTerminal()
            }}
            onRecheck={setup.refreshCliSkill}
            freshnessSkillName={
              activeSkillRuntime.canUseLocalSkillFreshness ? KINGU_CLI_SKILL_NAME : undefined
            }
          />
        </div>
