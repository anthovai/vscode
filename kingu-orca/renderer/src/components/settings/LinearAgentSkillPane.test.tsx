...
import {
  KINGU_LINEAR_SKILL_INSTALL_COMMAND,
...
const UPDATE_COMMAND = 'npx skills update kingu-linear --global'
...
describe('LinearAgentSkillPane', () => {
  afterEach(async () => {
    ...
    mocks.updateSkillName = 'kingu-linear'
  ...
  it('renders checklist with inlined skill panel and usage examples', () => {
    ...
    for (const example of examples) {
      ...
      expect(example.prompt).toContain('/kingu-linear')
  ...
  it('passes the kingu-linear install/update commands and freshness on a local runtime', async () => {
    await renderPane()

    expect(mocks.panelProps.at(-1)).toEqual(
      expect.objectContaining({
        command: KINGU_LINEAR_SKILL_INSTALL_COMMAND,
        installedCommand: UPDATE_COMMAND,
        freshnessSkillName: KINGU_LINEAR_SKILL_NAME
      })
    )
  })
