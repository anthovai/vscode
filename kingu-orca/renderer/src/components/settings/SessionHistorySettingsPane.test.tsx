...
it('lists one row per paired Kingu server under this computer, and says where SSH stands', async () => {
  mocks.environments = [
  ...
  expect(screen.getByText('Kingu remote servers')).toBeInTheDocument()
})
...
it('leaves a lone computer to its own switch, with no roll-up above it', async () => {
  ...
  expect(screen.queryByText('Kingu remote servers')).not.toBeInTheDocument()
})
