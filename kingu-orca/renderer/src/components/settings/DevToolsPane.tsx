...
// Dev-only preview of the first-party Kingu Cloud sign-in. The sidebar/titlebar
...
function KinguCloudDevSubsection(): React.JSX.Element {
  const authStatus = useAppStore((s) => s.kinguProfileAuthStatus)
  const connect = useAppStore((s) => s.connectCurrentKinguProfile)
  const signOut = useAppStore((s) => s.signOutCurrentKinguProfile)
  const refresh = useAppStore((s) => s.fetchKinguProfileAuthStatus)
    ...
    <section className="space-y-3">
      <div className="flex items-start justify-between gap-3">
        <SettingsSubsectionHeader
          title={translate('auto.components.settings.DevToolsPane.kinguCloud', 'Kingu Cloud')}
          description={translate(
            'auto.components.settings.DevToolsPane.kinguCloudDescription',
            'Dev-only preview of first-party cloud sign-in. Hidden in production; in dev it also appears in the sidebar account switcher once KINGU_CLOUD_API_URL and KINGU_CLOUD_CLIENT_ID are set.'
          )}
        />
      ...
      {configured ? (
        <div className="space-y-2">
          <p className="text-xs text-muted-foreground">
            {translate('auto.components.settings.DevToolsPane.kinguCloudStatus', 'Status')}:{' '}
            <span className="font-medium text-foreground">{authStatus?.state}</span>
          </p>
          <div className="flex flex-wrap gap-2">
            {connected ? (
              <Button type="button" variant="outline" size="sm" onClick={() => void signOut()}>
                {translate('auto.components.settings.DevToolsPane.kinguCloudSignOut', 'Sign out')}
              </Button>
            ) : (
              <Button type="button" variant="outline" size="sm" onClick={() => void connect()}>
                {translate(
                  'auto.components.settings.DevToolsPane.kinguCloudConnect',
                  'Connect profile'
                )}
              </Button>
            )}
            <Button type="button" variant="ghost" size="sm" onClick={() => void refresh()}>
              {translate(
                'auto.components.settings.DevToolsPane.kinguCloudRefresh',
                'Refresh status'
              )}
            </Button>
          </div>
        ...
        <p className="text-xs text-muted-foreground">
          {authStatus?.setupMessage ??
            translate(
              'auto.components.settings.DevToolsPane.kinguCloudNotConfigured',
              'Set KINGU_CLOUD_API_URL and KINGU_CLOUD_CLIENT_ID to preview Kingu Cloud sign-in in this dev build.'
            )}
        </p>
      )}
    </section>
