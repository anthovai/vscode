...
const repos = [repo('r1', 'Kingu'), repo('r2', 'Kingu (builder)', 'ssh-1'), repo('r3', 'Solo')]
const projects = [project('p1', 'Kingu')]
...
describe('buildPaletteFilterModel', () => {
  it('keeps filter options repo-granular while retaining project-row membership', () => {
    ...
    expect(model.repositories[0]?.searchText).toContain('kingu')
