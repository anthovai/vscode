...
const SHARED_WORKTREE_ID = 'repo-shared::/work/kingu-feature'
...
const localWorktree: Worktree = {
  ...
  repoId: 'repo-shared',
  path: '/work/kingu-feature',
  hostId: 'local',
  head: 'abc123',
  branch: 'feature',
  isBare: false,
  isMainWorktree: false,
  displayName: 'kingu-feature',
  comment: '',
  linkedIssue: null,
  linkedPR: null,
...
describe('projectWorkspaceSurfaces', () => {
  it('emits one surface per workspace id when two hosts publish the same worktree', () => {
    ...
    expect(surfaces).toEqual([{ id: SHARED_WORKTREE_ID, path: '/work/kingu-feature' }])
  })
  ...
  it('mounts the active folder workspace at its resolved host path, not the first row', () => {
    ...
    expect(surfaces).toEqual([{ id: 'folder:folder-shared', path: '/remote/kingu' }])
  })
  ...
  it('keeps first-wins for a colliding folder workspace that is not the active one', () => {
    ...
    expect(surfaces).toEqual([{ id: 'folder:folder-shared', path: '/work/kingu-local' }])
    expect(warn).toHaveBeenCalledWith(
      '[workspace-surface] dropping colliding folder path',
      expect.objectContaining({
        kept: '/work/kingu-local',
        dropped: '/remote/kingu'
      })
    )
    warn.mockRestore()
  })
  ...
  it('switches the active folder path from first-wins to the host that hydrates', () => {
    ...
    expect(hydrating).toEqual([{ id: 'folder:folder-shared', path: '/work/kingu-local' }])
    expect(hydrated).toEqual([{ id: 'folder:folder-shared', path: '/remote/kingu' }])
  ...
  it('keeps distinct workspaces on one host', () => {
    const otherWorktree: Worktree = {
      ...localWorktree,
      id: 'repo-shared::/work/kingu-other',
      path: '/work/kingu-other'
    }

    expect(project({ worktrees: [localWorktree, otherWorktree] })).toEqual([
      { id: SHARED_WORKTREE_ID, path: '/work/kingu-feature' },
      { id: 'repo-shared::/work/kingu-other', path: '/work/kingu-other' }
    ])
  })
})
...
describe('projectWorkspaceSurfaces never under-selects', () => {
  ...
  it('emits every distinct id from a mixed local, SSH, runtime and folder catalog', () => {
    const distinctWorktree: Worktree = {
      ...sshWorktree,
      id: 'repo-shared::/work/kingu-ssh-only',
      path: '/work/kingu-ssh-only'
    }
  ...
  it('mounts a local-only catalog whose rows never name a host', () => {
    ...
    expect(
      ...
      { id: SHARED_WORKTREE_ID, path: '/work/kingu-feature' },
      { id: 'repo-shared::/work/kingu-other', path: '/work/kingu-other' },
      { id: 'folder:folder-local-only', path: '/work/kingu-local' }
    ])
  })
  ...
  it('keeps a git worktree and a folder workspace apart even on the same id text', () => {
    ...
    expect(surfaces).toEqual([
      ...
      { id: 'folder:folder-shared', path: '/work/kingu-local' }
    ])
  })
  ...
  it('collapses folder rows across three hosts to one surface without losing the id', () => {
    ...
    expect(surfaces).toEqual([{ id: 'folder:folder-shared', path: '/ssh/kingu' }])
  })
  ...
  it('does not let a folder row that names no host win the local tie-break', () => {
    ...
    ).toEqual([{ id: 'folder:folder-shared', path: '/work/kingu-local' }])
  })
