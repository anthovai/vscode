...
import {
  KINGU_EDITOR_REQUEST_CMD_SAVE_EVENT,
...
export function useEditorCmdSaveRequest({
  ...
  useEffect(() => {
    ...
    window.addEventListener(KINGU_EDITOR_REQUEST_CMD_SAVE_EVENT, handler)
    return () => window.removeEventListener(KINGU_EDITOR_REQUEST_CMD_SAVE_EVENT, handler)
