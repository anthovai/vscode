...
// reads the clipboard with navigator.clipboard.readText(). Kingu runs the renderer with
...
// (execCommand('copy') from a user gesture) both work. We route the read through Kingu's
// trusted clipboard IPC bridge instead, matching how the terminal already reads it.
export const KINGU_CONTEXT_MENU_PASTE_PRIORITY = 10001
export const KINGU_CONTEXT_MENU_PASTE_NAME = 'kingu-ipc-paste'
...
export type KinguContextMenuPasteDeps = {
...
export type KinguContextMenuPasteOutcome =
 ...
 * bridge); otherwise performs the paste through Kingu's IPC clipboard read and
 * returns a Promise (truthy) so Monaco's blocked default never runs.
 */
export function runKinguContextMenuPaste(
  deps: KinguContextMenuPasteDeps
): false | Promise<KinguContextMenuPasteOutcome> {
  ...
  return performKinguContextMenuPaste(editorInstance, deps)
}

async function performKinguContextMenuPaste(
  editorInstance: PasteCapableEditor,
  deps: KinguContextMenuPasteDeps
): Promise<KinguContextMenuPasteOutcome> {
  let text: string
  try {
