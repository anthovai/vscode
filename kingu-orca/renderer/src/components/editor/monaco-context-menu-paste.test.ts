...
import {
  runKinguContextMenuPaste,
  type KinguContextMenuPasteDeps
...
function makeDeps(
  overrides: Partial<KinguContextMenuPasteDeps> & { editor?: ReturnType<typeof makeEditor> }
): KinguContextMenuPasteDeps {
...
describe('runKinguContextMenuPaste', () => {
  ...
  it('reads via IPC and dispatches a native paste for writable focused editors', async () => {
    ...
    const outcome = runKinguContextMenuPaste(deps)
  ...
  it('applies in-app copy metadata (empty-selection + multicursor) when enabled', async () => {
    ...
    await runKinguContextMenuPaste(deps)
    expect(handle.trigger).toHaveBeenCalledWith('keyboard', 'paste', {
      text: 'line text',
      pasteOnNewLine: true,
      multicursorText: ['a', 'b'],
  ...
  it('does nothing on an empty clipboard', async () => {
    ...
    await expect(runKinguContextMenuPaste(deps)).resolves.toEqual({
      status: 'noop',
      reason: 'empty'
    })
  ...
  it('routes oversized pastes through the chunked inserter instead of a native trigger', async () => {
    ...
    const outcome = await runKinguContextMenuPaste(deps)
