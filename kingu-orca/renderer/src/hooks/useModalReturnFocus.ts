...
import {
  KINGU_BROWSER_FOCUS_REQUEST_EVENT,
...
export function useModalReturnFocus(visible: boolean): {
  ...
  const focusSimulatorSurface = useCallback((): void => {
    ...
    focusFirstMatchingSurface(['[data-kingu-emulator-frame="true"] [tabindex]'])
  }, [focusCapturedElement, focusFirstMatchingSurface])
  ...
  const requestBrowserFocus = useCallback((detail: BrowserFocusRequestDetail): void => {
    ...
    window.dispatchEvent(new CustomEvent(KINGU_BROWSER_FOCUS_REQUEST_EVENT, { detail }))
  }, [])
  ...
  const captureReturnFocus = useCallback((): void => {
    ...
    const browserTarget =
      tabType === 'browser' && activeElement?.closest('[data-kingu-browser-address-bar="true"]')
        ? 'address-bar'
        : 'webview'
