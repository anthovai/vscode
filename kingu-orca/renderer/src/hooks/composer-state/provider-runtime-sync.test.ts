...
describe('useComposerProviderRuntimeSync', () => {
  it('drops an older provider slug result after the selected repo/runtime context changes', async () => {
    ...
    first.resolve({ owner: 'kingu', repo: 'stale' })
    await act(async () => first.promise)
    expect(setSelectedRepoSlug).not.toHaveBeenCalledWith({ owner: 'kingu', repo: 'stale' })

    second.resolve({ owner: 'kingu', repo: 'current' })
    await act(async () => second.promise)
    expect(setSelectedRepoSlug).toHaveBeenLastCalledWith({ owner: 'kingu', repo: 'current' })
  })
})
