...
describe('hasInstalledAgentSkill', () => {
  it('matches installed skills by summarized name', () => {
    expect(hasInstalledAgentSkill([skill({ name: 'kingu-cli' })], 'kingu-cli')).toBe(true)
  })
    ...
    expect(
      hasInstalledAgentSkill(
        [
          skill({
            name: 'Kingu CLI',
            directoryPath: 'C:\\Users\\test\\.agents\\skills\\kingu-cli'
          })
        ],
        'kingu-cli'
      )
  ...
  it('ignores non-installed discovery entries', () => {
    expect(
      hasInstalledAgentSkill([skill({ name: 'kingu-cli', installed: false })], 'kingu-cli')
    ).toBe(false)
  })
      ...
      hasInstalledAgentSkill(
        [
          skill({
            ...
            directoryPath: '/repo/.agents/skills/kingu-cli',
            skillFilePath: '/repo/.agents/skills/kingu-cli/SKILL.md'
          }),
          skill({
            id: 'skill-2',
            name: 'kingu-cli',
            ...
            directoryPath: '/Users/test/.codex/plugins/cache/vendor/kingu-cli',
            skillFilePath: '/Users/test/.codex/plugins/cache/vendor/kingu-cli/SKILL.md'
          })
        ],
        'kingu-cli',
  ...
  it('counts home skills when matching global installs', () => {
    expect(
      hasInstalledAgentSkill([skill({ name: 'kingu-cli' })], 'kingu-cli', {
        sourceKinds: GLOBAL_AGENT_SKILL_SOURCE_KINDS
      })
    ).toBe(true)
  })
    ...
    expect(
      hasInstalledAgentSkillNamed(
        [
          skill({
            name: 'Linear Tickets',
            directoryPath: 'C:\\Users\\test\\.agents\\skills\\kingu-linear'
          })
        ],
        ['kingu-linear', 'linear-tickets']
      )
  ...
  it('keeps aliases opt-in for unrelated single-name checks', () => {
    expect(hasInstalledAgentSkill([skill({ name: 'linear-tickets' })], 'kingu-linear')).toBe(false)
  })
})
...
describe('discoverInstalledAgentSkills', () => {
  ...
  it('starts a fresh scan when a forced refresh arrives during a background scan', async () => {
    ...
    const freshResult = discoveryResult([skill({ name: 'kingu-cli' })])
  ...
  it('lets completed-scan broadcasts reuse the cached result', async () => {
    const discover = vi
      ...
      .mockResolvedValue(discoveryResult([skill({ name: 'kingu-linear' })]))
    vi.stubGlobal('window', {
      api: { skills: { discover } },
      dispatchEvent: vi.fn(),
      CustomEvent: class {}
    ...
    for (const pending of fromSubscribers) {
      await expect(pending).resolves.toMatchObject({
        skills: [expect.objectContaining({ name: 'kingu-linear' })]
      })
    }
    expect(discover).toHaveBeenCalledTimes(1)
  })
