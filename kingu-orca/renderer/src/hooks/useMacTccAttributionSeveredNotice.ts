    ...
    const applySeveredNotice = (health: PtyManagementMacTccAttributionHealth): void => {
      ...
      toast.warning(
        translate(
          ...
          'macOS permissions may not reach Kingu terminals'
        ),
        {
          id: SEVERED_TCC_NOTICE_ID,
          description: translate(
            'auto.hooks.useMacTccAttributionSeveredNotice.description',
            'Running Kingu terminals are hosted by a daemon started by a previous Kingu installation. macOS may not apply Kingu’s Accessibility, Automation, or protected-file permissions to them. Restart the daemon from Manage Sessions to restore access. This will close all running Kingu terminals.'
          ),
          duration: Infinity,
          action: {
            label: translate(
      ...
      toast.warning(
        ...
        ),
        {
          ...
          description: translate(
            ...
            'macOS is blocking Kingu’s terminal service from this folder, so commands run there may fail until it’s fixed.'
          ),
          duration: Infinity,
          action: {
