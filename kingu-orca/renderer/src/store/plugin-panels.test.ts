...
describe('plugin panel list loading', () => {
  it('collects commands only from enabled plugin states', () => {
    ...
    const pending = { ...enabled, pluginKey: 'kingu-samples.pending', status: 'pending' as const }
  ...
  it('bounds watchdog errors to installed panels and clears them on recovery', () => {
    const installed = {
      ...plugin('kingu-samples.current'),
      panels: [
        {
          id: 'dashboard',
          title: 'Dashboard',
          tabKey: 'plugin:kingu-samples.current/dashboard' as const
        }
      ]
    }
    ...
    usePluginPanelsStore
      .getState()
      .setPanelHealth('plugin:kingu-samples.current/dashboard', 'error')
    expect(usePluginPanelsStore.getState().panelErrors).toEqual({
      'plugin:kingu-samples.current/dashboard': true
    })

    usePluginPanelsStore
      .getState()
      .setPanelHealth('plugin:kingu-samples.current/dashboard', 'healthy')
    expect(usePluginPanelsStore.getState().panelErrors).toEqual({})
  ...
  it('recovers automatically from a transient list failure with bounded backoff', async () => {
    ...
    const list = vi
      ...
      .mockResolvedValueOnce([plugin('kingu-samples.recovered')])
    vi.stubGlobal('window', { api: { plugins: { list } } })
    ...
    expect(usePluginPanelsStore.getState()).toMatchObject({
      fetchStatus: 'ready',
      plugins: [expect.objectContaining({ pluginKey: 'kingu-samples.recovered' })]
    })
  })
})
