...
// One project cloned on the local Mac and on a remote Kingu server under distinct repo ids —
...
function runtimeResult(method: string): unknown {
  ...
  if (method === 'projectHostSetup.list') {
    return { setups: [clone(setup('remote-repo', '/srv/kingu'))] }
  }
  return {}
}
...
beforeEach(() => {
  ...
  listHostSetups.mockImplementation(async () => [clone(setup('local-repo', '/local/kingu'))])
