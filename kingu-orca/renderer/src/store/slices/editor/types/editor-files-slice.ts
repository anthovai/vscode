...
export type EditorFilesSlice = {
  ...
  /** Atomically retargets open editor sessions across an Kingu-owned move — one commit-only update migrating every path-derived id + all id-keyed state, no close/reopen. Returns collision/stale without mutating. */
