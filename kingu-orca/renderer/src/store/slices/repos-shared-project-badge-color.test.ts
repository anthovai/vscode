...
// Mirrors the real report: one project name ("kingu") set up on the local Mac and on a
// remote Kingu server, where only the local repo row carries the user's chosen color.
const SHARED_PROJECT_ID = 'github:anthovai/kingu-intelligence'
...
function runtimeResult(method: string): unknown {
  ...
  if (method === 'projectHostSetup.list') {
    return { setups: [setup(SHARED_PROJECT_ID, 'remote-repo', '/srv/kingu')] }
  }
  return {}
}
...
beforeEach(() => {
  ...
  listHostSetups.mockResolvedValue([setup(SHARED_PROJECT_ID, 'local-repo', '/local/kingu')])
