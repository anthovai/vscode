...
describe('worktree remote runtime mutations', () => {
      ...
      .createWorktree(
        ...
        undefined,
        {
          ...
          launchConfig: {
            ...
            agentEnv: { KINGU_AGENT_MODE: 'direct' }
          }
        }
      )
    ...
    expect(runtimeEnvironmentCall).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        params: expect.objectContaining({
          ...
          startupEnv: { KINGU_AGENT_MODE: 'direct' },
          startupLaunchConfig: {
            agentCommand: 'codex',
            agentArgs: '--model gpt-5',
            agentEnv: { KINGU_AGENT_MODE: 'direct' }
  ...
  it('passes task startup drafts only to the owning remote runtime', async () => {
    ...
    args[10] = 'codex'
    args[25] = { startupDraft: 'https://github.com/anthovai/kingu-intelligence/issues/12' }
    ...
    expect(runtimeEnvironmentCall).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        params: expect.objectContaining({
          createdWithAgent: 'codex',
          startupDraft: 'https://github.com/anthovai/kingu-intelligence/issues/12'
        })
      })
    )
    expect(mockApi.worktrees.create).not.toHaveBeenCalled()
