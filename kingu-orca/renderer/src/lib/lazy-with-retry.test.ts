...
import { KINGU_RENDERER_UNLOAD_PREVENTED_EVENT } from '../../../shared/renderer-shutdown-events'
import { KINGU_APP_RESTART_ABORTED_EVENT } from '../../../shared/updater-renderer-events'
import {
  KINGU_EDITOR_PREPARE_HOT_EXIT_EVENT,
  type EditorPrepareHotExitDetail
} from '../../../shared/editor-save-events'

const RELOAD_GUARD_KEY = 'kingu:lazy-chunk-reload-attempted'
...
describe('loadLazyWithRetry recovery reload vs the dirty-editor-tab unload veto', () => {
  ...
  function installDirtyEditorTab(options: { hotExitBackupFails?: boolean } = {}): Harness {
    ...
    window.addEventListener(KINGU_EDITOR_PREPARE_HOT_EXIT_EVENT, hotExitBackup)
    vi.spyOn(window.location, 'reload').mockImplementation(() => {
      ...
      if (!accepted) {
        window.dispatchEvent(new Event(KINGU_RENDERER_UNLOAD_PREVENTED_EVENT))
      }
    })

    cleanupHarness = () => {
      window.removeEventListener('beforeunload', dirtyTabGuard)
      window.removeEventListener(KINGU_EDITOR_PREPARE_HOT_EXIT_EVENT, hotExitBackup)
      cleanupBypass()
    }
    return harness
  }
  ...
  it('refuses to reload when unsaved buffers cannot be backed up', async () => {
    ...
    window.addEventListener(KINGU_APP_RESTART_ABORTED_EVENT, restartAborted)
    const error = chunkParseError()
    ...
    window.removeEventListener(KINGU_APP_RESTART_ABORTED_EVENT, restartAborted)
  })
