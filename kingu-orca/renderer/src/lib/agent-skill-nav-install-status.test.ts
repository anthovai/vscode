  ...
  it('keeps loading and missing states ahead of freshness', () => {
    expect(
      getAgentSkillNavInstallStatus({
        ...
        installed: true,
        loading: true,
        inventory: inventory([placement('kingu-linear', 'current')])
      })
    ).toBe('checking')
    expect(
      getAgentSkillNavInstallStatus({
...
describe('getLinearAgentSkillNavInstallStatus', () => {
  it('reports canonical installs under kingu-linear', () => {
    const skills = [skill('kingu-linear')]
    ...
    expect(
      getLinearAgentSkillNavInstallStatus({
        ...
        installed: true,
        loading: false,
        inventory: inventory([placement('kingu-linear', 'outdated')], ['kingu-linear'])
      })
  ...
  it('prefers canonical freshness when both names are installed', () => {
    expect(
      getLinearAgentSkillNavInstallStatus({
        skills: [skill('kingu-linear'), skill('linear-tickets')],
        installed: true,
        loading: false,
        inventory: inventory(
          [placement('kingu-linear', 'current'), placement('linear-tickets', 'outdated')],
          ['linear-tickets']
        )
      })
    ).toBe('up-to-date')
