...
export function buildAgentSessionContinuationPrompt(
  ...
  const sourceLines = [
    ...
    source.sourceLabel ? `Kingu pane: ${source.sourceLabel}` : null,
  ...
  return [
    'Continue work from the prior Kingu session using the context below.',
    ...
    ...(statusHints.length > 0 ? ['', 'Latest Kingu status hints:', ...statusHints] : []),
    '',
