...
type DiagnosticWindow = Window & { __kinguTypingDiagnostic?: TypingDiagnosticBridge }

describe('typing latency diagnostic lifecycle', () => {
  afterEach(() => {
    delete (window as DiagnosticWindow).__kinguTypingDiagnostic
    vi.restoreAllMocks()
  })
  ...
  it('releases stopped pane state while preserving the instrumented count', () => {
    ...
    const bridge = (window as DiagnosticWindow).__kinguTypingDiagnostic
    if (!bridge) {
