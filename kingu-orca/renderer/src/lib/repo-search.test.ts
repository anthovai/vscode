...
describe('repo-search', () => {
  ...
  it('matches display names case-insensitively', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'anthovai/kingu-intelligence', path: '/repos/kingu' }),
      makeRepo({ id: '2', displayName: 'anthovai/noqa', path: '/repos/noqa' })
    ]

    expect(searchRepos(repos, 'KINGU').map((repo) => repo.id)).toEqual(['1'])
  })

  it('falls back to matching repo paths', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'frontend', path: '/src/team-a/kingu' }),
  ...
  it('keeps display-name matches ahead of path-only matches', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'misc', path: '/src/kingu-tools/misc' }),
      makeRepo({ id: '2', displayName: 'kingu', path: '/src/team-a/project' })
    ]

    expect(searchRepos(repos, 'kingu').map((repo) => repo.id)).toEqual(['2', '1'])
  })
