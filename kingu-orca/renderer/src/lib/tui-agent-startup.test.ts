...
describe('buildAgentDraftLaunchPlan', () => {
  ...
  it('uses KINGU_PI_PREFILL env var for pi (no CLI flag exists)', () => {
    // Why: pi has no `--prefill` flag, and bracketed-paste-after-ready races
    // against pi's lengthy startup output. The Kingu overlay installs an
    // `kingu-prefill` extension that reads KINGU_PI_PREFILL on session_start
    ...
    expect(
      ...
      agent: 'pi',
      launchCommand: `pi; command test -n "$fish_pid" && set --erase -g KINGU_PI_PREFILL; command test -z "$fish_pid" && unset KINGU_PI_PREFILL; true`,
      expectedProcess: 'pi',
      env: { KINGU_PI_PREFILL: 'https://github.com/acme/repo/issues/42' },
