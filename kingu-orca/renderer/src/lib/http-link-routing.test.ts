...
describe('openHttpLink', () => {
  it('routes into Kingu when openLinksInApp is on and a worktree is known', () => {
  ...
  it('routes floating workspace links into Kingu without changing the active repo worktree', () => {
    storeState.settings = { openLinksInApp: true }
  ...
  it('forceInApp opens a local link in Kingu when the setting is off', () => {
    storeState.settings = { openLinksInApp: false }
  ...
  it('does not force a remote link into the Kingu browser', () => {
    storeState.settings = { openLinksInApp: false }
  ...
  it('labels explicit local links from the local scan instead of a merged remote port', async () => {
    ...
    registerLocalhostLabelMock.mockResolvedValue({ url: 'http://wt-local.kingu.localhost:60016/' })
  ...
  it('labels localhost links from terminal output before opening the system browser', async () => {
    ...
    registerLocalhostLabelMock.mockResolvedValue({
      url: 'http://analytics.kingu.localhost:60016/episodes'
    })
    ...
    expect(openUrlMock).toHaveBeenCalledWith('http://analytics.kingu.localhost:60016/episodes')
  })
  ...
  it('resolves display URLs for labeled localhost links without opening them', async () => {
    ...
    registerLocalhostLabelMock.mockResolvedValue({
      url: 'http://snapstudio-main.kingu.localhost:60016/'
    })

    await expect(resolveLocalhostHttpLinkDisplayUrl('http://localhost:5180/')).resolves.toBe(
      'http://snapstudio-main.kingu.localhost:60016/'
    )
    expect(openUrlMock).not.toHaveBeenCalled()
    expect(createBrowserTabMock).not.toHaveBeenCalled()
  })
  ...
  it('labels a local-owned localhost link from the local scan', async () => {
    ...
    ).resolves.toBe('http://snapstudio-main.kingu.localhost:60016/')
  })
})

describe('openHttpLink modifier routing', () => {
  it('forces the system browser when inverting is off and links open in Kingu', () => {
  ...
  it('opens in Kingu when inverting is on and links open externally', () => {
  ...
  it('opens in the system browser when inverting is on and links open in Kingu', () => {
  ...
  // Why: remote-owned links must never land in an Kingu tab that cannot reach them.
  it('never routes a remote source into Kingu even when inverting', () => {
    storeState.settings = { openLinksInApp: false, openLinksInAppModifierInverts: true }
