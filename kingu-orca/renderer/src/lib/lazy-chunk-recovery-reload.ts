...
function waitForRefusedNavigation(win: Window): RefusedNavigationWait {
  ...
  const cancel = (): void => {
    ...
    win.removeEventListener(KINGU_RENDERER_UNLOAD_PREVENTED_EVENT, onUnloadPrevented)
  }
  const outcome = new Promise<'unload-vetoed' | 'never-landed'>((resolve) => {
    ...
    win.addEventListener(KINGU_RENDERER_UNLOAD_PREVENTED_EVENT, onUnloadPrevented)
...
export async function requestLazyChunkRecoveryReload(
  ...
  try {
    ...
    win.dispatchEvent(new Event(KINGU_APP_RESTART_ABORTED_EVENT))
  }
}
