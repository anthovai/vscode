...
import { hashKinguHookScript, type KinguHookScriptKind } from './kingu-hook-trust'
import {
  checkRuntimeHooks,
...
export type HookScriptKind = KinguHookScriptKind
...
function getSetupTrustContent(yamlHooks: KinguHooks | null): string {
...
function getVmRecipeTrustContent(yamlHooks: KinguHooks | null): string {
...
function canUseRepoWideTrust(state: AppState, repoId: string): boolean {
  ...
  return Boolean(state.trustedKinguHooks[repoId]?.all) && !hasDuplicateRepoId
}
...
async function confirmScriptContent(
  ...
  const contentHash = await hashKinguHookScript(scriptContent)
  if (isCancelled()) {
    return 'skip'
  }
  const existingHash = state.trustedKinguHooks[repoId]?.[scriptKind]?.contentHash
  ...
  return new Promise<'run' | 'skip'>((resolve) => {
    ...
    state.openModal('confirm-kingu-yaml-hooks', {
      repoId,
      repoName,
      scriptKind,
      scriptContent,
  ...
  return enqueueTrustPrompt(async () => {
    ...
    try {
      if (scriptKind === 'issueCommand') {
        // Local overrides are user-owned; only shared kingu.yaml commands need repo trust.
        ...
        const yamlHooks = (result.hooks as KinguHooks | null) ?? null
        scriptContent =
          scriptKind === 'setup'
