 ...
 *
 * The incident chain was: Cmd+Q leaves a packaged `kingu serve` owning the
...
/** Argv macOS delivers for `open -n -a Kingu` / Finder / Dock relaunch. */
const DESKTOP_RELAUNCH_ARGV = ['/Applications/Kingu.app/Contents/MacOS/Kingu'] as const
...
describe('#8457 headless serve promotion preserves surviving agent sessions', () => {
  ...
  it('ignores a duplicate `kingu serve` launch instead of promoting the headless owner', () => {
    const serve = bootHeadlessServeOwner()
