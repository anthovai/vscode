...
describe('terminal path helpers', () => {
  ...
  describe('plain-text file:// URIs', () => {
    it('extracts a printed file:// URI as a file link resolving to its path', () => {
      const line = 'Report: file:///Users/dev/kingu/report.html'
      const link = extractTerminalFileLinks(line).find(
        (candidate) => candidate.displayText === 'file:///Users/dev/kingu/report.html'
      )
      expect(link).toMatchObject({ pathText: '/Users/dev/kingu/report.html' })
      expect(resolveTerminalFileLink(link!, '/Users/dev/kingu')).toEqual({
        absolutePath: '/Users/dev/kingu/report.html',
        line: null,
        column: null
      })
    })
    ...
    it('does not also emit a bare-path link for the URI body', () => {
      const links = extractTerminalFileLinks('file:///Users/dev/kingu/report.html')
      expect(links.map((link) => link.displayText)).toEqual(['file:///Users/dev/kingu/report.html'])
    })
