...
export type LinearAgentSkillUpdateTarget = {
  skillName: typeof KINGU_LINEAR_SKILL_NAME | typeof LINEAR_TICKETS_SKILL_NAME
  command: string
}
...
export function getLinearAgentSkillUpdateTarget(
  ...
  const canonicalSkillInstalled = hasInstalledAgentSkill(skills, KINGU_LINEAR_SKILL_NAME, {
  ...
  return !installed || canonicalSkillInstalled || !legacySkillInstalled
    ? { skillName: KINGU_LINEAR_SKILL_NAME, command: KINGU_LINEAR_SKILL_UPDATE_COMMAND }
