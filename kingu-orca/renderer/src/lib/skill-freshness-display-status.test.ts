...
describe('getSkillFreshnessDisplayStatus', () => {
  ...
  it.each([
    ...
    // Why: these are Kingu's own bounds. A large but healthy plugin cache would
  ...
  // bound Kingu chose, so the only one a person can actually clear.
...
describe('hasSkillCopyNeedingAttention', () => {
  ...
  // copy inside a work directory. Kingu only updates global skills, so the project copy
  ...
  // presence rather than a freshness claim about a copy Kingu does not manage.
  ...
  // Why: an unreadable plugin path could hide a copy of anything, but a skill Kingu
