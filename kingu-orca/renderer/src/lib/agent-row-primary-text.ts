...
import {
  findKinguDispatchTaskMarkerIndex,
  KINGU_DISPATCH_STATUS_PREAMBLE_PREFIX,
  KINGU_DISPATCH_STATUS_TASK_MARKER
} from '../../../shared/kingu-dispatch-status-prompt'

export const KINGU_DISPATCH_PREAMBLE_PREFIX = KINGU_DISPATCH_STATUS_PREAMBLE_PREFIX
const KINGU_DISPATCH_TASK_MARKER = KINGU_DISPATCH_STATUS_TASK_MARKER
const KINGU_DISPATCH_TASK_ID_MARKER = 'Your task ID is:'
...
const KINGU_DISPATCH_TASK_PREVIEW_SCAN_LIMIT = 512
// Why: task id lives near the top of the preamble; keep that scan tight.
const KINGU_DISPATCH_TASK_ID_SCAN_LIMIT = 1024
...
const KINGU_DISPATCH_TASK_MARKER_SCAN_LIMIT = 32_768

/** True when the live prompt is still an Kingu dispatch turn (not sticky metadata alone). */
export function isKinguDispatchPrompt(prompt: string): boolean {
  return prompt.trimStart().startsWith(KINGU_DISPATCH_PREAMBLE_PREFIX)
}
...
export function orchestrationLabelsMatchLiveDispatch(
  ...
  const liveTaskId = getKinguDispatchTaskId(entry.prompt)
  if (!liveTaskId) {
    return true
  }
...
export function getAgentRowPrimaryText(
  ...
  // first characters are boilerplate ("You are working inside Kingu…").
  if (orchestrationLabelsMatchLiveDispatch(entry)) {
    return (
      entry.orchestration?.displayName?.trim() ||
      entry.orchestration?.taskTitle?.trim() ||
      getKinguDispatchTaskPreview(entry.prompt)
    )
  }
  if (isKinguDispatchPrompt(entry.prompt)) {
    return getKinguDispatchTaskPreview(entry.prompt)
  }
...
export function getKinguDispatchTaskId(prompt: string): string | null {
  if (!isKinguDispatchPrompt(prompt)) {
    return null
  }
  const scan = prompt.trimStart().slice(0, KINGU_DISPATCH_TASK_ID_SCAN_LIMIT)
  const markerIndex = scan.indexOf(KINGU_DISPATCH_TASK_ID_MARKER)
  if (markerIndex === -1) {
    return null
  }
  ...
  const afterMarker = scan.slice(markerIndex + KINGU_DISPATCH_TASK_ID_MARKER.length).trimStart()
  const idEnd = afterMarker.search(/\s/)
  const idLine = idEnd === -1 ? afterMarker : afterMarker.slice(0, idEnd)
  return idLine || null
}

function getKinguDispatchTaskPreview(prompt: string): string {
  ...
  const scan = prompt
    .trimStart()
    .slice(0, KINGU_DISPATCH_TASK_MARKER_SCAN_LIMIT + KINGU_DISPATCH_TASK_PREVIEW_SCAN_LIMIT)
  ...
  const taskBodyStart = taskMarkerIndex + KINGU_DISPATCH_TASK_MARKER.length
  const taskBody = scan.slice(taskBodyStart, taskBodyStart + KINGU_DISPATCH_TASK_PREVIEW_SCAN_LIMIT)
