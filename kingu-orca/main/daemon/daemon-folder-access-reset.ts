...
// (STA-7948) even though Kingu itself is allowed: clear Kingu's TCC row for that folder class so
// macOS asks again, have the app touch the folder so the prompt names Kingu, then re-probe.
...
/** `Kingu.app/Contents/MacOS/Kingu` → `Kingu.app`, the bundle whose id owns every TCC row. */
