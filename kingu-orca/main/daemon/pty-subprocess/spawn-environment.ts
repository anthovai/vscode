...
import { dropInheritedKinguHistFile } from '../../worktree-history-file-path'
import {
...
function deleteRequestedDaemonEnvKeys(
  ...
  // Why: persistent daemon state can differ from Electron; delete CODEX_HOME only when its Kingu overlay owns it.
  const deleteKinguOwnedCodexHome =
...
function promoteAgentTeamsShimPath(
  ...
  if (!env.KINGU_AGENT_TEAMS_TEAM_ID || !requestedPath) {
    return
  }
...
function removeInheritedDevAgentHookEndpoint(
  ...
  if (
    explicitEnv?.KINGU_AGENT_HOOK_ENV === 'development' &&
    !explicitEnv.KINGU_AGENT_HOOK_ENDPOINT
  ) {
...
export function createDaemonPtyEnvironment(opts: PtySubprocessOptions): Record<string, string> {
  const env: Record<string, string> = {
    ...
    COLORTERM: 'truecolor',
    TERM_PROGRAM: 'Kingu',
    TERM_PROGRAM_VERSION: process.env.KINGU_APP_VERSION ?? '0.0.0-dev',
    FORCE_HYPERLINK: '1',
    [KINGU_IMAGE_PROTOCOL_ENV]: KINGU_IMAGE_PROTOCOL_VALUE
  ...
  if (opts.env?.HISTFILE === undefined) {
    dropInheritedKinguHistFile(env)
  }
  if (opts.env?.KINGU_HISTFILE === undefined) {
    delete env.KINGU_HISTFILE
  }
