...
describe('isDurableDaemonScopeSupported', () => {
  ...
  it('is true when the process env XDG_RUNTIME_DIR is a hardened unit override, but the real per-UID dir has a reachable bus (mtl-02 regression)', () => {
    // Simulates kingu-serve@factory.service's RuntimeDirectory=factory hardening directive:
...
describe('buildDurableDaemonScopeCommand', () => {
  it('wraps the daemon command in systemd-run --user --scope with a collected unit', () => {
    const result = buildDurableDaemonScopeCommand(
      '/usr/bin/node',
      ['/opt/kingu/daemon-entry.js', '--socket', '/tmp/x.sock'],
      'nonce-1',
      { PATH: '/usr/bin' },
      null
    )
    ...
    expect(result.args).toEqual([
      '--user',
      '--scope',
      '--unit=kingu-daemon-nonce-1.scope',
      '--property=TimeoutStopSec=5s',
      '--collect',
      '--quiet',
      '--',
      '/usr/bin/node',
      '/opt/kingu/daemon-entry.js',
      '--socket',
      '/tmp/x.sock'
    ])
  })
...
describe('detectOwnCgroupScopeUnit', () => {
  ...
  it('parses a v2 unified-hierarchy line naming an kingu-daemon scope', () => {
    const path = writeCgroupFixture('0::/user.slice/user-1000.slice/kingu-daemon-abc123.scope\n')
    expect(detectOwnCgroupScopeUnit('linux', path)).toBe('kingu-daemon-abc123.scope')
  })

  it('parses a v1 systemd-controller line naming an kingu-daemon scope', () => {
    const path = writeCgroupFixture(
      '1:name=systemd:/user.slice/user-1000.slice/kingu-daemon-def456.scope\n'
    )
    expect(detectOwnCgroupScopeUnit('linux', path)).toBe('kingu-daemon-def456.scope')
  })

  it('returns null for a plain service-unit cgroup — the un-isolated case this fix targets', () => {
    const path = writeCgroupFixture('0::/system.slice/kingu-serve@factory.service\n')
    expect(detectOwnCgroupScopeUnit('linux', path)).toBeNull()
  })

  it('returns null for a scope unit that is not an kingu-daemon one', () => {
    const path = writeCgroupFixture('0::/user.slice/user-1000.slice/some-other-app.scope\n')
    expect(detectOwnCgroupScopeUnit('linux', path)).toBeNull()
  })

  it('recognizes a legacy app-kingu scope so an adopted daemon can migrate it', () => {
    const path = writeCgroupFixture('0::/user.slice/user-1000.slice/app-kingu-1420296.scope\n')
    expect(detectOwnCgroupScopeUnit('linux', path)).toBe('app-kingu-1420296.scope')
  })
})
...
describe('legacy daemon scope migration', () => {
  it('reads every process in the legacy scope, including detached descendants', () => {
    ...
    const cgroupDir = join(root, 'sys', 'user.slice', 'app-kingu-1420296.scope')
    mkdirSync(join(procDir), { recursive: true })
    mkdirSync(cgroupDir, { recursive: true })
    writeFileSync(join(procDir, 'cgroup'), '0::/user.slice/app-kingu-1420296.scope\n')
  ...
  it('lets busctl discover the resolved user bus when service hardening disables the inherited address', () => {
    ...
    const command = buildLegacyScopeMigrationCommand(
      ...
      [321],
      {
        XDG_RUNTIME_DIR: '/run/kingu_serve/factory',
  ...
  it('migrates only a proven legacy scope and fails closed when systemd rejects it', () => {
    ...
    const migrated = migrateLegacyDaemonScope(
      ...
      'linux',
      runtimeDir,
      () => ({ unit: 'app-kingu-1420296.scope', pids: [321, 400] }),
      fakeSystemdBootPath(),
      () => ({ code: 0, timedOut: false }),
      runMigration
    )
