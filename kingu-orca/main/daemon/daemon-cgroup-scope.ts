 ...
 * combined unit (`kingu-serve.service` / `kingu-serve@<slot>`) it and its PTYs stay in the unit's
...
const UNIT_NAME_PREFIX = 'kingu-daemon-'
...
const LEGACY_SCOPE_PREFIX = 'app-kingu-'
 ...
 *
 * Why not that env var first: `RuntimeDirectory=` hardening (`kingu-serve@factory.service` on
 * mtl-02) makes systemd export `XDG_RUNTIME_DIR=/run/kingu_serve/<slot>`, a private scratch dir
...
export function detectOwnCgroupScopeUnit(
  ...
  for (const line of contents.split('\n')) {
    // cgroup v2 unified hierarchy: "0::/user.slice/.../kingu-daemon-<nonce>.scope"
    // cgroup v1 systemd controller: "1:name=systemd:/user.slice/.../kingu-daemon-<nonce>.scope"
