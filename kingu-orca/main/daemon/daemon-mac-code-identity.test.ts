...
const HELPER_PATH =
  '/Applications/Kingu.app/Contents/Frameworks/Kingu Helper.app/Contents/MacOS/Kingu Helper'
const PARKED_PATH =
  '/private/var/folders/x/T/com.anthovai.kingu.ShipIt.abc/Kingu.app/Contents/MacOS/Kingu'
...
describe('classifyCodesignDisplayOutput', () => {
  it('resolves the executable path codesign reports for a live process', () => {
    expect(
      classifyCodesignDisplayOutput(
        `Executable=${HELPER_PATH}\nIdentifier=com.anthovai.kingu.helper\nFormat=pid diskrep\n`,
        0
      )
    ).toBe('resolved')
  })
  ...
  it('separates the Squirrel staging copy from the installed bundle', () => {
    ...
    expect(
      classifyCodesignDisplayOutput(
        'Executable=/Users/a/Library/Caches/com.anthovai.kingu.ShipIt/u/Kingu.app/Contents/MacOS/Kingu\n',
        0
      )
    ).toBe('parked')
  })
