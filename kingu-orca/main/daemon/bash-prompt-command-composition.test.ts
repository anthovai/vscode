...
function runInteractiveBash(
  ...
  const result = spawnSync(
    ...
    ['-lc', '"$BASH" --noprofile --rcfile "$1" -i 2>&1', 'bash', rcfile],
    {
      input,
      encoding: 'utf8',
      env: { ...process.env, HOME: tempHome, KINGU_SHELL_FEATURES: 'ready', TERM: 'xterm' },
      timeout: 5000
    }
  )
...
`
  ...
  itWithBash('keeps a scalar ending in an odd backslash isolated from Kingu hooks', () => {
...
`
    ...
    expect(output).not.toContain('PROMPT_BACKSLASH:<__kingu_')
    expectLifecycle(output)
  })
...
`
    ...
    expect(output).not.toContain('PROMPT_ARRAY_BACKSLASH:<__kingu_')
    expectLifecycle(output)
  })
...
`
  ...
  itWithBash('normalizes hooks without a DEBUG-trap command substitution', () => {
    ...
    expect(output).not.toContain('PROMPT_DEBUG:<(( __kingu_exit_code == 0 ))>')
    expect(output).not.toContain('PROMPT_DEBUG:<__kingu_restore_prompt_status')
    expectLifecycle(output)
  })
  ...
  itWithBash('preserves DEBUG visibility for inherited prompt commands without functrace', () => {
    ...
    expect(output).not.toContain('PROMPT_DEBUG:<(( __kingu_exit_code == 0 ))>')
    expect(output).not.toContain('PROMPT_DEBUG:<__kingu_restore_prompt_status')
    expect(output).not.toContain('PROMPT_DEBUG:<eval "$__kingu_prompt_part">')
    expectLifecycle(output)
  })
  ...
  itWithBash('does not emit command lifecycle for an empty prompt', () => {
    ...
    expect(output).not.toContain('PROMPT_DEBUG:<__kingu_prompt_status=')
    expect(output).not.toContain('PROMPT_DEBUG:<__kingu_prompt_had_functrace="">')
    expect(output).not.toContain('PROMPT_DEBUG:<__kingu_outer_debug_trap_spec=')
  })
  ...
  itWithBash('does not recurse when a user trap installs Kingu preexec', () => {
    const profile = [
      `trap 'trap '\\''__kingu_osc133_preexec'\\'' DEBUG 2>/dev/null; printf "PRIVATE_TRAP\\n"' DEBUG`,
  ...
  itWithBash('does not introduce RETURN traps for inherited prompt commands', () => {
    ...
    expect(output).not.toContain('PROMPT_RETURN:<__kingu_run_prompt_command_array>')
    expectLifecycle(output)
  })
  ...
  itWithBash('hides legacy dispatcher commands from a chained DEBUG trap', () => {
    ...
    const output = runInteractiveBash(
      profile,
      tempHome,
      'echo __kingu_osc133_probe\nfalse\nexit 0\n'
    )
    const commands = [...output.matchAll(/PROMPT_PREEXEC:<([^>]+)>/g)].map((match) => match[1])

    expect(commands).toEqual(['echo __kingu_osc133_probe', 'false', 'exit 0'])
    expectLifecycle(output)
  })
  ...
  itWithBash.each([
    ['status capture', '__kingu_prompt_status=$?'],
    ['trap capture', '__kingu_outer_debug_trap_spec="$(trap -p DEBUG)"'],
    ['trap re-arm', 'trap "__kingu_osc133_preexec" DEBUG']
