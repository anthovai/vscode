...
const {
  ...
  trackMock: vi.fn(),
  getPathMock: vi.fn(() => '/Applications/Kingu.app/Contents/MacOS/Kingu'),
  opendirMock: vi.fn(),
  readMacosBundleIdMock: vi.fn<() => Promise<string | null>>(async () => 'com.anthovai.kingu'),
...
beforeEach(() => {
  setPlatform('darwin')
  trackMock.mockReset()
  getPathMock.mockReset().mockReturnValue('/Applications/Kingu.app/Contents/MacOS/Kingu')
  opendirMock.mockReset().mockResolvedValue(fakeDir())
  readMacosBundleIdMock.mockReset().mockResolvedValue('com.anthovai.kingu')
...
describe('resetFolderAccessForDaemon runs the remedy', () => {
  it.each([
    ...
    expect(readMacosBundleIdMock).toHaveBeenCalledWith('/Applications/Kingu.app')
    expect(resetMacosTccPermissionMock).toHaveBeenCalledWith(service, 'com.anthovai.kingu')
  })
