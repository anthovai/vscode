...
// `app`, so kingud's daemon launch path can resolve without Electron in the graph.
const hostApp = {
  isPackaged: true,
  userDataPath: '',
  appPath: '',
...
function buildInstallFixture(root: string): void {
  ...
  writeFileSync(join(root, 'Kingu.exe'), 'exe-bytes')
...
beforeEach(() => {
  ...
  setProcessProp('execPath', join(installDir, 'Kingu.exe'))
...
describe('buildDaemonHostManifest', () => {
  it('mirrors the win-unpacked layout: exe + data blobs + resources tree, no GPU DLLs', () => {
    ...
    expect(byDest.get('Kingu.exe')?.kind).toBe('file')
    const exeOp = ops.find((op) => op.sourcePath === 'C:\\app\\Kingu.exe')
    expect(exeOp?.destRel).toBe('Kingu.exe')
...
describe('materializeRelocatedDaemonHost', () => {
  it('copies the tree, writes the marker, and returns mirrored fork paths', () => {
    ...
    const dest = join(localAppDataDir, 'Kingu', 'daemon-host', '9.9.9')
    expect(result?.execPath).toBe(join(dest, 'Kingu.exe'))
  ...
  it('copies the exe verbatim: same file name and same bytes as the install-dir exe', () => {
    const result = materializeRelocatedDaemonHost()
    const sourceExe = join(installDir, 'Kingu.exe')
  ...
  it('tracks a differently-named app exe rather than pinning an image name of its own', () => {
    ...
    renameSync(join(installDir, 'Kingu.exe'), join(installDir, 'Kingu Nightly.exe'))
    setProcessProp('execPath', join(installDir, 'Kingu Nightly.exe'))
    const result = materializeRelocatedDaemonHost()
    const dest = join(localAppDataDir, 'Kingu', 'daemon-host', '9.9.9')
    expect(result?.execPath).toBe(join(dest, 'Kingu Nightly.exe'))
    expect(existsSync(join(dest, 'kingu-terminal-daemon.exe'))).toBe(false)
    // Re-resolution must agree with materialization or the fork would target a missing exe.
    expect(getRelocatedDaemonHost()?.execPath).toBe(join(dest, 'Kingu Nightly.exe'))
  })
  ...
  it('fails open on a missing required input, leaving no dest or staging dir', () => {
    ...
    const hostRoot = join(localAppDataDir, 'Kingu', 'daemon-host')
  ...
  it('returns null off win32', () => {
    ...
    expect(existsSync(join(localAppDataDir, 'Kingu', 'daemon-host'))).toBe(false)
  })

  it('does nothing for a packaged host with no asar root (kingud on win32)', () => {
    // kingud answers isPackaged() true — it is a shipped build — but it is plain Node: no
...
describe('getRelocatedDaemonHost', () => {
  it('returns null when the marker version does not match the current version', () => {
    ...
    writeFileSync(join(dest, 'Kingu.exe'), 'exe')
...
describe('pruneOldDaemonHosts', () => {
  it('removes unpinned non-current version dirs, keeping current and pinned', () => {
    const root = join(localAppDataDir, 'Kingu', 'daemon-host')
  ...
  it('reclaims nothing for a packaged host with no asar root (kingud on win32)', () => {
    const root = join(localAppDataDir, 'Kingu', 'daemon-host')
    mkdirSync(join(root, '1.0.0'), { recursive: true })
    hostApp.appPath = join(installDir, 'resources', 'app')
    installHostApp()
