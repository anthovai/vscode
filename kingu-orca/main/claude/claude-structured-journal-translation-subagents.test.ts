...
describe('claude journal translation — which agent produced a row', () => {
  ...
  it("attributes a tool result naming its own call to the call's own agent", () => {
    ...
    const row = items.find(
      (entry) => kinguClientMessageId(entry.identity) === 'claude-tool:claude-session:toolu_bash'
    )
  ...
  const toolRowWrites = (
    ...
    items.filter(
      (entry) => kinguClientMessageId(entry.identity) === `claude-tool:claude-session:${toolUseId}`
    )
  ...
  it('never lets a correction change a row\u2019s content, only its attribution', () => {
    ...
    const idOf = (entry: { identity: AgentJournalItemIdentity }): string =>
      kinguClientMessageId(entry.identity) ?? JSON.stringify(entry.identity)
    ...
    const nested = items.findLast(
      (entry) => kinguClientMessageId(entry.identity) === 'claude-tool:claude-session:toolu_2'
    )
