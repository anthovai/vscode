...
describe('getManagedStatusLineScript (posix)', () => {
  it('guards on rate_limits before sourcing the endpoint or spawning curl', () => {
    ...
    const endpointIndex = script.indexOf('KINGU_AGENT_HOOK_ENDPOINT')
...
describe('getManagedStatusLineScript (win32 local)', () => {
  it('guards on rate_limits via findstr before the endpoint call and curl spawn', () => {
    ...
    const endpointIndex = script.indexOf('call "%KINGU_AGENT_HOOK_ENDPOINT%"')
    const curlIndex = script.indexOf('curl.exe')
    expect(captureIndex).toBeGreaterThan(-1)
    expect(guardIndex).toBeGreaterThan(captureIndex)
    expect(guardIndex).toBeLessThan(endpointIndex)
    expect(endpointIndex).toBeLessThan(curlIndex)
    expect(script).toContain('if errorlevel 1 goto :kingu_statusline_cleanup')
  })
  ...
  it('posts the buffered payload file and deletes it afterwards', () => {
    ...
    expect(script).toContain('set "KINGU_STATUSLINE_PANE_ID=%KINGU_PANE_KEY:~-36%"')
    expect(script).toContain('set "KINGU_STATUSLINE_PANE_ID=%KINGU_STATUSLINE_PANE_ID::=_%"')
    ...
    const curlIndex = script.indexOf('curl.exe')
    const delIndex = script.indexOf('del "%KINGU_STATUSLINE_PAYLOAD_FILE%"')
  ...
  it('never posts a literal %CLAUDE_CONFIG_DIR% token when the var is unset', () => {
    ...
    expect(script).toContain('set "KINGU_STATUSLINE_CONFIG_DIR_FIELD=configDir="')
    expect(script).toContain(
      'if defined CLAUDE_CONFIG_DIR set "KINGU_STATUSLINE_CONFIG_DIR_FIELD=configDir=%CLAUDE_CONFIG_DIR%"'
    )
    expect(script).toContain('--data-urlencode "%KINGU_STATUSLINE_CONFIG_DIR_FIELD%"')
  ...
  it('drains stdin before exiting when the pane key is missing', () => {
    ...
    const paneGuardIndex = script.indexOf(
      'if "%KINGU_PANE_KEY%"=="" goto :kingu_agent_hook_drain_stdin'
    )
    const captureIndex = script.indexOf('more.com')
    expect(paneGuardIndex).toBeGreaterThan(-1)
    expect(paneGuardIndex).toBeLessThan(captureIndex)
    expect(script).toContain(':kingu_agent_hook_drain_stdin')
  })
  ...
  it('throttles with an all-builtin seconds-of-day stamp that fails open to posting', () => {
    ...
    const throttleIndex = script.indexOf(
      `if %KINGU_STATUSLINE_ELAPSED% GEQ 0 if %KINGU_STATUSLINE_ELAPSED% LSS ${CLAUDE_STATUSLINE_MIN_POST_INTERVAL_SECONDS} goto :kingu_statusline_cleanup`
    )
    ...
    const tokenGuardIndex = script.indexOf('if "%KINGU_AGENT_HOOK_TOKEN%"=="" goto')
    const curlIndex = script.indexOf('curl.exe')
    ...
    expect(script).toContain('if not defined KINGU_STATUSLINE_ELAPSED goto :kingu_statusline_probe')
    expect(script).toContain(
      'for /f "delims=0123456789" %%d in ("%KINGU_STATUSLINE_LAST%") do set "KINGU_STATUSLINE_LAST="'
    )
    expect(script).toContain(
    ...
    expect(script).toContain('set "KINGU_STATUSLINE_TIME=%TIME: =0%"')
  })
})
...
describe('statusline curl throttle (posix)', () => {
  it('checks the per-pane stamp after the env guards and before curl', () => {
    stubPlatform('darwin')
    const script = getManagedStatusLineScript('local')
    const envGuardIndex = script.indexOf('-z "$KINGU_AGENT_HOOK_PORT"')
    const durationIndex = script.indexOf('"total_duration_ms"')
    const stampIndex = script.indexOf('kingu-claude-statusline-last-${kingu_statusline_pane_id}')
    ...
    expect(script).toContain(
      'case "$kingu_statusline_now" in 0|[1-9]|[1-9][0-9]*) ;; *) kingu_statusline_now='
    )
    expect(script).toContain(
      'case "$kingu_statusline_last" in 0|[1-9]|[1-9][0-9]*) ;; *) kingu_statusline_last='
    )
  })
})
...
describe.skipIf(process.platform === 'win32')('statusline curl throttle (posix behavioral)', () => {
  ...
  function makeHarness(): {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'kingu-statusline-throttle-'))
    dirs.push(dir)
  ...
  function stampPathFor(dir: string, leafId = LEAF_ID): string {
    return join(dir, `kingu-claude-statusline-last-${leafId}`)
  }
