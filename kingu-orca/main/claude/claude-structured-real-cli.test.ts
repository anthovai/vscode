 ...
 *  is not derived from Kingu's own path expressions. */
...
describe.skipIf(!realClaudeAvailable)('Claude structured real CLI handshake', () => {
  it.skipIf(!realClaudeAuthenticated)(
    ...
    async () => {
      ...
      const cwd = await mkdtemp(join(tmpdir(), 'kingu-command-init-'))
      await mkdir(join(cwd, '.claude', 'commands'), { recursive: true })
      await writeFile(
        join(cwd, '.claude', 'commands', 'kingu-init-catalog-proof.md'),
  ...
  // against the root the CLI itself reports. Deriving the expected root from Kingu's own
  ...
  // silently degrades to echoing back whatever Kingu sent.
  it.skipIf(!realClaudeAuthenticated)(
    ...
    async () => {
      ...
      try {
        ...
        // for in the catalog's resolved shape rather than the id Kingu sent.
  ...
  it('turns a real silent unauthenticated startup into sign-in guidance', async () => {
    const claudeConfigDir = await mkdtemp(join(tmpdir(), 'kingu-claude-no-auth-'))
    const providerSessionId = randomUUID()
    const adapter = realAdapter(providerSessionId, claudeConfigDir)
