...
beforeEach(async () => {
  accountHome = await mkdtemp(join(tmpdir(), 'kingu-claude-history-window-'))
})
...
describe('a crash between Claude saving a prompt and Kingu recording its echo', () => {
  ...
  it('reconciles the prompt Claude already holds as accepted, not undelivered', () => {
    // The durable anchor is Kingu's last completed turn: the reply it saw on the live stream.
