...
/**
 * PIDs of Kingu's own Chromium processes — browser, renderers, GPU, utilities.
 *
 ...
 * kill in every field Kingu records (#10680). A pid in this set is proof the
 ...
 * (main-process-preflight). The standalone daemon installs none and `kingud`
 ...
 * the *killing* process's own pid. Kingu's Chromium processes are children of
 * Electron main, so they never classify `own` from a daemon or kingud host, and
 * on an SSH/serve host there is no Chromium on the machine at all.
 */
export function readKinguChromiumProcessPids(): ReadonlySet<number> {
