export function getStatusPluginEndpointSource(): string[] {
  return [
    ...
    "// OpenCode's Node process (not Kingu's) and has no access to server.ts's",
    ...
    '// which reads the endpoint file. The file only changes on Kingu restart',
    ...
    '      // writeEndpointFile on the Kingu side) allocates a fresh inode on',
    ...
    '        // digits in the key for forward-compat with future KINGU_AGENT_HOOK_*',
    ...
    '      console.warn("[kingu-hook] failed to parse endpoint file:", err.message);',
    '    }',
    '    return null;',
    '  }',
    '}',
    ...
    '  // frozen when OpenCode was fork()ed — stale after an Kingu restart. The',
    '  // file is rewritten on every Kingu start(), so sourcing it per post lets',
    '  // a long-running OpenCode session reach the current server. Falls back',
    '  // to process.env when the file is absent (first-run / pre-endpoint-file / Kingu',
