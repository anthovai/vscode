...
export function getStatusPluginFactorySource(options: {
  ...
  // collision as an 'kingu-opencode-status' plugin failure.
  const pluginID =
    expectedAgent === 'opencode2' ? 'kingu-opencode2-status' : 'kingu-opencode-status'
  return [
    ...
    'export const KinguOpenCodeStatusPlugin = async (_ctx) => {',
    `  if (process.env.KINGU_OPENCODE_AGENT && process.env.KINGU_OPENCODE_AGENT !== '${expectedAgent}') return {};`,
    ...
    ...(options.emitNextEvents
      ? [
          ...
          '    // bounded Kingu lifecycle and preview posts.',
    ...
    '          // publish a final idle so Kingu does not retain a dead owner.',
    ...
    'export default {',
    `  id: "${pluginID}",`,
    '  server: KinguOpenCodeStatusPlugin,',
