...
describe.each(['opencode', 'opencode2'] as const)('%s plugin on OpenCode 2', (agent) => {
  ...
  beforeEach(() => {
    ...
    process.env.KINGU_OPENCODE_AGENT = agent
  ...
  it('does not register hooks for the other pane variant', async () => {
    process.env.KINGU_OPENCODE_AGENT = agent === 'opencode' ? 'opencode2' : 'opencode'
  ...
  it('exposes a distinct plugin id per agent variant', async () => {
    ...
    expect(module.default?.id).toBe(
      agent === 'opencode2' ? 'kingu-opencode2-status' : 'kingu-opencode-status'
    )
  })
  ...
  // id — not its metadata — decides whether Kingu can ever retire the blocker
