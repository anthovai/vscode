...
describe('OpenCode status plugin module contract', () => {
  ...
  // env. Pin every input here so the run does not depend on the developer's Kingu session
  // (an inherited KINGU_AGENT_HOOK_ENDPOINT would otherwise redirect the post to a live app).
  const ENV_KEYS = [
    'KINGU_PANE_KEY',
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'kingu-opencode-plugin-contract-'))
    ...
    // so an inherited value from the developer's own Kingu pane would leave
    // `hooks.event` undefined and fail the contract for the wrong reason.
    delete process.env.KINGU_OPENCODE_AGENT
    delete process.env.KINGU_AGENT_HOOK_ENDPOINT
    process.env.KINGU_AGENT_HOOK_PORT = '59999'
  ...
  async function loadPluginModule(
    ...
    const pluginPath = join(
      tempDir,
      `kingu-opencode-status-${Math.random().toString(36).slice(2)}.mjs`
    )
  ...
  it('keeps the named factory export so the factory-based loader still resolves', async () => {
    ...
    expect(module.KinguOpenCodeStatusPlugin).toBeTypeOf('function')
  })
  ...
  it('returns an event handler from the default export server(), like the named factory', async () => {
    ...
    const fromNamed = await module.KinguOpenCodeStatusPlugin?.({})
