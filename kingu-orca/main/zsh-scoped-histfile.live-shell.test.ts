 ...
 * that the rest of Kingu's startup features arrive with it.
 *
 * macOS `/etc/zshrc` assigns `HISTFILE=${ZDOTDIR:-$HOME}/.zsh_history` with no
 * check-before-set. Kingu used to fight that by keeping its own ZDOTDIR in place
 ...
 * Kingu would not wrap cannot pass here by construction.
 */
...
const systemZshrcClobbersHistfile = (() => {
  ...
  const home = mkdtempSync(join(tmpdir(), 'kingu-histfile-probe-'))
  try {
...
/** The launch config Kingu produces for a pane with exactly these features. */
function launchPane(
  home: string,
  ...
  const env: Record<string, string> = {
    HOME: home,
    ...(scopedHistfile ? { HISTFILE: scopedHistfile, KINGU_HISTFILE: scopedHistfile } : {})
  }
  ...
  return {
    features,
    launch,
    // Why KINGU_ORIG_ZDOTDIR overridden: the launch config resolves the user's
    // config dir from the real process env, and these runs must resolve against
    // the sandbox home instead.
    env: { PATH: '/usr/bin:/bin', ...env, ...launch.env, KINGU_ORIG_ZDOTDIR: home }
  }
}

const USER_FILES = {
  '.zshenv': 'export KINGU_TEST_USER_ZSHENV=1\n',
  '.zprofile': 'export KINGU_TEST_USER_ZPROFILE=1\n',
  '.zshrc': 'export KINGU_TEST_USER_ZSHRC=1\n'
}
...
describe.skipIf(process.platform === 'win32')(
  ...
  () => {
    itWithZsh(
      'wraps a plain pane once Kingu injected a worktree HISTFILE',
      withHome(USER_FILES, async (home) => {
        ...
        expect(launch.env.KINGU_SHELL_FEATURES).toBe('history')
      })
    )
    ...
    itWithClobber(
      ...
      withHome(USER_FILES, async (home) => {
        const scoped = join(home, 'kingu-history', 'zsh_history')
    ...
    itWithZsh(
      ...
      withHome(USER_FILES, async (home) => {
        const { env, launch } = launchPane(home, join(home, 'kingu-history', 'zsh_history'))

        const { values } = await runZshPty({
          env,
          report: [
            'ZDOTDIR',
            'KINGU_TEST_USER_ZSHENV',
            'KINGU_TEST_USER_ZPROFILE',
            'KINGU_TEST_USER_ZSHRC'
          ]
        })

        // Each user file is read from the user's own dir, exactly as unwrapped.
        expect(values.KINGU_TEST_USER_ZSHENV).toBe('1')
        expect(values.KINGU_TEST_USER_ZPROFILE).toBe('1')
        expect(values.KINGU_TEST_USER_ZSHRC).toBe('1')
    ...
    itWithZsh(
      'never leaves history inside Kingu’s own wrapper directory',
      withHome(USER_FILES, async (home) => {
        const { env, launch } = launchPane(home, join(home, 'kingu-history', 'zsh_history'))
    ...
    itWithZsh(
      'consumes KINGU_HISTFILE so nothing the shell spawns can inherit it',
      withHome(USER_FILES, async (home) => {
        const scoped = join(home, 'kingu-history', 'zsh_history')
        const { env } = launchPane(home, scoped)
        ...
        const { values } = await runZshPty({
          env,
          report: ['KINGU_HISTFILE', 'KINGU_SHELL_FEATURES', 'HISTFILE']
        })

        // Root-cause fix for #11146: the variables no longer exist after use.
        expect(values.KINGU_HISTFILE).toBe('UNSET')
    ...
    itWithZsh(
      'leaves HISTFILE exactly as an unwrapped zsh would when Kingu injects nothing',
      withHome(USER_FILES, async (home) => {
        ...
        // The contract is that Kingu's wrapper does not change it either way.
        const overlayEnv = { KINGU_CODEX_HOME: join(home, 'codex') }
        const features = selectShellStartupFeatures({
          shellPath: ZSH_PATH,
...
describe.skipIf(process.platform === 'win32')('the deferred hook delivers every feature', () => {
  ...
  itWithZsh(
    'restores Kingu’s overlay values after the user’s config overwrites them',
    withHome(
      {
        ...USER_FILES,
      ...
      async (home) => {
        const overlayEnv = {
          KINGU_CODEX_HOME: '/kingu/codex',
          KINGU_AGENT_TEAMS_SHIM_DIR: '/kingu/shim'
        }
        const features = selectShellStartupFeatures({
          shellPath: ZSH_PATH,
          env: { HOME: home, ...overlayEnv },
        ...
        // the spawn env did, and Kingu's values still win.
        expect(values.CODEX_HOME).toBe('/kingu/codex')
        expect(values.PATH.startsWith('/kingu/shim:')).toBe(true)
      }
    )
  )
})
...
describe.skipIf(process.platform === 'win32')(
  ...
  () => {
     ...
     * `.zshenv` or `.zprofile` cost the pane all of Kingu's features. Only one
    ...
    it.each([
      ...
      try {
        ...
        const { values } = await runZshPty({ env, report: ['HISTFILE', 'KINGU_HISTFILE'] })
    ...
    itWithZsh(
      ...
      withHome({ ...USER_FILES, '.zshrc': 'precmd_functions=()\n' }, async (home) => {
        ...
        const report = ['HISTFILE', 'KINGU_HISTFILE', 'ZDOTDIR']
        ...
        // host's call, not Kingu's. macOS /etc/zshrc overwrites HISTFILE, so it
        ...
        expect(values.HISTFILE).not.toContain(launch.env.ZDOTDIR)
        // KINGU_HISTFILE was consumed in .zshenv precisely so a dropped hook
    ...
    itWithZsh(
      ...
      withHome(
        ...
        async (home) => {
          const { env } = launchPane(home, join(home, 'kingu-history', 'zsh_history'))

          const { values } = await runZshPty({
            env,
            report: ['ZDOTDIR', 'KINGU_TEST_USER_ZSHRC']
          })
...
describe.skipIf(process.platform === 'win32')('the relay variant of the hook', () => {
  itWithZsh(
    ...
    withHome(USER_FILES, async (home) => {
      const relayRoot = mkdtempSync(join(tmpdir(), 'kingu-relay-wrapper-'))
      try {
        ...
        const { output, values } = await runZshPty({
          env: {
            ...
            KINGU_ORIG_ZDOTDIR: home,
            KINGU_SHELL_FEATURES: 'overlay,history,ready',
            KINGU_HISTFILE: scoped,
            KINGU_REMOTE_CLI_BIN_DIR: '/kingu/remote-bin'
          },
          commands: ['true'],
          report: ['HISTFILE', 'ZDOTDIR', 'PATH', 'KINGU_HISTFILE']
        })

        expect(values.HISTFILE).toBe(scoped)
        expect(values.ZDOTDIR).toBe(home)
        expect(values.KINGU_HISTFILE).toBe('UNSET')
        expect(values.PATH.startsWith('/kingu/remote-bin:')).toBe(true)
        expect(output).toContain(MARKERS.ready)
