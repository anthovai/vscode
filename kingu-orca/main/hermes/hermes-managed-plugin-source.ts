export const HERMES_PLUGIN_NAME = 'kingu-status'
export const HERMES_PLUGIN_MARKER = 'Managed by Kingu. Do not edit; changes may be overwritten.'
...
export function getPluginManifest(): string {
  return [
    ...
    'version: 1.0.0',
    'description: "Reports Hermes Agent lifecycle events to Kingu."',
    'author: "Kingu"',
    'kind: standalone',
    'provides_hooks:',
...
def _endpoint_env() -> dict[str, str]:
    env = dict(os.environ)
    endpoint = env.get("KINGU_AGENT_HOOK_ENDPOINT", "")
...
def _post_to_kingu(payload: dict[str, Any]) -> None:
    env = _endpoint_env()
    port = env.get("KINGU_AGENT_HOOK_PORT", "")
    token = env.get("KINGU_AGENT_HOOK_TOKEN", "")
    pane_key = env.get("KINGU_PANE_KEY", "")
    if not port or not token or not pane_key:
        return
    body = {
        "paneKey": pane_key,
        "launchToken": env.get("KINGU_AGENT_LAUNCH_TOKEN", ""),
        "tabId": env.get("KINGU_TAB_ID", ""),
        "worktreeId": env.get("KINGU_WORKTREE_ID", ""),
        "env": env.get("KINGU_AGENT_HOOK_ENV", ""),
        "version": env.get("KINGU_AGENT_HOOK_VERSION", ""),
        "payload": payload,
    }
...
def _make_hook(event_name: str) -> Callable[..., None]:
    def _hook(**kwargs: Any) -> None:
        _post_to_kingu(_payload_for_event(event_name, kwargs))
