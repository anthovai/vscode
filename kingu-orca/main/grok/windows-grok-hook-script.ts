...
const WINDOWS_GROK_HOOK_POST_COMMAND = buildWindowsAgentHookPostCommand('grok', [
  ...
  '  --data-urlencode "grokHome=%KINGU_GROK_HOME%" ^'
])
 ...
 * is unset (the default outside an Kingu-managed terminal), length/trailing
...
export function buildWindowsGrokHookScript(): string {
  return [
    ...
    ...buildWindowsHookEnvironmentGuardLines(),
    'set "KINGU_GROK_HOME="',
    'if not defined GROK_HOME goto :kingu_grok_home_ready',
    `if not "%GROK_HOME:~${GROK_HOME_ENVELOPE_MAX_LENGTH},1%"=="" goto :kingu_grok_home_ready`,
    ...
    'set "KINGU_GROK_HOME=%GROK_HOME:"=%"',
    'if not defined KINGU_GROK_HOME goto :kingu_grok_home_ready',
    'if "%KINGU_GROK_HOME:~-1%"=="\\" set "KINGU_GROK_HOME=%KINGU_GROK_HOME%."',
    // Why: the trailing-backslash safety sentinel counts toward the relay envelope.
    `if not "%KINGU_GROK_HOME:~${GROK_HOME_ENVELOPE_MAX_LENGTH},1%"=="" set "KINGU_GROK_HOME="`,
    ':kingu_grok_home_ready',
