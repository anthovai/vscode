...
function devSelf(): KinguOrgMember {
  return {
  ...
  return {
    members: [
      devSelf(),
      {
        ...
        email: 'teammate@kingu.local',
        displayName: 'Dev Teammate',
        role: 'admin'
      },
      // Why: userId null exercises the "hasn't signed in to Kingu yet" disabled row.
      { userId: null, email: 'invited-member@kingu.local', displayName: undefined, role: 'member' }
    ],
    pendingInvites: [{ email: 'pending@kingu.local', role: 'member', createdAt: Date.now() }]
  }
}
...
export function listDevKinguCloudOrgMembers(orgId: string): KinguOrgMembersRoster {
...
export function inviteDevKinguCloudOrgMember(
...
export function revokeDevKinguCloudOrgInvite(
...
export function changeDevKinguCloudOrgMemberRole(
...
export function removeDevKinguCloudOrgMember(
