import type {
  KinguOrgMember,
  KinguOrgMembersRoster,
  KinguOrgPendingInvite,
  KinguOrgRole
...
const ORG_ROLES: readonly KinguOrgRole[] = ['owner', 'admin', 'member']

function isOrgRole(value: unknown): value is KinguOrgRole {
  return typeof value === 'string' && (ORG_ROLES as readonly string[]).includes(value)
}

function normalizeRole(value: unknown, fallback: KinguOrgRole): KinguOrgRole {
...
function normalizeMember(value: unknown): KinguOrgMember | null {
...
function normalizePendingInvite(value: unknown): KinguOrgPendingInvite | null {
  if (!value || typeof value !== 'object') {
    return null
  }
  const record = value as Record<string, unknown>
...
function normalizeRoster(value: unknown): KinguOrgMembersRoster {
  if (!value || typeof value !== 'object') {
    throw new Error('invalid_kingu_org_members_roster')
  }
  const record = value as Record<string, unknown>
  const members = Array.isArray(record.members)
    ? record.members
        .map(normalizeMember)
        .filter((member): member is KinguOrgMember => member !== null)
    : []
  const pendingInvites = Array.isArray(record.pendingInvites)
    ? record.pendingInvites
        .map(normalizePendingInvite)
        .filter((invite): invite is KinguOrgPendingInvite => invite !== null)
    : []
  return {
    members,
    pendingInvites,
...
function orgMembersUrl(config: KinguCloudAuthConfig, orgId: string, path: string): string {
...
async function requestOrgMembers<T>(
  ...
  if (!response.ok) {
    throw new KinguCloudRequestError(response.status, await extractErrorCode(response))
  }
  return parse((await response.json()) as unknown)
}

export async function listKinguCloudOrgMembers(
  config: KinguCloudAuthConfig,
  session: KinguCloudSession,
  orgId: string
): Promise<KinguOrgMembersRoster> {
...
export async function inviteKinguCloudOrgMember(
  config: KinguCloudAuthConfig,
  session: KinguCloudSession,
  args: { orgId: string; email: string; role: KinguOrgRole }
): Promise<void> {
...
export async function revokeKinguCloudOrgInvite(
  config: KinguCloudAuthConfig,
  session: KinguCloudSession,
...
export async function changeKinguCloudOrgMemberRole(
  config: KinguCloudAuthConfig,
  session: KinguCloudSession,
  args: { orgId: string; userId: string; role: KinguOrgRole }
): Promise<void> {
  await requestOrgMembers(
...
export async function removeKinguCloudOrgMember(
  config: KinguCloudAuthConfig,
  session: KinguCloudSession,
