...
export class KinguCloudRequestError extends Error {
...
function assertString(value: unknown, field: string): string {
  if (typeof value !== 'string') {
    throw new Error(`invalid_kingu_cloud_${field}`)
  }
...
function normalizeCapabilities(value: unknown): KinguCloudCapabilities {
...
function normalizeOrganizations(value: unknown): KinguCloudOrgSummary[] | undefined {
...
function normalizeCloudSummary(value: unknown): KinguProfileCloudSummary {
  if (!value || typeof value !== 'object') {
    throw new Error('invalid_kingu_cloud_profile')
  }
  const record = value as Record<string, unknown>
  return {
...
function normalizeSessionResponse(value: unknown): KinguCloudSessionExchangeResponse {
  if (!value || typeof value !== 'object') {
    throw new Error('invalid_kingu_cloud_session')
  }
  const record = value as Record<string, unknown>
  return {
...
export function isAmbiguousCloudRequestFailure(error: unknown): boolean {
  return !(error instanceof KinguCloudRequestError)
}
...
async function postJson<T>(url: string, body: unknown, options?: PostJsonOptions): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new KinguCloudRequestError(response.status)
  }
  return (await response.json()) as T
}

export async function exchangeKinguCloudAuthCode(
  config: KinguCloudAuthConfig,
  args: ExchangeCodeArgs
): Promise<KinguCloudSessionExchangeResponse> {
...
export async function refreshKinguCloudCapabilities(
  config: KinguCloudAuthConfig,
...
export async function refreshKinguCloudSession(
  config: KinguCloudAuthConfig,
  session: KinguCloudSession
): Promise<KinguCloudSessionExchangeResponse> {
  return normalizeSessionResponse(
...
export async function createKinguCloudProfile(
  config: KinguCloudAuthConfig,
  session: KinguCloudSession,
...
export async function selectKinguCloudOrg(
  config: KinguCloudAuthConfig,
  session: KinguCloudSession,
  orgId: string
...
export async function revokeKinguCloudSession(
  config: KinguCloudAuthConfig,
  session: KinguCloudSession
): Promise<void> {
  await postJson(
