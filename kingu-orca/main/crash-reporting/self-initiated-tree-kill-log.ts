...
/**
 * Records the force-kills Kingu itself issues, so a later `render-process-gone`
 ...
 * (see `kingu-chromium-process-pids.ts`), and a group or Job-Object kill can
 * only contain what Kingu put in it. Absence is evidence, not proof.
 */
 ...
 * Kingu did not put in its target: `taskkill /T /F` walks whatever tree the pid
 * owns at kill time, including a recycled pid that is now our renderer. A
 * process group or Job Object contains only what Kingu placed there, so it is
 ...
 * direct proof that Kingu was about to kill its own renderer.
 */
...
/**
 * Kills Kingu issued near `goneAt`, split by whether the mechanism could have
