...
function getAuthConfig(): AzureDevOpsCreateAuthConfig {
  return {
    pat: envValue('KINGU_AZURE_DEVOPS_TOKEN') ?? envValue('KINGU_AZURE_DEVOPS_PAT'),
    accessToken: envValue('KINGU_AZURE_DEVOPS_ACCESS_TOKEN'),
    username: envValue('KINGU_AZURE_DEVOPS_USERNAME')
  }
}
...
function classifyCreateError(error: unknown): CreateHostedReviewResult {
  ...
  if (
    ...
    return {
      ...
      code: 'auth_required',
      error:
        'Create PR failed: Azure DevOps is not authenticated. Next step: set KINGU_AZURE_DEVOPS_TOKEN in this environment.'
    }
  }
