...
export type GetStatusOptions = GitRuntimeOptions & {
  ...
  /** Paths Kingu may have symlinked into this worktree (per-user shared paths
   *  plus `kingu.yaml` shared directories). Untracked entries that are one of
   *  these *and* really symlinks are dropped: Git cannot ignore them when the
   *  repo's rule is directory-only (`node_modules/`), but they are Kingu's own
