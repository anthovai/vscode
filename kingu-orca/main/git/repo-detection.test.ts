...
describe('isGitRepo', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(path.join(tmpdir(), 'kingu-repo-detect-'))
  })
  ...
  it('rejects a .git file that points at a missing gitdir when git cannot be run', () => {
    ...
    mkdirSync(fakeRepo)
    writeFileSync(path.join(fakeRepo, '.git'), 'gitdir: /missing/kingu/gitdir')
...
describe('getLinkedWorktreeMainRepoRoot', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(path.join(tmpdir(), 'kingu-linked-worktree-'))
  })
  ...
  function initRepoWithCommit(repoRoot: string): void {
    ...
    git(repoRoot, ['config', 'user.email', 'test@kingu.test'])
    git(repoRoot, ['config', 'user.name', 'Kingu Test'])
