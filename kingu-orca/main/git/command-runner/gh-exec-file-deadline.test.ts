...
describe('gh exec deadline', () => {
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      ...
      // whatever group it inherited — Kingu's own.
  ...
  it('spawns with hidden console and captured stdio, never an inherited or shell stdio', async () => {
    ...
    expect(args).toEqual(['api', '--include', 'user/starred/anthovai/kingu-intelligence'])
  ...
  it('fails rather than returning a clipped answer when gh overruns maxBuffer', async () => {
    ...
    await expect(
      ghExecFileAsync(['api', 'repos/anthovai/kingu-intelligence/issues'], {
        timeout: 15_000,
        maxBuffer: 8
      })
