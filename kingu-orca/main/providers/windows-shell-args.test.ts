...
const CODEX_LAUNCH_PREFLIGHT = 'C:\\Program Files\\Kingu\\kingu.exe'
const CMD_CODEX_LAUNCH_PREFLIGHT =
  'if defined KINGU_CODEX_LAUNCH_PREFLIGHT call %KINGU_CODEX_LAUNCH_PREFLIGHT_CMD_QUOTE%%KINGU_CODEX_LAUNCH_PREFLIGHT%%KINGU_CODEX_LAUNCH_PREFLIGHT_CMD_QUOTE% agent hooks prepare-codex > nul 2>&1'
...
describe('resolveWindowsShellLaunchArgs', () => {
  ...
  it('returns PowerShell args that install OSC 133 bootstrap after normal profile loading', () => {
    ...
    const mimocodeRestoreIndex = command.indexOf('$env:MIMOCODE_HOME = $env:KINGU_MIMOCODE_HOME')
    const duplicateStateGuardIndex = command.indexOf('Test-Path variable:global:__KinguOsc133State')
    ...
    const ompExtensionIndex = command.indexOf('--extension $env:KINGU_OMP_STATUS_EXTENSION')
    const codexRestoreIndex = command.indexOf('$env:CODEX_HOME = $env:KINGU_CODEX_HOME')
    ...
    expect(command).not.toContain('KINGU_PI_CODING_AGENT_DIR')
    expect(command).not.toContain('KINGU_OMP_CODING_AGENT_DIR')
    expect(command).not.toContain('$env:PI_CODING_AGENT_DIR = $env:KINGU_OMP_SOURCE_AGENT_DIR')
  ...
  it('preserves complex PowerShell startup command text through EncodedCommand', () => {
    const startupCommand =
      '& "C:\\Program Files\\Kingu CLI\\kingu.exe" "--label" "quoted value"; $env:KINGU_VALUE = "nested"'
    const result = resolveWindowsShellLaunchArgs(
      'powershell.exe',
      'C:\\Users\\alice',
      'C:\\Users\\alice',
  ...
  it('starts Git Bash as an interactive login shell with UTF-8 console setup', () => {
    ...
    expect(bashRcfile).toContain('"${KINGU_CODEX_LAUNCH_PREFLIGHT}" agent hooks prepare-codex')
  ...
  it('quotes a spaced preflight path through each shell environment', () => {
    ...
    expect(cmd.shellArgs[1]).toContain(
      'call %KINGU_CODEX_LAUNCH_PREFLIGHT_CMD_QUOTE%%KINGU_CODEX_LAUNCH_PREFLIGHT%%KINGU_CODEX_LAUNCH_PREFLIGHT_CMD_QUOTE%'
    )
    ...
    expect(readFileSync(rcfilePath, 'utf8')).toContain('"${KINGU_CODEX_LAUNCH_PREFLIGHT}"')
  })
  ...
  it('materializes shell-ready wrappers before building WSL shell args', () => {
    ...
    // commands must reach the user's binary without Kingu rewriting argv.
    ...
    for (const wrapperFile of [bashRcfile, zshEnv]) {
      expect(wrapperFile).toContain('command omp --extension "${KINGU_OMP_STATUS_EXTENSION}" "$@"')
      expect(wrapperFile).toContain('function omp { __kingu_omp "$@"; }')
  ...
  it('escapes single quotes when translating a WSL cwd', () => {
    ...
    expect(result.shellArgs[3]).toContain('exec "$_kingu_wsl_shell" -l')
  })
...
describe('issue #7236: PowerShell setup-runner command delivery', () => {
  ...
  const runnerPath = 'C:/Users/alice/repo/.git/kingu/setup-runner.cmd'
