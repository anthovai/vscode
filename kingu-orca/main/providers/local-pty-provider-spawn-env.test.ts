...
vi.mock('electron', () => ({
  app: {
    getPath: vi.fn(() => '/tmp/kingu-user-data')
  }
}))
...
describe('LocalPtyProvider', () => {
  ...
  describe('spawn', () => {
    ...
    it.each([
      // fish EXPORTS fish_history, so an Kingu launched from a fish pane hands every
      // pane the LAUNCHING worktree's session — even with isolation off (STA-4682).
      ['an inherited Kingu session', 'kingu_abc123', undefined],
    ...
    it.each([
      // HISTFILE is exported, so an Kingu launched from a pane in another worktree
      // hands every pane that worktree's history file — isolation off included.
      [
        'an inherited Kingu path',
    ...
    it('does not inherit NODE_ENV from the Kingu process env', async () => {
      // Why: NODE_ENV in Kingu's process is Kingu's build mode (electron-vite sets
    ...
    it('honors explicit terminal env overrides after deleting requested defaults', async () => {
      provider.configure({
        buildSpawnEnv: (_id, env) => {
          env.TERM_PROGRAM = 'Kingu'
          env.KINGU_STALE_TEST_ENV = '/tmp/kingu-stale'
          env.PATH = `/tmp/kingu-stale:${env.PATH ?? ''}`
          return env
        }
      })
      ...
      expect(spawnCall[2].env.PATH.split(':')[0]).toBe('/tmp/kingu-agent-teams-bin')
      expect(spawnCall[2].env.TERM_PROGRAM).toBeUndefined()
      expect(spawnCall[2].env.KINGU_STALE_TEST_ENV).toBeUndefined()
    })
    ...
    it('does not re-promote a legacy attribution path for Agent Teams', async () => {
      await provider.spawn({
        ...
        rows: 24,
        env: {
          PATH: '/tmp/kingu-terminal-attribution/posix:/usr/bin',
          KINGU_AGENT_TEAMS_TEAM_ID: 'team-test'
        }
      })
    ...
    it('does not inherit AppImage runtime env into Linux PTY shells', async () => {
      ...
      process.env.LD_LIBRARY_PATH = ['/tmp/.mount_kingu123/usr/lib', '/opt/audio/lib'].join(
        delimiter
      )
    ...
    it('uses shell wrapper when MiMo home must survive shell startup', async () => {
      provider.configure({
        buildSpawnEnv: (_id, env) => {
          env.MIMOCODE_HOME = '/tmp/kingu-mimocode-overlay'
          env.KINGU_MIMOCODE_HOME = '/tmp/kingu-mimocode-overlay'
          return env
        }
      })
      ...
      expect(spawnCall[2].env.KINGU_SHELL_FEATURES).not.toContain('ready')
    })
    ...
    it('promotes the agent-teams shim onto the Windows `Path` spelling', async () => {
      ...
      expect(spawnEnv.Path.split(':')[0]).toBe('/tmp/kingu-agent-teams-bin')
    })
    ...
    it('does not inherit parent Kingu pane identity when caller omits pane env', async () => {
      ...
      const spawnCall = spawnMock.mock.calls.at(-1)!
      expect(spawnCall[2].env.KINGU_PANE_KEY).toBeUndefined()
      expect(spawnCall[2].env.KINGU_TAB_ID).toBeUndefined()
      expect(spawnCall[2].env.KINGU_WORKTREE_ID).toBeUndefined()
    })
    ...
    it('preserves explicit child Kingu pane identity over parent env', async () => {
      ...
      const spawnCall = spawnMock.mock.calls.at(-1)!
      expect(spawnCall[2].env.KINGU_PANE_KEY).toBe('child-tab:child-leaf')
      expect(spawnCall[2].env.KINGU_TAB_ID).toBe('child-tab')
      expect(spawnCall[2].env.KINGU_WORKTREE_ID).toBe('child-worktree')
    })
  })
})
