...
describe('ensureShellReadyWrappersAt', () => {
  it('keeps required wrapper paths aligned with generated files', () => {
    const root = '/tmp/kingu-shell-ready'
  ...
  it('does not rewrite a tree that is already present', async () => {
    const userData = mkdtempSync(join(tmpdir(), 'kingu-warm-tree-'))
    try {
  ...
  it('regenerates a tree whose files went missing', async () => {
    const userData = mkdtempSync(join(tmpdir(), 'kingu-missing-tree-'))
    try {
      setTestUserDataPath(userData)
      const generation = await import('./local-pty-shell-ready-wrapper-generation')
      const { getShellReadyWrapperRoot: resolveRoot } =
...
describe('shell-ready wrapper root resolution', () => {
  // Why: daemon-entry fork is plain Node (no electron), so the wrapper root resolves from KINGU_USER_DATA_PATH, not app.getPath.
  it('resolves the wrapper root from KINGU_USER_DATA_PATH', async () => {
    const root = mkdtempSync(join(tmpdir(), 'kingu-userdata-env-'))
    try {
...
describePosix('local PTY shell-ready launch config', () => {
  ...
  let previousKinguOrigZdotdir: string | undefined

  beforeEach(() => {
    previousKinguOrigZdotdir = process.env.KINGU_ORIG_ZDOTDIR
    delete process.env.KINGU_ORIG_ZDOTDIR
    userDataPath = mkdtempSync(join(tmpdir(), 'local-pty-shell-ready-test-'))
    setTestUserDataPath(userDataPath)
  })

  afterEach(() => {
    if (previousKinguOrigZdotdir === undefined) {
      delete process.env.KINGU_ORIG_ZDOTDIR
    } else {
      process.env.KINGU_ORIG_ZDOTDIR = previousKinguOrigZdotdir
    }
  ...
  it('wraps fish launches with a fish_prompt shell-ready marker init command', async () => {
    ...
    expect(init).toContain('builtin printf "\\033]777;kingu-shell-ready\\007"')
    expect(init).toContain('functions -e __kingu_shell_ready_marker')
  })
  ...
  it('falls back to HOME for KINGU_ORIG_ZDOTDIR when inherited ZDOTDIR points at a wrapper dir', async () => {
    // Why: mirrors the daemon path — guards the same zsh recursion loop for renderer/local PTYs spawned inside an Kingu terminal.
    const previousZdotdir = process.env.ZDOTDIR
    const previousHome = process.env.HOME
    process.env.ZDOTDIR = '/some/other/kingu/shell-ready/zsh'
    process.env.HOME = '/Users/alice'
    try {
      const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
      const config = getShellReadyLaunchConfig('/bin/zsh')
      expect(config.env.KINGU_ORIG_ZDOTDIR).toBeUndefined()
    } finally {
  ...
  it('uses inherited KINGU_ORIG_ZDOTDIR when ZDOTDIR is an Kingu wrapper dir', async () => {
    const previousZdotdir = process.env.ZDOTDIR
    const previousOrigZdotdir = process.env.KINGU_ORIG_ZDOTDIR
    const previousHome = process.env.HOME
    const userZdotdir = makeUserZdotdir(userDataPath, '.config', 'zsh')
    process.env.ZDOTDIR = '/some/other/kingu/shell-ready/zsh'
    process.env.KINGU_ORIG_ZDOTDIR = userZdotdir
    process.env.HOME = userDataPath
    try {
      const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
      const config = getShellReadyLaunchConfig('/bin/zsh')
      expect(config.env.KINGU_ORIG_ZDOTDIR).toBe(userZdotdir)
    } finally {
      if (previousZdotdir === undefined) {
        delete process.env.ZDOTDIR
      } else {
      ...
      if (previousOrigZdotdir === undefined) {
        ...
        process.env.KINGU_ORIG_ZDOTDIR = previousOrigZdotdir
      }
      if (previousHome === undefined) {
        delete process.env.HOME
      } else {
  ...
  it('falls back to HOME when inherited KINGU_ORIG_ZDOTDIR points at a wrapper dir', async () => {
    const previousZdotdir = process.env.ZDOTDIR
    const previousOrigZdotdir = process.env.KINGU_ORIG_ZDOTDIR
    const previousHome = process.env.HOME
    delete process.env.ZDOTDIR
    process.env.KINGU_ORIG_ZDOTDIR = '/some/other/kingu/shell-ready/zsh'
    process.env.HOME = '/Users/alice'
    try {
      const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
      const config = getShellReadyLaunchConfig('/bin/zsh')
  ...
  it('writes a zsh hook that hands ZDOTDIR back before any user file loads', async () => {
    ...
    expect(zshenv).toContain('builtin export ZDOTDIR="$KINGU_ORIG_ZDOTDIR"')
    expect(zshenv).toContain('printf "\\033]777;kingu-shell-start:%s\\007" "$$"')
    // The handback is what makes a nested Kingu unable to inherit this dir, and
    // what stops /etc/zshrc deriving HISTFILE from it.
    expect(zshenv).toContain('builtin unset KINGU_ORIG_ZDOTDIR KINGU_ZSHENV_SOURCE_DIR')
    expect(zshenv.indexOf('builtin export ZDOTDIR=')).toBeLessThan(
      zshenv.indexOf('builtin source -- "$_kingu_user_zshenv"')
    )
  ...
  it('owns zle-line-init for the shell-ready marker instead of an azhw hook', async () => {
    ...
    const zshenv = readFileSync(join(getShellReadyWrapperRoot(), 'zsh', '.zshenv'), 'utf8')
    expect(zshenv).toContain('zle -N zle-line-init __kingu_prompt_mark')
    expect(zshenv).toContain('__kingu_prev_line_init_fn="${widgets[zle-line-init]#user:}"')
    expect(zshenv).toContain('printf "\\033]777;kingu-shell-ready\\007"')
    ...
    expect(zshenv).toContain('== "user:__kingu_prompt_mark"')
    expect(zshenv).toContain('__kingu_deferred_init')
  })
  ...
  it('writes wrappers without restoring Pi/OMP homes after user startup files', async () => {
    ...
    const agentTeamsPathRestoreLine = '[[ -n "${KINGU_AGENT_TEAMS_SHIM_DIR:-}" ]] || return 0'
    const ompWrapperLine = 'command omp --extension "${KINGU_OMP_STATUS_EXTENSION}" "$@"'
    ...
    expect(zshrc).not.toContain('KINGU_PI_CODING_AGENT_DIR')
    expect(zlogin).not.toContain('KINGU_PI_CODING_AGENT_DIR')
    expect(bashRc).not.toContain('KINGU_PI_CODING_AGENT_DIR')
    ...
    expect(zshrc).not.toContain('KINGU_OMP_CODING_AGENT_DIR')
    expect(zlogin).not.toContain('KINGU_OMP_CODING_AGENT_DIR')
    expect(bashRc).not.toContain('KINGU_OMP_CODING_AGENT_DIR')
    ...
    for (const wrapperFile of [zshrc, zlogin, bashRc]) {
      ...
      expect(wrapperFile).not.toContain('__kingu_prime_agent')
      expect(wrapperFile).not.toContain('KINGU_PRIME_AGENT_STATUS_EXTENSION')
  ...
  it('emits OSC 133 C/D markers in the bash wrapper so agent exit cleanup fires', async () => {
    ...
    expect(bashRc).toContain('[[ -z "${__kingu_in_command:-}" ]] || return 0')
    expect(bashRc).toContain('__kingu_prepend_prompt_command "__kingu_osc133_precmd"')
    // DEBUG is armed after setup; lastIndexOf skips the dispatcher's conditional re-arm.
    expect(bashRc.lastIndexOf("trap '__kingu_osc133_preexec' DEBUG")).toBeGreaterThan(
      bashRc.indexOf('if [[ -n "$__kingu_ready_marker" ]]; then')
    )
  ...
  itWithBash('emits lifecycle for foreground text ending like an internal hook', () => {
    const input = 'echo user:__kingu_osc133_prompt_done\nfalse\nexit 0\n'
    const output = runInteractiveBashRcfile(getBashShellReadyRcfileContent(), userDataPath, input)

    expect(output).toContain('user:__kingu_osc133_prompt_done')
    expectBashOsc133Lifecycle(output)
  })
  ...
  itWithBash(
    ...
    async () => {
      ...
      expect(output).not.toContain('USER_DEBUG_AFTER:<(( __kingu_exit_code == 0 ))>')
      expect(output).not.toContain('USER_DEBUG_AFTER:<__kingu_restore_prompt_status')
      expectBashOsc133Lifecycle(output)
    }
  )
  ...
  it('preserves a real inherited ZDOTDIR as KINGU_ORIG_ZDOTDIR', async () => {
    const previousZdotdir = process.env.ZDOTDIR
    const userZdotdir = makeUserZdotdir(userDataPath, '.config', 'zsh')
  ...
  it('sources the user .zshenv at wrapper top level, not inside a function', async () => {
    ...
    expect(zshenv).toContain('builtin source -- "$_kingu_user_zshenv"')
    ...
    expect(zshenv.indexOf('__kingu_deferred_init() {')).toBeLessThan(
      zshenv.indexOf('builtin source -- "$_kingu_user_zshenv"')
    )
  })
