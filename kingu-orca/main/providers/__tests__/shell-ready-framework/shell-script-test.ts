...
export async function shellScriptTest(
  ...
  try {
    ...
    const env: Record<string, string> = {
      ...
      KINGU_SHELL_FEATURES: (config.env.KINGU_SHELL_FEATURES ?? '')
        .split(',')
      ...
      HOME: testHome,
      KINGU_ORIG_ZDOTDIR: testHome,
      KINGU_ZSHENV_SOURCE_DIR: testHome
    }
...
const TEMP_PATH_PATTERN =
  /\/(?:var\/folders|tmp)\/[^\s]+?\/(?:shell-test|kingu|shell-ready)-[a-z]+-[a-z0-9-]+/g
