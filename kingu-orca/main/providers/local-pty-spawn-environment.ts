...
export function buildLocalPtySpawnEnvironment(args: {
  ...
  const spawnEnv: Record<string, string> = {
    ...
    // Why: supports-hyperlinks rejects TERM_PROGRAM=Kingu, so tools drop OSC 8 links; force it since xterm.js parses them.
    FORCE_HYPERLINK: '1',
    [KINGU_IMAGE_PROTOCOL_ENV]: KINGU_IMAGE_PROTOCOL_VALUE
  } satisfies Record<string, string>
  // Why: Kingu can be launched from an Kingu terminal; pane identity belongs to the child PTY, not the parent shell.
