/**
 * Byte-for-byte snapshots of every shell wrapper file Kingu generates, for all
...
/**
 * Every shell name the wrapper is allowed to write that is not Kingu-namespaced.
 *
 * Each is a deliberate contract with the shell or with Kingu's own features, not
...
function foreignGlobalsWritten(content: string): string[] {
  ...
  for (const line of content.split('\n')) {
    ...
    for (const name of [
      ...
      if (name && !/^_{0,2}kingu_/i.test(name) && !CONTRACT_GLOBALS.has(name)) {
        names.add(name)
      }
    }
  }
...
describePosix('generated shell wrapper files', () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'kingu-wrapper-snapshot-'))
  ...
  ])('%s wrappers write no shell global outside Kingu’s namespace', (_transport, generate, dir) => {
    generate()
