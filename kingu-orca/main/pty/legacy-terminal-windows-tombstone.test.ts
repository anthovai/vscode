...
describe('legacy terminal Windows tombstone text', () => {
  ...
  const makeUserDataDir = (): string => {
    const userData = mkdtempSync(join(tmpdir(), 'kingu-legacy-shim-win-'))
  ...
  it('rejects stale Windows real-command paths inside the wrapper directory', () => {
    ...
    expect(cmd).toContain('if defined kingu_real if not exist "%kingu_real%" set "kingu_real="')
    expectOrdered(cmd, 'if not exist "%kingu_real%"', ':kingu_try_candidate')
  ...
  it('resolves Windows fallbacks against PATH only, never the current directory', () => {
    ...
    for (const command of ['git', 'gh'] as const) {
      ...
      expect(cmd).toContain('for %%P in ("%kingu_clean_path:;=" "%") do (')
      expect(cmd).toContain(`if exist "%kingu_candidate_dir%\\${command}.exe"`)
      ...
      expect(cmd).toContain('set "kingu_sep=\\"')
      ...
      // Why: a captured KINGU_REAL_* may be relative, and both the existence test and the
      // invocation resolve it against the cwd.
      expect(cmd).toContain('if defined kingu_real call :kingu_check_rooted')
      expect(cmd).toContain('if defined kingu_real if not defined kingu_rooted set "kingu_real="')
      ...
      // Why the exact block, not an ordering pin: the first ':kingu_append_path' in the file is
      // the top-level CALL, and the first 'if not defined kingu_rooted' belongs to a different
      // subroutine, so an ordering pin stayed green with this subroutine's guard deleted.
      expect(cmd).toContain(':kingu_append_path\r\nif not defined kingu_entry exit /b\r\n')
      expect(cmd).toContain(
      ...
      expect(cmd).not.toContain('call :kingu_check_rooted "')
      expect(cmd).not.toContain('call :kingu_try_candidate "')
      expect(cmd).not.toContain('call :kingu_append_path "')
      expect(cmd).not.toContain('%~1')
      expect(cmd).toContain('set "kingu_probe=%kingu_entry%"\r\ncall :kingu_check_rooted\r\n')
      // Why: kingu_sep is compared against before the legacy dir is normalized; defining it later
      // made that strip silently no-op and left the legacy dir on PATH.
      expectOrdered(cmd, 'set "kingu_sep=', 'if "%kingu_legacy_norm:~-1%."')
      // Why: %kingu_sep% expands before the line is parsed, so `=="%kingu_sep%"` becomes `=="\"` —
      // the literal trap the variable exists to avoid. Both sides must carry the trailing dot.
      expect(cmd).not.toContain('=="%kingu_sep%"')
      ...
      expectOrdered(
        cmd,
        'call :kingu_check_rooted\r\nif not defined kingu_rooted exit /b',
        'for %%G in ("%kingu_entry%") do set "kingu_candidate_dir='
      )
      expect(cmd).toContain('if not defined kingu_rooted exit /b')
      expect(cmd).toContain('if "%kingu_candidate_dir:~-1%."=="%kingu_sep%."')
      expect(cmd).not.toContain(':\\#=#%')
      ...
      expect(cmd).toContain('if /I "%kingu_candidate_dir%\\"=="%kingu_wrapper_dir%" exit /b')
      ...
      expect(cmd).toContain('if "%kingu_probe:~1,2%"==":\\" set "kingu_rooted=1"')
      expect(cmd).toContain('if "%kingu_probe:~0,2%"=="\\\\" set "kingu_rooted=1"')
      ...
      expect(cmd).toContain('if not defined kingu_entry exit /b')
      ...
      expect(cmd).toContain('if "%kingu_legacy_norm:~-1%."=="%kingu_sep%."')
      ...
      expect(cmd).toContain('if defined kingu_legacy_wrapper_dir call :kingu_normalize_legacy_dir')
      expect(cmd).not.toContain('if defined kingu_legacy_wrapper_dir for ')
  ...
  it('guards both cmd PATH walks so an empty variable cannot break parsing', () => {
    ...
    for (const command of ['git', 'gh'] as const) {
      const cmd = readFileSync(join(win32Dir, `${command}.cmd`), 'utf8')
      expect(cmd).toContain('if not defined PATH goto :kingu_path_walked')
      expect(cmd).toContain('if not defined kingu_clean_path goto :kingu_candidates_walked')
      for (const [guard, loop] of [
        ['if not defined PATH goto :kingu_path_walked', 'for %%P in ("%PATH:;='],
        [
          'if not defined kingu_clean_path goto :kingu_candidates_walked',
          'for %%P in ("%kingu_clean_path:;='
        ]
      ] as const) {
