...
const POSIX_TOMBSTONE = String.raw`#!__KINGU_INTERPRETER__
set -u

command_name="__KINGU_COMMAND__"
...
legacy_wrapper_dir="${SHELL_DOLLAR}{KINGU_ATTRIBUTION_SHIM_DIR:-}"
...
filter_path() {
  ...
  while true; do
    ...
    if [[ "$entry" != /* ]]; then
      ...
      # directory, so a relative KINGU_ATTRIBUTION_SHIM_DIR let the cwd decide which PATH entry
...
filter_path
unset KINGU_ENABLE_GIT_ATTRIBUTION KINGU_GIT_COMMIT_TRAILER KINGU_GH_PR_FOOTER
unset KINGU_GH_ISSUE_FOOTER KINGU_ATTRIBUTION_SHIM_DIR KINGU_REAL_GIT KINGU_REAL_GH KINGU_ATTRIBUTION_BYPASS
...
if [[ -z "$real_command" ]]; then
  printf 'Kingu compatibility wrapper could not locate %s on PATH.\n' "$command_name" >&2
  exit 127
fi
...
export function renderLegacyTerminalPosixTombstone(
  ...
  return POSIX_TOMBSTONE.replaceAll('__KINGU_INTERPRETER__', interpreter).replaceAll(
    '__KINGU_COMMAND__',
    command
  )
}
