...
const DEV_COMMAND_NAME = 'kingu-dev'
...
/** Absolute path of the Kingu CLI the preflight must execute, or null to skip it.
 *
 *  Why absolute: the value rides in KINGU_CODEX_LAUNCH_PREFLIGHT and is invoked
 ...
 *  would be resolved against a PATH Kingu neither controls nor can predict —
 *  handing Kingu's managed Codex environment to an unidentified program. When no
...
set -l __kingu_codex_type (type -t codex 2>/dev/null)
if test -x "$KINGU_CODEX_LAUNCH_PREFLIGHT"; and test "$__kingu_codex_type" = file
  function codex
    command "$KINGU_CODEX_LAUNCH_PREFLIGHT" agent hooks prepare-codex >/dev/null 2>&1; or true
    command codex $argv
  end
end
set -e __kingu_codex_type`
}

export function getPowerShellCodexShellLaunchPreflight(): string {
  return `$kinguCodexCommand = Get-Command codex -ErrorAction SilentlyContinue | Select-Object -First 1
if ($env:KINGU_CODEX_LAUNCH_PREFLIGHT -and $kinguCodexCommand -and
    $kinguCodexCommand.CommandType -in @("Application", "ExternalScript")) {
    function Global:codex {
        try {
            & $env:KINGU_CODEX_LAUNCH_PREFLIGHT agent hooks prepare-codex *> $null
        } catch {
        }
        $kinguCodexExecutable = Get-Command codex -CommandType Application,ExternalScript -ErrorAction SilentlyContinue | Select-Object -First 1
        if (-not $kinguCodexExecutable) {
            Write-Error "codex executable not found"
            $global:LASTEXITCODE = 127
            return
        }
        & $kinguCodexExecutable.Source @args
        $global:LASTEXITCODE = $LASTEXITCODE
    }
}
Remove-Variable kinguCodexCommand -ErrorAction SilentlyContinue`
}
