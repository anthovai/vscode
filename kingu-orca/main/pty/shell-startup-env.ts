...
// Why: Kingu launches bash as a login shell (see local-pty-shell-ready.ts
...
// .bashrc. Scanning .bashrc would mirror values the live Kingu bash never sees.
...
function shellStartupFiles(
  ...
  if (!shell) {
    // Why: Kingu's POSIX default shell is /bin/zsh when $SHELL is unset.
  ...
  // Kingu's zsh/bash/fish shell-ready startup files, so scanning those files
 ...
 *
 * Why: GUI-launched Kingu does not inherit interactive shell exports, but the
