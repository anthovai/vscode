...
const LEGACY_TERMINAL_ATTRIBUTION_ENABLE_ENV_KEY = 'KINGU_ENABLE_GIT_ATTRIBUTION'
const LEGACY_TERMINAL_ATTRIBUTION_BYPASS_ENV_KEY = 'KINGU_ATTRIBUTION_BYPASS'

const LEGACY_SHIM_ROOT_DIR = 'kingu-terminal-attribution'
...
export const LEGACY_TERMINAL_SHIM_ENV_KEYS = [
  'KINGU_ENABLE_GIT_ATTRIBUTION',
  'KINGU_GIT_COMMIT_TRAILER',
  'KINGU_GH_PR_FOOTER',
  'KINGU_GH_ISSUE_FOOTER',
...
function writeFileAtomically(filePath: string, contents: string, mode: number): void {
  const temporaryPath = `${filePath}.kingu-neutralizing-${process.pid}`
  try {
...
export function isLegacyTerminalShimPathEntry(entry: string): boolean {
  // Why `windows` unconditionally here: this classifier only ever matches Kingu's own
  // `kingu-terminal-attribution/{posix,win32}` layout, and a Windows PATH can reach it through the
...
export function stripLegacyTerminalShimEnv(
  ...
  const shimDirKey = 'KINGU_ATTRIBUTION_SHIM_DIR'.toLowerCase()
  const explicitShimDirs = Object.entries(env)
    .filter(([key]) =>
      windows ? key.toLowerCase() === shimDirKey : key === 'KINGU_ATTRIBUTION_SHIM_DIR'
    )
    .map(([, value]) => value)
    .filter(Boolean)
