...
function makeTempDir(): string {
  const dir = mkdtempSync(join(tmpdir(), 'kingu-omp-node-pty-'))
  tempDirs.push(dir)
  return dir
}
...
function writeFakeOmp(binDir: string): void {
  ...
  writeFileSync(
    ompPath,
    `#!/bin/sh
agent_dir="\${PI_CODING_AGENT_DIR:-\${KINGU_FAKE_OMP_DEFAULT_DIR:-}}"
...
exit "\${KINGU_TEST_FAKE_OMP_EXIT_CODE:-0}"
`,
    { mode: 0o755 }
  )
...
async function runInteractivePosixPty(args: {
  ...
  const proc = pty.spawn(shell, shellArgs, {
    ...
    rows: 30,
    cwd: args.cwd,
    env: { ...args.env, KINGU_TEST_RCFILE: rcfile }
  })
  ...
  try {
    const input = shell === 'zsh' ? `source "$KINGU_TEST_RCFILE"\n${args.input}` : args.input
...
describePosix('OMP shell wrapper node-pty reproduction', () => {
  ...
  itWithBash('reproduces why restored shells miss OMP status without the wrapper', async () => {
    ...
    const statusExtension = join(extensionDir, 'kingu-agent-status.ts')
    ...
    const makeEnv = (captureFile: string, afterPiFile: string): Record<string, string> => ({
      ...
      KINGU_OMP_STATUS_EXTENSION: statusExtension,
      KINGU_FAKE_OMP_DEFAULT_DIR: ompDir,
      KINGU_CAPTURE_FILE: captureFile,
      KINGU_AFTER_PI_FILE: afterPiFile,
    ...
    await runInteractivePosixPty({
      ...
      input: `omp ask
printf '%s' "$PI_CODING_AGENT_DIR" > "$KINGU_AFTER_PI_FILE"
exit 0
`
    })
  ...
  itWithBash('runs OMP config subcommands without redirecting the home', async () => {
    ...
    await runInteractivePosixPty({
      ...
      env: {
        ...
        KINGU_PI_CODING_AGENT_DIR: '',
        KINGU_OMP_CODING_AGENT_DIR: '',
        KINGU_OMP_SOURCE_AGENT_DIR: sourceDir,
        KINGU_OMP_STATUS_EXTENSION: statusExtension,
        KINGU_FAKE_OMP_DEFAULT_DIR: sourceDir,
        KINGU_CAPTURE_FILE: captureFile,
        TERM: process.env.TERM || 'xterm-256color'
      },
      input: `omp config
    ...
    async () => {
      ...
      await runInteractivePosixPty({
        ...
        env: {
          ...
          KINGU_OMP_CODING_AGENT_DIR: '',
          KINGU_OMP_STATUS_EXTENSION: statusExtension,
          KINGU_FAKE_OMP_DEFAULT_DIR: defaultOmpDir,
          KINGU_CAPTURE_FILE: captureFile,
          TERM: process.env.TERM || 'xterm-256color'
        },
        input: `omp config
...
`
  ...
  async function expectStaleCwdRecovery(shell: PosixShell): Promise<void> {
    ...
    writeFileSync(
      scenarioFile,
      `KINGU_CAPTURE_FILE="$KINGU_UNSET_PWD_CAPTURE_FILE"
unset PWD
omp
__kingu_test_unset_status=$?
if [[ -z "\${PWD+x}" ]]; then
  __kingu_test_pwd_state=unset
else
  __kingu_test_pwd_state=set
fi
builtin cd -- "$KINGU_LOGICAL_PROJECT_LINK"
/bin/rm -- "$KINGU_LOGICAL_PROJECT_LINK"
KINGU_CAPTURE_FILE="$KINGU_DELETED_LINK_CAPTURE_FILE"
omp
__kingu_test_deleted_link_status=$?
builtin cd -P -- "$KINGU_STALE_PROJECT_DIR"
KINGU_CAPTURE_FILE="$KINGU_STALE_CAPTURE_FILE"
/bin/rm -rf -- "$KINGU_STALE_PROJECT_DIR"
/bin/mkdir -p -- "$KINGU_STALE_PROJECT_DIR"
omp
__kingu_test_first_status=$?
KINGU_CAPTURE_FILE="$KINGU_SKIP_CAPTURE_FILE"
omp --version
__kingu_test_skip_status=$?
if [[ "$PWD" -ef . ]]; then
  __kingu_test_parent_state=live
else
  __kingu_test_parent_state=stale
fi
unset PWD
KINGU_CAPTURE_FILE="$KINGU_STALE_UNSET_CAPTURE_FILE"
omp
__kingu_test_stale_unset_status=$?
unset KINGU_WORKTREE_PATH KINGU_ROOT_PATH
KINGU_CAPTURE_FILE="$KINGU_NO_LOGICAL_CAPTURE_FILE"
omp
__kingu_test_no_logical_status=$?
PWD="$KINGU_STALE_PROJECT_DIR"
/bin/rm -rf -- "$KINGU_STALE_PROJECT_DIR"
omp
__kingu_test_missing_status=$?
{
  echo "UNSET=$__kingu_test_unset_status"
  echo "PWD=$__kingu_test_pwd_state"
  echo "LINK=$__kingu_test_deleted_link_status"
  echo "FIRST=$__kingu_test_first_status"
  echo "SKIP=$__kingu_test_skip_status"
  echo "PARENT=$__kingu_test_parent_state"
  echo "STALE_UNSET=$__kingu_test_stale_unset_status"
  echo "NO_LOGICAL=$__kingu_test_no_logical_status"
  echo "MISSING=$__kingu_test_missing_status"
} > "$KINGU_RESULT_FILE"
exit 0
`
    )
...
${getPosixOmpShellWrapper()}`,
      env: {
        INPUTRC: '/dev/null',
        PROMPT_COMMAND: '',
        KINGU_STALE_PROJECT_DIR: projectDir,
        KINGU_LOGICAL_PROJECT_LINK: logicalProjectLink,
        KINGU_UNSET_PWD_CAPTURE_FILE: unsetPwdCaptureFile,
        KINGU_DELETED_LINK_CAPTURE_FILE: deletedLinkCaptureFile,
        KINGU_STALE_CAPTURE_FILE: staleCaptureFile,
        KINGU_SKIP_CAPTURE_FILE: skipCaptureFile,
        KINGU_STALE_UNSET_CAPTURE_FILE: staleUnsetCaptureFile,
        KINGU_NO_LOGICAL_CAPTURE_FILE: noLogicalCaptureFile,
        KINGU_WORKTREE_PATH: workspaceDir,
        HOME: homeDir,
        PATH: `${binDir}:/usr/bin:/bin:/usr/sbin:/sbin`,
        KINGU_OMP_STATUS_EXTENSION: statusExtension,
        KINGU_CAPTURE_FILE: staleCaptureFile,
        KINGU_RESULT_FILE: resultFile,
        KINGU_SCENARIO_FILE: scenarioFile,
        KINGU_TEST_FAKE_OMP_EXIT_CODE: '23',
        TERM: 'xterm-256color'
      },
      input: 'source "$KINGU_SCENARIO_FILE"\n'
    })
    ...
    expect(output).toContain('Kingu: OMP cannot access the terminal working directory')
  }
