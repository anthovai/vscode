...
import type { KingudHealth } from '../kingud/kingud-health'

const ready: ServeReadiness = {
  runtimeId: 'runtime-1',
  boundEndpoint: 'ws://0.0.0.0:6768',
  advertisedEndpoint: 'wss://kingu.example.test/runtime',
  managedWslCliReconciliation: 'settled',
  pairing: {
    available: true,
    url: 'kingu://pair?code=secret',
    endpoint: 'wss://kingu.example.test/runtime',
    deviceId: 'device-1',
    webClientUrl: 'https://kingu.example.test/runtime/web-index.html#pairing=secret',
    scope: 'runtime',
    qr: null
  }
}

const health: KingudHealth = {
...
describe('ServeReadinessPublisher', () => {
  it('writes one complete human-readable ready block', async () => {
    ...
    expect(write).toHaveBeenCalledWith(
      expect.stringContaining(
        'Kingu server ready\nBound endpoint: ws://0.0.0.0:6768\nAdvertised endpoint: wss://kingu.example.test/runtime'
      )
    )
    expect(write).toHaveBeenCalledWith(
      expect.stringContaining('Pairing URL: kingu://pair?code=secret\n')
    )
  })
  ...
  it('preserves the recipe JSON contract', () => {
    expect(renderServeReadiness(ready, { mode: 'recipe-json', projectRoot: '/workspace' })).toBe(
      '{"schemaVersion":1,"pairingCode":"kingu://pair?code=secret","projectRoot":"/workspace"}'
    )
  })
  ...
  it('says out loud when the daemon self-test failed', () => {
    ...
    expect(human).toContain('terminals survive an kingud restart: NO')
  })
