export function getOpenCode2SetupSource(): string[] {
  return String.raw`
// Why: OpenCode owns a form under a session id, and Kingu retires a blocker when
...
// here to drop forms Kingu could otherwise strand. OpenCode's own schema calls
...
async function setupOpenCode2Status(ctx) {
  ...
  // as an 'kingu-opencode-status' plugin failed error in the TUI, which is worse
  ...
  try {
    ...
    const hooks = await KinguOpenCodeStatusPlugin({ client });
    ...
    const consuming = consume().catch((error) => {
      if (!controller.signal.aborted) console.warn("[kingu-hook] event subscription failed:", error.message);
    });
    return async () => {
      try {
        controller.abort();
