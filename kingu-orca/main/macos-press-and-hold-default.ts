...
/**
 * Turns off the macOS accent picker for Kingu's own preferences domain (#14746).
 *
 ...
 * `defaults write com.anthovai.kingu ApplePressAndHoldEnabled -bool true` (or deletes the key), and
...
const KINGU_BUNDLE_ID = 'com.anthovai.kingu'
...
export type PressAndHoldDecision =
  ...
  | 'already-decided'
  /** The running bundle is not Kingu's (e.g. a bare `Electron.app`), whose domain we do not own. */
  | 'foreign-bundle'
...
/** Only Kingu's own bundle: an unpackaged run is `com.github.Electron`, shared with every other
 *  unpackaged Electron app on the machine. */
export function isKinguPreferencesDomain(domain: string): boolean {
  return domain === KINGU_BUNDLE_ID || domain.startsWith(`${KINGU_BUNDLE_ID}.`)
}
...
/**
 * Apply Kingu's press-and-hold default at most once, leaving any explicit user value alone.
 *
...
export function ensureMacPressAndHoldDefault(host: PressAndHoldHost): PressAndHoldDecision {
  ...
  if (!domain || !isKinguPreferencesDomain(domain)) {
