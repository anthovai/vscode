...
if (process.env.CODEX_HOME !== process.env.KINGU_EXPECTED_CODEX_HOME) {
...
describe('Codex rate-limit process contract', () => {
  beforeEach(() => {
    tempRoot = mkdtempSync(join(tmpdir(), 'kingu-codex-rate-limit-contract-'))
    stubPath = join(tempRoot, 'codex-contract.cjs')
    writeFileSync(stubPath, STUB_CODEX_SOURCE)
    previousExpectedHome = process.env.KINGU_EXPECTED_CODEX_HOME
    process.env.KINGU_EXPECTED_CODEX_HOME = join(tempRoot, 'managed-codex-home')
  ...
  afterEach(() => {
    if (previousExpectedHome === undefined) {
      delete process.env.KINGU_EXPECTED_CODEX_HOME
    } else {
      process.env.KINGU_EXPECTED_CODEX_HOME = previousExpectedHome
    }
  ...
  it('starts a read-only non-interactive app-server with the managed home', async () => {
    await expect(
      fetchCodexRateLimits({
        codexHomePath: process.env.KINGU_EXPECTED_CODEX_HOME,
