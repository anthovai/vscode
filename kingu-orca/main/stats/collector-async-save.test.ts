...
const statsPath = (): string => join(userDataDir, 'kingu-stats.json')
...
describe('StatsCollector async debounced save', () => {
  beforeEach(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'kingu-stats-test-'))
    gate.blocked = false
  ...
  it('writes valid JSON via the async fs/promises path on the debounced timer, no stray temp files', async () => {
    ...
    await vi.waitFor(() => {
      expect(readdirSync(userDataDir)).toContain('kingu-stats.json')
    })
