...
} from '../kingu-profiles/profile-storage-paths'
import { getKinguProfileIndexPath, readProfileIndex } from '../kingu-profiles/profile-index-store'

/**
 * Worktree ids owned by Kingu profiles OTHER than the running one.
 *
...
export function getOtherProfileWorktreeIdsForHistoryGc(userDataPath = getProfileUserDataPath()): {
  ...
  const index = readProfileIndex(getKinguProfileIndexPath(userDataPath))
  if (!index) {
  ...
  for (const profile of index.profiles) {
    ...
    const collected = readProfileWorktreeIds(getKinguProfileDataFile(profile.id, userDataPath))
    if (!collected) {
