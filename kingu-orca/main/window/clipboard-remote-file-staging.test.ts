...
const STAGING_ROOT = join(TEMP_ROOT, `kingu-clipboard-files${UID_SUFFIX}`)
...
const LEGACY_EXPIRED = `kingu-clipboard-file-${EXPIRED_TRANSFER}`
const LEGACY_FRESH = `kingu-clipboard-file-${FRESH_TRANSFER}`
...
describe('legacy remote clipboard staging compatibility', () => {
  ...
  it('removes only exact expired legacy children encountered inside the bound', async () => {
    opendirMock.mockResolvedValue(
      openedDirectory([
        directoryEntry('kingu-clipboard-file-lookalike'),
