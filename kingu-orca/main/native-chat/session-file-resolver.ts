...
import { resolveKinguManagedCodexHomePath } from '../codex/codex-home-paths'
import {
...
// Why: Kingu launches Codex with KINGU_CODEX_HOME pointing at its own managed
// runtime home, so Kingu-started Codex rollout files land under
...
// fall back to CODEX_HOME/~/.codex so a non-Kingu Codex transcript still resolves.
...
// Why: resolveKinguManagedCodexHomePath avoids the mkdirSync performed by the
// getter; creating the runtime home belongs to launch, not this resolve poll.
function codexSessionsDirs(): string[] {
  const candidates = [
    join(resolveKinguManagedCodexHomePath(), 'sessions'),
...
export type ResolveSessionFileOptions = {
   ...
   *  scans). Defaults to the kingu-managed home then CODEX_HOME/~/.codex. */
