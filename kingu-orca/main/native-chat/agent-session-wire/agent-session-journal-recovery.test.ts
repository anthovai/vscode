...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), 'kingu-wire-recovery-'))
...
describe('providerHistoryId', () => {
  it('uses the provider handle, never the Kingu session id', () => {
...
describe('openAgentSessionJournalWithRecovery', () => {
  ...
  // Kingu minted the submission, receipt and lifecycle identities; no provider
  ...
  it('rebuilds from provider history when the epoch row itself is gone', async () => {
    ...
    await journal.appendLifecycleBatch({
      ...
      mutations: [
        {
          kind: 'item',
          identity: { provider: 'kingu', clientMessageId: 'approval-1' },
