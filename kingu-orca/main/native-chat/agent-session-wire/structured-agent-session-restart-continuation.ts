 ...
 *
 * The send layer answers `ok: true` as soon as Kingu OWNS the message — a rejected or unverifiable
 ...
 *  waiter for the verdict, and the journal note that attributes the send to Kingu. */
...
function restartNoteWriter(
  ...
  return async (sessionId, text, tone) => {
    ...
    await session.journal.appendItem(
      { provider: 'kingu', clientMessageId: `restart-continuation:${sessionId}:${host.now()}` },
...
export type StructuredAgentSessionContinuationDeps = {
  ...
  send: (input: {
    ...
    ok: boolean
    refusal?: { code: string }
    /** The submission is where the provider's answer lives; the envelope only says Kingu took it. */
  ...
  /** Records a host-authored journal note: that this send was Kingu's, not the user's, or that the
...
async function sendContinuation(
  ...
  // The send result carries the dispatch as it stood when Kingu took the message, which for a normal
  ...
  // Only an accepted dispatch gets the note: it is a durable claim that Kingu asked this agent to
