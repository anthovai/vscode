...
export abstract class UpdaterInstallSupport extends UpdaterCheckState {
  ...
  protected deferHeadlessServeInstall(phase: 'download' | 'install', version: string): boolean {
    ...
    if (this.lastInstallDeferralVersion[phase] !== diagnosticVersion) {
      ...
      recordUpdaterLifecycle(
        ...
        {
          level: 'warn',
          message: 'Update install deferred while hosting kingu serve'
        }
      )
    }
    this.sendErrorStatus(
      'This kingu serve process was not started by an update-capable supervisor. Keep it running and update Kingu through its service manager.',
      true
    )
    return true
  }
  ...
  protected getPreCommitInstallFailureMessage(): string {
    return process.platform === 'darwin'
      ? 'Could not restart to install the update. Quit and reopen Kingu, then try again.'
      : 'Could not start the update installer. Kingu remains open.'
  }
