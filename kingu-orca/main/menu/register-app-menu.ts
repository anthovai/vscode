...
function buildAndApplyMenu(options: RegisterAppMenuOptions): void {
  ...
  const featureTourItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.exploreKingu', 'Explore Kingu'),
    click: (_menuItem, window) => onOpenFeatureTour(window)
  }

  const setupGuideItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.gettingStarted', 'Getting Started with Kingu'),
  ...
  // redundant "Kingu" entry with roles that don't apply, so we omit it there
    ...
    submenu: [
      ...
      }),
      {
        ...
        click: () => {
          ...
          // control, so raw Electron paste cannot know which Kingu surface owns it.
  ...
  const appearanceSubmenu: Electron.MenuItemConstructorOptions = {
    ...
    submenu: [
      ...
      {
        label: translateMain('menu.showMobileButton', 'Show Kingu Mobile Button'),
        type: 'checkbox',
