...
export async function createUninstallFixture() {
  const root = await mkdtemp(join(tmpdir(), 'kingu-plugin-uninstall-'))
  roots.push(root)
  let settings: {
  ...
  async function install(
    ...
    const manifest = {
      manifestVersion: 1,
      id,
      publisher: official ? 'anthovai' : 'memory-audit',
      name: id,
      version: '1.0.0',
      engines: { kingu: '>=1.0.0' },
      pluginApi: 1,
    ...
    await writeFile(join(sourcePath, 'kingu-plugin.json'), JSON.stringify(manifest))
