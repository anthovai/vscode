...
beforeAll(async () => {
  bundleRoot = await mkdtemp(join(tmpdir(), 'kingu-plugin-host-bundle-'))
...
async function createPluginSpec(
  source = `export default function activate(kingu) { kingu.commands.register('run', async () => ({ ok: true })); }`
): Promise<PluginWorkerSpawnSpec> {
  const rootDir = await mkdtemp(join(tmpdir(), 'kingu-plugin-supervision-'))
  pluginRoots.push(rootDir)
  await writeFile(join(rootDir, 'main.mjs'), source)
  return {
    pluginKey: 'kingu-samples.supervision',
    rootDir,
    mainEntry: 'main.mjs',
    grantedCapabilities: []
  }
  ...
  it('terminates and supervises a live worker that disconnects IPC', async () => {
    const spec = await createPluginSpec(`
      export default function activate(kingu) {
        kingu.commands.register('disconnect', async () => {
          process.disconnect?.()
          setInterval(() => {}, 1_000)
          await new Promise(() => {})
        })
