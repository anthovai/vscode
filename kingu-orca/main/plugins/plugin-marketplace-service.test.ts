...
async function tempRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'kingu-marketplace-service-'))
  roots.push(root)
  return root
}
...
describe('PluginMarketplaceService', () => {
  ...
  it('rejects reserved identities outside the official organization', async () => {
    const service = new PluginMarketplaceService({
      ...
      fetcher: async () =>
        fetched(
          marketplace('Attack', 'community.kingu-secrets', 'https://github.com/attacker/x.git')
        )
    })

    await expect(service.addSource(source())).rejects.toThrow(
      'reserved plugin identity community.kingu-secrets'
    )
    await expect(service.listSources()).resolves.toEqual([])
  })
  ...
  it('derives the Official badge only from the canonical marketplace and source organization', async () => {
    const officialMarketplace: PluginMarketplace = {
      ...
      plugins: [
        {
          id: 'anthovai.kingu-shortcuts',
          source: {
            kind: 'git',
            url: 'git@github.com:anthovai/kingu-intelligence-shortcuts.git',
            ref: 'main'
          },
    ...
    await service.addSource(source('https://github.com/anthovai/kingu-intelligence-plugins.git'))

    await expect(service.listPlugins()).resolves.toEqual([
      expect.objectContaining({ pluginKey: 'anthovai.kingu-shortcuts', official: true })
    ])
  })
  ...
  it('seeds the official marketplace once and keeps it configured across restarts', async () => {
    ...
    const officialMarketplace = marketplace(
      'Kingu Plugins',
      'anthovai.kingu-notes',
      'https://github.com/anthovai/kingu-intelligence-notes.git'
    )
    officialMarketplace.owner = 'anthovai'
    ...
    await expect(first.seedOfficialSource()).resolves.toMatchObject({
      official: true,
      marketplace: { name: 'Kingu Plugins' }
    })
