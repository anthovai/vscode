...
async function pluginRoot(pluginManifest = manifest()): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'kingu-plugin-reconcile-'))
  roots.push(root)
  await writeFile(join(root, 'kingu-plugin.json'), JSON.stringify(pluginManifest))
  ...
  const service = new PluginService({
    ...
    getPluginKillListEntry: (key) =>
      killed && key === pluginKey
        ? { pluginKey, reason: 'Security incident', advisoryUrl: 'https://kingu.example/advisory' }
        : null,
...
describe('PluginService worker reconciliation', () => {
  it('blocks every runtime surface until a saved override resolves a content conflict', async () => {
    ...
    const service = new PluginService({
      ...
      getDisabledPlugins: () => [],
      getPluginConsents: () => ({
        'kingu-samples.first': fingerprintPluginConsent(firstManifest, firstHash.hash),
        'kingu-samples.second': fingerprintPluginConsent(secondManifest, secondHash.hash)
      }),
    ...
    expect(service.activationError('kingu-samples.first')).toContain('conflicts')
    expect(service.getGrantedCapabilities('kingu-samples.first')).toBeNull()
    await expect(service.invokeCommand('kingu-samples.first', 'run')).rejects.toThrow('not enabled')
    await expect(service.panels.readEntry('kingu-samples.first', 'panel')).resolves.toBeNull()
    ...
    keybindings = { 'plugin:kingu-samples.first/run': ['Mod+Shift+T'] }
    await service.reconcileActivationState()

    expect(service.activationError('kingu-samples.first')).toBeNull()
    await expect(service.panels.readEntry('kingu-samples.first', 'panel')).resolves.toMatchObject({
      html: expect.stringContaining('<h1>Panel</h1>')
    })
    await expect(service.invokeCommand('kingu-samples.first', 'run')).resolves.toBeNull()
