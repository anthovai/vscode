...
describe('writePluginFileAtomically', () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'kingu-plugin-atomic-'))
  })
...
describe('renamePluginFileWithWindowsRetry', () => {
  it('renames when the source exists', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'kingu-plugin-rename-'))
    try {
  ...
  describe('on Windows', () => {
    ...
    beforeEach(async () => {
      dir = await mkdtemp(join(tmpdir(), 'kingu-plugin-rename-win-'))
  ...
  it('does not retry off Windows', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'kingu-plugin-rename-posix-'))
