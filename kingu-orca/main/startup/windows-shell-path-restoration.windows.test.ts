...
describe.runIf(process.platform === 'win32')('Windows shell PATH restoration', () => {
  ...
  beforeEach(() => {
    _resetHydrateShellPathCache()
    fixtureRoot = mkdtempSync(join(tmpdir(), 'kingu-shell-path-'))
    ...
    writeFileSync(join(shellADir, 'kingu-path-probe.cmd'), '@echo shell-a\r\n')
    writeFileSync(join(shellBDir, 'kingu-path-probe.cmd'), '@echo shell-b\r\n')
  })
