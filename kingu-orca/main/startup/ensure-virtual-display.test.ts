...
describe('ensureVirtualDisplayForHeadlessServe', () => {
  ...
  // leaves a lockless socket on Kingu's OWN :99. Adopting it would resurrect the orphan-socket bug.
  ...
  // A root-owned stale :99 socket (crashed system Xvfb, serve running as User=kingu) cannot be
  ...
  describe('hasUsableLinuxDisplay', () => {
    ...
    // Kingu's own teardown unlinks the lock before the socket, so a lockless :99 is our own
