...
// our own file idempotently but never clobber a user's own ~/.local/bin/kingu.
const DISPATCHER_MARKER = '# kingu-serve-bare-kingu-dispatcher'

export type LinuxBareKinguDispatcherOptions = {
  /** Packaged app resources root; the bundled `kingu-ide` launcher lives under it. */
  ...
  /** Test seam — defaults to $XDG_CACHE_HOME/kingu/appimage. */
...
export type LinuxBareKinguDispatcherState =
  | 'installed'
  | 'skipped-foreign'
  | 'skipped-launcher-missing'

export type LinuxBareKinguDispatcherResult = {
  state: LinuxBareKinguDispatcherState
  dispatcherPath: string
  /** The bundled `kingu-ide` launcher the dispatcher execs. */
  target: string | null
}

// Why: on Linux the CLI installs as `kingu-ide`, not bare `kingu`, to avoid
// shadowing GNOME Kingu's /usr/bin/kingu. But the Claude Team launcher typed into
// the initial managed terminal invokes the literal `kingu claude-teams`, so a
// headless serve box needs a bare-`kingu` dispatcher on the managed-terminal PATH
...
// never reclaims it.
export async function installLinuxBareKinguDispatcher(
  options: LinuxBareKinguDispatcherOptions
): Promise<LinuxBareKinguDispatcherResult> {
  const dispatcherPath = join(options.homePath ?? homedir(), '.local', 'bin', 'kingu')
  ...
  const installed = await publishDispatcher(
    dispatcherPath,
    insertDispatcherMarker(buildBareKinguCliScript(launcher))
  )
  return installed
...
/** Bare-`kingu` script that execs the one Linux CLI launcher. */
export function buildBareKinguCliScript(launcherPath: string): string {
...
async function publishDispatcher(dispatcherPath: string, content: string): Promise<boolean> {
  ...
  const temporaryPath = join(directoryPath, `.kingu-dispatcher-${process.pid}-${randomUUID()}`)
  ...
  try {
    ...
    const displacedPath = join(
      directoryPath,
      `.kingu-preserved-dispatcher-${process.pid}-${randomUUID()}`
    )
    try {
