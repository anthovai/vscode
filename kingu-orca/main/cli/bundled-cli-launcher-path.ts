...
// Why `kingu-ide` on Linux: GNOME Kingu ships /usr/bin/kingu, so the CLI never claims that name.
export const LINUX_CLI_COMMAND_NAME = 'kingu-ide'
...
export function getBundledLauncherPath(
  ...
  if (platform === 'darwin') {
    return join(resourcesPath, 'bin', 'kingu')
  }
  if (platform === 'linux') {
    return join(resourcesPath, 'bin', LINUX_CLI_COMMAND_NAME)
  }
  if (platform === 'win32') {
    return join(resourcesPath, 'bin', 'kingu.exe')
  }
  return null
}
