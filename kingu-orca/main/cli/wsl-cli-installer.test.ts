...
function makeHostStatus(
  launcherPath = 'C:\\Users\\me\\AppData\\Local\\Programs\\Kingu\\resources\\bin\\kingu.exe'
) {
  return {
    platform: 'win32',
    commandName: 'kingu',
    commandPath: launcherPath,
    pathDirectory: 'C:\\Users\\me\\AppData\\Local\\Programs\\Kingu\\resources\\bin',
    pathConfigured: true,
    launcherPath,
...
set -euo pipefail
# Kingu managed WSL CLI launcher
# KINGU_WIN_LAUNCHER_B64=QzpcUHJvZ3JhbSBGaWxlc1xPcmNhXHJlc291cmNlc1xiaW5cb3JjYS5jbWQ=
KINGU_WIN_LAUNCHER='C:\\Program Files\\Kingu\\resources\\bin\\kingu.cmd'
KINGU_BRIDGE_PS1='/home/alice/.local/share/kingu/kingu-wsl-bridge.ps1'
if command -v powershell.exe >/dev/null 2>&1; then
  KINGU_POWERSHELL=powershell.exe
elif [ -x /mnt/c/Windows/System32/WindowsPowerShell/v1.0/powershell.exe ]; then
  KINGU_POWERSHELL=/mnt/c/Windows/System32/WindowsPowerShell/v1.0/powershell.exe
else
  echo "Kingu WSL CLI requires Windows interop and could not find powershell.exe." >&2
  exit 1
fi
KINGU_BRIDGE_PS1_WIN=$(wslpath -w "$KINGU_BRIDGE_PS1")
exec "$KINGU_POWERSHELL" -NoProfile -ExecutionPolicy Bypass -File "$KINGU_BRIDGE_PS1_WIN" "$KINGU_WIN_LAUNCHER" "$@"
`
...
function createWslRunner(
  ...
  const commandPath = '/home/alice/.local/bin/kingu-ide'
  const legacyCommandPath = '/home/alice/.local/bin/kingu'
  const bridgePath = '/home/alice/.local/share/kingu/kingu-wsl-bridge.ps1'
  ...
  const runner = vi.fn(async (_distro: string, command: string) => {
    ...
    if (command.includes('cat > "$command_tmp"')) {
      ...
      if (
        ...
        !files.get(bridgePath)?.includes('# Kingu managed WSL CLI PowerShell bridge')
      ) {
        throw new Error('__KINGU_CONFLICT__')
      }
      const launcher =
        command.match(/cat > "\$command_tmp" <<'KINGU_WSL_CLI'\n([\s\S]*)\nKINGU_WSL_CLI/)?.[1] ??
        ''
      const bridge =
        command.match(
          /cat > "\$bridge_tmp" <<'KINGU_WSL_BRIDGE'\n([\s\S]*)\nKINGU_WSL_BRIDGE/
        )?.[1] ?? ''
      files.set(commandPath, launcher)
      files.set(bridgePath, bridge)
      if (files.get(legacyCommandPath)?.includes('# Kingu managed WSL CLI launcher')) {
    ...
    if (command.includes('rm -f')) {
      ...
      if (
        ...
        files.get(legacyCommandPath)?.includes('# Kingu managed WSL CLI launcher')
      ) {
        files.delete(legacyCommandPath)
      }
      return ''
    ...
    if (command.includes('cat ')) {
      if (command.includes(commandPath)) {
        return files.get(commandPath) ?? '__KINGU_MISSING__'
      }
      if (command.includes(bridgePath)) {
        return files.get(bridgePath) ?? '__KINGU_MISSING__'
      }
      if (command.includes(legacyCommandPath)) {
        return files.get(legacyCommandPath) ?? '__KINGU_MISSING__'
      }
    }
...
describe('WslCliInstaller', () => {
  ...
  it('installs a WSL launcher that forwards to the Windows Kingu launcher', async () => {
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'not_installed',
      commandPath: '/home/alice/.local/bin/kingu-ide'
    })
    ...
    expect(wsl.getFile()).toBe(
      _internals.buildWslLauncher(
        'C:\\Users\\me\\AppData\\Local\\Programs\\Kingu\\resources\\bin\\kingu.exe',
        '/home/alice/.local/share/kingu/kingu-wsl-bridge.ps1'
      )
    )
    ...
    expect(installCommand).toContain("legacy_command_path='/home/alice/.local/bin/kingu'")
  ...
  it('continues checking WSL when the host launcher exists but host PATH is unknown', async () => {
    ...
    const hostStatus = {
      ...makeHostStatus(),
      pathConfigured: null,
      detail: 'Kingu could not read the Windows user PATH registry value.'
    } satisfies CliInstallStatus
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
  ...
  it('derives the shared WSL bridge path for current and legacy command names', () => {
    expect(_internals.getBridgePathFromCommandPath('/home/alice/.local/bin/kingu-ide')).toBe(
      '/home/alice/.local/share/kingu/kingu-wsl-bridge.ps1'
    )
    expect(_internals.getBridgePathFromCommandPath('/home/alice/.local/bin/kingu')).toBe(
      '/home/alice/.local/share/kingu/kingu-wsl-bridge.ps1'
    )
  })
  ...
  it('reports installed WSL launchers whose bin directory is missing from PATH', async () => {
    ...
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
      hostInstaller: { getStatus: async () => makeHostStatus('C:\\Kingu\\kingu.cmd') },
      wslRunner: wsl.runner
    })
  ...
  it('accepts current managed WSL scripts with an extra heredoc trailing newline', async () => {
    ...
    const installer = new WslCliInstaller({
      ...
      wslRunner: async (distro, command) => {
        if (command.includes('cat /home/alice/.local/share/kingu/kingu-wsl-bridge.ps1')) {
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'installed',
      currentTarget: 'C:\\Kingu\\kingu.cmd'
    })
  })
  ...
  it('generates a launcher that forwards arguments through a PowerShell file bridge', () => {
    const launcher = _internals.buildWslLauncher(
      'C:\\Program Files\\Kingu\\resources\\bin\\kingu.exe',
      '/home/alice/.local/share/kingu/kingu-wsl-bridge.ps1'
    )
    ...
    expect(launcher).toContain(
      'Kingu WSL CLI requires Windows interop and could not find powershell.exe.'
    )
    expect(launcher).toContain('"$KINGU_POWERSHELL" -NoProfile -ExecutionPolicy Bypass -File')
    expect(launcher).toContain('KINGU_WSL_CWD=$(pwd -P 2>/dev/null) || {')
    expect(launcher).toContain('KINGU_WSL_CWD=/')
    expect(launcher).toContain('cd /')
    expect(launcher).toContain('KINGU_WSL_CWD_WIN=$(wslpath -w "$KINGU_WSL_CWD")')
    expect(launcher.indexOf('KINGU_WSL_CWD=$(pwd -P')).toBeLessThan(
      launcher.indexOf('KINGU_BRIDGE_PS1_WIN=$(wslpath')
    )
    expect(launcher).toContain('"$KINGU_WIN_LAUNCHER" -WslCwd "$KINGU_WSL_CWD_WIN" "$@"')
    ...
    expect(bridge).toContain('[string]$KinguLauncher = $args[0]')
    ...
    expect(bridge).toContain('$env:KINGU_CLI_CWD = $WslCwd')
    expect(bridge).toContain('$LauncherDirectory = Split-Path -Parent $KinguLauncher')
    ...
    expect(bridge).not.toContain('& $KinguLauncher @ForwardArgs')
    expect(bridge).toContain('Remove-Item Env:KINGU_CLI_CWD -ErrorAction SilentlyContinue')
  ...
  it('marks stale managed launchers that point at the old app bin instead of packaged resources', async () => {
    const oldLauncher = _internals.buildWslLauncher(
      'C:\\Users\\me\\AppData\\Local\\Programs\\Kingu\\bin\\kingu.cmd',
      '/home/alice/.local/share/kingu/kingu-wsl-bridge.ps1'
    )
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'stale',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\Kingu\\bin\\kingu.cmd',
      launcherPath: 'C:\\Users\\me\\AppData\\Local\\Programs\\Kingu\\resources\\bin\\kingu.exe'
    })

    await expect(installer.install()).resolves.toMatchObject({
      state: 'installed',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\Kingu\\resources\\bin\\kingu.exe'
    })
  })

  it('repairs the frozen pre-rc4 registration so orchestration send/reply reach native rc4', async () => {
    const nativeLauncher = 'C:\\Program Files\\Kingu\\resources\\bin\\kingu.exe'
    ...
    const simulateRc4Launch = (args: string[]): number => {
      ...
      return target?.toLowerCase().endsWith('kingu.cmd') &&
  ...
  it('repairs a managed launcher whose bridge is missing, but preserves a conflicting bridge', async () => {
    const nativeLauncher = 'C:\\Kingu\\resources\\bin\\kingu.exe'
    ...
    const staleBridge = createWslRunner(PRE_RC4_MANAGED_WSL_LAUNCHER, true, {
      initialBridge: '# Kingu managed WSL CLI PowerShell bridge\nWrite-Output "stale"\n'
    })
  ...
  it('moves a legacy-only managed registration to kingu-ide without touching unmanaged names', async () => {
    const nativeLauncher = 'C:\\Kingu\\resources\\bin\\kingu.exe'
  ...
  it('keeps the pre-rc4 files on a transactional replacement failure', async () => {
    ...
    const installer = new WslCliInstaller({
      ...
      distro: 'Ubuntu',
      hostInstaller: {
        getStatus: async () => makeHostStatus('C:\\Program Files\\Kingu\\resources\\bin\\kingu.exe')
      },
      wslRunner: wsl.runner
    })
    ...
    // would leave a window where a concurrent shell finds no kingu-ide at all.
    expect(installCommand).not.toContain('command_backup')
    expect(installCommand).not.toContain(`mv -f '/home/alice/.local/bin/kingu-ide'`)
  })

  it.skipIf(process.platform === 'win32')(
    'rolls both files back when the command replacement fails after the bridge move',
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'kingu-wsl-cli-rollback-'))
      const home = join(root, 'home with spaces')
      const commandPath = join(home, '.local', 'bin', 'kingu-ide')
      const bridgePath = join(home, '.local', 'share', 'kingu', 'kingu-wsl-bridge.ps1')
      const bridge = _internals.buildWslBridgeScript()
      await mkdir(join(home, '.local', 'bin'), { recursive: true })
      await mkdir(join(home, '.local', 'share', 'kingu'), { recursive: true })
      ...
      const installer = new WslCliInstaller({
        ...
        hostInstaller: {
          getStatus: async () =>
            makeHostStatus('C:\\Program Files\\Kingu\\resources\\bin\\kingu.exe')
        },
        wslRunner: runner
      })
  ...
  it('is idempotent after repairing an old managed registration', async () => {
    const nativeLauncher = 'D:\\Custom Kingu\\resources\\bin\\kingu.exe'
    const wsl = createWslRunner(PRE_RC4_MANAGED_WSL_LAUNCHER)
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
    ...
    expect(wsl.getFile()).toContain(
      "KINGU_WIN_LAUNCHER='D:\\Custom Kingu\\resources\\bin\\kingu.exe'"
    )
  })
  ...
  it('refuses to remove an old managed launcher when the bridge path is user-owned', async () => {
    ...
    await expect(installer.remove()).rejects.toThrow('__KINGU_CONFLICT__')
  })
})
