...
export async function inspectStableCommand(
  ...
  throw new Error(`The command at ${commandPath} changed while Kingu inspected it.`)
}
...
export async function quarantineCommandPath(commandPath: string): Promise<CommandQuarantine> {
  ...
  const directoryPath = join(commandDirectory, `.kingu-cli-${process.pid}-${randomUUID()}`)
...
export function buildMacPrivilegedSymlinkTransaction(
  ...
  const transactionDirectory = join(commandDirectory, `.kingu-cli-${process.pid}-${randomUUID()}`)
