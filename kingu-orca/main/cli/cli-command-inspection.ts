...
// previous packaged Kingu and is ours to reclaim; anything else stays a conflict.
const PACKAGED_LINUX_LAUNCHER_DIRECTORIES = ['/opt/Kingu', '/opt/kingu-ide', '/opt/kingu']
...
export class CliCommandInspection extends CliInstallLocation {
    ...
    try {
      ...
      if (!stats.isSymbolicLink()) {
        if (stats.isFile()) {
          ...
          if (managedTarget) {
            return this.buildStatus({
              ...
              detail: `${commandPath} contains an older Kingu launcher.`
            })
          }
        }
        ...
        return this.buildStatus({
          ...
          state: 'conflict',
          currentTarget: null,
          detail: `${commandPath} exists but is not an Kingu symlink.`
        })
      }
      ...
      return this.buildStatus({
        ...
        detail: isInstalled
          ...
          : isManagedStaleTarget
            ? `${commandPath} points to an older Kingu launcher.`
            : `${commandPath} points to a non-Kingu launcher.`
      })
    } catch (error) {
      if (isMissingError(error)) {
        return this.buildStatus({
          ...
          state: 'not_installed',
          currentTarget: null,
          detail: `Register ${commandPath} to use Kingu from the terminal.`
        })
      }
      throw error
    }
  ...
  protected isManagedSymlinkTarget(resolvedTarget: string, launcherPath: string): boolean {
    ...
    if (this.platform === 'darwin') {
      // Why: reclaim symlinks to an older Kingu.app launcher, but never replace arbitrary user-owned symlinks.
  ...
  /** A launcher inside a packaged Linux install tree, left behind by a deb/rpm Kingu. */
  ...
  protected isSiblingDevLauncherTarget(
    ...
    // Why: dev builds generate launchers under the sibling `*-dev` profile; packaged Kingu must reclaim that command.
    return (
    ...
    try {
      ...
      if (!stats.isFile()) {
        return this.buildStatus({
          ...
          state: 'conflict',
          currentTarget: null,
          detail: `${commandPath} exists but is not an Kingu launcher script.`
        })
      }
      ...
      if (isMissingError(error)) {
        return this.buildStatus({
          ...
          state: 'not_installed',
          currentTarget: null,
          detail: `Register ${commandPath} to use Kingu from Command Prompt or PowerShell.`
        })
      }
      throw error
    }
