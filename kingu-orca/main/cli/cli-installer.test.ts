...
async function fakeAppImageExtractRunner(_appImagePath: string, cwd: string): Promise<void> {
  ...
  await writeFile(join(launcherDir, 'kingu-ide'), '#!/usr/bin/env bash\n', {
    encoding: 'utf8',
    mode: 0o755
  })
}
...
describe('CliInstaller', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const installPath = join(fixture.root, 'bin', 'kingu')
      ...
      expect(initial.launcherPath).toContain(join('userData', 'cli', 'bin', 'kingu'))
      ...
      expect(launcherContent).toContain(`export KINGU_USER_DATA_PATH='${fixture.userDataPath}'`)
      expect(launcherContent).toContain('export KINGU_APP_EXECUTABLE="$ELECTRON"')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const fixture = await makeFixture()
      const installPath = join(fixture.root, '.local', 'bin', 'kingu-ide')
      const installer = new CliInstaller({
        platform: 'linux',
        isPackaged: false,
        userDataPath: fixture.userDataPath,
      ...
      const installed = await installer.install()
      expect(installed.state).toBe('installed')
      expect(installed.commandName).toBe('kingu-ide')
  ...
  // packaged `kingu` / `kingu-ide` commands developers rely on day to day.
  it.skipIf(process.platform === 'win32')(
    'uses a separate kingu-dev command for default development installs',
    async () => {
      const fixture = await makeFixture()
      ...
      const installed = await installer.install()
      expect(installed.state).toBe('installed')
      expect(installed.commandName).toBe('kingu-dev')
      expect(installed.commandPath).toBe(join(commandDir, 'kingu-dev'))
      expect(installed.launcherPath).toBe(join(fixture.userDataPath, 'cli', 'bin', 'kingu-dev'))
      await expect(readlink(installed.commandPath as string)).resolves.toBe(installed.launcherPath)
      await expect(
        readFile(join(fixture.userDataPath, 'cli', 'bin', 'kingu'), 'utf8')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const installPath = join(commandDir, 'kingu-ide')
      const appImagePath = join(fixture.root, 'Kingu.AppImage')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const commandDir = join(fixture.root, '.local', 'bin')
      const installPath = join(commandDir, 'kingu-ide')
      const appImagePath = join(fixture.root, "Kingu's AppImage.AppImage")
      const cacheRootPath = join(fixture.root, 'cache')
  ...
  // Why: Linux renamed the public command to avoid shadowing GNOME Kingu, so
  // upgrading must clean up only the old symlink owned by prior Kingu installs.
  it.skipIf(process.platform === 'win32')(
    'removes the old managed linux kingu symlink when installing kingu-ide',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const commandDir = join(homePath, '.local', 'bin')
      ...
      const oldLauncherPath = join(resourcesPath, 'bin', 'kingu')
      const legacyCommandPath = join(commandDir, 'kingu')
      await mkdir(commandDir, { recursive: true })
      ...
      const installed = await installer.install()
      expect(installed.commandPath).toBe(join(commandDir, 'kingu-ide'))
      await expect(lstat(legacyCommandPath)).rejects.toMatchObject({ code: 'ENOENT' })
    }
  )

  it.skipIf(process.platform === 'win32')(
    'removes a legacy linux kingu symlink when registering from an AppImage',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const commandDir = join(homePath, '.local', 'bin')
      ...
      const extractedRoot = resolveAppImageExtractedRoot({ appImagePath, cacheRootPath })!
      await symlink(join(dirname(extractedRoot.payloadLauncherPath), 'kingu'), legacyCommandPath)
  ...
  it.skipIf(process.platform === 'win32' || process.getuid?.() === 0)(
    ...
    async () => {
      ...
      const installPath = join(protectedDir, 'bin', 'kingu')
