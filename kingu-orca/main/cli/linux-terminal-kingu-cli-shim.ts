...
import { buildBareKinguCliScript } from './linux-bare-kingu-dispatcher'
import { quoteShell } from './cli-install-path-format'

const SHIM_DIR_NAME = 'linux-kingu-cli-shim'

export type LinuxTerminalKinguCliShimOptions = {
  userDataPath: string
...
// Why: on Linux the CLI installs as `kingu-ide` so it never shadows the GNOME
// Kingu screen reader at /usr/bin/kingu — but agent-facing surfaces (skills,
// dispatch preambles, CLI hints) all invoke bare `kingu`, so on stock Ubuntu an
// agent inside an Kingu terminal would launch the screen reader instead
// (anthovai/kingu-intelligence#7904). Prepending this userData-scoped shim dir to managed-PTY
// PATH makes bare `kingu` resolve to the Kingu CLI inside Kingu terminals only,
// leaving the user's own shells (and their screen reader) untouched.
export function ensureLinuxTerminalKinguCliShimDir(
  options: LinuxTerminalKinguCliShimOptions
): string | null {
  const resourcesPath =
...
fail() {
  printf 'Kingu CLI is unavailable; reopen Kingu or register the CLI again.\\n' >&2
  exit 1
}
...
function ensureShimForLauncher(userDataPath: string, launcherPath: string): string | null {
  const script = buildBareKinguCliScript(launcherPath)
