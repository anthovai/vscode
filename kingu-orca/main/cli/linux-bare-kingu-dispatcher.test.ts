...
import { installLinuxBareKinguDispatcher } from './linux-bare-kingu-dispatcher'
...
async function makeFixture(): Promise<{ homePath: string; resourcesPath: string }> {
  const root = await mkdtemp(join(tmpdir(), 'kingu-bare-dispatcher-'))
  created.push(root)
  const resourcesPath = join(root, 'resources')
  // The bundled kingu-ide launcher must exist for the dispatcher to be written.
  await mkdir(join(resourcesPath, 'bin'), { recursive: true })
  await writeFile(join(resourcesPath, 'bin', 'kingu-ide'), '#!/usr/bin/env bash\n', 'utf8')
...
describe('installLinuxBareKinguDispatcher', () => {
  it('uses the mounted bundled launcher when only APPDIR is inherited', async () => {
    ...
    const result = await installLinuxBareKinguDispatcher({ resourcesPath, homePath })

    expect(result.state).toBe('installed')
    expect(result.target).toBe(join(resourcesPath, 'bin', 'kingu-ide'))
  })

  it('writes an executable bare-kingu dispatcher that execs the bundled kingu-ide launcher', async () => {
    const { homePath, resourcesPath } = await makeFixture()

    const result = await installLinuxBareKinguDispatcher({
      resourcesPath,
      homePath,
      appImagePath: null
    })

    const expectedTarget = join(resourcesPath, 'bin', 'kingu-ide')
    expect(result.state).toBe('installed')
    expect(result.target).toBe(expectedTarget)
    expect(result.dispatcherPath).toBe(join(homePath, '.local', 'bin', 'kingu'))
  ...
  it('is idempotent — a second install rewrites its own dispatcher without throwing', async () => {
    ...
    const first = await installLinuxBareKinguDispatcher({
      resourcesPath,
      homePath,
      appImagePath: null
    })
    const second = await installLinuxBareKinguDispatcher({
      resourcesPath,
      homePath,
      appImagePath: null
    })
  ...
  it('publishes when the home filesystem does not support hard links', async () => {
    ...
    await expect(readFile(result.dispatcherPath, 'utf8')).resolves.toContain(
      '# kingu-serve-bare-kingu-dispatcher'
    )
  })

  it('restores a displaced foreign dispatcher when hard links are unsupported', async () => {
    const { homePath, resourcesPath } = await makeFixture()
    await installLinuxBareKinguDispatcher({ resourcesPath, homePath, appImagePath: null })
  ...
  it('quotes a resources path containing spaces so the exec line cannot be split', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kingu-bare-dispatcher-space-'))
    created.push(root)
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const { homePath, resourcesPath } = await makeFixture()
      const appImagePath = join(homePath, 'Kingu.AppImage')
      ...
      const result = await installLinuxBareKinguDispatcher({
        ...
        appImageExtractRunner: async (_appImagePath, cwd) => {
          ...
          await writeFile(join(launcherDir, 'kingu-ide'), '', { encoding: 'utf8', mode: 0o755 })
        }
      })
  ...
  it('skips (does not clobber) a user-owned kingu already at ~/.local/bin', async () => {
    const { homePath, resourcesPath } = await makeFixture()
    const dispatcherPath = join(homePath, '.local', 'bin', 'kingu')
    const appImagePath = join(homePath, 'Kingu.AppImage')
  ...
  it('preserves a foreign dispatcher created while AppImage extraction is in flight', async () => {
    ...
    const installation = installLinuxBareKinguDispatcher({
      resourcesPath,
      homePath,
      appImagePath,
      appImageCacheRootPath: cacheRootPath,
  ...
  it('prunes old owner generations without touching a sibling namespace', async () => {
    ...
    await installLinuxBareKinguDispatcher(options)
    ...
    const installation = installLinuxBareKinguDispatcher(options)
  ...
  it('skips when the bundled kingu-ide launcher is missing from the build', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kingu-bare-dispatcher-nolauncher-'))
    created.push(root)
...
async function writePayload(cwd: string): Promise<void> {
  ...
  await writeFile(join(launcherDirectory, 'kingu-ide'), '#!/usr/bin/env bash\n', { mode: 0o755 })
}
