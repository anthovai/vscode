...
export type RemoteKinguCliRequest = {
  argv: string[]
  cwd: string
...
export type RemoteKinguCliResult = {
  stdout: string
  stderr: string
  exitCode: number
...
export type RemoteKinguCliPostOutput =
  | {
...
export function buildHostCliEnv(args: {
  ...
  env.KINGU_NODE_OPTIONS = args.hostEnv.NODE_OPTIONS ?? ''
  env.KINGU_NODE_REPL_EXTERNAL_MODULE = args.hostEnv.NODE_REPL_EXTERNAL_MODULE ?? ''
...
export async function runHostKinguCliPassthrough(
  ...
  try {
    ...
    // to (see index.ts KinguRuntimeRpcServer wiring), or the CLI subprocess
    // reports "Kingu is not running" against a healthy app.
  ...
  if (!entryExists(cliEntryPath)) {
    throw new HostCliUnavailableError(`Kingu CLI entry not found at ${cliEntryPath}`)
  }
  ...
  return await new Promise<RemoteKinguCliResult>((resolve, reject) => {
    let settled = false
    ...
    const killTimer = setTimeout(() => {
      ...
      resolve({
        ...
        stderr: `${stderr.toString()}Kingu CLI bridge timed out after ${killTimeoutMs}ms on the host.\n`,
        exitCode: 1
      })
    }, killTimeoutMs)
    killTimer.unref?.()
    ...
    child.on('error', (err) => {
      ...
      reject(
        new HostCliUnavailableError(`Failed to launch the Kingu CLI on the host: ${err.message}`)
      )
    })
...
class CappedOutputCollector {
  ...
  toString(): string {
    ...
    return this.truncated ? `${text}\n[kingu ssh cli] output truncated\n` : text
  }
}
