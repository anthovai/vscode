...
export function searchSshConfigHosts(
  ...
  for (const entry of hosts) {
    ...
    const alreadyInKingu = existingAliases.has(normalizedAlias)
    // Why: tombstones only block passive bulk import — the picker still lists the
    // Host so deleting one Kingu target never looks like "~/.ssh/config is empty".
    const previouslyRemoved = !alreadyInKingu && suppressedAliasSet.has(normalizedAlias)
    totalHostCount += 1
    // Why: "Add all" must match importFromSshConfig without reAdopt (tombstones stay).
    newHostCount += alreadyInKingu || previouslyRemoved ? 0 : 1
    if (!matchesQuery(entry, normalizedQuery)) {
      continue
    }
    matchCount += 1
    if (summaries.length < SSH_CONFIG_HOST_RESULT_LIMIT) {
      summaries.push(toSummary(entry, alreadyInKingu, previouslyRemoved))
    }
  }
