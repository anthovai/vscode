/**
 * What a client does when it finds both a relay and an kingud installed on one host.
 *
 * `docs/design/shipping-kingud.html` §06 draws the line the boundary doc actually draws:
 ...
 * "an kingud dir exists, so prefer kingud" — would let a GC pass, a half-finished install or
 ...
 */
export type RemoteHostRegistration = 'ssh-target' | 'kingud-peer' | 'both' | 'none'
...
export function selectRemoteInstallModel(input: {
  ...
  /** Raw directory names under `~/.kingu-remote/`, as listed on the host. */
  ...
  if (input.registration === 'both') {
    return {
      ...
      reason:
        'This machine is registered both as an SSH target and as a paired kingud peer. One ' +
  ...
  if (input.registration === 'none') {
    return {
      ...
      reason:
        ...
        'as an kingud peer; what is already installed on it does not decide which it is.'
    }
  }
  const model: RemoteInstallModelId = input.registration === 'ssh-target' ? 'relay' : 'kingud'
  const inventory = inventoryRemoteInstallDirs(input.installedDirNames)
  const coexisting = model === 'relay' ? inventory.kingud : inventory.relay
  return {
    outcome: 'use',
    model,
    coexisting,
    note:
      coexisting.length > 0
        ? `This host also has ${coexisting.length} ${model === 'relay' ? 'kingud' : 'relay'} ` +
