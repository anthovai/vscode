...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'kingud-shell-'))
  dataDir = join(root, '.kingu')
  snapshotDir = join(root, 'snapshots', 'pre-0.2.0+bb01-1000')
  versionDir = join(root, '.kingu-remote', 'kingud-0.2.0+bb01')
  mkdirSync(join(dataDir, 'profiles', 'p1'), { recursive: true })
  mkdirSync(join(dataDir, 'daemon'), { recursive: true })
  mkdirSync(versionDir, { recursive: true })
  writeFileSync(join(dataDir, 'kingu-profile-index.json'), '{"v":"before"}')
  writeFileSync(join(dataDir, 'profiles', 'p1', 'kingu-data.json'), '{"repos":"before"}')
...
describe('state snapshot commands, run for real', () => {
  it('captures, then restores state the newer build overwrote', () => {
    ...
    expect(sh(probeKingudStateSnapshotCommand(host, snapshotDir)).trim()).toBe('PRESENT')

    // The new version migrates the store and adds a file of its own.
    writeFileSync(join(dataDir, 'kingu-profile-index.json'), '{"v":"migrated"}')
  ...
  it('leaves the live daemon runtime dir untouched through capture and restore', () => {
    ...
    sh(restoreKingudStateSnapshotCommand(host, dataDir, snapshotDir))
  ...
  it('reports EMPTY on a data root with nothing to lose, instead of an archive of nothing', () => {
    ...
    expect(
      parseKingudSnapshotCapture(
        sh(captureKingudStateSnapshotCommand(host, emptyRoot, snapshotDir))
      )
    ).toBe('empty')
    expect(sh(probeKingudStateSnapshotCommand(host, snapshotDir)).trim()).toBe('ABSENT')
  })

  it('reports MISSING rather than claiming a restore it did not perform', () => {
    expect(
      parseKingudSnapshotRestore(
        sh(restoreKingudStateSnapshotCommand(host, dataDir, join(root, 'nope')))
      )
    ).toBe('missing')
  })
  ...
  it('survives a data root whose path contains a quote and a space', () => {
    ...
    writeFileSync(join(nasty, 'kingu-profile-index.json'), '{"v":"quoted"}')
    expect(
    ...
    expect(readFileSync(join(nasty, 'kingu-profile-index.json'), 'utf8')).toBe('{"v":"quoted"}')
  })
})
...
describe('liveness and stop commands, run for real', () => {
  ...
  it('reports LIVE for a running process and stops it with SIGTERM', async () => {
    ...
    try {
      writeFileSync(join(versionDir, KINGUD_PID_FILENAME), String(child.pid))
      expect(parseKingudLiveness(sh(kingudLivenessProbeCommand(host, versionDir)))).toBe('LIVE')
