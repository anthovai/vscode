...
internal static class KinguRemoteCliLauncher
{
    private static int Main(string[] args)
    {
        try
        {
            string nodePath = RequireEnvironmentVariable("KINGU_RELAY_NODE_PATH");
            string relayDirectory = RequireEnvironmentVariable("KINGU_RELAY_DIR");
            string socketPath = RequireEnvironmentVariable("KINGU_RELAY_SOCKET_PATH");
            string credentialFile = Environment.GetEnvironmentVariable("KINGU_RELAY_CREDENTIAL_FILE");
            ...
            {
                Console.Error.WriteLine("Kingu SSH CLI bridge cannot find Node.js at \"{0}\"", nodePath);
                return 1;
            }
            if (!File.Exists(relayPath))
            {
                Console.Error.WriteLine("Kingu SSH CLI bridge cannot find the relay at \"{0}\"", relayPath);
                return 1;
            }
        ...
        {
            Console.Error.WriteLine("Unable to start the Kingu SSH CLI bridge: {0}", error.Message);
            return 1;
        }
    }
    ...
    private static string BuildArguments(string relayPath, string socketPath, string credentialFile, string[] args)
    {
        ...
        AppendArgument(commandLine, "--kingu-cli");
...
function createWindowsLauncherCompileCommand(
  ...
  return powerShellCommand(
    [
      ...
      `if (-not (Test-Path -LiteralPath ${powerShellLiteral(launcherPath)} -PathType Leaf)) { Write-Error 'The Kingu SSH CLI launcher compiler produced no executable.'; exit 1 }`,
      // Why: remove the legacy %* bridge only after a successful compile, so a
      // host missing csc.exe keeps its existing CLI (kingu.exe shadows kingu.cmd).
...
export function createRemoteCliInstallPlan(env: RemoteCliInstallEnv): RemoteCliInstallPlan {
  if (isWindowsRemoteHost(env.hostPlatform)) {
    const launcherFileName = 'kingu.exe'
    const sourceFileName = 'kingu-launcher.cs'
    ...
    const legacyShimPath = joinRemotePath(env.hostPlatform, env.binDir, 'kingu.cmd')
  ...
  const launcherPath = joinRemotePath(env.hostPlatform, env.binDir, 'kingu')
  return {
    launcherPath,
    files: [
      {
        ...
        contents: [
          '#!/usr/bin/env sh',
          'set -eu',
          `KINGU_RELAY_NODE_PATH=\${KINGU_RELAY_NODE_PATH:-${quoteSh(env.nodePath)}}`,
          `KINGU_RELAY_DIR=\${KINGU_RELAY_DIR:-${quoteSh(env.relayDir)}}`,
          `KINGU_RELAY_SOCKET_PATH=\${KINGU_RELAY_SOCKET_PATH:-${quoteSh(env.sockPath)}}`,
          `KINGU_RELAY_CREDENTIAL_FILE=\${KINGU_RELAY_CREDENTIAL_FILE:-${quoteSh(env.credentialFile ?? `${env.sockPath}.credential`)}}`,
          'if [ ! -S "$KINGU_RELAY_SOCKET_PATH" ]; then',
          '  echo "Kingu SSH CLI bridge cannot find the relay socket: $KINGU_RELAY_SOCKET_PATH" >&2',
          '  exit 1',
          'fi',
          'exec "$KINGU_RELAY_NODE_PATH" "$KINGU_RELAY_DIR/relay.js" --sock-path "$KINGU_RELAY_SOCKET_PATH" --credential-file "$KINGU_RELAY_CREDENTIAL_FILE" --kingu-cli "$@"',
          ''
        ].join('\n')
      }
    ],
