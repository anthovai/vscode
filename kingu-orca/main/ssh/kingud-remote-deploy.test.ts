...
vi.mock('./kingud-local-build-hash', () => ({
  computeLocalKingudBuildHash: () => 'abc123def4567890'
}))

import { execCommand } from './ssh-relay-deploy-helpers'
import { acquireInstallLock } from './ssh-relay-install-lock'
import { uploadRelayDirectory, writeRelayFile } from './ssh-relay-install-transfers'
import { deployKingud, type KingudDeployOptions } from './kingud-remote-deploy'
import { emptyKingudActivationRecord, withActivatedVersion } from './kingud-activation-record'
...
function scriptHost(script: HostScript): void {
  mockExec.mockImplementation(async (_conn, command: string) => {
    ...
    if (text.startsWith('cat ') && text.includes('kingud-active.json')) {
      return script.activationRecord
    }
    if (text.includes('.kingud-readiness') && text.startsWith('cat ')) {
...
function options(overrides: Partial<KingudDeployOptions> = {}): KingudDeployOptions {
  return {
    conn: {} as SshConnection,
    host: getRemoteHostPlatform('linux-x64'),
    remoteHome: '/home/u',
    localKingudDir: '/local/out/kingud',
...
const ACTIVE_OLD = JSON.stringify(
  withActivatedVersion(emptyKingudActivationRecord(), OLD_VERSION, null, new Date(0))
)

describe('deployKingud', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('installs under the kingud namespace, not the relay one', async () => {
    ...
    scriptHost(script)
    await deployKingud(options())
    expect(vi.mocked(acquireInstallLock).mock.calls[0][1]).toBe(
      `/home/u/.kingu-remote/kingud-${NEW_VERSION}`
    )
    expect(vi.mocked(uploadRelayDirectory).mock.calls[0][2]).toContain(`kingud-${NEW_VERSION}`)
  })
  ...
  it('installs but does not activate when terminals are running', async () => {
    ...
    scriptHost(script)
    const result = await deployKingud(
  ...
  it('does not write the activation record when the candidate fails its health gate', async () => {
    ...
    scriptHost(script)
    const result = await deployKingud(options())
    expect(result).toMatchObject({ code: 'kingud_activation_daemon_degraded' })
    expect(
      vi
        .mocked(writeRelayFile)
        .mock.calls.some((call) => String(call[2]).endsWith('kingud-active.json'))
    ).toBe(false)
  })
  ...
  it('puts the previous version back after a rejected candidate, rather than leaving the host down', async () => {
    ...
    expect(result.outcome === 'installed-not-activated' && result.reason).toContain(
      `kingud ${OLD_VERSION} was restarted and is serving again`
    )
  })
  ...
  it('refuses to activate when a different build answered the port', async () => {
    ...
    scriptHost(script)
    const result = await deployKingud(options())
    expect(result).toMatchObject({ code: 'kingud_activation_build_mismatch' })
  })
  ...
  it('refuses to treat an unreadable activation record as an empty one', async () => {
    ...
    scriptHost(script)
    await expect(deployKingud(options())).rejects.toThrow('activation record')
