...
import { evaluateKingudActivation } from './kingud-activation-gate'
import type { ServeReadiness } from '../server/serve-readiness'
import type { KingudHealth, TerminalDaemonHealth } from '../kingud/kingud-health'
...
function daemon(overrides: Partial<TerminalDaemonHealth> = {}): TerminalDaemonHealth {
  return {
    ...
    entryPath: '/home/u/.kingu-remote/kingud-0.2.0+bb01/daemon-entry.js',
    protocolVersion: 3,
    cgroupUnit: null,
...
function health(overrides: Partial<KingudHealth> = {}): KingudHealth {
  return {
...
describe('evaluateKingudActivation', () => {
  it('activates a candidate that proved a real PTY round trip', () => {
    const verdict = evaluateKingudActivation(readiness(), EXPECTED)
  ...
  it('refuses when the candidate never published readiness', () => {
    const verdict = evaluateKingudActivation(null, EXPECTED)
    expect(verdict).toMatchObject({ decision: 'reject', code: 'kingud_activation_no_readiness' })
  })
  ...
  it('refuses a readiness payload with no health, rather than reading silence as healthy', () => {
    ...
    const verdict = evaluateKingudActivation(withoutHealth as ServeReadiness, EXPECTED)
    expect(verdict).toMatchObject({ decision: 'reject', code: 'kingud_activation_no_health' })
  })

  it('refuses when a different build answered — a stale process holding the port', () => {
    const verdict = evaluateKingudActivation(
      readiness({ health: health({ buildHash: '0000000000000000' }) }),
      EXPECTED
    )
    expect(verdict).toMatchObject({ decision: 'reject', code: 'kingud_activation_build_mismatch' })
  })

  it('refuses a listening kingud whose terminal daemon is absent', () => {
    const verdict = evaluateKingudActivation(
      readiness({
        health: health({
          terminalDaemon: daemon({
    ...
    expect(verdict).toMatchObject({ decision: 'reject', code: 'kingud_activation_daemon_absent' })
  })
  ...
  it('refuses a degraded daemon, whose fresh terminals would not survive a restart', () => {
    ...
    expect(verdict).toMatchObject({ decision: 'reject', code: 'kingud_activation_daemon_degraded' })
  })
  ...
  it('refuses a candidate that is not listening', () => {
    const verdict = evaluateKingudActivation(readiness({ boundEndpoint: null }), EXPECTED)
    expect(verdict).toMatchObject({ decision: 'reject', code: 'kingud_activation_not_listening' })
  })
  ...
  it('checks identity before health, so a wrong-build green payload cannot pass', () => {
    ...
    expect(verdict).toMatchObject({ code: 'kingud_activation_build_mismatch' })
  })
})
