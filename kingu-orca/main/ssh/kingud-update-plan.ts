/**
 * When an update may restart kingud, and when going back is still sound.
 *
 * Two constraints shape everything here.
 *
 * **The daemon must outlive the restart.** kingud forks the terminal daemon and deliberately
 * does not kill it on stop (`kingud-daemon-supervision.ts` uses `disconnectDaemon`, never
 ...
 *
 * **The state root is shared across versions.** `~/.kingu/` (or `$KINGU_USER_DATA`) is outside
 * every version dir, and Kingu's persisted state carries no schema version — migrations run
 ...
 */
import type { KingudActivationRecord } from './kingud-activation-record'

export type KingudTerminalCensus = {
  /**
...
export type KingudUpdateDecision =
  ...
  | { action: 'defer'; code: KingudUpdateDeferCode; reason: string }

export type KingudUpdateDeferCode =
  | 'kingud_update_terminals_running'
  | 'kingud_update_terminal_census_unavailable'

/**
 * Decide whether to restart kingud onto `candidateVersion`.
 *
 * Deferring on live terminals is a deliberate choice, not caution. The restart itself is
 * non-destructive, but it leaves the host running a NEW kingud against an OLD daemon until
...
export function planKingudUpdate(input: {
  ...
  if (liveSessions === null) {
    if (!input.force) {
      return {
        action: 'defer',
        code: 'kingud_update_terminal_census_unavailable',
        reason:
  ...
  if (liveSessions > 0 && !input.force) {
    return {
      action: 'defer',
      code: 'kingud_update_terminals_running',
      reason:
        ...
        `kingud ${input.candidateVersion} against a daemon forked from ` +
...
export type KingudRollbackSafety =
  | { safety: 'clean'; target: string; notes: string[] }
  | { safety: 'lossy'; target: string; discards: string[] }
  | { safety: 'unsafe'; code: KingudRollbackUnsafeCode; reason: string }

export type KingudRollbackUnsafeCode =
  | 'kingud_rollback_no_target'
  | 'kingud_rollback_snapshot_missing'
  | 'kingud_rollback_orphans_live_terminals'
  | 'kingud_rollback_census_unavailable'
 ...
 */
export function assessKingudRollback(input: {
  record: KingudActivationRecord
...
}): KingudRollbackSafety {
  const target = input.record.previous
  if (!target) {
    return {
      safety: 'unsafe',
      code: 'kingud_rollback_no_target',
      reason:
        'This host has no previous kingud version recorded, so there is nothing to roll back ' +
  ...
  if (!input.record.snapshot || !input.snapshotPresent) {
    return {
      safety: 'unsafe',
      code: 'kingud_rollback_snapshot_missing',
      reason:
        `The pre-activation state snapshot for ${input.record.active ?? 'the active version'} ` +
        'is gone, and Kingu state carries no schema version that could prove the older build ' +
  ...
  if (startedSinceActivation === null) {
    return {
      safety: 'unsafe',
      code: 'kingud_rollback_census_unavailable',
      reason:
  ...
  if (startedSinceActivation > 0) {
    return {
      safety: 'unsafe',
      code: 'kingud_rollback_orphans_live_terminals',
      reason:
