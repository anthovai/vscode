...
vi.mock('./relay-protocol', () => ({
  ...
  RELAY_SENTINEL: 'KINGU-RELAY v0.1.0 READY\n',
...
vi.mock('./ssh-relay-deploy-helpers', () => ({
  ...
  execCommand: vi.fn().mockResolvedValue('__KINGU_REMOTE_PLATFORM__ Linux x86_64')
}))
...
vi.mock('./ssh-relay-versioned-install', () => ({
  ...
  computeRemoteRelayDir: (home: string, v: string) => `${home}/.kingu-remote/relay-${v}`,
...
describe('deployAndLaunchRelay', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.mocked(execCommand).mockReset().mockResolvedValue('__KINGU_REMOTE_PLATFORM__ Linux x86_64')
  ...
  it('calls exec to detect remote platform', async () => {
    ...
    mockExecCommand.mockResolvedValueOnce('__KINGU_REMOTE_PLATFORM__ Linux x86_64') // tagged POSIX platform probe
    mockExecCommand.mockResolvedValueOnce('/home/user') // echo $HOME
    mockExecCommand.mockResolvedValueOnce('KINGU-NATIVE-DEPS-OK') // native deps probe
    ...
    expect(mockExecCommand).toHaveBeenCalledWith(
      conn,
      "printf '\\n%s ' '__KINGU_REMOTE_PLATFORM__'; uname -sm",
  ...
  it('reports progress via callback', async () => {
    const conn = makeMockConnection()
    const mockExecCommand = vi.mocked(execCommand)
    mockExecCommand.mockResolvedValueOnce('__KINGU_REMOTE_PLATFORM__ Linux x86_64')
  ...
  it('does not launch fresh after an unconfirmed endpoint-incumbent probe', async () => {
    ...
    vi.mocked(execCommand)
      .mockResolvedValueOnce('__KINGU_REMOTE_PLATFORM__ Linux x86_64')
      .mockResolvedValueOnce('/home/user')
      .mockResolvedValueOnce('KINGU-NATIVE-DEPS-OK')
  ...
  it('resolves the remote node path once per deploy', async () => {
    ...
    mockExecCommand.mockResolvedValueOnce('__KINGU_REMOTE_PLATFORM__ Linux x86_64')
    mockExecCommand.mockResolvedValueOnce('/home/user')
    mockExecCommand.mockResolvedValueOnce('KINGU-NATIVE-DEPS-OK')
    queueLaunchNamespaceAndDeadSocketProbe()
  ...
  it('uses a content-hashed versioned remote install directory', async () => {
    ...
    const sawVersionedDir = allCmds.some((cmd) =>
      cmd.includes('/.kingu-remote/relay-0.1.0+abcdef012345')
    )
    ...
    try {
      ...
      vi.mocked(execCommand).mockImplementation((_conn, command) => {
        if (command.includes('uname')) {
          return Promise.resolve('__KINGU_REMOTE_PLATFORM__ Linux x86_64')
        }
        if (command === 'echo $HOME') {
          return Promise.resolve('/home/user')
        }
        const marker = command.match(/\.sftp-namespace-[0-9a-f]{32}/u)?.[0]
        if (command.includes('__KINGU_UPLOAD_STAGE_SLOT__') && marker) {
          return Promise.resolve(`__KINGU_UPLOAD_STAGE_SLOT__${marker}:slot-0`)
        }
    ...
    try {
      ...
      mockExecCommand
        ...
        .mockImplementationOnce(
          () =>
            new Promise<string>((resolve) =>
              setTimeout(() => resolve('KINGU-NATIVE-DEPS-OK'), 899_900)
            )
        )
        .mockResolvedValueOnce('') // launch namespace marker
  ...
  it('uses distinct target-specific relay socket paths', async () => {
    ...
    mockExecCommand.mockImplementation((_conn, command) => {
      if (command.includes('__KINGU_REMOTE_PLATFORM__')) {
        return Promise.resolve('__KINGU_REMOTE_PLATFORM__ Linux x86_64')
      }
      if (command === 'echo $HOME') {
        return Promise.resolve('/home/user')
      }
      if (command.includes('KINGU-NATIVE')) {
        return Promise.resolve('KINGU-NATIVE-DEPS-OK')
      }
  ...
  it('launches Windows remotes via a named pipe endpoint', async () => {
    ...
    mockExecCommand
      ...
      .mockResolvedValueOnce('__KINGU_REMOTE_PLATFORM__ Windows X64') // tagged PowerShell platform probe
      .mockResolvedValueOnce('C:\\Users\\me user') // remote home
      .mockResolvedValueOnce('KINGU-NATIVE-DEPS-OK') // native deps probe
    ...
    expect(result.sockPath).toMatch(/^\\\\\.\\pipe\\kingu-relay-[0-9a-f]{20}$/)
    ...
    expect(launchScript).toContain(
      '"C:/Users/me user/.kingu-remote/relay-0.1.0+abcdef012345/relay.js"'
    )
    expect(launchScript).toContain(
      '"C:/Users/me user/.kingu-remote/relay-0.1.0+abcdef012345/agent-hooks/kingu-relay-'
    )
  ...
  it('relaunches Windows remotes on a fallback pipe when reconnecting the occupied pipe fails', async () => {
    ...
    expect(primaryPipe).toMatch(/^\\\\\.\\pipe\\kingu-relay-[0-9a-f]{20}$/)
    expect(fallbackPipe).toMatch(/^\\\\\.\\pipe\\kingu-relay-[0-9a-f]{20}$/)
  ...
  it('prefers a persisted Windows fallback pipe on later reconnects', async () => {
    const conn = makeMockConnection()
    const mockExecCommand = vi.mocked(execCommand)
    const persistedPipe = '\\\\.\\pipe\\kingu-relay-1234567890abcdef1234'
    vi.mocked(resolveRemoteNodePath).mockResolvedValue('C:/Program Files/nodejs/node.exe')
    mockExecCommand
      .mockRejectedValueOnce(new Error('uname not found')) // tagged POSIX platform probe
      .mockResolvedValueOnce('__KINGU_REMOTE_PLATFORM__ Windows X64') // tagged PowerShell platform probe
  ...
  it('scopes persisted Windows active pipe markers by relay target', async () => {
    ...
    mockExecCommand
      ...
      .mockResolvedValueOnce('__KINGU_REMOTE_PLATFORM__ Windows X64')
