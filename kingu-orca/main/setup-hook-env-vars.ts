...
// Why: kept in sync with pty/wsl-kingu-env.ts, which path-translates the same keys for WSL.
// KINGU_WORKSPACE_NAME is a display name and the credential-guard policy is an enum — never paths.
...
export function getSetupEnvVars(repo: Repo, worktreePath: string): Record<string, string> {
  return {
    KINGU_ROOT_PATH: repo.path,
    KINGU_WORKTREE_PATH: worktreePath,
    KINGU_WORKSPACE_NAME: getRuntimePathBasename(worktreePath),
