...
export async function runBrowserRouteH3EgressProbe(
  ...
  const root = mkdtempSync(join(tmpdir(), 'kingu-browser-h3-egress-'))
...
function electronArgs(protectedSession: boolean, forcedQuicPort: number): string[] {
  ...
  if (protectedSession) {
    // Why: the guest arm launches with the exact list Kingu ships, so this proves the shipped switch, not a bare flag.
