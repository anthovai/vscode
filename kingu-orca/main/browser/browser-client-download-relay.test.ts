...
describe('BrowserClientDownloadRelay', () => {
  it('streams the staged file to the remote and reports the remote destination', async () => {
    ...
    const transport = negotiatedTransport((_method, params) => {
      ...
      return chunk.final
        ? { accepted: true, workspaceRelativePath: '.kingu/browser-downloads/report.pdf' }
        : { accepted: true }
    })
    ...
    expect(destination).toEqual({
      workspaceRelativePath: '.kingu/browser-downloads/report.pdf',
      hostLabel: 'build-box'
    })
  ...
  it('leaves nothing on disk under the real staging root once transfers settle', async () => {
    const root = await realpath(await mkdtemp(path.join(tmpdir(), 'kingu-download-relay-')))
