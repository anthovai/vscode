...
let activeKinguProfileId: string | null = null
      ...
      return new PairedRuntimeBrowserClientHostComposition({
        ...
        createExecutor: (next, { retainNetworkRoute, onPageUnavailable }) => {
          executor = new BrowserClientPageCommandExecutor({
            kinguProfileId: next.kinguProfileId,
...
export function configurePairedRuntimeBrowserClientHostsForKinguProfile(options: {
  kinguProfileId: string
}): void {
  if (activeKinguProfileId && activeKinguProfileId !== options.kinguProfileId) {
    throw new Error('paired_runtime_browser_client_host_profile_conflict')
  }
  activeKinguProfileId = options.kinguProfileId
}
...
export async function startPairedRuntimeBrowserClientHost(options: {
  ...
  const kinguProfileId = activeKinguProfileId
  if (!kinguProfileId) {
...
function browserClientFileStagingRoot(environmentId: string): string {
  ...
  return path.join(app.getPath('temp'), 'kingu-browser-file-channel', scope)
}
