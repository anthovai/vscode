...
const BIN = '/opt/kingu/agent-browser'
const SCOPED = {
  env: { AGENT_BROWSER_SOCKET_DIR: '/tmp/kingu-ab-0123456789abcdef' },
...
describe('agent-browser orphan sweep', () => {
  ...
  it('closes tab daemons left by a previous run', async () => {
    respond(['kingu-tab-aaa', 'kingu-tab-bbb'])

    const closed = await sweepOrphanedAgentBrowserSessions({ binaryPath: BIN, ...SCOPED })

    expect(closed).toEqual(['kingu-tab-aaa', 'kingu-tab-bbb'])
    expect(closedArgs()).toEqual([
      ['--session', 'kingu-tab-aaa', 'close'],
      ['--session', 'kingu-tab-bbb', 'close']
    ])
  })

  it('never closes a daemon outside Kingu tab naming', async () => {
    respond(['default', 'agent1', 'kingu-kingud-deadbeef', 'kingu-tab-aaa'])

    await sweepOrphanedAgentBrowserSessions({ binaryPath: BIN, ...SCOPED })

    expect(closedArgs()).toEqual([['--session', 'kingu-tab-aaa', 'close']])
  })

  it('leaves sessions this run already owns alone', async () => {
    respond(['kingu-tab-live', 'kingu-tab-orphan'])

    await sweepOrphanedAgentBrowserSessions({
      binaryPath: BIN,
      ...SCOPED,
      isSessionLive: (name) => name === 'kingu-tab-live'
    })

    expect(closedArgs()).toEqual([['--session', 'kingu-tab-orphan', 'close']])
  })

  // Why: without a socket dir Kingu derived itself, `session list` can reach daemons another Kingu
  ...
  it.each([
    ...
    ['a socket directory Kingu inherited', { AGENT_BROWSER_SOCKET_DIR: '/tmp/shared-ab' }]
  ])('does not enumerate with %s', async (_label, env) => {
    respond(['kingu-tab-aaa'])
  ...
  it('keeps sweeping after one close fails', async () => {
    runProcessMock.mockImplementation((spec: Spec) => {
      if (spec.args?.[0] === 'session') {
        return Promise.resolve({
          code: 0,
          signal: null,
          stdout: JSON.stringify({ data: { sessions: ['kingu-tab-aaa', 'kingu-tab-bbb'] } }),
          stderr: '',
          timedOut: false
        })
      }
      if (spec.args?.[1] === 'kingu-tab-aaa') {
    ...
    expect(closed).toEqual(['kingu-tab-bbb'])
  })
...
describe('sweep kill switch', () => {
  const previous = process.env.KINGU_DISABLE_AGENT_BROWSER_SWEEP

  afterEach(() => {
    if (previous === undefined) {
      delete process.env.KINGU_DISABLE_AGENT_BROWSER_SWEEP
    } else {
      process.env.KINGU_DISABLE_AGENT_BROWSER_SWEEP = previous
    }
  })
  ...
  it('enumerates nothing when disabled, even when Kingu owns the socket directory', async () => {
    process.env.KINGU_DISABLE_AGENT_BROWSER_SWEEP = '1'
  ...
  it('still sweeps when the flag holds any other value', async () => {
    process.env.KINGU_DISABLE_AGENT_BROWSER_SWEEP = '0'
    runProcessMock.mockClear()
