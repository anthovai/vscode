...
const AGENT_BROWSER_SOCKET_DIRECTORY_PREFIX = 'kingu-ab-'

/**
 * Lifetime bound for the agent-browser daemon.
 *
 * `agent-browser` is a client/daemon CLI: Kingu only ever spawns the short-lived
 * client, which forks a daemon Kingu holds no handle on and that reparents to
 * pid 1 immediately. Nothing in Kingu can reap it — not teardown, not a pid walk
 * (see `windows-pty-job.ts` for why walking your own orphans is guesswork) —
 * and a SIGKILL'd Kingu never runs teardown at all. The daemon's own idle timer
 * is the only bound that survives every way Kingu can die (#16367).
 *
...
export type AgentBrowserProcessEnvironment = {
  ...
  /**
   * True only when Kingu derived the socket directory itself. An inherited
   * `AGENT_BROWSER_SOCKET_DIR` can be shared with another Kingu profile, so it is
