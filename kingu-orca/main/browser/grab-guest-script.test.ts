...
describe('buildGuestOverlayScript', () => {
  ...
  it('arm script contains shadow DOM setup', () => {
    ...
    expect(script).toContain('__kingu-grab-host')
    expect(script).toContain('__kinguGrab')
  })
  ...
  it('teardown script cancels pending awaitClick', () => {
    ...
    expect(buildGuestOverlayScript('awaitClick')).toContain('__kinguCancelled')
  })
...
describe('awaitClick under a Zone.js-patched global Promise', () => {
  ...
  function armGrabHarness(options?: {
    ...
    window: { __kinguGrab: Record<string, unknown> }
    click: () => void
    contextmenu: () => void
    cancel: () => void
  } {
    ...
    const window = { __kinguGrab: grab }
    return {
      window,
      click: () => handlers.click?.(noopEvent),
      contextmenu: () => handlers.contextmenu?.(noopEvent),
      // cancelAwait is installed on __kinguGrab by the script itself at runtime.
      cancel: () => (window.__kinguGrab.cancelAwait as (() => void) | undefined)?.()
    }
  }
  ...
  it('resolves the context-menu marker across the boundary despite ZoneAwarePromise', async () => {
    ...
    expect(received).toHaveProperty('__kinguContextMenu', true)
  ...
  it('resolves the teardown cancel marker across the boundary despite ZoneAwarePromise', async () => {
    ...
    expect(received).toEqual({ __kinguCancelled: true })
  })
