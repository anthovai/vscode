...
describe('PowerShell OSC 133 bootstrap', () => {
  it('wraps prompt/readline without bypassing profiles or execution policy', () => {
    ...
    expect(script).toContain('KINGU_OPENCODE_CONFIG_DIR')
    expect(script).toContain('KINGU_MIMOCODE_HOME')
    expect(script).not.toContain('KINGU_PI_CODING_AGENT_DIR')
    expect(script).not.toContain('KINGU_OMP_CODING_AGENT_DIR')
    expect(script).toContain('KINGU_OMP_STATUS_EXTENSION')
    expect(script).toContain('function Global:omp')
    expect(script).toContain('--extension $env:KINGU_OMP_STATUS_EXTENSION')
    expect(script).toContain('KINGU_CODEX_HOME')
    expect(script).toContain('KINGU_CODEX_LAUNCH_PREFLIGHT')
    expect(script).toContain('function Global:codex')
    expect(script).not.toContain('$Global:__KinguCodexExecutable')
    ...
    const codexHomeRestore = script.indexOf('if ($env:KINGU_CODEX_HOME)')
