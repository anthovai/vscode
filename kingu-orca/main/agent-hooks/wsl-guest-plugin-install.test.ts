...
describe('requestGuestOpenCodeOverlayDir', () => {
  it('reports the guest overlay dir', async () => {
    const { mux } = fakeMux(async () => ({ overlayDirs: { opencode: '/home/jin/.kingu-relay/x' } }))
    await expect(requestGuestOpenCodeOverlayDir(mux, deps(), 'Ubuntu')).resolves.toEqual({
      kind: 'dir',
      dir: '/home/jin/.kingu-relay/x'
    })
  })
  ...
  it('carries the per-major guest overlay dirs independently', async () => {
    const { mux } = fakeMux(async () => ({
      overlayDirs: {
        opencode: '/home/jin/.kingu-relay/opencode-overlays/x',
        opencode2: '/home/jin/.kingu-relay/opencode2-overlays/y'
      }
    }))
    await expect(requestGuestOpenCodeOverlayDir(mux, deps(), 'Ubuntu')).resolves.toEqual({
      kind: 'dir',
      dir: '/home/jin/.kingu-relay/opencode-overlays/x',
      dir2: '/home/jin/.kingu-relay/opencode2-overlays/y'
    })
  })

  it('reports an OpenCode 2 overlay even when the v1 overlay is missing', async () => {
    const { mux } = fakeMux(async () => ({
      overlayDirs: { opencode2: '/home/jin/.kingu-relay/opencode2-overlays/y' }
    }))
    await expect(requestGuestOpenCodeOverlayDir(mux, deps(), 'Ubuntu')).resolves.toEqual({
      kind: 'dir',
      dir2: '/home/jin/.kingu-relay/opencode2-overlays/y'
