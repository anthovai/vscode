...
// user-global (~/.claude/settings.json, ~/.cursor/hooks.json). A second Kingu profile with the off
...
// Kingu's script, but the presence gate below then skips install() forever, freezing the
// script at whatever Kingu generated last. Existing scripts are Kingu-owned, so bring them
