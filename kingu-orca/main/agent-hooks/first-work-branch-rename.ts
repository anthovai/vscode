...
export type FirstWorkBranchRenameDeps = {
  ...
  /** True only for Kingu-created worktrees whose branch Kingu is allowed to rename. */
  canRenameKinguCreatedBranch: (worktreeId: string) => boolean
...
async function runAutoRename(
  ...
  if (!deps.canRenameKinguCreatedBranch(worktreeId)) {
    return stop(`worktree is not eligible for auto-rename`, true)
  }
  // Only rename a branch Kingu auto-named — never one the user chose.
