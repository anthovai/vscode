...
/** Opens a POST that announces a body and then never sends it, so Kingu's own slowloris cap ends it. */
...
describe('AgentHookServer transport interference', () => {
  ...
  async function startServer(): Promise<{
    ...
    return {
      server,
      port: Number(env.KINGU_AGENT_HOOK_PORT),
      token: env.KINGU_AGENT_HOOK_TOKEN,
      reports
    }
  }
  ...
  it('excludes requests the slowloris cap destroyed, so the count stays honest', async () => {
    ...
    await postTruncatedHook(port, token)
    await postTruncatedHook(port, token)
    // Why: Kingu destroys this one itself at HOOK_REQUEST_SLOWLORIS_MS; counting it would make
