 ...
 * `readKinguChromiumProcessPids()` is empty and this always admits. That is not
 ...
 * `kingu-chromium-process-pids.ts`.
 */
export function admitSelfInitiatedTreeKill(target: {
  ...
  // land there: a POSIX group holds only what Kingu put in it, so that arm is
  ...
  const isOwnChromiumPid =
    target.scope === 'win-taskkill-tree' && readKinguChromiumProcessPids().has(target.pid)
  try {
    if (isOwnChromiumPid) {
