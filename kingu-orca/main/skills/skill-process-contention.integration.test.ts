...
const RUN_REAL_PROCESS = process.env.KINGU_REAL_PROCESS_SKILL_TEST === '1'
...
function startChild(input: {
  ...
  const child = spawn(
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        KINGU_REAL_PROCESS_SKILL_TEST: '0',
        KINGU_SKILL_CONTENTION_CHILD: '1',
        KINGU_SKILL_CONTENTION_ROOT: input.root,
        KINGU_SKILL_CONTENTION_ARCHIVE: input.archivePath,
        KINGU_SKILL_CONTENTION_ROLE: input.role,
        KINGU_SKILL_CONTENTION_READY: input.readyPath,
        KINGU_SKILL_CONTENTION_RELEASE: input.releasePath,
        KINGU_SKILL_CONTENTION_RESULT: input.resultPath
      },
...
describe.runIf(RUN_REAL_PROCESS)('skill multi-process contention', () => {
  it('fails busy without residue, then converges after the owner releases', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kingu-skill-process-contention-'))
    roots.push(root)
    ...
    expect(
      (await readdir(join(root, 'skills'))).filter((name) => name.includes('.kingu-'))
    ).toEqual([])
  }, 30_000)
})
