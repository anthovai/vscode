...
const CHILD = process.env.KINGU_SKILL_BUNDLE_PROCESS_CHILD === '1'
...
describe.runIf(CHILD)('skill bundle process termination child', () => {
  it('extracts a bundle until the parent terminates this process', async () => {
    const root = process.env.KINGU_SKILL_BUNDLE_CRASH_ROOT
    const marker = process.env.KINGU_SKILL_BUNDLE_CRASH_MARKER
    if (!root || !marker) {
    ...
    await installSkillBundle({
      ...
      kinguStateDirectory: join(root, 'state'),
      detectedProviders: [],
