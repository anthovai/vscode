...
describe.runIf(RUN_REAL_WSL)('real WSL POSIX skill semantics', () => {
  ...
  beforeAll(async () => {
    localRoot = await mkdtemp(join(tmpdir(), 'kingu-wsl-posix-semantics-'))
    guestRoot = await runWsl('mktemp', '-d', '/tmp/kingu-skill-posix.XXXXXX')
    if (!guestRoot.startsWith('/tmp/kingu-skill-posix.')) {
  ...
  it('preserves case and applies owner-private executable modes', async () => {
    ...
    const result = await installSharedSkill({
      ...
      scope: 'global',
      homeDirectory,
      kinguStateDirectory: join(localRoot, 'state'),
      detectedProviders: [],
  ...
  it('installs only selected bundle skills into the distro-owned home', async () => {
    ...
    const result = await installSkillBundle({
      ...
      scope: 'global',
      homeDirectory,
      kinguStateDirectory: join(localRoot, 'bundle-state'),
      detectedProviders: [],
