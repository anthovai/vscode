...
async function fixture(options: { sha512?: string; url?: string } = {}) {
  const directory = await mkdtemp(join(tmpdir(), 'kingu-local-build-'))
  tempDirectories.push(directory)
  const artifactName = 'kingu-macos-arm64.zip'
...
describe('loadLocalBuildCandidate', () => {
  it('returns a sanitized, architecture-specific feed after hash validation', async () => {
    ...
    expect([...candidate.artifacts.keys()]).toEqual(['kingu-macos-arm64.zip'])
    expect(candidate.manifestContent).toContain('kingu-macos-arm64.zip')
  ...
  it('rejects mismatched hashes and traversal paths', async () => {
    ...
    const traversal = await fixture({ url: '../kingu-macos-arm64.zip' })
    await expect(
  ...
  it('serves the same artifact descriptor that passed validation', async () => {
    ...
    try {
      await expect(
        fetch(`${feed.url}kingu-macos-arm64.zip`).then((response) => response.text())
  ...
  it.runIf(process.platform === 'darwin')(
    ...
    async () => {
      const directory = await mkdtemp(join(tmpdir(), 'kingu-local-build-zip-'))
      tempDirectories.push(directory)
      const zipRoot = join(directory, 'zip-root')
      const resources = join(zipRoot, 'Kingu.app', 'Contents', 'Resources')
      await mkdir(resources, { recursive: true })
      await writeFile(join(resources, 'kingu-local-build.json'), JSON.stringify(compatibility()))
      const artifactName = 'kingu-macos-arm64.zip'
      const artifactPath = join(directory, artifactName)
      await execFileAsync('/usr/bin/zip', ['-qry', artifactPath, 'Kingu.app'], { cwd: zipRoot })
  ...
  it('requires exactly one ZIP for the running architecture', async () => {
    ...
    ).rejects.toThrow('exactly one x64 Kingu ZIP')
  })
})
