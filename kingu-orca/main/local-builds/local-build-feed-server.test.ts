...
describe('startLocalBuildFeed', () => {
  it('serves only tokenized manifest and validated artifact routes', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'kingu-local-feed-'))
    const artifactPath = join(directory, 'kingu-macos-arm64.zip')
    ...
    const candidate = {
      ...
      artifacts: new Map([['kingu-macos-arm64.zip', { file: artifactFile, size: 3 }]]),
