...
// persists Kingu's hook coordinates in that allowlist so events stay attributed.
...
function buildStatus(
  ...
  if (managedText === null) {
    return {
      ...
      detail: 'Could not read Kingu managed hooks file'
    }
  }
  if (pointer !== managedHooksPath) {
    return {
      ...
      detail:
        pointer === undefined
          ? null
          : `managed_hooks_path points at ${pointer}, not the Kingu managed hooks file`
    }
  }
  const parsed = parseMuseSettingsText(managedText, 'Kingu managed Muse hooks')
...
export class MuseHookService {
  ...
  async installRemote(sftp: SFTPWrapper, remoteHome: string): Promise<AgentHookInstallStatus> {
    ...
    const remoteScriptPath = `${remoteHome.replace(/\/$/, '')}/.kingu/agent-hooks/muse-hook.sh`
