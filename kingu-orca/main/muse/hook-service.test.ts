...
// Why: getSharedManagedScriptPath() writes under homedir()/.kingu and the
...
// real ~/.kingu or ~/.config/muse. os.homedir() resolves $HOME on POSIX.
let home: string
let originalHome: string | undefined
let originalXdg: string | undefined

beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), 'kingu-muse-hook-'))
...
const managedHooksPath = (): string => join(home, '.kingu', 'agent-hooks', 'muse-hooks.json')
const scriptPath = (): string => join(home, '.kingu', 'agent-hooks', 'muse-hook.sh')
...
describe('MuseHookService', () => {
  ...
  it('installs the managed hooks pointer, file, and script', () => {
    ...
    // The settings pointer aims at the Kingu-owned managed file, and a fresh
    ...
    expect(settings?.managed_hooks_env_vars).toContain('KINGU_PANE_KEY')
  ...
  it('treats malformed managed hook entries as absent instead of throwing', () => {
    mkdirSync(join(home, '.config', 'muse'), { recursive: true })
    mkdirSync(join(home, '.kingu', 'agent-hooks'), { recursive: true })
    const managedPath = join(home, '.kingu', 'agent-hooks', 'muse-hooks.json')
