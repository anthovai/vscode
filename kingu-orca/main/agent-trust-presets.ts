...
import { getKinguManagedCodexHomePath } from './codex/codex-home-paths'
 ...
 * first launch.
 *
 * Why: Kingu's "drop URL into agent input as a draft" flow injects the URL
...
export function markCodexProjectTrusted(workspacePath: string): Promise<void> {
  ...
  // Why: Kingu-launched Codex runs with an Kingu-owned CODEX_HOME, so the trust
  // preset must also update the runtime config Codex will actually read.
  const runtimeTomlPath = join(getKinguManagedCodexHomePath(), 'config.toml')
...
function canonicalize(p: string): string {
  ...
  // (kingu caches realpath()'d worktree paths) matches the agent's lookup.
  try {
    if (existsSync(p)) {
