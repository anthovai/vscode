...
export const KINGU_PI_PREFILL_EXTENSION_FILE = 'kingu-prefill.ts'

// Why: prefill-without-submit needs an env-var the bundled `kingu-prefill.ts`
...
export function getPiPrefillHandlerSourceLines(
  ...
  return [
    ...
    '    if (!process.env.KINGU_PANE_KEY || ctx?.hasUI === false) return',
