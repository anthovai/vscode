...
export function getPiAgentStatusHandlerSourceLines(kind: PiAgentKind): string[] {
  ...
  return [
    ...
    '  const lifecycleState = (piEventBus as { __kinguPiSubagents?: { active: Set<string>; waiting: boolean; onEvent?: (event: unknown, forcedStatus?: string) => void; listener?: (event: unknown) => void } } | undefined)?.__kinguPiSubagents ?? { active: new Set<string>(), waiting: false }',
    '  if (piEventBus) (piEventBus as { __kinguPiSubagents?: unknown }).__kinguPiSubagents = lifecycleState',
