export function getPiAgentStatusWslCurlSourceLines(): string[] {
  return [
    ...
    '  const windowsPort = process.env.KINGU_AGENT_HOOK_PORT',
    '  const windowsToken = process.env.KINGU_AGENT_HOOK_TOKEN',
    "  if (!curlPath || !windowsPort || !windowsToken) throw new Error('Kingu WSL hook bridge unavailable')",
    ...
    "        '-H', `X-Kingu-Agent-Hook-Token: ${windowsToken}`,",
    "        '--data-binary', '@-',",
    '        windowsUrl',
    '      ],',
    ...
    "      reject(new Error('Kingu WSL hook delivery timed out'))",
    '    }, 11000)',
    ...
    '      clearTimeout(timer)',
    "      if (code === 0) resolve(); else reject(new Error('Kingu WSL hook exit ' + code))",
    '    })',
