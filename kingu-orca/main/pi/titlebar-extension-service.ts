...
import {
  KINGU_PI_AGENT_STATUS_EXTENSION_FILE,
  getPiAgentStatusExtensionSource
} from './agent-status-extension-source'
import {
  KINGU_PI_PREFILL_EXTENSION_FILE,
  getPiPrefillExtensionSource
} from './prefill-extension-source'
import { KINGU_PI_EXTENSION_FILE, getPiTitlebarExtensionSource } from './titlebar-extension-source'
import {
...
// Why: old Kingu versions used per-kind overlay roots. Keep the names so
...
// shadowed one in Kingu's per-launch agent picker.
...
function withKinguManagedExtensionMarker(source: string): string {
...
export class PiTitlebarExtensionService {
  ...
  private getPtyOverlayDir(ptyId: string, kind: LegacyOverlayAgentKind): string {
    // Why: old Kingu versions used PTY-scoped hashed overlays. Keep resolving
  ...
  private writeOmpFallbackStatusExtension(source: string): string | undefined {
    ...
    const fallbackPath = join(fallbackDir, KINGU_PI_AGENT_STATUS_EXTENSION_FILE)
  ...
  private installManagedExtensions(
    ...
    if (kind !== 'prime-agent') {
      this.writeManagedExtension(
        join(extensionsDir, KINGU_PI_EXTENSION_FILE),
        withKinguManagedExtensionMarker(getPiTitlebarExtensionSource(kind))
      )
      this.writeManagedExtension(
        join(extensionsDir, KINGU_PI_PREFILL_EXTENSION_FILE),
        withKinguManagedExtensionMarker(getPiPrefillExtensionSource(kind))
      )
    }
    const statusExtensionPath = join(extensionsDir, KINGU_PI_AGENT_STATUS_EXTENSION_FILE)
    const statusSource = withKinguManagedExtensionMarker(getPiAgentStatusExtensionSource(kind))
  ...
  buildFreshOmpEnv(): Record<string, string> {
    return {
      KINGU_OMP_FRESH_CONFIG: materializeOmpFreshConfig(
  ...
  buildPtyEnv(
    ...
    if (!existsSync(sourceAgentDir) && !materializeDefaultHome) {
      if (kind === 'omp') {
        ...
        return statusExtensionPath
          ? { ...freshConfigEnv, KINGU_OMP_STATUS_EXTENSION: statusExtensionPath }
          : freshConfigEnv
      }
      return {}
    }
    ...
    if (kind === 'omp') {
      env.KINGU_OMP_SOURCE_AGENT_DIR = installed.sourceAgentDir
      if (installed.statusExtensionPath) {
        env.KINGU_OMP_STATUS_EXTENSION = installed.statusExtensionPath
      }
    } else if (kind === 'prime-agent') {
      env.KINGU_PRIME_AGENT_SOURCE_AGENT_DIR = installed.sourceAgentDir
    } else {
      env.KINGU_PI_SOURCE_AGENT_DIR = installed.sourceAgentDir
    }
    return env
  }
