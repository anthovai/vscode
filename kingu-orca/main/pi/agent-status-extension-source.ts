...
// /hook/<kind> using the same KINGU_AGENT_HOOK_* + KINGU_PANE_KEY env that every
...
// any Kingu dep into the pi runtime.
...
export const KINGU_PI_AGENT_STATUS_EXTENSION_FILE = 'kingu-agent-status.ts'

/** Source of the status extension installed into a Pi-family agent's extension
 *  dir: it POSTs lifecycle, tool, and (under OMP) model events to Kingu's hook
...
export function getPiAgentStatusExtensionSource(kind: PiAgentKind = 'pi'): string {
  ...
  const modelMetadataSourceLines = [
    ...
    "    if (provider && id) modelMetadata = { model: provider + '/' + id, ...(ompModelSwitchSupported ? { model_switch_command: 'kingu-model' } : {}) }",
    '  } catch {',
  ...
  // so it cannot import from Kingu's main bundle. fs/http coords come from
  // the same endpoint file the OpenCode plugin reads (process.env is frozen
  // at PTY spawn, so on Kingu restart we have to re-read it from disk).
  return [
    ...
    '// Kingu receiver from building an unbounded queue of obsolete snapshots.',
    ...
    '  const path = process.env.KINGU_AGENT_HOOK_ENDPOINT',
    '  if (!path) return null',
    '  try {',
    "    const fs = require('fs')",
    '    try {',
    ...
    "      console.warn('[kingu-pi-status] failed to parse endpoint file:', (err as Error).message)",
    '    }',
    '    return null',
    '  }',
    '}',
    ...
    "    env: fileEnv.KINGU_AGENT_HOOK_ENV || process.env.KINGU_AGENT_HOOK_ENV || '',",
    "    version: fileEnv.KINGU_AGENT_HOOK_VERSION || process.env.KINGU_AGENT_HOOK_VERSION || '',",
    '  }',
    '}',
    '',
    ...
    '    timeout = setTimeout(() => {',
    '      controller?.abort()',
    "      reject(new Error('Kingu hook delivery timed out'))",
    ...
    "          'X-Kingu-Agent-Hook-Token': coords.token,",
    '        },',
    '        body,',
    ...
    '      timeoutPromise,',
    '    ])',
    "    if (!response.ok) throw new Error('Kingu hook HTTP ' + response.status)",
    '  } catch (error) {',
    '    // Why: status reporting must never fail the pi run just because Kingu',
    '    // is unavailable or the loopback request failed (e.g. Kingu restart).',
