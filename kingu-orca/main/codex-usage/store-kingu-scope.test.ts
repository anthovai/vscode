...
describe('CodexUsageStore', () => {
  ...
  it('reports no data for Kingu scope when only non-Kingu Codex usage exists', async () => {
  ...
  it('uses only Kingu-scoped models when projecting mixed-scope sessions', async () => {
    const store = createStoreWithState({
      sessions: [
        {
          sessionId: 'session-1',
    ...
    const breakdown = await store.getBreakdown('kingu', '30d', 'model')
    const recentSessions = await store.getRecentSessions('kingu', '30d', 10)
