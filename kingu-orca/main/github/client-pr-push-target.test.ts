...
describe('getPRForBranch', () => {
  ...
  it('resolves fork PR push target using the origin URL protocol', async () => {
    ...
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'anthovai', repo: 'kingu' })
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'prateek/fix-sidebar-agents-toggle',
          repo: {
            full_name: 'prateek/kingu',
            name: 'kingu',
            clone_url: 'https://github.com/prateek/kingu.git',
            ssh_url: 'git@github.com:prateek/kingu.git',
            owner: { login: 'prateek' }
          }
        }
      })
    })
    getRemoteUrlForRepoMock.mockResolvedValueOnce('git@github.com:anthovai/kingu-intelligence.git')

    const target = await getPullRequestPushTarget('/repo-root', 1738)

    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['api', 'repos/anthovai/kingu-intelligence/pulls/1738'],
      {
        cwd: '/repo-root'
      }
    )
    ...
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'contributor/fix',
          repo: {
            full_name: 'contributor/kingu',
            name: 'kingu',
            clone_url: 'https://github.com/contributor/kingu.git',
            ssh_url: 'git@github.com:contributor/kingu.git',
            owner: { login: 'contributor' }
          }
        }
      })
    })
    getRemoteUrlForRepoMock.mockResolvedValueOnce('git@github.com:fork/kingu.git')

    await getPullRequestPushTarget('/repo-root', 1738, null, {}, 'origin')

    expect(resolvePRRepositoryCandidatesMock).not.toHaveBeenCalled()
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(['api', 'repos/fork/kingu/pulls/1738'], {
      cwd: '/repo-root',
      host: 'github.com'
    })
    expect(ghExecFileAsyncMock).not.toHaveBeenCalledWith(
      ['api', 'repos/upstream/kingu/pulls/1738'],
      expect.anything()
    )
  })
    ...
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'fix-sidebar',
          repo: {
            full_name: 'anthovai/kingu-intelligence',
            name: 'kingu',
            clone_url: 'https://github.com/anthovai/kingu-intelligence.git',
            ssh_url: 'git@github.com:anthovai/kingu-intelligence.git',
            owner: { login: 'anthovai' }
          }
        }
      })
    })
  ...
  it('keeps getRepoSlug origin-based on a fork checkout (#7331)', async () => {
    ...
    getOwnerRepoMock.mockResolvedValue({ owner: 'fsdwen', repo: 'kingu' })
  ...
  it('resolves a distinct upstream remote as the repo upstream', async () => {
    ...
    getOwnerRepoMock.mockResolvedValue({ owner: 'tmchow', repo: 'kingu' })
    getOwnerRepoForRemoteMock.mockImplementation(async (_repoPath: string, remoteName: string) =>
      remoteName === 'origin'
        ? { owner: 'tmchow', repo: 'kingu' }
        : { owner: 'anthovai', repo: 'kingu' }
    )
  ...
  it('does not treat a same-repository upstream remote as a fork', async () => {
    getOwnerRepoMock.mockResolvedValue({ owner: 'StablyAI', repo: 'Kingu' })
    getOwnerRepoForRemoteMock.mockImplementation(async (_repoPath: string, remoteName: string) =>
      remoteName === 'origin'
        ? { owner: 'StablyAI', repo: 'Kingu' }
        : { owner: 'anthovai', repo: 'kingu' }
    )
    ghExecFileAsyncMock.mockResolvedValueOnce({
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ...
      ['repo', 'view', 'github.com/StablyAI/Kingu', '--json', 'isFork,parent'],
  ...
  it('falls back to the GitHub parent when no upstream remote is configured', async () => {
    getOwnerRepoForRemoteMock
      .mockResolvedValueOnce({ owner: 'tmchow', repo: 'kingu' })
      .mockResolvedValueOnce(null)
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        isFork: true,
        parent: { name: 'kingu', owner: { login: 'anthovai' } }
      })
    })
  ...
  it('routes GHES push-target probes through the Enterprise host', async () => {
    ...
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'feature',
          repo: {
            full_name: 'team/kingu',
            name: 'kingu',
            clone_url: 'https://github.acme-corp.com/team/kingu.git',
            ssh_url: 'git@github.acme-corp.com:team/kingu.git',
            owner: { login: 'team' }
          }
        }
      })
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['api', 'repos/team/kingu/pulls/7'],
      expect.objectContaining({ host: 'github.acme-corp.com' })
    )
  })

  it('does not confuse same-slug PR repositories across GitHub hosts', async () => {
    const enterprise = { owner: 'team', repo: 'kingu', host: 'github.acme-corp.com' }
    const dotCom = { owner: 'team', repo: 'kingu', host: 'github.com' }
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
      candidates: [enterprise, dotCom],
      headRepo: dotCom
    })
    getOwnerRepoForRemoteMock.mockResolvedValueOnce(dotCom)
    getRemoteUrlForRepoMock.mockResolvedValueOnce('git@github.com:team/kingu.git')
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'feature',
    ...
    await expect(getPullRequestPushTarget('/repo-root', 7)).resolves.toEqual({
      pushTarget: {
        remoteName: 'pr-team-kingu',
        branchName: 'feature',
        remoteUrl: 'git@github.acme-corp.com:team/kingu.git'
      }
    })
  })
  ...
  it('probes additional PR repo candidates when the first lookup is not found', async () => {
    ...
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'fork', repo: 'kingu' })
    ghExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          head: {
            ref: 'feature/test',
            repo: {
              full_name: 'fork/kingu',
              name: 'kingu',
              clone_url: 'https://github.com/fork/kingu.git',
              ssh_url: 'git@github.com:fork/kingu.git',
              owner: { login: 'fork' }
            }
          }
        })
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(1, ['api', 'repos/fork/kingu/pulls/1849'], {
      cwd: '/repo-root'
    })
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      2,
      ['api', 'repos/anthovai/kingu-intelligence/pulls/1849'],
      { cwd: '/repo-root' }
    )
  })

  it('probes the next PR work-item candidate after a permission denial', async () => {
    const upstream = { owner: 'upstream', repo: 'kingu', host: 'github.com' }
    const origin = { owner: 'fork', repo: 'kingu', host: 'github.com' }
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
    ...
    ghExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          title: 'Origin PR',
          state: 'OPEN',
          url: 'https://github.com/fork/kingu/pull/42',
          labels: [],
    ...
    expect(ghExecFileAsyncMock.mock.calls[0][0]).toEqual(
      expect.arrayContaining(['pr', 'view', '--repo', 'upstream/kingu'])
    )
    expect(ghExecFileAsyncMock.mock.calls[1][0]).toEqual(['api', 'repos/upstream/kingu/pulls/42'])
    expect(ghExecFileAsyncMock.mock.calls[2][0]).toEqual(
      expect.arrayContaining(['pr', 'view', '--repo', 'fork/kingu'])
    )
  })
