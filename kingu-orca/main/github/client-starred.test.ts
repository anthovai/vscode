...
import { __resetKinguStarCheckForTests, checkKinguStarred, starKingu } from './client'
...
describe('checkKinguStarred', () => {
  ...
  it('returns true only for an included successful GitHub response', async () => {
    ...
    await expect(checkKinguStarred()).resolves.toBe(true)

    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['api', '--include', 'user/starred/anthovai/kingu-intelligence'],
  ...
  it('returns false for GitHub 404 not starred responses', async () => {
    ...
    await expect(checkKinguStarred()).resolves.toBe(false)
  })
  ...
  it('returns null when gh exits successfully without response headers', async () => {
    ...
    await expect(checkKinguStarred()).resolves.toBe(null)
  })
  ...
  it('coalesces concurrent checks onto one gh child', async () => {
    ...
    const first = checkKinguStarred()
    const second = checkKinguStarred()
    const third = checkKinguStarred()
...
describe('starKingu', () => {
  ...
  it('stars through the bounded gh runner and releases its slot', async () => {
    ...
    await expect(starKingu()).resolves.toBe(true)

    expect(execFileAsyncMock).not.toHaveBeenCalled()
    const [args, options] = ghExecFileAsyncMock.mock.calls[0]
    expect(args).toEqual(['api', '-X', 'PUT', 'user/starred/anthovai/kingu-intelligence'])
    expect(options.timeout).toBeGreaterThan(0)
    expect(releaseMock).toHaveBeenCalledTimes(1)
  })
  ...
  it('reports failure and still releases its slot when gh times out', async () => {
    ...
    await expect(starKingu()).resolves.toBe(false)
    expect(releaseMock).toHaveBeenCalledTimes(1)
  })
})
