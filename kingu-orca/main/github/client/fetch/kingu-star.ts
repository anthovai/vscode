...
export const KINGU_REPO = 'anthovai/kingu-intelligence'
...
/**
 * Check if the authenticated user has starred the Kingu repo.
 * Returns true if starred, false if not, null if unable to determine (gh unavailable).
 */
export function checkKinguStarred(): Promise<boolean | null> {
  ...
  inFlightStarCheck ??= runKinguStarredCheck().finally(() => {
...
export function __resetKinguStarCheckForTests(): void {
  inFlightStarCheck = null
}

async function runKinguStarredCheck(): Promise<boolean | null> {
  await acquire()
  try {
    const { stdout, stderr } = await ghExecFileAsync(
      ['api', '--include', `user/starred/${KINGU_REPO}`],
...
/**
 * Star the Kingu repo for the authenticated user.
 */
export async function starKingu(): Promise<boolean> {
  await acquire()
  try {
    await ghExecFileAsync(['api', '-X', 'PUT', `user/starred/${KINGU_REPO}`], {
      encoding: 'utf-8',
