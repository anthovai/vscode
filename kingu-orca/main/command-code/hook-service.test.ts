...
describe('CommandCodeHookService', () => {
  ...
  beforeEach(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'kingu-command-code-home-'))
  ...
  it('installs the managed command for Command Code status events', () => {
    ...
    if (process.platform !== 'win32') {
      expect(config.hooks.PreToolUse[0].hooks[0].command).toContain(join(homeDir, '.kingu'))
    }
    if (process.platform !== 'win32') {
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'kingu command-code home with spaces')
  ...
  it('installs a hook script that can recover the endpoint when Command Code strips token env', () => {
    ...
    const script = readFileSync(join(homeDir, '.kingu', 'agent-hooks', scriptFileName), 'utf8')

    if (process.platform === 'win32') {
      expect(script).toContain('sourceEndpointByPort')
      expect(script).toContain('kingu-dev\\agent-hooks')
      expect(script).toContain('set KINGU_AGENT_HOOK_PORT=')
    } else {
      ...
      expect(script).toContain('__kingu_read_ancestor_var')
      expect(script).toContain('__kingu_fill_from_endpoint_file')
      expect(script).toContain('[ "$__kingu_endpoint_port" != "$KINGU_AGENT_HOOK_PORT" ]')
      expect(script).toContain('KINGU_PANE_KEY')
      expect(script).toContain('KINGU_AGENT_LAUNCH_TOKEN')
      expect(script).toContain('kingu-dev/agent-hooks')
  ...
  itPosix('does not let stale endpoint files clobber recovered connection fields', async () => {
    ...
    const server = createServer((req, res) => {
      ...
      req.on('end', () => {
        requests.push({
          body,
          token: req.headers['x-kingu-agent-hook-token']
        })
        res.statusCode = 204
        res.end()
      })
    ...
    try {
      ...
      const scriptPath = join(homeDir, '.kingu', 'agent-hooks', 'command-code-hook.sh')
      const child = spawn('/bin/sh', [scriptPath], {
        env: {
          ...process.env,
          HOME: homeDir,
          KINGU_AGENT_HOOK_ENDPOINT: staleEndpointPath,
          KINGU_AGENT_HOOK_PORT: String(address.port),
          KINGU_AGENT_HOOK_TOKEN: 'current-token',
          KINGU_PANE_KEY: 'tab:leaf',
          KINGU_TAB_ID: 'tab',
          KINGU_WORKTREE_ID: 'worktree',
          KINGU_AGENT_HOOK_ENV: 'development',
          KINGU_AGENT_HOOK_VERSION: '1'
        },
