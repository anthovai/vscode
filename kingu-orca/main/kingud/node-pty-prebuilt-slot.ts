 ...
 * by config/scripts/build-kingud-prebuilds.mjs. An upstream tarball would not do: the patch
 ...
 * `kingud.js` itself, so this stays correct wherever the install directory ends up.
 */
export function resolveKingudPrebuildsDir(entryScript = process.argv[1]): string | null {
  const override = process.env.KINGU_KINGUD_PREBUILDS_DIR
  if (override) {
    return override
  }
...
export function installPrebuiltSlot(options: {
  ...
  const prebuildsDir = options.prebuildsDir ?? resolveKingudPrebuildsDir()
