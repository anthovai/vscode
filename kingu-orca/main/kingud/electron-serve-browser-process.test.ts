...
import type { RuntimeBrowserCommandHost } from '../runtime/kingu-runtime-browser'
...
const FAKE_SIDECAR = join(import.meta.dirname, '__fixtures__', 'fake-kingud-electron-sidecar.cjs')
const INSTALLED_EXECUTABLE = join('/Applications', 'Kingu.app', 'Contents', 'MacOS', 'Kingu')

/** Every key the provider must strip so the sidecar cannot inherit kingud's own browser config. */
...
beforeEach(async () => {
  harnessRoot = await mkdtemp(join(tmpdir(), 'kingud-electron-harness-'))
  ...
  spawnProcessMock.mockImplementation((spec: RunProcessModule.ProcessSpec) =>
    actual.spawnProcess({
      ...
      env: {
        ...spec.env,
        KINGU_FAKE_SIDECAR_LOG: logPath,
        KINGU_FAKE_SIDECAR_CONTROL: controlPath,
        ...(sidecarMode ? { KINGU_FAKE_SIDECAR_MODE: sidecarMode } : {})
      }
    })
  )
})
...
describe('ElectronServeBrowserProcess start-up', () => {
  it('launches the installed app in headless serve mode without kingud browser env', async () => {
    for (const key of AGENT_BROWSER_ENVIRONMENT_KEYS) {
      vi.stubEnv(key, `leaked-${key}`)
    }
    vi.stubEnv('KINGU_HARNESS_UNRELATED', 'preserved')
    ...
    expect(spec.env?.KINGU_HARNESS_UNRELATED).toBe('preserved')
  ...
  it('stops the sidecar process and removes its user data directory', async () => {
    ...
    const metadata = JSON.parse(
      await readFile(join(userDataPath, 'kingu-runtime.json'), 'utf8')
    ) as { pid: number }
