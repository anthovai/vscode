...
import { resolveKingudBrowserProvider, type KingudBrowserProvider } from './kingud-browser-provider'
...
let provider: KingudBrowserProvider | null
...
beforeEach(async () => {
  harnessRoot = await mkdtemp(join(tmpdir(), 'kingud-provider-selection-'))
...
describe('resolveKingudBrowserProvider Electron preference', () => {
  it('uses the installed Electron app even when a Chromium executable is configured', async () => {
    provider = await resolveKingudBrowserProvider({
      userDataPath: join(harnessRoot, 'state'),
      environment: { KINGU_BROWSER_EXECUTABLE: chromiumExecutable },
  ...
  it('falls back to the operator Chromium when the installed Electron app cannot start', async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      '[kingud] Installed Electron browser provider unavailable:',
      expect.anything()
    )
  })
