...
const { collectKingudHealth, collectTerminalDaemonHealth, computeKingudBuildHash } =
  await import('./kingud-health')
...
const PID_RECORD: ParsedDaemonPid = {
  pid: 4242,
  startedAtMs: 1_000,
  entryPath: '/opt/kingud/daemon-entry.js',
  appVersion: '1.2.2',
  launchNonce: 'n',
  linuxStartTicks: null,
  bootId: null,
  spawnerExecPath: null,
  cgroupUnit: 'kingu-daemon-n.scope'
}
...
describe('collectTerminalDaemonHealth', () => {
  it('reports live only when the daemon answered its own PTY spawn probe', async () => {
    ...
    // The build the LIVE daemon came from, which can legitimately predate this kingud.
    expect(health.buildVersion).toBe('1.2.2')
    expect(health.entryPath).toBe('/opt/kingud/daemon-entry.js')
    ...
    expect(health.cgroupUnit).toBe('kingu-daemon-n.scope')
...
describe('computeKingudBuildHash', () => {
  it('changes when the bundle bytes change, even at the same version string', () => {
    const dir = mkdtempSync(join(tmpdir(), 'kingud-build-hash-'))
    const entry = join(dir, 'kingud.js')
    writeFileSync(entry, 'build-a')
    const first = computeKingudBuildHash(entry)
    ...
    expect(computeKingudBuildHash(entry)).not.toBe(first)
  })

  it('answers unknown rather than throwing when the entry cannot be read', () => {
    expect(computeKingudBuildHash(join(tmpdir(), 'definitely-absent-kingud.js'))).toBe('unknown')
    // A process with no argv[1] (an embedded host) still has to publish a readiness payload.
    expect(computeKingudBuildHash('')).toBe('unknown')
  })
})
