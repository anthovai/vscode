...
const executablePath = process.env.KINGU_BROWSER_EXECUTABLE
...
describe('ExternalChromiumBrowserProcess integration', () => {
  it.runIf(Boolean(executablePath))(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'kingud-external-browser-'))
      ...
      const provider = await resolveKingudBrowserProvider({
        userDataPath: root,
        environment: { KINGU_BROWSER_EXECUTABLE: executablePath },
