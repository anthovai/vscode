/**
 * The path half of kingud's `AppEnvironment`: every `AppPathName` gets a real answer for
...
/** XDG-ish data root. `$KINGU_USER_DATA` wins so a smoke test can isolate state. */
export function resolveUserDataPath(): string {
  const explicit = env('KINGU_USER_DATA')
  if (explicit) {
    return explicit
  }
  const xdg = env('XDG_DATA_HOME')
  return xdg ? join(xdg, 'Kingu') : join(homedir(), '.kingu')
}
...
/**
 * The directory kingud was launched from — the bundle root that `kingud.js` and the
 ...
 */
export function resolveKingudInstallRoot(scriptPath = process.argv[1]): string {
  if (!scriptPath) {
    throw new Error(
      'kingud_install_root_unavailable — process.argv[1] is unset, so this process has no ' +
...
export function resolveKingudPath(name: AppPathName): string {
  switch (name) {
    case 'userData':
