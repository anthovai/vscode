...
import { installKingudHostAdapters } from './kingud-entry'

/**
 * The composition, not the pieces: kingud's real host adapters against the real resolver
 * and a real deployment layout on disk. Both halves of this passed their own unit tests
 * while the running server forked a path that has never existed in an kingud deployment.
 */
describe('kingud forked-child paths', () => {
  let deployRoot: string
  let originalArgv: string[]

  beforeEach(() => {
    deployRoot = mkdtempSync(join(tmpdir(), 'kingud-deploy-'))
    // The layout build-kingud.mjs emits: the bundle and its children side by side.
    writeFileSync(join(deployRoot, 'kingud.js'), '')
    writeFileSync(join(deployRoot, 'parcel-watcher-process-entry.js'), '')
    originalArgv = process.argv
    process.argv = [process.execPath, join(deployRoot, 'kingud.js')]
  ...
  it('forks the watcher child shipped beside kingud.js', () => {
  ...
  it('ignores a decoy child under the working directory', () => {
    ...
    const elsewhere = mkdtempSync(join(tmpdir(), 'kingud-cwd-'))
