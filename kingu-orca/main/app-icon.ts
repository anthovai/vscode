...
import watercolorIcon from '../../resources/app-icons/kingu-watercolor.png?asset'
import watercolorMacDockIcon from '../../resources/app-icons/kingu-watercolor.png?asset&asarUnpack'
import blueIcon from '../../resources/app-icons/kingu-blue.png?asset'
import blueMacDockIcon from '../../resources/app-icons/kingu-blue.png?asset&asarUnpack'
...
const MAC_DOCK_ICON_SCRIPT = [
  ...
  'set appPath to system attribute "KINGU_APP_BUNDLE_PATH"',
  'set iconPath to system attribute "KINGU_APP_ICON_PATH"',
  "set image to current application's NSImage's alloc()'s initWithContentsOfFile:iconPath",
  'if image is missing value then error "Kingu app icon image could not be loaded"',
  "set ok to current application's NSWorkspace's sharedWorkspace()'s setIcon:image forFile:appPath options:0",
  'if ok is false then error "Kingu app icon could not be persisted"'
]
...
const MAC_DOCK_ICON_CLEAR_SCRIPT = [
  ...
  'if ok is false then error "Kingu app icon could not be cleared"'
]
...
function runMacCustomIconCommand(
  ...
  return runBoundedMacDockIconCommand({
    ...
    options: {
      env: {
        ...process.env,
        KINGU_APP_BUNDLE_PATH: appBundlePath,
        KINGU_APP_ICON_PATH: iconPath
      }
    },
