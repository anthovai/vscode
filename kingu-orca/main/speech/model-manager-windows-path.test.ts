...
describe('ModelManager Windows model path handling', () => {
  beforeEach(() => {
    ...
    appGetPathMock.mockImplementation(() => join(tmpdir(), 'kingu-speech-models-test'))
  })
  ...
  it('uses an ASCII cache path when the Windows default user data path has non-ASCII characters', () => {
    const dir = mkdtempSync(join(tmpdir(), 'kingu-model-manager-'))
    try {
      setPlatform('win32')
      const programDataDir = join(dir, 'ProgramData')
      const userDataDir = join(dir, '用户', 'Kingu')
      ...
      expect(manager.getModelsDir()).toContain(join(programDataDir, 'Kingu', 'speech-models'))
