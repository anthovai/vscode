...
import { createMobileCreateTestNotifier } from './kingu-runtime-test-scenario-builders.spec'
...
describe('terminal rename before renderer graph hydration', () => {
  it.each(['Media Engine Orch', null])(
    ...
    async (title) => {
      ...
      const runtime = new KinguRuntimeService(checkedStore)
      ...
      const restored = new KinguRuntimeService(checkedStore)
  ...
  it('does not recreate a closed persisted tab from a surviving PTY record', async () => {
    ...
    const runtime = new KinguRuntimeService(runtimeStore as RuntimeStore)
