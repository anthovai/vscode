import { KinguRuntimeWithGetRuntimeId } from './kingu-runtime-get-runtime-id'
...
import type { RuntimeWorktreeLifecycleEvent } from './kingu-runtime-core'
import { WORKTREE_CREATE_RESULT_TTL_MS } from './kingu-runtime-core'
...
export class KinguRuntimeWithGetStatus extends KinguRuntimeWithGetRuntimeId {
  ...
  getStatus(): RuntimeStatus {
    ...
    const capabilities: RuntimeCapability[] = RUNTIME_CAPABILITIES.filter(
      (capability) =>
        ...
        (process.env.KINGU_E2E_DISABLE_RUNTIME_SHARED_CONTROL !== '1' ||
        ...
        (process.env.KINGU_E2E_DISABLE_AUTHORITATIVE_SESSION_TABS_INVENTORY !== '1' ||
    ...
    // Why not a static capability: dev trees and `kingu serve` installs may carry no
    ...
    // that actually applies, and a host that says "set KINGU_BROWSER_EXECUTABLE" to someone
    ...
    return {
      ...
      // Why: headless kingu serve cannot create/stream BrowserViews, so clients
