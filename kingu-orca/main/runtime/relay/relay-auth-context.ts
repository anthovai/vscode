...
import { readFreshKinguCloudSession } from '../../kingu-profiles/profile-cloud-session-refresh'
...
export async function readRelayAuthContext(
  ...
  const session = await readFreshKinguCloudSession(authConfig, active, userDataPath)
  ...
  const refreshed = ensureActiveKinguProfile(userDataPath)
