...
import { KinguRuntimeWithVerifyOrchestrationCompatibilityCaller } from './kingu-runtime-verify-orchestration-compatibility-caller'
...
export class KinguRuntimeWithGetOrchestrationDispatchAuthority extends KinguRuntimeWithVerifyOrchestrationCompatibilityCaller {
  ...
  getTerminalOrchestrationCliCommand(handle: string): OrchestrationCliCommand {
    ...
    return resolveTerminalOrchestrationCliCommand({
      ...
      isWsl: pty.isWsl,
      worktreeId: pty.worktreeId,
      // Dev builds run the CLI as `kingu-dev`; a packaged app must not advertise it.
      runtimeCliCommand: getAppEnvironment().isPackaged() ? undefined : 'kingu-dev',
