...
import type { RuntimeBrowserCommandHost } from './kingu-runtime-browser'
    ...
    profiles: new Map([
      ...
      [
        'profile-isolated',
        {
          id: 'profile-isolated',
          scope: 'isolated',
          partition: 'persist:kingu-browser-session-profile-isolated',
          label: 'Isolated',
          source: null
        }
      ]
...
describe('RuntimeBrowserCommands browser screencast', () => {
  ...
  it('creates profiles with the requested scope and label', async () => {
    const { RuntimeBrowserCommands } = await import('./kingu-runtime-browser')
    const profile = {
      id: 'profile-google',
      scope: 'isolated',
      partition: 'persist:kingu-browser-session-profile-google',
      label: 'Google',
      source: null
    }
  ...
  it('sends the resolved isolated profile partition when creating a renderer tab', async () => {
    ...
    expect(send).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        sessionPartition: 'persist:kingu-browser-session-profile-isolated'
      })
    )
  })
