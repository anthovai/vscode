...
import { KinguRuntimeWithGetAgentSessionExecutionNamespace } from './kingu-runtime-get-agent-session-execution-namespace'
import type {
...
export class KinguRuntimeWithCreateAgentSession extends KinguRuntimeWithGetAgentSessionExecutionNamespace {
  async createAgentSession(
    ...
    const operation = (async (): Promise<RuntimeCreateAgentSessionResult> => {
      ...
      // KINGU_TERMINAL_HANDLE, so it is the only name a lost spawn can be re-found by.
      reclaim.identity = {
