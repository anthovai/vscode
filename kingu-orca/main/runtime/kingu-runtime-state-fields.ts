...
import { KinguRuntimeWithLinearCommands } from './kingu-runtime-linear-commands'
...
export class KinguRuntimeWithStateFields extends KinguRuntimeWithLinearCommands {
  ...
  constructor(
    ...
    deps?: {
      ...
      // runs under `kingu serve`, so remote/SSH hosts would silently drop
      ...
      // differs per host (scanner child on the desktop, in-process on kingud), and on kingud
    ...
    // even on headless `kingu serve` hosts where registerCoreHandlers never runs.
