...
import type { KinguRuntimeService as KinguRuntimeServiceConstructor } from '../kingu-runtime'
...
export function acknowledgeAgentPromptSubmit(
  runtime: InstanceType<typeof KinguRuntimeServiceConstructor>,
  ptyId: string,
  data: string
): void {
  if (data === '\r') {
...
vi.mock('../../hooks', () => ({
  ...
  parseKinguYaml: vi.fn().mockReturnValue(null)
}))
