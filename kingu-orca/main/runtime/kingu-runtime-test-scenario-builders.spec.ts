import * as mocks from './kingu-runtime-test-mocks.spec'
import type { Mock } from 'vitest'

const { HEADLESS_RUNTIME_WINDOW_ID, KinguRuntimeService, electronMocks } = mocks
const { getBrowserHostLeaseRegistry, getDefaultWorkspaceSession, getRuntimeBrowserPageRegistry } =
  mocks
const { gitRunner, vi } = mocks

import * as fixtures from './kingu-runtime-test-fixtures.spec'
...
// Why (#10333): `kingu serve` publishes a ready graph under
...
function createWorktreeRemovalRuntime(runtimeStore: unknown = store): RuntimeService {
  ...
  return new KinguRuntimeService(runtimeStore as never, undefined, {
