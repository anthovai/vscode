...
const FIREWALL_RULE_NAME = 'Kingu.MobilePairing'
const FIREWALL_RULE_DISPLAY_NAME = 'Kingu Mobile Pairing'
...
New-NetFirewallRule -Name ${quotePowerShell(FIREWALL_RULE_NAME)} -DisplayName ${quotePowerShell(FIREWALL_RULE_DISPLAY_NAME)} -Description 'Allows Kingu Mobile to connect to this Kingu desktop on private networks.' -Direction Inbound -Action Allow -Enabled True -Profile Private -Protocol TCP -LocalPort ${port} -Program ${quotePowerShell(executablePath)} -EdgeTraversalPolicy Block | Out-Null`
}
