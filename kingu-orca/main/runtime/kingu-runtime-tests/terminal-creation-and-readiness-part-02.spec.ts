...
describe('KinguRuntimeService', () => {
  ...
  it('applies Settings agent defaults to bare agent command terminal creates', async () => {
    ...
    expect(spawnCall?.env?.KINGU_AGENT_LAUNCH_TOKEN).toMatch(UUID_RE)
  ...
  // the CLI Kingu can host (issue #11926).
  ...
  it.each([
    { platform: 'win32' as const, expected: 'kingu.cmd claude-teams' },
    { platform: 'linux' as const, expected: 'kingu-ide claude-teams' },
    { platform: 'darwin' as const, expected: 'kingu claude-teams' }
  ])(
  ...
  it('resolves a startupAgent in a repo-less folder workspace', async () => {
    const folderPath = await mkdtemp(join(tmpdir(), 'kingu-runtime-folder-startup-agent-'))
    const spawn = vi.fn().mockResolvedValue({ id: 'pty-bg' })
