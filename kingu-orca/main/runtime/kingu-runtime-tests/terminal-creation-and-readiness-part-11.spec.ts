...
describe('KinguRuntimeService', () => {
  ...
  it('strips retained terminal path line and hash locators before matching', () => {
    const colonPath = '/tmp/kingu report/result.json'
  ...
  it('keeps tmpdir artifact paths in bounded PTY path candidates', () => {
    const artifactPath = join(tmpdir(), 'kingu-runtime-retained-result.json')
    const candidates = appendRecentPtyPathCandidates(undefined, `wrote ${artifactPath}\n`)
  ...
  it('matches terminal artifact paths only when they appear in recent terminal output', () => {
    expect(
      recentTerminalOutputIncludesPath(
        'wrote /tmp/kingu report/result.json:12:3',
        '/tmp/kingu report/result.json',
        '/tmp/kingu report/result.json'
      )
    ).toBe(true)
    expect(
      recentTerminalOutputIncludesPath(
        '\x1b]8;;file:///tmp/kingu%20report/result.json\x1b\\result\x1b]8;;\x1b\\',
        '/tmp/kingu report/result.json',
        '/tmp/kingu report/result.json'
      )
    ).toBe(true)
    ...
    expect(
      recentTerminalOutputIncludesPath(
        'wrote /tmp/kingu report/other.json',
        '/tmp/kingu report/result.json',
        '/tmp/kingu report/result.json'
      )
    ).toBe(false)
