...
import type { WorktreeMeta } from '../kingu-runtime-test-mocks.spec'
import { TEST_REPO_ID, makeWorktreeMeta, store } from '../kingu-runtime-test-fixtures.spec'
...
describe('KinguRuntimeService', () => {
  ...
  it('records the resolved fallback agent when the requested startup draft agent is disabled', async () => {
    ...
    const result = await runtime.createManagedWorktree({
      ...
      startupDraft: 'https://github.com/anthovai/kingu-intelligence/issues/456',
  ...
  it('honors split setup placement for opted-in local startup-draft worktrees', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      runnerScriptPath: '/tmp/repo/.git/kingu/setup-runner.sh',
      envVars: {
        KINGU_ROOT_PATH: '/tmp/repo',
        KINGU_WORKTREE_PATH: '/tmp/workspaces/runtime-startup-setup-split'
      },
    ...
    const result = await runtime.createManagedWorktree({
      ...
      startupDraft: 'https://github.com/anthovai/kingu-intelligence/issues/123',
      setupDecision: 'run',
      activate: true,
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        cwd: '/tmp/workspaces/runtime-startup-setup-split',
        command: expect.stringContaining('bash /tmp/repo/.git/kingu/setup-runner.sh'),
        env: expect.objectContaining({
          KINGU_ROOT_PATH: '/tmp/repo',
          KINGU_WORKTREE_PATH: '/tmp/workspaces/runtime-startup-setup-split',
          KINGU_WORKTREE_ID: result.worktree.id
        }),
        worktreeId: result.worktree.id
      })
    )
    ...
    const markerPath = `/tmp/repo/.git/kingu/setup-runner.sh.${nonceMatch![1]}.done`
    ...
    expect(mainEnv.KINGU_TAB_ID).toBeDefined()
    expect(mainEnv.KINGU_PANE_KEY).toBeDefined()
    expect(setupEnv.KINGU_TAB_ID).toBe(mainEnv.KINGU_TAB_ID)
    const mainLeafId = mainEnv.KINGU_PANE_KEY!.slice(`${mainEnv.KINGU_TAB_ID!}:`.length)
    expect(revealTerminalSession).toHaveBeenLastCalledWith(
      result.worktree.id,
      expect.objectContaining({
        ptyId: 'pty-startup-split-setup',
        tabId: mainEnv.KINGU_TAB_ID,
        activate: false,
  ...
  it('passes the wrapped setup command to activation when startup spawned but setup did not', async () => {
    ...
    vi.mocked(shouldRunSetupForCreate).mockReturnValue(true)
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      runnerScriptPath: 'C:\\tmp\\repo\\.git\\kingu\\setup-runner.sh',
      shell: { family: 'posix', executable: 'wsl.exe' },
      envVars: {
        KINGU_ROOT_PATH: '/tmp/repo',
        KINGU_WORKTREE_PATH: '/tmp/workspaces/runtime-startup-setup-retry'
      },
      waitForAgentStartup: true
    })
    vi.mocked(listWorktrees).mockResolvedValue([
    ...
    expect(activateWorktree).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        runnerScriptPath: 'C:\\tmp\\repo\\.git\\kingu\\setup-runner.sh',
        command: expect.stringContaining('bash /mnt/c/tmp/repo/.git/kingu/setup-runner.sh')
      }),
      undefined,
      undefined
    )
  ...
  it('lets explicit startup draft agents override the desktop default', async () => {
    ...
    const draftUrl = 'https://github.com/anthovai/kingu-intelligence/issues/789'
    const result = await runtime.createManagedWorktree({
      repoSelector: 'id:repo-1',
