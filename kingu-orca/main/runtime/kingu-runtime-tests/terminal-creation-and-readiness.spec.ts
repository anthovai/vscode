...
import { KinguRuntimeService, electronMocks, makePaneKey } from '../kingu-runtime-test-mocks.spec'
import {
  HEADLESS_LEAF_ID,
...
describe('KinguRuntimeService', () => {
  it('creates visible terminal sessions without asking the renderer to focus a tab', async () => {
    ...
    expect(spawnedEnv.KINGU_WORKTREE_ID).toBe(TEST_WORKTREE_ID)
    expect(spawnedEnv.KINGU_AGENT_LAUNCH_TOKEN).toMatch(UUID_RE)
  ...
  it('retires inherited launch authority when the agent command exits', async () => {
    ...
    const evidence = {
      ...
      paneKey: spawnEnv.KINGU_PANE_KEY,
