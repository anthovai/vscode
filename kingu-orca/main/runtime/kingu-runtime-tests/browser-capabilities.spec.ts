...
} from '../kingu-runtime-test-scenario-builders.spec'
...
describe('KinguRuntimeService', () => {
  ...
  it('advertises the browser identity capability only where an identity store exists', () => {
    ...
    initializeBrowserIdentityModeStore(mkdtempSync(join(tmpdir(), 'kingu-identity-capability-')))
    try {
  ...
  it('reports the driver as missing instead of telling a configured operator to configure it', () => {
    ...
    expect(degradation?.message).not.toMatch(/set KINGU_BROWSER_EXECUTABLE/)
  })
  ...
  it('keeps the degradation wire-safe for peers that predate structured causes', () => {
    ...
    expect(degradation?.message).toBe(
      'KINGU_BROWSER_EXECUTABLE points at a path that does not exist. (/nope/chromium)'
    )
  })
