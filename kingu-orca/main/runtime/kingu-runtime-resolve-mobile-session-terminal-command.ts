...
import { KinguRuntimeWithRunCreateMobileSessionTerminal } from './kingu-runtime-run-create-mobile-session-terminal'
...
export class KinguRuntimeWithResolveMobileSessionTerminalCommand extends KinguRuntimeWithRunCreateMobileSessionTerminal {
  protected async resolveMobileSessionTerminalCommand(
    ...
    // Why: SSH runs the CLI through the relay shim (plain `kingu`), so the Linux-only `kingu-ide` rename must not apply.
