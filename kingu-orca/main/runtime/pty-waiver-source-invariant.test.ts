...
const FILE_GROUPS = [
  ...
  },
  {
    ...
    files: [
      join(__dirname, 'kingu-runtime-pty-foreground-process-reads.ts'),
      join(__dirname, 'kingu-runtime-resolve-worktree-removal-target.ts'),
      join(__dirname, 'kingu-runtime-remove-managed-worktree.ts')
    ]
  }
] as const
