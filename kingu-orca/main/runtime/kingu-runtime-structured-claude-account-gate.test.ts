...
function runtimeWithAccounts(claude: ClaudeManagedAccountGateSettings | null): KinguRuntimeService {
  // No store at all is the unreadable-settings case the gate must fail closed on.
  const runtime = claude
    ? new KinguRuntimeService({ getSettings: () => claude } as never)
    : new KinguRuntimeService()
