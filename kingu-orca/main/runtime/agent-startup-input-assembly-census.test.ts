...
const MIGRATED_HOST_LAUNCH_MODULES = [
  'src/main/runtime/kingu-runtime-create-agent-session.ts',
  'src/main/runtime/kingu-runtime-resolve-mobile-session-terminal-command.ts',
  'src/main/runtime/kingu-runtime-resolve-worktree-removal-target.ts',
...
describe('host agent-startup input assembly census', () => {
  ...
  it('detects hand assembly when it is present', () => {
    ...
    const resumeSite = read(
      'src/main/runtime/kingu-runtime-get-agent-session-execution-namespace.ts'
    )
