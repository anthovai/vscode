...
async function launchOpenCodePane(options: {
  ...
  attestAgentHookCompatibilityAuthority?: KinguRuntimeServiceDeps['attestAgentHookCompatibilityAuthority']
  ...
  const paneKey = env.KINGU_PANE_KEY as string
  const launchToken = env.KINGU_AGENT_LAUNCH_TOKEN as string
...
type KinguRuntimeServiceDeps = NonNullable<ConstructorParameters<typeof KinguRuntimeService>[2]>
...
describe('OpenCode finished-session launch authority (STA-4557)', () => {
  ...
  it('stops attesting the finished session token for the reused pane', async () => {
    ...
    const post = (payload: Record<string, unknown>): Promise<Response> =>
      fetch(`http://127.0.0.1:${hookEnv.KINGU_AGENT_HOOK_PORT}/hook/opencode`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Kingu-Agent-Hook-Token': hookEnv.KINGU_AGENT_HOOK_TOKEN
        },
        body: JSON.stringify({
    ...
    // Every later process in this shell inherits KINGU_AGENT_LAUNCH_TOKEN from the PTY env,
  ...
  it('does not rehydrate a finished session token as restored authority after a restart', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'kingu-sta4557-'))
    ...
    await fetch(`http://127.0.0.1:${hookEnv.KINGU_AGENT_HOOK_PORT}/hook/opencode`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Kingu-Agent-Hook-Token': hookEnv.KINGU_AGENT_HOOK_TOKEN
    ...
    // After a restart the PTY survives with KINGU_AGENT_LAUNCH_TOKEN still in its env and
