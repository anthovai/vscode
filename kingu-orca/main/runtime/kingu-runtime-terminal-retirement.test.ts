...
function syncSplit(runtime: KinguRuntimeService, snapshot = makeSplitSnapshot()): void {
...
describe('KinguRuntimeService terminal surface retirement', () => {
  ...
  it('rejects one stale shared-PTY surface without removing its live sibling', () => {
    ...
    const runtime = new KinguRuntimeService(runtimeStore({ getWorkspaceSession: () => session }))
    runtime.attachWindow(1)
