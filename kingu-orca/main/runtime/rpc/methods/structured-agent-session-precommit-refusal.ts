...
function precommitRefusal(error: unknown): AgentSessionWireRefusal {
  ...
  if (code) {
    return { code, message: 'Kingu cannot open a structured agent chat for this workspace.' }
  }
  ...
  return {
    ...
    message: `Kingu could not prepare a structured agent chat for this workspace: ${message}`
  }
}
