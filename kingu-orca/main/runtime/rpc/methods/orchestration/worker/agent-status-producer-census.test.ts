...
const CENSUS: readonly CensusRow[] = [
  ...
  {
    path: 'main/kingud/kingud-entry.ts',
    kind: 'wiring',
    role: 'binds the same snapshot, OSC producer and structured sink into the headless kingud runtime deps'
  },
  {
    path: 'main/runtime/kingu-runtime-state-fields.ts',
    kind: 'wiring',
    role: 'stores the snapshot deps on the runtime'
  },
  {
    path: 'main/runtime/kingu-runtime-preserved-branch-cleanup.ts',
    kind: 'wiring',
    role: 'declares the snapshot dep fields'
  },
  {
    path: 'main/runtime/kingu-runtime-get-orchestration-dispatch-authority.ts',
    kind: 'produces',
    role: 'getOrchestrationFleetAgentStatusSnapshot — delegates to the checked snapshot module'
  },
  {
    path: 'main/runtime/kingu-runtime-stop-requested-pty-ids.ts',
    kind: 'wiring',
  ...
  {
    path: 'main/runtime/kingu-runtime-get-worktree-ps.ts',
    kind: 'consumes',
    role: 'worktree.ps inline agent rows (driven below)'
  },
  {
    path: 'main/runtime/kingu-runtime-get-terminal-interactive-wait.ts',
    kind: 'consumes',
    role: 'exact-worker provider session selection, matched on pane key'
  },
  {
    path: 'main/runtime/kingu-runtime-serialize-agent-prompt-submission.ts',
    kind: 'consumes',
    role: 'prompt-submission serialization, matched on pane key'
  },
  {
    path: 'main/runtime/kingu-runtime-resolve-recovered-structured-tui-transcript.ts',
    kind: 'consumes',
    role: 'recovered transcript resolution from provider-session rows, matched on pane key'
  },
  {
    path: 'main/runtime/kingu-runtime-prune-mobile-session-tab-group-layout.ts',
    kind: 'consumes',
...
function censusRuntime(): KinguRuntimeService {
...
describe('agent status producer census', () => {
  ...
  it('lists a worktree.ps agent row from a hook row that carries only a pane key', async () => {
    ...
    const runtime = new KinguRuntimeService(censusStore() as never, undefined, {
