...
// Why these shapes: Codex agents working on Kingu print `rg` hits from this very detector and its
...
function codexIdleScreen(): string[] {
  return [
    ...
    '',
    '  gpt-6-astra medium · ~/kingu/workspaces/kingu/fix-wait-detector-scrollback'
  ]
}
...
function codexScrollback(quotedLines: string[], trailingLineCount: number): string[] {
  const lines: string[] = [
    ...
    '',
    '• Ran rg -n "press enter to confirm" src/main/runtime/terminal-wait-detection.ts src/main/runtime/kingu-runtime-tests/agent-status-and-waits.spec.ts',
    '  └ src/main/runtime/terminal-wait-detection.ts',
    '    src/main/runtime/kingu-runtime-tests/agent-status-and-waits.spec.ts',
    '    src/main/runtime/kingu-runtime-tests/terminal-creation-and-readiness-part-07.spec.ts',
    ...quotedLines
  ]
...
describe('detectTerminalWaitBlockedReason scrollback bounding', () => {
  ...
  it('does not let quoted scrollback wording veto a Codex ready header', () => {
    const waitText = waitTextFor([
      ...
      ' directory:   ~/kingu/workspaces/kingu/fix-wait-detector-scrollback'
    ])
...
describe('detectTerminalWaitBlockedReason live prompts', () => {
  ...
  it('reports the newest prompt when a live dialog follows a stale one at the bottom', () => {
    const waitText = waitTextFor([
      ...
      ' directory:   ~/kingu/workspaces/kingu/cli-debug',
      'Hooks need review',
    ...
    [
      ...
      },
      {
        ...
        lines: [
          'Antigravity CLI 1.0.3',
          'Do you trust the files in this folder?',
          '~/kingu/workspaces/kingu/agy-dispatch-issue',
          '1. Yes',
          '2. No',
          '> Yes'
        ],
