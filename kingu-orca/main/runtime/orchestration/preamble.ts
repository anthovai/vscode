...
import { KINGU_DISPATCH_PROMPT_LEAD_LINE } from '../../../shared/kingu-dispatch-status-prompt'
...
export type PreambleParams = {
  ...
  workerHandle: string
  devMode?: boolean
  // Why: packaged WSL panes install the scoped launcher as `kingu-ide`;
  // other execution hosts keep their existing bare `kingu` bridge.
  ...
  // shells have no agent prompt for Kingu to reuse.
...
// Why: the dispatch preamble teaches agents about Kingu's CLI commands for
...
export function buildDispatchPreamble(params: PreambleParams): string {
  // Why: in dev mode, agents must use kingu-dev to connect to the dev runtime's
  // socket. Without this, agents inside the dev Electron app would call the
  // production CLI and talk to the wrong Kingu instance (Section 6.4).
  const cli = params.devMode ? 'kingu-dev' : (params.cliCommand ?? 'kingu')
  ...
  const header = `You are working inside Kingu, a multi-agent IDE. You are a dispatched worker.
...
export function dispatchPreambleSendOptions(requestId: string): DispatchPreambleSendOptions {
  ...
  return {
    leadLine: KINGU_DISPATCH_PROMPT_LEAD_LINE,
    acceptQueued: true,
...
prompt for Kingu to reuse; if the coordinator has more for you it will
