/**
 * Drives the runtime's own wiring: a real KinguRuntimeService, the real page registry, and the real
...
function startRuntime(profile: ReturnType<typeof createProfile>): KinguRuntimeService {
  const runtime = new KinguRuntimeService(profile.store, undefined, {})
  runtime.rehydrateClientHostedBrowserPages()
  return runtime
}

function placeAndPublish(runtime: KinguRuntimeService, browserPageId: string): void {
...
function attachHost(runtime: KinguRuntimeService) {
