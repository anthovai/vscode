...
/** The launched CLI name is the whole decision: `kingu` is the relay shim, `kingu-ide` is local. */
...
describe('buildWorktreeStartupForAgent host resolution', () => {
  ...
  it('drops the Linux-only rename for both spellings of SSH ownership on two hosts', () => {
    expect(launchCliNameFor(makeRepo({ connectionId: 'm4air' }))).toBe('kingu')
    expect(launchCliNameFor(makeRepo({ executionHostId: 'ssh:openclaw' }))).toBe('kingu')
  })
  ...
  it('keeps the rename for a runtime host with no nested SSH target', () => {
    expect(launchCliNameFor(makeRepo({ executionHostId: 'runtime:vm-1' }))).toBe('kingu-ide')
  })
...
describe('markLocalWorktreeTrusted', () => {
  ...
  /**
   * Why this test exists: Kingu has two trust dispatch chains — the renderer's
