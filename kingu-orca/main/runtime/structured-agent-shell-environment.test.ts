...
const processEnv = { PATH: '/kingu/bin', HOME: '/home/me', KINGU_USER_DATA_PATH: '/kingu' }
...
describe('structuredAgentBaseEnvironment', () => {
  ...
  it('passes only the baseline and listed shell names over Kingu env when off', () => {
    expect(
      structuredAgentBaseEnvironment({
        shellEnv,
  ...
  it('matches names case-insensitively on Windows without duplicating Path', () => {
    expect(
      structuredAgentBaseEnvironment({
        ...
        policy: { inheritAll: false, names: ['CODEX_LB_API_KEY'] },
        processEnv: { Path: 'C:\\kingu', USERPROFILE: 'C:\\Users\\me' },
        platform: 'win32'
      })
