...
import { KinguRuntimeWithBuildPtyTerminalSummary } from './kingu-runtime-build-pty-terminal-summary'
import type { PtyIncarnationHandleRecord } from './kingu-runtime-core'
import type { RuntimeLeafRecord, RuntimePtyWorktreeRecord } from './runtime-terminal-state-records'
import { randomUUID } from 'node:crypto'

export class KinguRuntimeWithBindPtyIncarnationHandle extends KinguRuntimeWithBuildPtyTerminalSummary {
  ...
  protected rememberDetachedPreAllocatedLeaves(): void {
    for (const leaf of this.leaves.values()) {
      if (leaf.ptyId && this.handleByPtyId.has(leaf.ptyId)) {
        // Why: KINGU_TERMINAL_HANDLE is an agent identity, so CLI control survives renderer graph loss while the PTY is alive.
