...
describe('KinguRuntimeRpcServer WebSocket bind host (STA-2370)', () => {
  ...
  it('binds all interfaces at startup when exposeNetworkByDefault is set (kingu serve)', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'kingu-runtime-rpc-'))
    const server = new KinguRuntimeRpcServer({
      runtime: new KinguRuntimeService(),
      userDataPath,
  ...
  it('upgrades a reused pending grant to network reach so its link survives a relaunch', async () => {
    ...
    const relaunched = new KinguRuntimeRpcServer({
      runtime: new KinguRuntimeService(),
      userDataPath,
      enableWebSocket: true,
      wsPort: 0
  ...
  it('honours a pinned bind host over exposeNetworkByDefault (kingud --bind)', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'kingu-runtime-rpc-'))
    const server = new KinguRuntimeRpcServer({
      runtime: new KinguRuntimeService(),
      userDataPath,
  ...
  it('stays pinned to loopback even after a device has connected once', async () => {
    ...
    // network-reach device has connected. A loopback kingud would therefore go wide one
