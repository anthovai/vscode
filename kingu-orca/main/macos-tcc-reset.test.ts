...
describe('readMacosBundleId', () => {
  it('reads CFBundleIdentifier out of the bundle’s Info.plist', async () => {
    runProcessMock.mockResolvedValue(processResult({ stdout: 'com.anthovai.kingu\n' }))

    await expect(readMacosBundleId('/Applications/Kingu.app')).resolves.toBe('com.anthovai.kingu')
    expect(runProcessMock).toHaveBeenCalledWith(
      expect.objectContaining({
        program: '/usr/libexec/PlistBuddy',
        args: ['-c', 'Print :CFBundleIdentifier', '/Applications/Kingu.app/Contents/Info.plist']
      })
    )
  })
  ...
  it.each([
    ...
    await expect(readMacosBundleId('/Applications/Kingu.app')).resolves.toBeNull()
  })
...
describe('resetMacosTccPermission', () => {
  it('clears the service’s row for the bundle id', async () => {
    ...
    await expect(
      resetMacosTccPermission('SystemPolicyDocumentsFolder', 'com.anthovai.kingu')
    ).resolves.toEqual({ ok: true })
    expect(runProcessMock).toHaveBeenCalledWith(
      expect.objectContaining({
        program: '/usr/bin/tccutil',
        args: ['reset', 'SystemPolicyDocumentsFolder', 'com.anthovai.kingu']
      })
    )
  })
  ...
  it.each([
    ...
    await expect(
      resetMacosTccPermission('SystemPolicyDownloadsFolder', 'com.anthovai.kingu')
