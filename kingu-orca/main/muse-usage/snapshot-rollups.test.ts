...
// One session that ran model `a` inside the Kingu worktree and model `b` outside it.
...
describe('Muse usage snapshot rollups', () => {
  it('counts a session only toward models it used inside the selected scope', () => {
    const kinguRows = buildMuseUsageBreakdownRows(
      'model',
      'kingu',
      [daily('a', 'worktree:wt-1', 'wt-1')],
      [session]
    )
    expect(kinguRows.map((row) => [row.key, row.sessions])).toEqual([['a', 1]])
