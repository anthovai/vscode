...
export function describeStrandedAutomationRun(run: AutomationRun): string {
  if (run.status === 'dispatching') {
    return 'Kingu stopped before this run reported that its agent started.'
  }
  return 'Kingu lost the terminal for this run before it reported completion.'
}
...
function describeObservationError(error: unknown): string {
  ...
  return 'Kingu stopped watching this run before it reported completion.'
}
