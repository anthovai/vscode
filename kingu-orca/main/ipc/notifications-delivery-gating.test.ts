...
describe('registerNotificationHandlers', () => {
  ...
  it('opens the current macOS app notification settings entry', async () => {
    ...
    const originalBundleId = process.env.KINGU_DEV_MACOS_BUNDLE_ID
    Object.defineProperty(process, 'platform', { value: 'darwin', configurable: true })
    process.env.KINGU_DEV_MACOS_BUNDLE_ID = 'com.anthovai.kingu.dev.fb5a47066f08'
    try {
      registerNotificationHandlers({
        getSettings: () => ({
          notifications: {
      ...
      expect(shellOpenExternalMock).toHaveBeenCalledWith(
        'x-apple.systempreferences:com.apple.Notifications-Settings.extension?id=com.anthovai.kingu.dev.fb5a47066f08'
      )
    } finally {
      Object.defineProperty(process, 'platform', { value: originalPlatform, configurable: true })
      if (originalBundleId === undefined) {
        delete process.env.KINGU_DEV_MACOS_BUNDLE_ID
      } else {
        process.env.KINGU_DEV_MACOS_BUNDLE_ID = originalBundleId
      }
    }
  })
  ...
  it('suppresses active-worktree notifications while Kingu is focused', async () => {
