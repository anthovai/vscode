...
describe('registerPtyHandlers', () => {
  ...
  it('spawns a plain POSIX login shell and queues startup commands for the live session', async () => {
    ...
    const originalKinguOrigZdotdir = process.env.KINGU_ORIG_ZDOTDIR
    ...
    try {
      ...
      // config dir to hand back and Kingu must not invent one — the wrapper
      // leaves ZDOTDIR unset, exactly as an unwrapped login zsh would.
      expect(options.env.KINGU_ORIG_ZDOTDIR).toBeUndefined()
    } finally {
      Object.defineProperty(process, 'platform', {
        configurable: true,
        value: originalPlatform
      ...
      if (originalKinguOrigZdotdir === undefined) {
        delete process.env.KINGU_ORIG_ZDOTDIR
      } else {
        process.env.KINGU_ORIG_ZDOTDIR = originalKinguOrigZdotdir
      }
