...
const FEEDBACK_API_URL = 'https://www.onkingu.dev/v1/feedback'
...
// which Kingu build and which OS the feedback came from. The main process is
...
function feedbackRequestBodyInit(body: FeedbackSubmitBody): Pick<RequestInit, 'body' | 'headers'> {
  ...
  if (body.diagnosticBundle) {
    ...
    formData.append(
      ...
      `kingu-diagnostics-${body.diagnosticBundle.bundleSubmissionId}.ndjson`
    )
  }
...
export async function submitFeedback(
  ...
  try {
    ...
    // Why: api.onkingu.dev serves a different product, so transient failures
