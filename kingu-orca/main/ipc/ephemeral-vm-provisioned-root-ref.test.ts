...
beforeEach(() => {
  handlers.clear()
  userDataPath = mkdtempSync(join(tmpdir(), 'kingu-vm-ref-user-data-'))
  repoPath = mkdtempSync(join(tmpdir(), 'kingu-vm-ref-repo-'))
...
function writeRecipe(path: string, createCountPath: string, createEnvPath?: string): void {
  ...
  writeFileSync(
    ...
    [
      ...
      ...(createEnvPath
        ? [
            `require('node:fs').writeFileSync(${JSON.stringify(createEnvPath)}, JSON.stringify({ref:process.env.KINGU_REPO_REF,refHead:process.env.KINGU_REPO_REF_HEAD,repoUrl:process.env.KINGU_REPO_URL}))`
          ]
        : []),
