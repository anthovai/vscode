...
describe('resolveWatcherProcessEntryPath', () => {
  ...
  it('uses the unpacked nested entry for packaged apps', () => {
    const appPath = path.join('C:', 'Kingu', 'resources', 'app.asar')
  ...
  it('uses the adjacent entry for a packaged host whose app root is not an asar', () => {
    // kingud: a packaged Node bundle that ships this child beside kingud.js. Gating the
    // adjacent probe on isPackaged sent it to a desktop out/main that never exists here.
    const kingudRoot = path.join(path.sep, 'opt', 'kingu')
    const adjacentEntry = path.join(kingudRoot, 'parcel-watcher-process-entry.js')

    expect(
      resolveWatcherProcessEntryPath(kingudRoot, true, (candidate) => candidate === adjacentEntry)
    ).toBe(adjacentEntry)
  })
  ...
  it('falls back to the nested entry when a non-asar packaged host ships no adjacent child', () => {
    ...
    expect(resolveWatcherProcessEntryPath(kingudRoot, true, () => false)).toBe(
      path.join(kingudRoot, 'out', 'main', 'parcel-watcher-process-entry.js')
    )
  })

  it('uses resourcesPath for packaged Electron-as-Node serve processes', () => {
    const resourcesPath = path.join('Applications', 'Kingu.app', 'Contents', 'Resources')
  ...
  it('keeps the cwd build fallback when resourcesPath has no packaged entry', () => {
    const appRoot = path.join('workspace', 'kingu')
