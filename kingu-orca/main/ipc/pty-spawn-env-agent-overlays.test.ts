...
describe('registerPtyHandlers', () => {
  ...
  describe('spawn environment', () => {
    ...
    it('injects the OpenCode hook env into Kingu terminal PTYs', async () => {
      ...
      expect(env.KINGU_OPENCODE_HOOK_PORT).toBe('4567')
      expect(env.KINGU_OPENCODE_HOOK_TOKEN).toBe('opencode-token')
      expect(env.KINGU_OPENCODE_PTY_ID).toBe('test-pty')
      expect(env.OPENCODE_CONFIG_DIR).toEqual(expect.any(String))
      expect(env.KINGU_OPENCODE_CONFIG_DIR).toBe(env.OPENCODE_CONFIG_DIR)
    })
    it('mirrors the original OpenCode source dir when launched from an Kingu overlay shell', async () => {
      const env = await spawnAndGetEnv({
        OPENCODE_CONFIG_DIR: '/tmp/parent-kingu-opencode-overlay',
        KINGU_OPENCODE_SOURCE_CONFIG_DIR: '/tmp/user-opencode-config'
      })
      expect(openCodeBuildPtyEnvMock).toHaveBeenCalledWith(
        expect.any(String),
        '/tmp/user-opencode-config'
      )
      expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/kingu-opencode-overlay')
      expect(env.KINGU_OPENCODE_CONFIG_DIR).toBe('/tmp/kingu-opencode-overlay')
      expect(env.KINGU_OPENCODE_SOURCE_CONFIG_DIR).toBe('/tmp/user-opencode-config')
    })
    it('does not treat inherited Kingu OpenCode config as user config without a source dir', async () => {
      const env = await spawnAndGetEnv({
        OPENCODE_CONFIG_DIR: '/tmp/parent-kingu-opencode-overlay',
        KINGU_OPENCODE_CONFIG_DIR: '/tmp/parent-kingu-opencode-overlay'
      })

      expect(openCodeBuildPtyEnvMock).toHaveBeenCalledWith(expect.any(String), undefined)
      expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/kingu-opencode-config')
      expect(env.KINGU_OPENCODE_CONFIG_DIR).toBe('/tmp/kingu-opencode-config')
      expect(env.KINGU_OPENCODE_SOURCE_CONFIG_DIR).toBeUndefined()
    })
    it('restores user OpenCode config when agent status hooks are disabled in a nested Kingu shell', async () => {
      const env = await spawnAndGetEnv(
        {
          OPENCODE_CONFIG_DIR: '/tmp/parent-kingu-opencode-overlay',
          KINGU_OPENCODE_CONFIG_DIR: '/tmp/parent-kingu-opencode-overlay',
      ...
      expect(env.KINGU_OPENCODE_CONFIG_DIR).toBeUndefined()
      expect(env.KINGU_OPENCODE_SOURCE_CONFIG_DIR).toBeUndefined()
    })
    ...
    it('injects MiMo overlay env only when launch command is mimo', async () => {
      ...
      expect(env.MIMOCODE_HOME).toBe('/tmp/kingu-mimocode-shared')
      expect(env.KINGU_MIMOCODE_HOME).toBe('/tmp/kingu-mimocode-shared')
      expect(env.KINGU_MIMOCODE_SOURCE_HOME).toBeUndefined()
    })
    ...
    it('restores user MiMo home when agent status hooks are disabled in a nested Kingu shell', async () => {
      const env = await spawnAndGetEnv(
        {
          MIMOCODE_HOME: '/tmp/parent-kingu-mimocode-overlay',
          KINGU_MIMOCODE_HOME: '/tmp/parent-kingu-mimocode-overlay',
          KINGU_MIMOCODE_SOURCE_HOME: '/tmp/user-mimocode-home'
        },
        undefined,
        undefined,
        () => ({ agentStatusHooksEnabled: false }),
      ...
      expect(env.KINGU_MIMOCODE_HOME).toBeUndefined()
      expect(env.KINGU_MIMOCODE_SOURCE_HOME).toBeUndefined()
    })
    posixOnlyIt(
      'reproduces issue #1534: GUI-launched Kingu mirrors zshrc-only OpenCode config',
      async () => {
        ...
        const env = await spawnAndGetEnv(undefined, {
          ...
          KINGU_OPENCODE_SOURCE_CONFIG_DIR: undefined
        })
        ...
        expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/kingu-opencode-overlay')
        expect(env.KINGU_OPENCODE_CONFIG_DIR).toBe('/tmp/kingu-opencode-overlay')
        expect(env.KINGU_OPENCODE_SOURCE_CONFIG_DIR).toBe('/home/pim/company/opencode-config')
        expect(env.OPENCODE_CONFIG_DIR).not.toBe(env.KINGU_OPENCODE_SOURCE_CONFIG_DIR)
      }
    )
    it('installs Pi managed extensions without redirecting Kingu terminal PTY homes', async () => {
      ...
      expect(env.KINGU_PI_CODING_AGENT_DIR).toBeUndefined()
      expect(env.KINGU_PI_SOURCE_AGENT_DIR).toBe('/tmp/user-pi-agent')
      expect(env.KINGU_OMP_CODING_AGENT_DIR).toBeUndefined()
      expect(env.KINGU_OMP_STATUS_EXTENSION).toBe(
        '/tmp/kingu-user-data/omp-managed-status-extension/kingu-agent-status.ts'
      )
      expect(env.KINGU_OMP_SOURCE_AGENT_DIR).toBeUndefined()
    })
    it('does not materialize a missing Pi home when another agent mentions Pi', async () => {
      ...
      expect(env.KINGU_PI_SOURCE_AGENT_DIR).toBeUndefined()
    })
    it('materializes Pi home for an explicit Pi launch through a custom command', async () => {
      ...
      expect(env.KINGU_PI_SOURCE_AGENT_DIR).toBe('/tmp/default-pi-agent')
    })
    it.each([
      'omp',
      ...
      // Why: OMP launches emit KINGU_OMP_* shadow vars, not Pi-named ones; only PI_CODING_AGENT_DIR stays (OMP's own binary reads it).
      const env = await spawnAndGetEnv(
        undefined,
      ...
      expect(env.KINGU_OMP_CODING_AGENT_DIR).toBeUndefined()
      expect(env.KINGU_OMP_STATUS_EXTENSION).toBe(
        '/tmp/user-omp-agent/extensions/kingu-agent-status.ts'
      )
      expect(env.KINGU_OMP_SOURCE_AGENT_DIR).toBe('/tmp/user-omp-agent')
    ...
    it('installs Prime status into its independent agent dir on explicit launch', async () => {
      ...
      expect(env.PI_CODING_AGENT_DIR).toBe('/tmp/user-pi-agent')
      expect(env.KINGU_PRIME_AGENT_SOURCE_AGENT_DIR).toBe('/tmp/user-prime-agent')
      expect(env.KINGU_PI_SOURCE_AGENT_DIR).toBeUndefined()
      expect(env.KINGU_OMP_SOURCE_AGENT_DIR).toBeUndefined()
    })
    ...
    it('mirrors the original Pi source dir when launched from an Kingu overlay shell', async () => {
      const env = await spawnAndGetEnv({
        PI_CODING_AGENT_DIR: '/tmp/parent-kingu-pi-overlay',
        KINGU_PI_SOURCE_AGENT_DIR: '/tmp/user-pi-agent'
      })
      expect(piBuildPtyEnvMock).toHaveBeenCalledWith(
        expect.any(String),
        '/tmp/user-pi-agent',
      ...
      expect(env.PI_CODING_AGENT_DIR).toBe('/tmp/parent-kingu-pi-overlay')
      expect(env.KINGU_PI_CODING_AGENT_DIR).toBeUndefined()
      expect(env.KINGU_PI_SOURCE_AGENT_DIR).toBe('/tmp/user-pi-agent')
    })
  })
