...
import { parseKinguYaml, hasHooksFile, loadHooks, hasUnrecognizedKinguYamlKeys } from '../../hooks'
...
export function registerWorktreeHookCheckHandler(context: WorktreeIpcContext): void {
  ...
  ipcMain.handle(
    ...
    async (_event, args: { repoId: string; hostId?: ExecutionHostId }) => {
      ...
      if (repo.connectionId) {
        ...
        try {
          ...
          return {
            ...
            hooks: result.isBinary ? null : parseKinguYaml(result.content),
            mayNeedUpdate: false
          }
        } catch (error) {
          return {
      ...
      // Why: unrecognised top-level keys mean the file is well-formed but from a newer Kingu; suggest updating rather than "could not be parsed".
      const mayNeedUpdate = has && !hooks && hasUnrecognizedKinguYamlKeys(repo.path)
      return {
        status: 'ok',
        hasHooks: has,
        hooks,
