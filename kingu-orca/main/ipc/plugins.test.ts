...
describe('plugin consent IPC schema', () => {
  it('requires the fingerprint reviewed by the caller', () => {
    expect(() =>
      parsePluginConsentArgs({ pluginKey: 'kingu-samples.demo', decision: 'approve' })
    ).toThrow()
  })
...
describe('plugin removal authority', () => {
  it('allows installed rows but refuses dev overrides and unknown keys', () => {
    const service = {
      getDiscovered: () => [
        { pluginKey: 'kingu-samples.installed', isDev: false },
        { pluginKey: 'kingu-samples.dev', isDev: true }
      ]
    } as unknown as PluginService

    expect(canRemoveInstalledPlugin(service, 'kingu-samples.installed')).toBe(true)
    expect(canRemoveInstalledPlugin(service, 'kingu-samples.dev')).toBe(false)
    expect(canRemoveInstalledPlugin(service, 'kingu-samples.unknown')).toBe(false)
  })

  it('refuses bundled installs because startup would restore them', () => {
    const service = {
      getDiscovered: () => [{ pluginKey: 'anthovai.kingu-theme', isDev: false }]
    } as unknown as PluginService
    const lock = {
      version: 1,
      plugins: {
        'anthovai.kingu-theme': {
          pluginKey: 'anthovai.kingu-theme',
          version: '1.0.0',
          source: { kind: 'bundled', bundleId: 'anthovai.kingu-theme' },
          resolvedCommit: null,
    ...
    expect(canRemoveInstalledPlugin(service, 'anthovai.kingu-theme', lock)).toBe(false)
  })
})
