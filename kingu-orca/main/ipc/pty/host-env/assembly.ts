...
import { prependKinguCliDirToChildPath } from '../../../cli/kingu-cli-child-path'
...
import { stripInheritedKinguCodexHomeOverride } from './codex-home'
import {
...
export function buildPtyHostEnv(
  ...
  if (opts.agentStatusHooksEnabled) {
    // Why: OPENCODE_CONFIG_DIR is a single path, not a colon-list; mirror the user's value into an overlay so their plugins and Kingu's status plugin coexist. See docs/opencode-config-dir-collision.md.
    const openCodeStatusService =
      openCodeAgent === 'opencode2' ? openCode2HookService : openCodeHookService
    baseEnv.KINGU_OPENCODE_AGENT = openCodeAgent
    ...
    if (baseEnv.OPENCODE_CONFIG_DIR) {
      ...
      baseEnv.KINGU_OPENCODE_CONFIG_DIR = baseEnv.OPENCODE_CONFIG_DIR
      if (preexistingOpenCodeConfigDir) {
        // Why: nested Kingu terminals inherit the overlay as OPENCODE_CONFIG_DIR; keep the real source so overlays don't mirror overlays.
        baseEnv.KINGU_OPENCODE_SOURCE_CONFIG_DIR = preexistingOpenCodeConfigDir
      } else {
        delete baseEnv.KINGU_OPENCODE_SOURCE_CONFIG_DIR
      }
    }
    if (isMimoLaunchCommand(launchCommandHint)) {
      ...
      if (baseEnv.MIMOCODE_HOME) {
        baseEnv.KINGU_MIMOCODE_HOME = baseEnv.MIMOCODE_HOME
        if (preexistingMimocodeHome) {
          baseEnv.KINGU_MIMOCODE_SOURCE_HOME = preexistingMimocodeHome
        } else {
          delete baseEnv.KINGU_MIMOCODE_SOURCE_HOME
        }
      }
    }
  } else {
    ...
    restoreOrStripOverlayEnv(baseEnv, {
      ...
      overlay: 'KINGU_MIMOCODE_HOME',
      source: 'KINGU_MIMOCODE_SOURCE_HOME'
    })
  }
  ...
  if (opts.agentStatusHooksEnabled) {
    ...
    if (opts.isWsl === true) {
      ...
      if (guestEndpoint) {
        baseEnv.KINGU_AGENT_HOOK_ENDPOINT = guestEndpoint
      }
      ...
      if (opencodeOverlayDir) {
        ...
        baseEnv.KINGU_OPENCODE_CONFIG_DIR = opencodeOverlayDir
        delete baseEnv.KINGU_OPENCODE_SOURCE_CONFIG_DIR
      } else {
        ...
        delete baseEnv.KINGU_OPENCODE_CONFIG_DIR
        delete baseEnv.KINGU_OPENCODE_SOURCE_CONFIG_DIR
      }
    }
  }

  // Why: PI_CODING_AGENT_DIR is the user's config/session root; install only Kingu-owned extension files, don't override it.
  if (opts.agentStatusHooksEnabled) {
    ...
    restoreOrStripOverlayEnv(baseEnv, {
      ...
      overlay: 'KINGU_PI_CODING_AGENT_DIR',
      source: 'KINGU_PI_SOURCE_AGENT_DIR'
    })
    restoreOrStripOverlayEnv(baseEnv, {
      primary: 'PI_CODING_AGENT_DIR',
      overlay: 'KINGU_OMP_CODING_AGENT_DIR',
      source: 'KINGU_OMP_SOURCE_AGENT_DIR'
    })
  ...
  if (opts.isWsl && opts.agentStatusHooksEnabled) {
    ...
    if (explicitPiAgentKind === 'pi') {
      ...
      if (guestPiDir) {
        baseEnv.KINGU_PI_SOURCE_AGENT_DIR = guestPiDir
      }
      ...
      if (guestOmpExtension) {
        baseEnv.KINGU_OMP_STATUS_EXTENSION = guestOmpExtension
      }
    }
  }

  // Why: keep the Codex home override PTY-scoped so dev/prod Kingus don't share hooks through ~/.codex.
  if (opts.skipCodexHomeEnv) {
    delete baseEnv.CODEX_HOME
    delete baseEnv.KINGU_CODEX_HOME
    delete baseEnv.KINGU_CODEX_LAUNCH_PREFLIGHT
    ...
    baseEnv.KINGU_CODEX_HOME = opts.selectedCodexHomePath
    ...
    if (preflightCommand) {
      baseEnv.KINGU_CODEX_LAUNCH_PREFLIGHT = preflightCommand
    } else {
      delete baseEnv.KINGU_CODEX_LAUNCH_PREFLIGHT
    }
  } else if (opts.stripInheritedKinguCodexHome) {
    stripInheritedKinguCodexHomeOverride(baseEnv)
    delete baseEnv.KINGU_CODEX_LAUNCH_PREFLIGHT
  } else {
    delete baseEnv.KINGU_CODEX_LAUNCH_PREFLIGHT
  }

  // Why: WSL shells need the managed userData root for shell-ready wrappers; dev-mode terminals need the same export so `kingu` targets the live dev instance.
  if (opts.isWsl) {
    baseEnv.KINGU_USER_DATA_PATH = opts.userDataPath
    // Why: managed WSL registration uses `kingu-ide`; exposing that literal scopes agent guidance to WSL without a bare-kingu shim.
    baseEnv.KINGU_CLI_COMMAND = opts.isPackaged ? 'kingu-ide' : 'kingu-dev'
  } else {
    if (!opts.isPackaged) {
      baseEnv.KINGU_USER_DATA_PATH ??= opts.userDataPath
    }
    delete baseEnv.KINGU_CLI_COMMAND
  }
  prependKinguCliDirToChildPath(baseEnv, {
    isPackaged: opts.isPackaged,
    userDataPath: opts.userDataPath,
    resourcesPath: opts.resourcesPath
  })
  ...
  if (
    ...
    const cliCommand = opts.isWsl ? (opts.isPackaged ? 'kingu-ide' : 'kingu-dev') : 'kingu'
