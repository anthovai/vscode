...
export function registerDaemonManagementHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (): Promise<{
      ...
      // returning to Kingu is the only moment anything can notice. The refresh owns when to skip.
  ...
  // Why a separate channel from the poll: this one has a side effect — it clears Kingu's TCC row and
