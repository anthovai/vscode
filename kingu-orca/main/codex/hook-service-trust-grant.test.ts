...
beforeEach(() => {
  previousDisableTrustRpc = process.env.KINGU_DISABLE_CODEX_TRUST_RPC
  delete process.env.KINGU_DISABLE_CODEX_TRUST_RPC
...
afterEach(() => {
  ...
  if (previousDisableTrustRpc === undefined) {
    ...
    process.env.KINGU_DISABLE_CODEX_TRUST_RPC = previousDisableTrustRpc
  }
...
// separated (the shape the real 0.144.x binary writes). Kingu's
...
describe('CodexHookService app-server trust grant lane', () => {
  it('treats Codex hashes as authoritative and records the verified grant', async () => {
    ...
    const selfComputed = computeTrustedHash({
      ...
      groupIndex: 0,
      handlerIndex: 0,
      command: wrapPosixHookCommand(join(tmpHome, '.kingu', 'agent-hooks', 'codex-hook.sh')),
      timeoutSec: 10
    })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const material = getCodexManagedHookInstallMaterial()
      const userHook = { type: 'command' as const, command: 'after-kingu.sh' }
      writeFileSync(
        targetPath,
        `${JSON.stringify(
          {
  ...
  it('upgrades self-computed trust in place without duplicate logical entries', async () => {
    prepareSystemHome()
    const service = new CodexHookService()
    process.env.KINGU_DISABLE_CODEX_TRUST_RPC = '1'
    expect((await service.install()).state).toBe('installed')
    const managedHome = join(userDataDir, 'codex-runtime-home', 'home')
    delete process.env.KINGU_DISABLE_CODEX_TRUST_RPC
    installCodexLikeGrantRunner()
