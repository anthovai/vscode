...
export function hasCustomCodexHomeOverride(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  const kinguCodexHome = env.KINGU_CODEX_HOME?.trim()
  const normalizedCodexHome = codexHome ? normalizePathForComparison(codexHome) : undefined
  const normalizedKinguCodexHome = kinguCodexHome
    ? normalizePathForComparison(kinguCodexHome)
    : undefined
  // Why: phase 1 owns only ~/.codex and can clean that path on downgrade. A
  // custom home needs cross-home ownership tracking before Kingu may mutate it.
  return Boolean(
    normalizedCodexHome &&
    normalizedCodexHome !== normalizedKinguCodexHome &&
...
export function getCustomCodexHomeOverrideForLaunch(
  ...
  const effectiveEnv = launchEnv
    ? {
        ...
        KINGU_CODEX_HOME: getLaunchEnvValue(launchEnv, 'KINGU_CODEX_HOME')
      }
    : process.env
...
function getLaunchEnvValue(
  launchEnv: NodeJS.ProcessEnv,
  key: 'CODEX_HOME' | 'KINGU_CODEX_HOME' | 'HOME' | 'SHELL' | 'XDG_CONFIG_HOME'
): string | undefined {
