...
export async function refreshCodexRuntimeUserHooksExclusively(
  ...
  // Why: same as install() — capture in-Kingu approvals before this refresh
  ...
  try {
    ...
    // Why: this path is used when Kingu status hooks are disabled. The
    // runtime CODEX_HOME should keep user hooks, but not Kingu-managed trust.
