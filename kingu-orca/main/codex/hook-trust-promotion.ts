...
import { getKinguManagedCodexHomePath, getSystemCodexHomePath } from './codex-home-paths'
import {
...
function getProvenancePath(runtimeHomePath: string): string {
  return join(runtimeHomePath, '.kingu-hook-trust-provenance.json')
}
...
/**
 * Records the runtime config.toml trust state Kingu leaves behind after an
 * install/refresh, so the next launch can tell "entry Kingu wrote" apart from
 ...
 * `config.toml` trust entries Kingu wrote apart from which the user approved
 * inside Codex. Overwriting it from the current config state after a failed
 * read stamps the user's approval as Kingu-written, and promotion then skips it
...
/**
 * Promotes hook approvals the user made inside Kingu-launched Codex (written
...
function promoteCodexRuntimeHookApprovalsToSystemUnsafe(runtimeHomePath: string): void {
  ...
  // Why: without a snapshot of what Kingu last wrote (first launch after
  ...
  // in-Kingu approval. Promoting would resurrect trust the user revoked in
  ...
  for (const [key, state] of runtimeTrust) {
    ...
    if (
      ...
      // Kingu wrote this entry and nothing touched it since — not an approval.
      continue
    }
    ...
    const hook = Array.isArray(definition?.hooks)
      ...
      : undefined
    // Why: never write trust for Kingu's managed status hook into the user's
    // real config — mutating ~/.codex for Kingu's own hooks is exactly what the
