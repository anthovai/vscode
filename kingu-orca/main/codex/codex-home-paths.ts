...
export function resolveKinguManagedCodexHomePath(): string {
  return join(getKinguUserDataPath(), 'codex-runtime-home', 'home')
}

export function getKinguManagedCodexHomePath(): string {
  const managedHomePath = resolveKinguManagedCodexHomePath()
  mkdirSync(managedHomePath, { recursive: true })
  return managedHomePath
}

export function getCodexSessionBackfillStateDirPath(): string {
  return join(getKinguUserDataPath(), 'codex-session-backfill')
}

export function getKinguUserDataPath(): string {
  ...
  if (process.platform === 'win32') {
    return join(process.env.APPDATA ?? join(homedir(), 'AppData', 'Roaming'), 'kingu')
  }
...
export function syncSystemCodexResourcesIntoManagedHome(managedHomePath?: string): void {
  const targetHome = managedHomePath ?? getKinguManagedCodexHomePath()
...
function linkSystemCodexResource(
  ...
  // Why: both branches below DELETE Kingu's mirrored copy because the system
