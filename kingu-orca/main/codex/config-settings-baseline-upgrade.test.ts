...
beforeEach(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'kingu-codex-settings-upgrade-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'kingu-codex-settings-upgrade-data-'))
...
function baselinePath(): string {
  return join(runtimeHomePath(), '.kingu-config-settings-baseline.json')
}
