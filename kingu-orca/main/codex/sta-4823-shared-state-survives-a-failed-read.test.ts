...
const baselinePath = (): string => join(runtimeHomePath, '.kingu-config-settings-baseline.json')

beforeEach(() => {
  denials.reset()
  fakeHomeDir = realFs.mkdtempSync(join(tmpdir(), 'kingu-sta4823-home-'))
  userDataDir = realFs.mkdtempSync(join(tmpdir(), 'kingu-sta4823-data-'))
