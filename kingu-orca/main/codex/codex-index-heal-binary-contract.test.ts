...
// asserts "healed" as "the `thread/read` call did not error". That pins Kingu's half
// of the contract and nothing about Codex's. The behavior Kingu actually depends on
...
//
// Keep it narrow. It pins the four arms that ablation established Kingu relies on,
...
const binary = process.env.KINGU_CODEX_CONTRACT_BINARY
const expectedVersion = process.env.KINGU_CODEX_CONTRACT_VERSION
...
// KINGU_CODEX_CONTRACT_REQUIRED=1, which turns a missing binary into a red test.
describe.runIf(process.env.KINGU_CODEX_CONTRACT_REQUIRED === '1' && !binary)(
  'codex binary index-heal contract prerequisites',
  () => {
    it('was given a Codex binary to run against', () => {
      expect.fail(
        'KINGU_CODEX_CONTRACT_REQUIRED=1 but KINGU_CODEX_CONTRACT_BINARY is unset, so the contract would have silently skipped'
      )
    })
  }
)
...
describeCodexContract(
  ...
  () => {
    ...
    /**
     * Builds a disposable CODEX_HOME in the state Kingu actually heals from: Codex's
    ...
    async function createBackfilledCodexHome(): Promise<string> {
      const home = mkdtempSync(join(tmpdir(), 'kingu-codex-heal-contract-'))
