...
// 'severed': macOS can no longer attribute daemon terminals to Kingu, so Accessibility/
...
// The daemon spawned a terminal into a folder it can't read while Kingu can (STA-7948).
...
// whole remedy, 'denied' means Kingu must be re-allowed first, 'unknown' means main could not tell.
