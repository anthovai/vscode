 ...
 * already delivers the configured root as `KINGU_WORKSPACE_ROOT`; read it before falling back.
 *
...
export function resolveRelaySpawnCwd(args: {
  ...
  const workspaceId = trimmedString(args.worktreeId) ?? trimmedString(args.env?.KINGU_WORKSPACE_ID)
  ...
  const workspaceRoot = trimmedString(args.env?.KINGU_WORKSPACE_ROOT)
