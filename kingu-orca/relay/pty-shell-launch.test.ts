...
describe('getRelayShellLaunchConfig', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const config = getRelayShellLaunchConfig('/bin/zsh', {
        HOME: homeDir,
        KINGU_OPENCODE_CONFIG_DIR: '/tmp/kingu-opencode-overlay'
      })
      const zshRoot = join(homeDir, '.kingu-relay', 'shell-ready', 'zsh')

      expect(config.args).toEqual(['-l'])
      expect(config.env.ZDOTDIR).toBe(zshRoot)
      const zshenv = readFileSync(join(zshRoot, '.zshenv'), 'utf8')
      // Why no KINGU_USER_ZDOTDIR: the relay used to republish the inherited
      ...
      expect(zshenv).not.toContain('KINGU_USER_ZDOTDIR')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      // bridge sets KINGU_REMOTE_CLI_BIN_DIR, which was already enough to wrap.
      const env = buildSshPtySpawnEnv({
        env: { HOME: homeDir, PATH: '/usr/bin:/bin' },
        remoteCliBridgeEnv: {
          binDir: '/home/remote/.kingu-relay/bin',
          relayDir: '/home/remote/.kingu-relay',
          nodePath: '/home/remote/.kingu-relay/node',
          sockPath: '/home/remote/.kingu-relay/relay.sock'
        }
      })
      env.KINGU_HISTFILE = join(homeDir, 'kingu-history', 'zsh_history')

      const config = getRelayShellLaunchConfig('/bin/zsh', env)

      expect(config.env.ZDOTDIR).toBe(join(homeDir, '.kingu-relay', 'shell-ready', 'zsh'))
      expect(config.env.KINGU_SHELL_FEATURES).toBe('overlay,history,markers')
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    'keeps a remote zsh with nothing Kingu-owned on the plain login path',
    () => {
      const env = buildSshPtySpawnEnv({ env: { HOME: homeDir, PATH: '/usr/bin:/bin' } })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const config = getRelayShellLaunchConfig('/bin/zsh', {
        HOME: homeDir,
        KINGU_MIMOCODE_HOME: '/tmp/kingu-mimocode-overlay'
      })
      const zshRoot = join(homeDir, '.kingu-relay', 'shell-ready', 'zsh')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env).toEqual({ KINGU_SHELL_FEATURES: '' })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env.KINGU_SHELL_FEATURES).toContain('ready')
      ...
      expect(zshenv).toContain('builtin unset KINGU_SHELL_FEATURES')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.supportsReadyMarker).toBe(true)
      expect(config.env.KINGU_SHELL_FEATURES).toContain('ready')
      expect(bashRc).toContain('printf "\\033]777;kingu-shell-start:%s\\007" "$$"')
      expect(bashRc).toContain('builtin unset KINGU_SHELL_FEATURES')
      expect(bashRc).toContain('__kingu_append_prompt_command "__kingu_prompt_mark"')
      expect(bashRc).toContain('printf "\\033]777;kingu-shell-ready\\007"')
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env.ZDOTDIR).toBe(join(homeDir, '.kingu-relay', 'shell-ready', 'zsh'))
      expect(config.env.KINGU_SHELL_FEATURES).toContain('identity')
      ...
      expect(config.env.KINGU_SHELL_FEATURES).not.toContain('ready')
  ...
  // Kingu inherits ends in a ";"+whitespace separator. Prepend/append must not
