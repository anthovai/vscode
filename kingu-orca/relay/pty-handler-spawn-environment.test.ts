...
describe('PtyHandler', () => {
  ...
  it("does not forward Kingu's own NODE_ENV into the spawned shell", async () => {
  ...
  it('does not inherit legacy attribution state from the relay process', async () => {
    const keys = ['KINGU_ENABLE_GIT_ATTRIBUTION', 'KINGU_ATTRIBUTION_SHIM_DIR', 'PATH'] as const
    ...
    process.env.PATH = '/tmp/kingu-terminal-attribution/posix:/usr/bin'
    ...
    try {
      ...
      expect(spawnedEnv.env.KINGU_ENABLE_GIT_ATTRIBUTION).toBeUndefined()
      expect(spawnedEnv.env.KINGU_ATTRIBUTION_SHIM_DIR).toBeUndefined()
      ...
      expect(revivedEnv.env.KINGU_ENABLE_GIT_ATTRIBUTION).toBeUndefined()
      expect(revivedEnv.env.KINGU_ATTRIBUTION_SHIM_DIR).toBeUndefined()
    } finally {
  ...
  describe('history isolation off', () => {
    ...
    // isolation on, but fish EXPORTS fish_history, so a relay launched from an Kingu
    ...
    it.each([
      [
        'a relay-minted path',
        `${process.env.HOME ?? ''}/.kingu-remote/terminal-history/aabbccddeeff0011-zsh_history`,
        undefined
      ],
      [
    ...
    // what normally mints (and first clears) KINGU_HISTFILE, and it runs only
    // with isolation on. An inherited one — the relay can be launched from an
    // Kingu pane — would otherwise reach the remote wrapper on the disabled and
    ...
    it.each([
      ...
      'drops %s inherited as KINGU_HISTFILE from the relay process env',
      async (_kind, inherited) => {
        const previous = process.env.KINGU_HISTFILE
        process.env.KINGU_HISTFILE = inherited
        try {
          await dispatcher.callRequest('pty.spawn', { cols: 80, rows: 24 })
        } finally {
          if (previous === undefined) {
            ...
            process.env.KINGU_HISTFILE = previous
          }
        }

        const spawnEnv = mockPtySpawn.mock.calls.at(-1)?.[2]?.env as Record<string, string>
        expect(spawnEnv.KINGU_HISTFILE).toBeUndefined()
      }
    )

    it('drops an KINGU_HISTFILE handed over in the client env', async () => {
      await dispatcher.callRequest('pty.spawn', {
        cols: 80,
        rows: 24,
        env: { KINGU_HISTFILE: '/fake/userData/terminal-history/aabbccddeeff0011/zsh_history' }
      })
  ...
  it('keeps the image protocol hint consistent after renderer and augmenter overrides', async () => {
    handler.addEnvAugmenter(() => ({ KINGU_IMAGE_PROTOCOL: 'sixel' }))
    await dispatcher.callRequest('pty.spawn', {
      cols: 80,
      rows: 24,
      env: { KINGU_IMAGE_PROTOCOL: 'none' }
    })
    expect(mockPtySpawn.mock.calls[0][2].env.KINGU_IMAGE_PROTOCOL).toBe('kitty')
  })
  ...
  it('applies env augmenters after process.env and renderer-supplied env (augmenter wins on key conflict)', async () => {
    handler.addEnvAugmenter(() => ({
      ...
      KINGU_PANE_KEY: 'augmenter-wins'
    }))

    await dispatcher.callRequest('pty.spawn', {
      cols: 80,
      rows: 24,
      env: { KINGU_PANE_KEY: 'tab-1:0', KINGU_TAB_ID: 'tab-1' }
    })
    ...
    expect(callArgs.env.KINGU_PANE_KEY).toBe('augmenter-wins')
    // Renderer-supplied keys not in augmenter map flow through:
    expect(callArgs.env.KINGU_TAB_ID).toBe('tab-1')
  })
  ...
  it('passes PTY and explicit launch identity to env augmenters', async () => {
    ...
    await dispatcher.callRequest('pty.spawn', {
      env: { KINGU_PANE_KEY: 'tab-context:0' },
      launchAgent: 'pi'
    })
    await dispatcher.callRequest('pty.spawn', {})
  ...
  it('applies identity defaults, then deletions, while preserving explicit TERM', async () => {
    handler.addEnvAugmenter(() => ({
      ...
      KINGU_STALE_TEST_ENV: '/tmp/augmenter-stale'
    }))
    ...
    await dispatcher.callRequest('pty.spawn', {
      env: {
        ...
        KINGU_STALE_TEST_ENV: '/tmp/renderer-stale'
      },
      envToDelete: ['TERM_PROGRAM', 'KINGU_STALE_TEST_ENV']
    })
    ...
    expect(spawnEnv.env.KINGU_STALE_TEST_ENV).toBeUndefined()
  })
  ...
  it('replaces an ambient TERM=dumb when no explicit TERM is supplied', async () => {
    ...
    expect(spawnEnv.env.TERM_PROGRAM).toBe('Kingu')
    expect(spawnEnv.env.KINGU_IMAGE_PROTOCOL).toBe('kitty')
  })
  ...
  it('expands variables in PATH before spawning a Windows relay shell', async () => {
    ...
    expect(spawnEnv.env.PATH).toBe('C:\\Users\\kingu\\AppData\\Local\\agy\\bin;C:\\Windows')
  })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const oldKinguPi = process.env.KINGU_PI_CODING_AGENT_DIR
      ...
      try {
        ...
        handler.addEnvAugmenter(() => ({
          ...
          KINGU_OPENCODE_CONFIG_DIR: '/remote/overlay/opencode',
          KINGU_OMP_STATUS_EXTENSION: '/remote/.omp/agent/extensions/kingu-agent-status.ts'
        }))
        ...
        if (oldKinguPi === undefined) {
          delete process.env.KINGU_PI_CODING_AGENT_DIR
        } else {
          process.env.KINGU_PI_CODING_AGENT_DIR = oldKinguPi
        }
      }

      const shellArgs = mockPtySpawn.mock.calls[0][1]
      const spawnOptions = mockPtySpawn.mock.calls[0][2] as { env: Record<string, string> }
      const rcfile = join(homeDir, '.kingu-relay', 'shell-ready', 'bash', 'rcfile')

      expect(shellArgs).toEqual(['--rcfile', rcfile])
      expect(spawnOptions.env.KINGU_OPENCODE_CONFIG_DIR).toBe('/remote/overlay/opencode')
      expect(spawnOptions.env.KINGU_PI_CODING_AGENT_DIR).toBeUndefined()
      expect(readFileSync(rcfile, 'utf8')).toContain(
        'export OPENCODE_CONFIG_DIR="${KINGU_OPENCODE_CONFIG_DIR}"'
      )
      expect(readFileSync(rcfile, 'utf8')).not.toContain('KINGU_PI_CODING_AGENT_DIR')
