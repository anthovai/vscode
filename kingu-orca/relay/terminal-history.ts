...
const HISTORY_ROOT = join(homedir(), '.kingu-remote', 'terminal-history')
...
export function injectRelayHistoryEnv(
  ...
  // Why first: same reason as the desktop path — an inherited KINGU_HISTFILE
  ...
  // Why: HISTFILE stays exported, so a relay (or a client) launched from an Kingu
  ...
  try {
    ...
    env.KINGU_HISTFILE = env.HISTFILE
    return HISTORY_ROOT
  } catch {
    return null
  }
...
export function injectRelayFishHistoryEnv(env: Record<string, string>, worktreeId: string): void {
  ...
  // user value, and fish EXPORTS `fish_history` so an Kingu-minted name arrives from
