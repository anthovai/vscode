...
export function writeRelayOmpStatusExtension(homeDir: string, source: string): string | null {
  const directory = join(homeDir, '.kingu-relay', 'omp-managed-status-extension')
  try {
    mkdirSync(directory, { recursive: true })
    const path = join(directory, 'kingu-agent-status.ts')
    try {
      if (!readFileSync(path, 'utf8').includes('@kingu-managed-pi-extension')) {
        return null
      }
    } catch {
