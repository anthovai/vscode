  ...
  private async buildPluginEnvironment(
    ...
    const opencodeAgent =
      context.launchAgent === 'opencode2' || isOpenCode2LaunchCommand(launchCommandHint)
        ? 'opencode2'
        : 'opencode'
    env.KINGU_OPENCODE_AGENT = opencodeAgent
    if (this.pluginOverlay.hasOpenCodeSource(opencodeAgent)) {
      ...
      if (sourceDir && !inheritedRelayOverlay) {
        ...
        if (dir) {
          ...
          env.KINGU_OPENCODE_CONFIG_DIR = dir
          env.KINGU_OPENCODE_SOURCE_CONFIG_DIR = sourceDir
        }
      } else {
    ...
    if (kind === 'omp' || !hasLaunchCommand) {
      env.KINGU_OMP_FRESH_CONFIG = this.pluginOverlay.materializeOmpFreshConfig()
    }
    ...
    if (kind === 'pi') {
      ...
      if (result?.sourceAgentDir) {
        env.KINGU_PI_SOURCE_AGENT_DIR = result.sourceAgentDir
      }
    }
    if (kind === 'omp' || !hasLaunchCommand) {
      const sourceDir =
        kind === 'omp'
          ...
          : context.env.KINGU_OMP_SOURCE_AGENT_DIR
      ...
      if (result?.statusExtensionPath) {
        env.KINGU_OMP_STATUS_EXTENSION = result.statusExtensionPath
      }
      if (result?.sourceAgentDir) {
        env.KINGU_OMP_SOURCE_AGENT_DIR = result.sourceAgentDir
      }
    }
    if (kind === 'prime-agent') {
      ...
      if (result?.sourceAgentDir) {
        env.KINGU_PRIME_AGENT_SOURCE_AGENT_DIR = result.sourceAgentDir
      }
    }
    return env
  }
