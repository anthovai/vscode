  ...
  private async runPullWithArgsUnlocked(
    ...
    const runPull = async (effectiveArgs: string[]): Promise<void> => {
      ...
      if (upstream && !upstream.isConfiguredUpstream) {
        // Why: legacy Kingu branches may track origin/main while pushes target origin/<branch>; pull the same effective branch the UI reports.
        await this.git(
  ...
  private async runRebaseFromBase(params: Record<string, unknown>, context?: RequestContext) {
    ...
    try {
      try {
        ...
        rebaseRef = `refs/kingu/rebase/${randomUUID()}`
        const fetchArgs = [
          source.remoteName,
