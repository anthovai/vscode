...
describe.skipIf(process.platform === 'win32')('createInstallPluginsHandler (guest side)', () => {
  ...
  it('writes kingu-opencode-status.js into the overlay and returns that dir', () => {
    withHome((home) => {
      const install = createInstallPluginsHandler(new PluginOverlayManager({ homeDir: home }), {
        HOME: home,
        KINGU_WSL_HOOK_INSTANCE: 'inst1'
      })
      const source = '// kingu opencode status plugin\nexport const Plugin = () => ({})\n'
      ...
      const pluginPath = join(dir as string, 'plugins', 'kingu-opencode-status.js')
  ...
  it('materializes the requested Pi extension in the guest home', () => {
    withHome((home) => {
      const install = createInstallPluginsHandler(new PluginOverlayManager({ homeDir: home }), {
        HOME: home,
        KINGU_WSL_HOOK_INSTANCE: 'inst-pi'
      })
      const source = '// @kingu-managed-pi-extension\nexport default {}\n'
      const res = install({ piExtensionSource: source, launchKind: 'pi' })
      expect(res.overlayDirs.pi).toBe(join(home, '.pi', 'agent'))
      const extension = join(home, '.pi', 'agent', 'extensions', 'kingu-agent-status.ts')
  ...
  it('writes the OpenCode 2 plugin to its separate overlay', () => {
    withHome((home) => {
      const install = createInstallPluginsHandler(new PluginOverlayManager({ homeDir: home }), {
        HOME: home,
        KINGU_WSL_HOOK_INSTANCE: 'inst-v2'
      })
      ...
      // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: Runtime validation or the local test fixture establishes the asserted shape.
      expect(
        readFileSync(join(dir as string, 'plugins', 'kingu-opencode2-status.js'), 'utf8')
      ).toBe(source)
  ...
  it('rebuilds if the resolved source dir ever changes (defensive)', () => {
    withHome((home) => {
      ...
      const env: NodeJS.ProcessEnv = { HOME: home, KINGU_WSL_HOOK_INSTANCE: 'inst1' }
      ...
      env.KINGU_OPENCODE_SOURCE_CONFIG_DIR = userConfig
  ...
  it('rebuilds when the cached overlay lost its plugin file', () => {
    withHome((home) => {
      ...
      rmSync(join(dir, 'plugins', 'kingu-opencode-status.js'))
  ...
  it('re-materializes when the shipped source changes', () => {
    withHome((home) => {
      ...
      // Why: a mid-session Kingu upgrade ships new plugin source; future spawns must see it.
      // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: Runtime validation or the local test fixture establishes the asserted shape.
      const dir = install({ opencodePluginSource: '// v2\n' }).overlayDirs.opencode as string
      expect(readFileSync(join(dir, 'plugins', 'kingu-opencode-status.js'), 'utf8')).toBe('// v2\n')
    })
  })
  ...
  it('mirrors an explicitly-set config root so overriding the var does not drop it', () => {
    withHome((home) => {
      ...
      const install = createInstallPluginsHandler(new PluginOverlayManager({ homeDir: home }), {
        HOME: home,
        KINGU_OPENCODE_SOURCE_CONFIG_DIR: userConfig,
        KINGU_WSL_HOOK_INSTANCE: 'inst1'
      })
      // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: Runtime validation or the local test fixture establishes the asserted shape.
  ...
  it('mirrors the XDG default config root when using an overlay', () => {
    withHome((home) => {
      ...
      // carry the user's default config and Kingu's plugin together.
