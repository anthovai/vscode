...
describe('PluginOverlayManager', () => {
  ...
  it('materializes OpenCode plugin into <overlay>/plugins/<file>', () => {
    ...
    const expected = join(dir!, 'plugins', 'kingu-opencode-status.js')
  ...
  it('keeps the OpenCode 2 plugin in a separate overlay and filename', () => {
    ...
    expect(dir).not.toBeNull()
    expect(readFileSync(join(dir!, 'plugins', 'kingu-opencode2-status.js'), 'utf8')).toBe(
      'export const V2 = 1'
    )
    expect(existsSync(join(dir!, 'plugins', 'kingu-opencode-status.js'))).toBe(false)
  })
  ...
  it('installs OpenCode plugins in the canonical XDG config roots', () => {
    ...
    expect(
      readFileSync(join(homeDir, 'xdg', 'opencode', 'plugins', 'kingu-opencode-status.js'), 'utf8')
    ).toBe('v1 plugin')
    expect(
      readFileSync(join(homeDir, 'xdg', 'opencode', 'plugins', 'kingu-opencode2-status.js'), 'utf8')
    ).toBe('v2 plugin')
  })

  it('mirrors a preexisting remote OpenCode config dir before adding Kingu plugin', () => {
    ...
    writeFileSync(join(userConfigDir, 'plugins', 'kingu-opencode-status.js'), 'user same-name')

    manager.setSources({ opencodePluginSource: 'kingu plugin' })
    ...
    expect(readFileSync(join(dir!, 'plugins', 'kingu-opencode-status.js'), 'utf8')).toBe(
      'kingu plugin'
    )
  ...
  // Why: the remote/guest config root keeps whichever Kingu plugin files earlier
  ...
  it.each([
    { agent: 'opencode', stale: 'kingu-opencode2-status.js', own: 'kingu-opencode-status.js' },
    { agent: 'opencode2', stale: 'kingu-opencode-status.js', own: 'kingu-opencode2-status.js' }
  ] as const)(
  ...
  it('installs Pi extension into the real agent extensions dir', () => {
    ...
    const file = join(result!.sourceAgentDir!, 'extensions', 'kingu-agent-status.ts')
    expect(result?.statusExtensionPath).toBe(file)
    expect(existsSync(file)).toBe(true)
    expect(readFileSync(file, 'utf8')).toContain('@kingu-managed-pi-extension')
  })

  it("does not overwrite a user's same-named remote Pi extension file", () => {
    const piAgentDir = join(homeDir, '.pi', 'agent')
    const extensionFile = join(piAgentDir, 'extensions', 'kingu-agent-status.ts')
  ...
  it('uses the kind-specific Pi-compatible extension source when available', () => {
    ...
    expect(
      readFileSync(join(piResult!.sourceAgentDir!, 'extensions', 'kingu-agent-status.ts'), 'utf8')
    ).toContain('// pi extension')
    expect(
      readFileSync(join(ompResult!.sourceAgentDir!, 'extensions', 'kingu-agent-status.ts'), 'utf8')
  ...
  it('installs Kingu status extension into the remote default Pi agent dir', () => {
    const piAgentDir = join(homeDir, '.pi', 'agent')
  ...
  it('mirrors a preexisting remote Pi agent dir instead of the default', () => {
    ...
    expect(readFileSync(join(dir!, 'extensions', 'kingu-agent-status.ts'), 'utf8')).toContain(
      '// pi extension'
    )
  })
  ...
  it('leaves lazy OMP agent.db in the real remote home on the relay', () => {
    ...
    expect(existsSync(join(homeDir, '.kingu-relay', 'omp-overlays'))).toBe(false)
  ...
  describe('per-agent default source dir (no cross-agent fallback)', () => {
    ...
    it('launching pi with both ~/.pi/agent and ~/.omp/agent present installs into ~/.pi/agent', () => {
      ...
      expect(extensions).toContain('kingu-agent-status.ts')
    ...
    it('launching omp when only ~/.pi/agent exists does NOT mirror Pi state', () => {
      ...
      const extensions = readdirSync(join(dir!, 'extensions')).sort()
      expect(extensions).toEqual(['kingu-agent-status.ts'])
    })
    ...
    it('bare-shell prep does not create missing remote agent homes (#10196)', () => {
      ...
      // Why: bare OMP keeps status via ~/.kingu-relay/… without SOURCE_AGENT_DIR or ~/.omp.
      expect(bareOmp?.sourceAgentDir).toBeUndefined()
      expect(bareOmp?.statusExtensionPath).toEqual(
        expect.stringContaining(join('.kingu-relay', 'omp-managed-status-extension'))
      )
  ...
  it('produces stable overlay dirs for a given id (idempotent re-materialization)', () => {
    ...
    expect(readFileSync(join(dirA, 'plugins', 'kingu-opencode-status.js'), 'utf8')).toBe('second')
  })
  ...
  it('hashes unsafe pane ids into portable overlay directory names', () => {
    ...
    expect(existsSync(join(dir!, 'plugins', 'kingu-opencode-status.js'))).toBe(true)
  })
})
...
describe('resolvePiSourceAgentDir', () => {
  it('uses only the selected kind source shadow when resolving inherited overlays', () => {
    const env = {
      ...
      KINGU_PI_CODING_AGENT_DIR: '/tmp/parent-kingu-pi-overlay',
      KINGU_PI_SOURCE_AGENT_DIR: '/user/.pi/agent'
    }
    try {
  ...
  it('keeps explicit PI_CODING_AGENT_DIR values when they are not Kingu overlays', () => {
    const env = {
      HOME: mkdtempSync(join(tmpdir(), 'plugin-overlay-env-')),
