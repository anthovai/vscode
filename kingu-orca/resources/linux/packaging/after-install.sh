#!/bin/bash
# Why: register the bundled `kingu-ide` CLI on PATH at package-install time.
# The in-app "Install CLI" action (CliInstaller) can never run on a headless
# server, so without this symlink `kingu serve` is unreachable from the shell on
...
# symlink works. We discover the install dir instead of hardcoding /opt/Kingu
...
is_owned_link() {
  ...
  for candidate in /opt/Kingu/resources/bin/kingu-ide /opt/kingu-ide/resources/bin/kingu-ide /opt/kingu/resources/bin/kingu-ide; do
...
for dir in /opt/Kingu /opt/kingu-ide /opt/kingu; do
  ...
  shim="$dir/resources/bin/kingu-ide"
  if [ -x "$shim" ]; then
    # Only manage our own symlink; never clobber an unrelated /usr/bin/kingu-ide.
