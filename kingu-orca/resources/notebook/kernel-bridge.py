"""Runs one Jupyter kernel for Kingu in the interpreter that launched this script.

Protocol: one JSON object per line. Kingu writes {"op": "execute", "code": ...} and
{"op": "interrupt"} to stdin; this writes {"type": ...} frames to stdout. Closing
stdin shuts the kernel down, so the kernel never outlives Kingu. When the kernel
dies this process exits, so its exit is the one death signal Kingu watches.
"""
...
def execute_all(client, codes):
    while True:
        code = codes.get()
        # allow_stdin=False: input() raises a clear error instead of reading Kingu's command pipe.
