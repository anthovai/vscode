...
describe('pairing offer', () => {
  ...
  it('encode then decode round-trips correctly', () => {
    ...
    expect(url).toMatch(/^kingu:\/\/pair\?code=/)
  ...
  it('rejects kingu URLs outside the exact pairing route', () => {
    ...
    expect(parsePairingCode(`kingu://pairing?code=${code}`)).toBeNull()
    expect(parsePairingCode(`kingu://pair-extra?code=${code}`)).toBeNull()
    expect(() => decodePairingOffer(`kingu://pairing?code=${code}`)).toThrow('Invalid pairing URL')
  })

  it('rejects URLs without a pairing code', () => {
    expect(() => decodePairingOffer('kingu://pair')).toThrow('Invalid pairing URL')
  })

  it('decodes legacy hash URLs', () => {
    const url = encodePairingOffer(offer)
    const code = new URLSearchParams(url.slice(url.indexOf('?') + 1)).get('code')!
    expect(decodePairingOffer(`kingu://pair#${code}`)).toEqual(offer)
  })
  ...
  it('rejects payloads with missing fields', () => {
    ...
    expect(() => decodePairingOffer(`kingu://pair#${base64}`)).toThrow()
  })
...
describe('parsePairingCode', () => {
  ...
  it('parses a full kingu://pair# URL', () => {
    const url = encodePairingOffer(offer)
