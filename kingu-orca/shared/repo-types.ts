...
import type { RepoHookSettings } from './kingu-yaml-hook-types'
...
export type Repo = {
   ...
   * At spawn, Kingu resolves a short-lived token via `gh auth token --user`
   * and injects it into that child env only.
   */
  ghAccount?: GhAccountBinding
  /** Controls Kingu's fork-default-branch sync offer for repos with upstream metadata. */
  forkSyncMode?: ForkSyncMode
  /** Canonical identity for the repo remote Kingu should use for provider-level grouping. */
  gitRemoteIdentity?: GitRemoteIdentity | null
  /** Controls whether worktrees Kingu did not create appear in the sidebar. */
