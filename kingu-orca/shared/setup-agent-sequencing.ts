...
export const SETUP_AGENT_SEQUENCE_STARTUP_COMMAND_ENV = 'KINGU_SEQUENCED_STARTUP_COMMAND'
export const SETUP_AGENT_SEQUENCE_STARTUP_SCRIPT_ENV = 'KINGU_SEQUENCED_STARTUP_SCRIPT'
...
function buildWindowsSetupCommand(
  ...
  const script = [
    ...
    '$processInfo.Arguments = \'/d /s /v:on /c ""!KINGU_SETUP_RUNNER!""\'',
    '$processInfo.UseShellExecute = $false',
    '$processInfo.EnvironmentVariables["KINGU_SETUP_RUNNER"] = $runner',
...
function buildWindowsStartupCommand(
  ...
  const script = [
    ...
    "$kinguProgress = $ProgressPreference; $ProgressPreference = 'SilentlyContinue'",
    'try { Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force -ErrorAction Stop } ' +
      'catch { [Console]::Error.WriteLine("Kingu: could not relax the execution policy for this " + ' +
      '"session (" + $_.FullyQualifiedErrorId + "). A startup command that runs a .ps1 " + ' +
      '"may be blocked.") }',
    '$ProgressPreference = $kinguProgress',
    `$marker = ${quotePowerShellString(markerPath)}`,
