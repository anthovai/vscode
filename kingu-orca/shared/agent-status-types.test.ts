...
import {
  KINGU_DISPATCH_PROMPT_LEAD_LINE,
  KINGU_DISPATCH_STATUS_PREAMBLE_PREFIX
} from './kingu-dispatch-status-prompt'
...
describe('parseAgentStatusPayload', () => {
  ...
  it('compacts Kingu dispatch preambles so the task body survives 200-char truncation', () => {
...
Fix dispatch fallback preview for normalized status prompts`
    ...
    expect(result!.prompt.startsWith('You are working inside Kingu, a multi-agent IDE.')).toBe(true)
  ...
  it('compacts a Claude hook prompt carrying the typed lead line and paste wrapper', () => {
    const preamble = `${KINGU_DISPATCH_STATUS_PREAMBLE_PREFIX}\nYour task ID is: task_lead\n\n=== TASK ===\nAdd greet()`
    const normalize = (prompt: string): string =>
      normalizeAgentStatusPayload({ state: 'working', prompt })!.prompt
    const compact = `${KINGU_DISPATCH_STATUS_PREAMBLE_PREFIX} Your task ID is: task_lead === TASK === Add greet()`
    ...
    ).toBe(KINGU_DISPATCH_STATUS_PREAMBLE_PREFIX)
  })
