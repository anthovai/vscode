...
const BROWSER_UNAVAILABLE_MESSAGES: Record<RuntimeBrowserUnavailableReason, string> = {
  unconfigured:
    'Browser automation has no backend on this host. Install the Kingu desktop app, or set KINGU_BROWSER_EXECUTABLE to a Chromium executable.',
  driver_missing:
    'KINGU_BROWSER_EXECUTABLE is set, but the bundled agent-browser driver is missing or not executable on this host, so Chromium cannot be driven.',
  executable_not_found: 'KINGU_BROWSER_EXECUTABLE points at a path that does not exist.',
  executable_not_executable:
    'KINGU_BROWSER_EXECUTABLE points at a file that is not executable by this host.',
  electron_start_failed: 'The installed Electron browser provider failed to start.',
  chromium_start_failed:
    'The Chromium browser provider named by KINGU_BROWSER_EXECUTABLE failed to start.',
