...
describe('buildSetupRunnerCommand', () => {
  it('uses bash for WSL UNC runner scripts regardless of host casing', () => {
    expect(
      buildSetupRunnerCommand(
        '\\\\WSL.LOCALHOST\\Ubuntu\\home\\jin\\repo\\.git\\worktrees\\feature\\kingu\\setup-runner.sh',
        'windows'
      )
    ).toBe('bash /home/jin/repo/.git/worktrees/feature/kingu/setup-runner.sh')
  })
  ...
  it('keeps generic forward-slash UNC runner scripts on cmd.exe', () => {
    expect(
      buildSetupRunnerCommand('//server/share/repo/.git/kingu/setup-runner.cmd', 'windows')
    ).toBe('cmd.exe /c "//server/share/repo/.git/kingu/setup-runner.cmd"')
  })

  it('uses POSIX launch semantics for native Windows runners when the setup shell is POSIX', () => {
    expect(
      buildSetupRunnerCommand('C:\\repo\\.git\\kingu\\setup-runner.sh', 'windows', {
        family: 'posix'
      })
    ).toBe('bash /c/repo/.git/kingu/setup-runner.sh')
  })
  ...
  it('uses the active WSL shell with WSL paths for native Windows POSIX runners', () => {
    ...
    ).toBe('bash /mnt/c/repo/.git/kingu/setup-runner.sh')
  })

  it('keeps cmd.exe launch semantics for cmd setup runners', () => {
    expect(
      buildSetupRunnerCommand('C:\\repo\\.git\\kingu\\setup-runner.cmd', 'windows', {
        family: 'cmd'
      })
    ).toBe('cmd.exe /c "C:\\repo\\.git\\kingu\\setup-runner.cmd"')
  })

  it('infers generated POSIX runner shell semantics from extension when metadata is absent', () => {
    expect(buildSetupRunnerCommand('C:\\repo\\.git\\kingu\\setup-runner.sh', 'windows')).toBe(
      'bash /c/repo/.git/kingu/setup-runner.sh'
    )
  })
  ...
  it('never hands a batch runner to bash, even from a Git Bash pane', () => {
    ...
    const command = buildSetupRunnerCommand('C:\\repo\\.git\\kingu\\setup-runner.cmd', 'windows', {
      family: 'posix'
    })
  ...
  it('keeps the batch runner path in native form for a POSIX pane launch', () => {
    ...
    expect(
      resolveSetupRunnerCommand('C:\\repo\\.git\\kingu\\setup-runner.cmd', 'windows', {
        family: 'posix'
      })
    ).toMatchObject({
      runnerScriptPathForShell: 'C:\\repo\\.git\\kingu\\setup-runner.cmd',
      shell: 'windows'
    })
  })
...
describe('buildSetupRunnerCommand cmd metacharacter guard', () => {
  const cmdRunner = (segment: string) => `C:\\repo${segment}\\.git\\kingu\\setup-runner.cmd`
  ...
  it.each([
    ['plain', 'C:\\repo\\.git\\kingu\\setup-runner.cmd'],
    ['spaces', 'C:\\Program Files\\repo\\.git\\kingu\\setup-runner.cmd'],
    ['single quote', "C:\\o'brien\\.git\\kingu\\setup-runner.cmd"],
    ['brackets and dash', 'C:\\repo-[2]\\.git\\kingu\\setup-runner.cmd']
  ...
  it('passes the runner path through the environment rather than the cmd argument string', () => {
    ...
    expect(script).toContain('$processInfo.EnvironmentVariables["KINGU_SETUP_RUNNER"] = $runner')
    expect(script).toContain('/d /s /v:on /c ""!KINGU_SETUP_RUNNER!""')
  ...
  it('escapes single quotes when embedding the path in the PowerShell literal', () => {
    const script = decodePowerShellCommand(
      buildSetupRunnerCommand("C:\\o'brien&co\\.git\\kingu\\setup-runner.cmd", 'windows', {
        family: 'cmd'
      })
    )

    expect(script).toContain("$runner = 'C:\\o''brien&co\\.git\\kingu\\setup-runner.cmd'")
  })
  ...
  it.each([
    ['native POSIX runner', 'C:\\repo\\a&b\\.git\\kingu\\setup-runner.sh', undefined],
    ['WSL UNC runner', '\\\\wsl.localhost\\Ubuntu\\home\\a&b\\kingu\\setup-runner.sh', undefined]
  ...
  it('does not disturb the wsl.exe POSIX launch', () => {
    expect(
      buildSetupRunnerCommand('C:\\repo\\a&b\\.git\\kingu\\setup-runner.sh', 'windows', {
        family: 'posix',
        executable: 'wsl.exe'
      })
    ).toBe("bash '/mnt/c/repo/a&b/.git/kingu/setup-runner.sh'")
  })
})
...
describe('getSetupRunnerCommandPlatformForPath', () => {
  it('prefers POSIX for absolute POSIX runner paths even from Windows clients', () => {
    expect(
      getSetupRunnerCommandPlatformForPath('/remote/repo/.git/kingu/setup-runner.sh', 'windows')
    ).toBe('posix')
  })

  it('prefers Windows for native Windows runner paths even from POSIX clients', () => {
    expect(
      getSetupRunnerCommandPlatformForPath('C:\\repo\\.git\\kingu\\setup-runner.cmd', 'posix')
    ).toBe('windows')
  })
  ...
  it('keeps WSL UNC paths on the Windows resolver so they can be converted', () => {
    expect(
      getSetupRunnerCommandPlatformForPath(
        '\\\\wsl.localhost\\Ubuntu\\home\\jin\\repo\\.git\\kingu\\setup-runner.sh',
        'posix'
      )
    ).toBe('windows')
  })
  ...
  it('keeps forward-slash UNC paths on the Windows resolver', () => {
    ...
    expect(
      getSetupRunnerCommandPlatformForPath(
        '//server/share/repo/.git/kingu/setup-runner.cmd',
        'posix'
      )
    ).toBe('windows')
  })
