...
it.skipIf(process.platform !== 'win32').each(['cmd', 'powershell'] as const)(
  ...
  async (shell) => {
    const root = await mkdtemp(join(tmpdir(), 'kingu-omp-draft-'))
    try {
      const config = join(root, 'fresh (%KINGU_EXPANSION_PROBE%) & settings!.yml')
      const calls = join(root, 'calls')
      await writeFile(
        join(root, 'omp.cmd'),
        '@echo off\r\necho %KINGU_OMP_PREFILL%>>"%CAPTURE%"\r\nexit /b 17\r\n'
      )
      ...
      for (const state of ['present', 'missing', 'unset', 'directory']) {
        ...
        const env = {
          ...process.env,
          ...plan.env,
          KINGU_OMP_FRESH_CONFIG: state === 'unset' ? undefined : config,
          CAPTURE: calls,
          KINGU_EXPANSION_PROBE: 'unexpected'
        }
        const result =
          shell === 'cmd'
            ? await runProcess({
                ...
                cwd: root,
                env,
                input: `${plan.launchCommand}\r\nset "kingu_result=%errorlevel%"\r\nif defined KINGU_OMP_PREFILL exit 91\r\nexit %kingu_result%\r\n`
              })
            : await runProcess({
                ...
                args: [
                  ...
                  '-NonInteractive',
                  '-Command',
                  `function omp { Add-Content -LiteralPath $env:CAPTURE -Value $env:KINGU_OMP_PREFILL; $global:LASTEXITCODE = 17 }; ${plan.launchCommand}; $result = $LASTEXITCODE; if (Test-Path Env:KINGU_OMP_PREFILL) { exit 91 }; exit $result`
                ],
                cwd: root,
                env
              })
