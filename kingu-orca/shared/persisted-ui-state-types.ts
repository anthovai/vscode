...
import type { PersistedTrustedKinguHooks } from './kingu-yaml-hook-types'
...
export type PersistedUIState = {
  ...
  /** Hide workspaces created through `kingu worktree create`. */
  ...
  /** Whether Kingu already tried triggering the macOS notification permission dialog; prevents re-firing every launch. */
  ...
  /** Why: Windows-only. Set once on first hide to tray so the "Kingu is still running" notice shows only once. */
  ...
  /** Once the user has starred Kingu (any entry point), permanently suppress the nag. */
