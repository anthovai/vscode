...
it.each(shells)(
  ...
  async (shell) => {
    const root = await mkdtemp(join(tmpdir(), 'kingu-fresh-shell-'))
    roots.push(root)
    ...
    const result = await runProcess({
      ...
      cwd: root,
      env: { ...process.env, KINGU_OMP_FRESH_CONFIG: config, CAPTURE: capture }
    })
    ...
    delete unsetEnv.KINGU_OMP_FRESH_CONFIG
