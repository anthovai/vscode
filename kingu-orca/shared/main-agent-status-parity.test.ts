...
/** Not a hook: current Claude sends none on a cancel, so Kingu infers it from the keystroke. */
const KINGU_INFERRED_INTERRUPT = { kingu_inferred_interrupt: true }
...
const STORIES: Story[] = [
  ...
  },
  {
    ...
    // must update this story, not delete it. The Claude row here is the primary path: Kingu's
  ...
  },
  {
    ...
    // verdict Kingu inferred rather than downgrade it to "unknown".
...
describe('mainAgent status parity across lanes', () => {
  ...
  function drive(
    ...
    for (const payload of events) {
      if (payload === KINGU_INFERRED_INTERRUPT) {
        if (source === 'codex') {
