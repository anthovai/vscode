...
it.each(
  ...
  async ({ shell, nounset }) => {
    ...
    try {
      ...
      const define =
        shell === 'fish'
          ? 'function omp; printf "%s\\n" "$KINGU_OMP_PREFILL" >> "$CAPTURE"; printf agent-stderr >&2; return 17; end; '
          : 'omp() { printf "%s\\n" "$KINGU_OMP_PREFILL" >> "$CAPTURE"; printf agent-stderr >&2; return 17; }; '
      const observe =
        shell === 'fish'
          ? '; set -l result $status; set -q KINGU_OMP_PREFILL; and exit 91; exit $result'
          : '; result=$?; test -z "${KINGU_OMP_PREFILL+x}" || exit 91; exit "$result"'
      for (const present of [true, false]) {
        ...
        const result = await runProcess({
          ...
          cwd: root,
          env: { ...process.env, ...plan.env, KINGU_OMP_FRESH_CONFIG: config, CAPTURE: calls }
        })
