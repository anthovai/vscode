 ...
 *
 * Why all three streams and not just `message` + `stderr`: the errors Kingu classifies
 * do not all come straight out of `execFile`. Node puts Git's stderr in both `message`
 * and `stderr`, but Kingu also throws its own failures with the Git output on `stdout`
