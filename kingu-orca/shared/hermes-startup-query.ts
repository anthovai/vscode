...
const QUERY_PLACEHOLDER = '__KINGU_HERMES_STARTUP_QUERY__'
const QUERY_ARG_PLACEHOLDER = `--query=${QUERY_PLACEHOLDER}`
const POSIX_QUERY_VARIABLE = '__kingu_hermes_startup_query'
const POWERSHELL_QUERY_VARIABLE = 'kinguHermesStartupQuery'
const POWERSHELL_NATIVE_QUERY_VARIABLE = 'kinguHermesNativeQuery'

export const KINGU_HERMES_STARTUP_QUERY_ENV = 'KINGU_HERMES_STARTUP_QUERY'
...
function stripKinguOwnedHermesArgs(args: readonly string[]): string[] {
...
function normalizeHermesArgv(
  ...
  return [
    ...
    ...stripKinguOwnedHermesArgs(baseArgsWithoutChat),
    ...stripKinguOwnedHermesArgs(configuredArgsWithoutChat),
    '--tui'
  ]
}
...
function buildQueryCommand(argv: string[], shell: AgentStartupShell): string {
  if (!isPosixStartupShell(shell)) {
    ...
    const script = `$${POWERSHELL_QUERY_VARIABLE} = $env:${KINGU_HERMES_STARTUP_QUERY_ENV}; $${POWERSHELL_NATIVE_QUERY_VARIABLE} = $${POWERSHELL_QUERY_VARIABLE} -replace '(\\\\*)"', '$1$1\\"'; Remove-Item Env:${KINGU_HERMES_STARTUP_QUERY_ENV} -ErrorAction SilentlyContinue; ${invocation}`
  ...
  const script = `${POSIX_QUERY_VARIABLE}="\${${KINGU_HERMES_STARTUP_QUERY_ENV}}"; unset ${KINGU_HERMES_STARTUP_QUERY_ENV}; eval "$(printf %b "${encodedInvocation}")"`
...
export function planHermesStartupQuery(args: {
  ...
  const env = {
    ...args.agentEnv,
    [KINGU_HERMES_STARTUP_QUERY_ENV]: args.prompt
  }
