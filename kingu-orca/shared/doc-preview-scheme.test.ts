...
describe('doc preview URLs', () => {
  it('round-trips a nested relative path', () => {
    ...
    expect(url).toBe(`kingu-preview://${GRANT}/assets/logo.png`)
  ...
  it('encodes characters that would otherwise split the URL', () => {
    ...
    expect(url).toBe(`kingu-preview://${GRANT}/a%20b/c%23d%3Fe.html`)
  ...
  it('reports an empty relative path for a root request', () => {
    expect(parseDocPreviewUrl(`kingu-preview://${GRANT}/`)).toEqual({
      grantId: GRANT,
      relativePath: ''
    })
  })
  ...
  it('rejects malformed grant ids and other schemes', () => {
    expect(parseDocPreviewUrl('kingu-preview://SHORT/index.html')).toBeNull()
    expect(parseDocPreviewUrl(`kingu-preview://${'g'.repeat(32)}/index.html`)).toBeNull()
  ...
  it('rejects an undecodable percent sequence rather than passing raw bytes through', () => {
    expect(parseDocPreviewUrl(`kingu-preview://${GRANT}/%E0%A4%A.html`)).toBeNull()
  })
})
