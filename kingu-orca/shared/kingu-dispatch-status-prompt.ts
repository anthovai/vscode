// Why: full Kingu dispatch preambles are multi-KB (CLI instructions before
...
export const KINGU_DISPATCH_STATUS_PREAMBLE_PREFIX =
  'You are working inside Kingu, a multi-agent IDE.'
export const KINGU_DISPATCH_STATUS_TASK_MARKER = '=== TASK ==='
...
export const KINGU_DISPATCH_PROMPT_LEAD_LINE =
  'Please carry out this task from my Kingu coordinator by following the brief I pasted below.'
const KINGU_DISPATCH_STATUS_TASK_ID_MARKER = 'Your task ID is:'
...
const KINGU_DISPATCH_STATUS_SOURCE_SCAN_LIMIT = 24_576
 ...
 */
export function findKinguDispatchPreambleStart(value: string): number {
  ...
  const scanEnd = Math.min(value.length, KINGU_DISPATCH_STATUS_SOURCE_SCAN_LIMIT)
  let start = skipTrimWhitespace(value, 0, scanEnd)
  if (value.startsWith(KINGU_DISPATCH_PROMPT_LEAD_LINE, start)) {
    start = skipTrimWhitespace(value, start + KINGU_DISPATCH_PROMPT_LEAD_LINE.length, scanEnd)
  }
  ...
  return start + KINGU_DISPATCH_STATUS_PREAMBLE_PREFIX.length <= scanEnd &&
    value.startsWith(KINGU_DISPATCH_STATUS_PREAMBLE_PREFIX, start)
    ? start
    : -1
}
...
export function compactDispatchPromptForStatus(
  ...
  const start = findKinguDispatchPreambleStart(value)
  if (start === -1) {
    return null
  }
  const scan = value.slice(start, Math.min(value.length, KINGU_DISPATCH_STATUS_SOURCE_SCAN_LIMIT))

  let taskId = ''
  const idMarkerIndex = scan.indexOf(KINGU_DISPATCH_STATUS_TASK_ID_MARKER)
  if (idMarkerIndex !== -1) {
    const afterId = scan.slice(idMarkerIndex + KINGU_DISPATCH_STATUS_TASK_ID_MARKER.length)
    let idStart = 0
  ...
  let taskBody = ''
  const taskMarkerIndex = findKinguDispatchTaskMarkerIndex(scan)
  if (taskMarkerIndex !== -1) {
    const body = scan.slice(taskMarkerIndex + KINGU_DISPATCH_STATUS_TASK_MARKER.length)
  ...
  // Why: keep the dispatch prefix (isKinguDispatchPrompt) + task id (label match)
  // + task body (fallback preview) so UI helpers still work on the 200-char field.
  let compact = KINGU_DISPATCH_STATUS_PREAMBLE_PREFIX
  if (taskId) {
    compact += ` ${KINGU_DISPATCH_STATUS_TASK_ID_MARKER} ${taskId}`
  }
  if (taskBody) {
    compact += ` ${KINGU_DISPATCH_STATUS_TASK_MARKER} ${taskBody}`
  }
  return normalizeSingleLine(compact, maxLength)
}

/**
 * Locate the Kingu task separator in a dispatch prompt scan window.
 * Why: base-drift commit subjects are repository-controlled and may mention
 * `=== TASK ===`. Raw multi-line preambles must use the standalone line Kingu
 ...
 */
export function findKinguDispatchTaskMarkerIndex(value: string): number {
  let searchFrom = 0
  while (searchFrom < value.length) {
    const markerIndex = value.indexOf(KINGU_DISPATCH_STATUS_TASK_MARKER, searchFrom)
    if (markerIndex === -1) {
      break
    }
    const markerEnd = markerIndex + KINGU_DISPATCH_STATUS_TASK_MARKER.length
  ...
  return value.includes('\n') || value.includes('\r')
    ? -1
    : value.indexOf(KINGU_DISPATCH_STATUS_TASK_MARKER)
}
