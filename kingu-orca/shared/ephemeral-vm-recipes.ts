...
const EphemeralVmRecipeKinguServerConnectionSchema = z
  .object({
    type: z.literal('kingu-server'),
...
export const EphemeralVmRecipeConnectionSchema = z.discriminatedUnion('type', [
  EphemeralVmRecipeKinguServerConnectionSchema,
...
export function parseEphemeralVmRecipeResult(stdout: string): EphemeralVmRecipeResultParseResult {
  ...
  if (connection.type === 'kingu-server' && !parsePairingCode(connection.pairingCode)) {
    return { ok: false, error: 'Recipe result pairingCode is not a valid Kingu pairing code.' }
  }
...
export function getEphemeralVmRecipeResultCheckoutMode(
  result: EphemeralVmRecipeResult
): 'kingu-worktree' | 'provisioned-root' {
  return result.schemaVersion === 2 ? 'provisioned-root' : 'kingu-worktree'
}
...
export function getEphemeralVmRecipeResultPairingCode(
  ...
  return connection.type === 'kingu-server' ? connection.pairingCode : null
}
