 ...
 * MDE incident against Kingu. These shims are generated files whose entire body
...
const DISABLE_FLAG = 'KINGU_DISABLE_CMD_SHIM_RESOLUTION'
...
function readParsedShim(program: string): ParsedWindowsCmdShim | null {
  ...
  // `.cmd` path Kingu ever spawns; reaching it means a re-read, not a wrong
  // answer.
...
function resolveShimNode(directory: string, env: NodeJS.ProcessEnv): string | null {
  ...
  // per-process and one entry per distinct shim directory Kingu ever spawns from.
