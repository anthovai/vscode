...
export const KINGU_OMP_FRESH_CONFIG_ENV = 'KINGU_OMP_FRESH_CONFIG'
...
export function withFreshOmpLaunch(command: string, shell: AgentStartupShell, suffix = ''): string {
  ...
  const path =
    shell === 'cmd'
      ? `"%${KINGU_OMP_FRESH_CONFIG_ENV}%"`
      : shell === 'powershell'
        ? `"$env:${KINGU_OMP_FRESH_CONFIG_ENV}"`
        : `"$${KINGU_OMP_FRESH_CONFIG_ENV}"`
  const launch = `${command} --config ${path}${suffix}`
  const error =
    'Kingu: fresh OMP settings are unavailable on this host. Restart the terminal after updating Kingu.'
  if (shell === 'powershell') {
  ...
  const available = `printenv ${KINGU_OMP_FRESH_CONFIG_ENV} >/dev/null && test -f ${path}`
...
export function isFreshOmpLaunchCommand(command: string | undefined): boolean {
  ...
  return Boolean(
    guarded &&
    [
      `printenv ${KINGU_OMP_FRESH_CONFIG_ENV} >/dev/null && test -f "$${KINGU_OMP_FRESH_CONFIG_ENV}"`,
      `if ("$env:${KINGU_OMP_FRESH_CONFIG_ENV}" -and (Test-Path -LiteralPath "$env:${KINGU_OMP_FRESH_CONFIG_ENV}" -PathType Leaf))`,
      `if exist "%${KINGU_OMP_FRESH_CONFIG_ENV}%" (if not exist "%${KINGU_OMP_FRESH_CONFIG_ENV}%\\*"`
