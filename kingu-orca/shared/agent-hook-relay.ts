...
// the relay sends to Kingu. Consumed by `src/relay/agent-hook-server.ts` (which
...
// between Kingu's main process and the remote relay.
//
// Invariants both ends rely on:
// - The relay normalizes; Kingu routes. The envelope's `payload` field has
//   already been through `normalizeHookPayload` (which calls
//   `parseAgentStatusPayload` → `normalizeAgentStatusObject`) on the relay
//   side. Kingu's `ingestRemote` re-runs the canonical payload normalizer at
...
// - The wire `connectionId` is **always `null`**: a `connectionId` is Kingu's
//   local handle on an `ssh2` connection, not a wire identity. Kingu stamps the
...
//   POST body so Kingu's warn-once protocol diagnostics still fire. The relay
...
// with the same value so Kingu can attribute the event back to the right CLI.
...
/** Wire envelope for a single hook event flowing relay → Kingu. */
export type AgentHookRelayEnvelope = {
  source: AgentHookSource
  paneKey: string
  /** Ephemeral Kingu launch identity stamped into the PTY env for this process. */
  launchToken?: string
  tabId?: string
  worktreeId?: string
  /** Always `null` on the wire — relay does not know Kingu's local connectionId. */
  connectionId: null
  /** Preserved from the relay-side normalized hook event so Kingu can
  ...
  /** True when the relay is replaying its cache after Kingu reconnects. */
  isReplay?: boolean
  ...
  env?: string
  /** Forwarded verbatim from the agent CLI POST body. Lets Kingu's warn-once
   ...
   *  Kingu's `ingestRemote` validates it again at the SSH trust boundary. */
...
/**
 * Re-attaches shed fields from Kingu's cached payload for this pane, so a transport-level
...
/** JSON-RPC request method Kingu issues after `--connect` reattach to ask the
 *  relay to replay its per-paneKey last-payload cache. Pull, not push: a relay
 *  that pushed on `setWrite` can emit before Kingu has wired its `agent.hook`
...
/** JSON-RPC request method Kingu issues at session-ready to ship the
...
export type AgentHookInstallManagedHooksParams = {
  /** SHA-256 fingerprint of the server key negotiated by Kingu's SSH transport. */
...
/** Feature-flag env var. Read once at process start by Kingu and the relay.
 *  Remote agent hooks ship as the default SSH behavior; set "0" to opt out. */
export const KINGU_FEATURE_REMOTE_AGENT_HOOKS_ENV = 'KINGU_FEATURE_REMOTE_AGENT_HOOKS' as const

export function isRemoteAgentHooksEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = env[KINGU_FEATURE_REMOTE_AGENT_HOOKS_ENV]
  if (raw === undefined) {
    return true
  }
