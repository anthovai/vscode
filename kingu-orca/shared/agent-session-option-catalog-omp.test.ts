...
describe('omp session option catalog', () => {
  ...
  it('switches mid-session with /kingu-model <selector>, which omp resolves exactly', () => {
    ...
    expect(command).toBe('/kingu-model minimax-cn/MiniMax-M3')
    expect(parseBuiltSessionOptionCommand(midSession.build, command)).toBe('minimax-cn/MiniMax-M3')
    expect(parseBuiltSessionOptionCommand(midSession.build, '/kingu-model ')).toBeNull()
  })
})
