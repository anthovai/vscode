...
export function buildAgentStartupPlan(args: {
   ...
   * `kingu-ide` rename must be skipped for remote launches. */
  isRemote?: boolean
  ...
  if (config.promptInjectionMode === 'hermes-query') {
    ...
    return {
      agent,
      // Why: Hermes owns readiness and submission for `chat --query`; Kingu
...
export function buildAgentDraftLaunchPlan(args: {
  ...
  sessionOptions?: Record<string, SessionOptionValue>
  sessionOptionsOverrideAgentArgs?: boolean
  /** Why: see buildAgentStartupPlan — remote launches use the plain `kingu` shim. */
  isRemote?: boolean
