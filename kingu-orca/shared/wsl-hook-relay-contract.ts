...
export const WSL_HOOK_RELAY_DIR = '.kingu-wsl/hook-relay'
 ...
 *  namespaces the guest install dir, so concurrent Kingu instances with
 *  different bundle versions (dev + prod) never reinstall over each other. */
export const WSL_HOOK_RELAY_VERSION_ENV = 'KINGU_WSL_HOOK_RELAY_VERSION'
 ...
 *  after an Kingu restart — the exact re-coordination this exists to serve. */
export const WSL_HOOK_RELAY_INSTANCE_ENV = 'KINGU_WSL_HOOK_INSTANCE'
...
export function wslHookRelayEndpointDir(guestHome: string, instanceKey: string): string {
  ...
  return `${home}/.kingu-wsl/agent-hooks/instance-${instanceKey}`
}
