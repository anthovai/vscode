 ...
 * ONE emitted string is correct in sh, bash, zsh, dash and fish alike, so Kingu
...
/** Every Unix shell on this machine a queued Kingu command line could land in. */
...
const SANDBOX_HOME = mkdtempSync(path.join(tmpdir(), 'kingu-shell-portability-'))

/** Env with no user shell config reachable, so only Kingu's own text is exercised. */
  ...
  () => {
    ...
    describe.each(SHELLS)('$name', (shell) => {
      ...
      it('clears an exported variable with no wrapper installed', () => {
        const clear = clearEnvCommand('KINGU_PI_PREFILL', 'posix')
        const probe =
          shell.name === 'fish'
            ? `set -gx KINGU_PI_PREFILL draft; ${clear}; set -q KINGU_PI_PREFILL; and echo STILL; or echo CLEARED`
            : `KINGU_PI_PREFILL=draft; export KINGU_PI_PREFILL; ${clear}; echo "\${KINGU_PI_PREFILL:+STILL}\${KINGU_PI_PREFILL:-CLEARED}"`
        expect(runInShell(shell, probe).trim()).toBe('CLEARED')
      })

      it('clears several variables in one statement', () => {
        const clear = clearEnvCommand(['KINGU_A', 'KINGU_B'], 'posix')
        const probe =
          shell.name === 'fish'
            ? `set -gx KINGU_A 1; set -gx KINGU_B 2; ${clear}; set -q KINGU_A; or set -q KINGU_B; and echo STILL; or echo CLEARED`
            : `KINGU_A=1 KINGU_B=2; export KINGU_A KINGU_B; ${clear}; echo "\${KINGU_A:+STILL}\${KINGU_B:+STILL}\${KINGU_A:-CLEARED}"`
        expect(runInShell(shell, probe).trim()).toBe('CLEARED')
      })
      ...
      it.each([
        ...
        const setUp = preset
          ? shell.name === 'fish'
            ? 'set -gx KINGU_PI_PREFILL draft; '
            : 'KINGU_PI_PREFILL=draft; export KINGU_PI_PREFILL; '
          : ''
      ...
      // interactive shell, which is the mode Kingu types into — a `-c` run cannot
      ...
      it.runIf(['bash', 'zsh'].includes(shell.name))(
        ...
        () => {
          ...
          const script = [
            ...
            'KINGU_PI_PREFILL=draft; export KINGU_PI_PREFILL',
            clear,
            'echo "RESULT=${KINGU_PI_PREFILL:+STILL}${KINGU_PI_PREFILL:-CLEARED}"'
          ].join('\n')
      ...
      // observed. Interactively — the mode Kingu types into — they survive without
      ...
      it.runIf(shell.name === 'fish')('never deletes a lone universal variable', () => {
        const clear = clearEnvCommand('KINGU_PI_PREFILL', 'posix')
        const probe = `set -Ux KINGU_PI_PREFILL persisted; ${clear}; set -q KINGU_PI_PREFILL; and echo "RESULT=$KINGU_PI_PREFILL"; or echo RESULT=DESTROYED; set -Ue KINGU_PI_PREFILL`
      ...
      it.runIf(shell.name === 'fish')('reveals a shadowed universal instead of deleting it', () => {
        const clear = clearEnvCommand('KINGU_PI_PREFILL', 'posix')
        const probe = `set -Ux KINGU_PI_PREFILL mine; set -gx KINGU_PI_PREFILL injected; ${clear}; echo "RESULT=$KINGU_PI_PREFILL"; set -Ue KINGU_PI_PREFILL`
