...
describe('crash-reporting shared helpers', () => {
  ...
  it('keeps details on a strict primitive allowlist', () => {
    ...
    expect(
      sanitizeCrashReportDetails({
        ...
        minidumpPath: '/Users/alice/Library/Application Support/Kingu/reports/abc.dmp',
        nested: { nope: true },
  ...
  it('redacts path tokens without deleting surrounding prose', () => {
    const note = [
      ...
      'The log is at /opt/kingu/logs/app.log and the repo is /Users/alice/x but this survives.'
    ].join(' ')
    ...
    expect(text).not.toContain('/opt/kingu/logs/app.log')
    expect(text).not.toContain('alice')
  })

  it.each([
    ['POSIX', '/home/alice/kingu/app.log then recovered.'],
    ['Windows', 'C:\\Users\\alice\\Kingu\\app.log then recovered.'],
    ['UNC', '\\\\server\\share\\Kingu\\app.log then recovered.']
