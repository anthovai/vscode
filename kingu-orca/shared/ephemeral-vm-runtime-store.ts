...
const EPHEMERAL_VM_RUNTIMES_FILE = 'kingu-ephemeral-vm-runtimes.json'
...
export function upsertEphemeralVmRuntimeRollbackRecovery(
  ...
  try {
    ...
    if (error instanceof JsonStringifyByteLimitError) {
      throw new EphemeralVmRuntimeStoreError(
        'runtime_error',
        `Could not write Kingu ephemeral VM runtimes at ${path}; the store exceeds its durable capacity.`
      )
    }
    throw error
  }
...
function readEphemeralVmRuntimeStore(userDataPath: string): LoadedEphemeralVmRuntimeStore {
  ...
  try {
    ...
    throw new EphemeralVmRuntimeStoreError(
      'runtime_error',
      `Could not read Kingu ephemeral VM runtimes at ${path}; the file is invalid.`
    )
  }
}
