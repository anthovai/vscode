...
import { KINGU_UPDATER_QUIT_AND_INSTALL_ABORTED_EVENT } from '../shared/updater-renderer-events'
import {
  KINGU_INTERNAL_FILE_DRAG_TYPE,
...
export const startupDiagnosticsEnabled = process.env.KINGU_STARTUP_DIAGNOSTICS === '1'
...
export function installNativeFileDropHandlers(): void {
  ...
  document.addEventListener(
    'drop',
    (event) => {
      if (event.dataTransfer?.types.includes(KINGU_INTERNAL_FILE_DRAG_TYPE)) {
        return
      }
      event.preventDefault()
