...
export function computerUseErrorRecoveryData(
  ...
  switch (code) {
    ...
    case 'window_not_found':
      return recoverWith(
        'Run `kingu computer list-windows --app <app> --json` and target one of the listed windows.',
        'If the app is listed but no usable window is visible, retry observation once with `--restore-window`.',
        'If no window is listed, open or focus the app first; `kingu computer` does not launch closed desktop apps.'
      )
    case 'window_not_focused':
      if (message?.includes('may already have been delivered')) {
        return recoverWith(
          'Run `kingu computer get-app-state --app <app> --json` and verify whether the intended action already occurred before retrying.',
    ...
    case 'window_stale':
      return recoverWith(
        'Run `kingu computer list-windows --app <app> --json` and choose a current window selector.',
        'Then rerun `kingu computer get-app-state --app <app> --json` before acting.'
      )
    case 'provider_incompatible':
      return recoverWith(
        'Run `kingu computer capabilities --json` and verify the local provider supports the requested operation.',
        'Update Kingu or use a supported platform/provider path before retrying.'
      )
    case 'unsupported_capability':
      return recoverWith(
        'Run `kingu computer capabilities --json` and choose a supported action.',
    ...
    case 'permission_denied':
      return recoverWith(
        'Run `kingu computer permissions --json`, or `kingu computer permissions --id accessibility --json` / `--id screenshots --json` when the message names one missing permission.',
        'For remote or SSH targets, verify the command is running inside an active graphical desktop session.'
      )
    case 'element_not_found':
      return recoverWith(
        'Run `kingu computer get-app-state --app <app> --json` again and use an element index from the fresh tree.',
    ...
    case 'action_timeout':
      return recoverWith(
        'Run `kingu computer get-app-state --app <app> --json` before retrying so you know whether the UI changed.',
    ...
    case 'screenshot_failed':
      return recoverWith(
        ...
        'If the message mentions Screen Recording or screenshots permission, run `kingu computer permissions --id screenshots --json` and grant access before retrying.',
        'If the message mentions the payload cap, target a smaller/current window or use `--no-screenshot`.'
      )
    case 'accessibility_error':
      return recoverWith(
        'Run `kingu computer capabilities --json` to confirm the provider is available before retrying.',
        'If the message mentions permissions, run `kingu computer permissions --id accessibility --json` and grant access.',
