...
const image = process.env.KINGU_GIT_COMPAT_IMAGE
const binary = process.env.KINGU_GIT_COMPAT_BINARY
const expectedVersion = process.env.KINGU_GIT_COMPAT_VERSION
...
describeBinaryCompatibility('real Git binary compatibility', () => {
  ...
  beforeAll(async () => {
    repoPath = await mkdtemp(join(tmpdir(), 'kingu-git-binary-compat-'))
  ...
  it('recognizes worktree-list and rev-parse compatibility boundaries', async () => {
    ...
    // Kingu's path-existence fallback (issue #8389).
  ...
  it('deregisters a worktree whose directory was renamed away', async () => {
    // Kingu renames the checkout into a trash directory and then clears the registration, so every
  ...
  it('supports prepared worktree creation and finalization', async () => {
    ...
    await runGit([
      ...
      'lock',
      '--reason',
      'kingu-create-preparation:v1:compat',
      'compat-prepared'
    ])
  ...
  // Why pin this: Kingu answers "which remote has this URL" from one `git remote -v`
  ...
  it('recognizes ref and merge-tree compatibility boundaries', async () => {
    ...
    await expectPreferredOrRecognizedFallback(
      ['fetch', '--no-write-fetch-head', '.', '+HEAD:refs/kingu/compat/no-write-fetch-head'],
      supports(2, 29),
