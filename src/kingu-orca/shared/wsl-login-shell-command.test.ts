...
describe('wsl login shell command helpers', () => {
  ...
  it('runs commands through the distro user login shell', () => {
    ...
    expect(command).toContain('exec "$_kingu_wsl_shell" -ilc')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const root = mkdtempSync(join(tmpdir(), 'kingu-wsl-login-codex-'))
      ...
      writeFileSync(
        join(tools, 'getent'),
        `#!/bin/sh\nprintf '%s\\n' "user:x:1000:1000::/home/user:$KINGU_TEST_LOGIN_SHELL"\n`
      )
      writeFileSync(
        loginShell,
        '#!/bin/sh\nexport PATH="$KINGU_TEST_CODEX_BIN:/usr/bin:/bin"\nexec /bin/sh -c "$2"\n'
      )
      ...
      const run = (codexBin: string): string =>
        execFileSync('/bin/sh', ['-c', command], {
          ...
          env: {
            ...
            KINGU_TEST_LOGIN_SHELL: loginShell,
            KINGU_TEST_CODEX_BIN: codexBin
          }
        })
  ...
  it('keeps command-scoped environment variables in the quoted payload', () => {
    const command = buildWslLoginShellCommand('HISTFILE=/tmp/kingu-history printf "$HISTFILE"')

    expect(command).toContain('\'HISTFILE=/tmp/kingu-history printf "$HISTFILE"\'')
  ...
  it('preserves user command variables across the Windows-to-WSL argv boundary', () => {
    ...
    const captured = buildWslCapturedLoginShellCommand(
      'kingu_value=ok; printf "<%s>" "$kingu_value"'
    )
  ...
  describe('captured login shell', () => {
      ...
      expect(
        captured.readStdout(
          'To run a command as administrator (user "root"), use "sudo <command>".\n\n' +
            '__KINGU_WSL_CAPTURE_BEGIN_nonce1__directory__KINGU_WSL_CAPTURE_END_nonce1__'
        )
      ).toBe('directory')
    })
    ...
    it('emits the status plumbing the payload needs', () => {
      ...
      expect(captured.command).toContain('_kingu_capture_status=$?')
      expect(captured.command).toContain('exit $_kingu_capture_status')
    })
    ...
    it('keeps payload bytes that themselves contain a fence', () => {
      ...
      const payload =
        'see __KINGU_WSL_CAPTURE_BEGIN_nonce1__ and __KINGU_WSL_CAPTURE_END_nonce1__\n'

      expect(
        captured.readStdout(
          `banner\n__KINGU_WSL_CAPTURE_BEGIN_nonce2__${payload}__KINGU_WSL_CAPTURE_END_nonce2__`
        )
      ).toBe(payload)
    })
    ...
    it('returns the tail when a payload exits before the closing fence', () => {
      ...
      expect(captured.readStdout('banner\n__KINGU_WSL_CAPTURE_BEGIN_nonce1__partial')).toBe(
        'partial'
      )
    })
  ...
  it('starts an interactive login shell without assuming bash', () => {
    ...
    expect(command).toContain('getent passwd')
    expect(command).toContain('if [ -z "$_kingu_wsl_shell" ] || [ ! -x "$_kingu_wsl_shell" ]; then')
    expect(command).toContain('_kingu_shell_ready_root=""')
    expect(command).toContain('if [ -n "${KINGU_USER_DATA_PATH:-}" ]; then')
    expect(command).toContain('_kingu_wsl_shell_name=$(basename "$_kingu_wsl_shell"')
    expect(command).toContain('bash)')
    expect(command).toContain('--rcfile "${_kingu_shell_ready_root}/bash/rcfile"')
    expect(command).toContain('zsh)')
    expect(command).toContain('export ZDOTDIR="${_kingu_shell_ready_root}/zsh"')
    expect(command).toContain('exec "$_kingu_wsl_shell" -l')
    expectValidShSyntax(command)
  })
})
...
describe('in-guest wrapper root resolution', () => {
  ...
  // rebuilt the root as `${KINGU_USER_DATA_PATH}/shell-ready`, which stopped
  ...
  it('prefers the host-published root over the legacy user-data guess', () => {
    ...
    expect(script).toContain('if [ -n "${KINGU_SHELL_READY_ROOT:-}" ]; then')
    expect(script).toContain('_kingu_shell_ready_root="${KINGU_SHELL_READY_ROOT%/}"')
    // The legacy branch must remain reachable only as a fallback, so an older
    // host that exports just KINGU_USER_DATA_PATH still wraps its shells.
    expect(script).toContain('elif [ -n "${KINGU_USER_DATA_PATH:-}" ]; then')
  })
  ...
  it('resolves the published root ahead of the legacy path under a real shell', () => {
    ...
    const prologue = script.split('_kingu_wsl_shell_name=')[0] as string
    const probe = [
      'KINGU_SHELL_READY_ROOT=/mnt/c/ud/shell-wrappers/deadbeefdeadbeef/shell-ready',
      'KINGU_USER_DATA_PATH=/mnt/c/ud',
      'export KINGU_SHELL_READY_ROOT KINGU_USER_DATA_PATH',
      prologue,
      'printf "%s" "$_kingu_shell_ready_root"'
    ].join('\n')
