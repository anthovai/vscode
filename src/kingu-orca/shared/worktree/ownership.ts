...
import type { GlobalSettings, KinguWorkspaceLayout } from '../global-settings-types'
...
export function buildKnownKinguWorkspaceLayouts(
...
): KinguWorkspaceLayout[] {
  const layouts: KinguWorkspaceLayout[] = []
...
function appendWorkspaceLayouts(
  target: KinguWorkspaceLayout[],
  source: readonly KinguWorkspaceLayout[]
): void {
...
function buildWslWorkspaceLayouts(
  ...
  const root = `//wsl.localhost/${parsed.distro}${linuxHome}/kingu/workspaces`
...
export function classifyWorktreeOwnership(args: {
  ...
  meta?: WorktreeMeta
  settings: Pick<GlobalSettings, 'workspaceDir' | 'nestWorkspaces' | 'workspaceDirHistory'>
  knownKinguLayouts: KinguWorkspaceLayout[]
  ...
  if (hasStrongKinguMetadata(args.meta)) {
    return 'kingu-managed'
  }
  ...
  if (isUnderFlatOrUntrustedKinguRoot(args.worktree.path, args.knownKinguLayouts)) {
    return 'unknown-legacy'
  }

  if (canClassifyAsExternal(args.worktree.path, args.knownKinguLayouts)) {
    // Why: a plain `git worktree add` can target Kingu's nested workspace
    // folder. Only metadata proves Kingu created it.
    return 'external'
  }
...
export function applyMetadataFallbackVisibility(detected: DetectedWorktree): DetectedWorktree {
  ...
  return {
    ...detected,
    visible: true,
    ownership: detected.ownership === 'kingu-managed' ? 'kingu-managed' : 'unknown-legacy'
  }
}
...
function hasStrongKinguMetadata(meta: WorktreeMeta | undefined): boolean {
  return Boolean(
    meta?.kinguCreatedAt ||
    meta?.kinguCreationWorkspaceLayout ||
    meta?.createdAt ||
...
function isUnderFlatOrUntrustedKinguRoot(
  worktreePath: string,
  knownKinguLayouts: KinguWorkspaceLayout[]
): boolean {
  for (const layout of knownKinguLayouts) {
