...
describe('worktree ownership classification', () => {
  it('treats explicit Kingu metadata as managed even outside the workspace root', () => {
    const repo = makeRepo()
    ...
    expect(
      classifyWorktreeOwnership({
        ...
        meta: makeMeta({ kinguCreatedAt: 1 }),
        knownKinguLayouts: buildKnownKinguWorkspaceLayouts(settings, repo)
      })
    ).toBe('kingu-managed')
  })

  it('treats nested Kingu workspace paths without metadata as external', () => {
    const repo = makeRepo()
    const settings = makeSettings()
    const layouts = buildKnownKinguWorkspaceLayouts(settings, repo)
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({ path: '/kingu/workspaces/app/feature' }),
        knownKinguLayouts: layouts
      })
    ).toBe('external')
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({ path: '/kingu/workspaces/other/feature' }),
        knownKinguLayouts: layouts
      })
    ).toBe('external')
  })

  it('treats explicit Kingu creation layout metadata as managed', () => {
    const repo = makeRepo()
    const settings = makeSettings()
    expect(
      classifyWorktreeOwnership({
  ...
  it('does not treat metadata-free nested workspace paths as Kingu-managed for new repos', () => {
    ...
    const detected = toDetectedWorktree({
      ...
      worktree: makeWorktree({
        path: '/kingu/workspaces/app/manual-git-worktree',
  ...
  it('does not treat generic discovery metadata on nested workspace paths as Kingu-managed', () => {
    const repo = makeRepo({ externalWorktreeVisibility: 'hide' })
    const settings = makeSettings()
    const detected = toDetectedWorktree({
      repo,
  ...
  it('treats flat workspace-root descendants as unknown legacy without strong metadata', () => {
    ...
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({ path: '/kingu/workspaces/feature' }),
        knownKinguLayouts: buildKnownKinguWorkspaceLayouts(settings, repo)
      })
    ).toBe('unknown-legacy')
  })
  ...
  it('keeps flat-layout history weak after switching the same root to nested mode', () => {
    ...
    const settings = makeSettings({
      nestWorkspaces: true,
      workspaceDirHistory: [{ path: '/kingu/workspaces', nestWorkspaces: false }]
    })
    expect(
      classifyWorktreeOwnership({
        repo,
  ...
  it('handles Windows drive casing and separators', () => {
    ...
    const settings = makeSettings({ workspaceDir: 'C:\\Kingu\\Workspaces' })
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({
          id: 'repo-1::C:\\KINGU\\WORKSPACES\\App\\Feature',
          path: 'C:\\KINGU\\WORKSPACES\\App\\Feature',
          isMainWorktree: false
        }),
        knownKinguLayouts: buildKnownKinguWorkspaceLayouts(settings, repo)
      })
...
describe('agent scratch worktrees', () => {
  ...
  it('classifies scratch worktrees created inside another linked checkout', () => {
    const repo = makeRepo()
    const settings = makeSettings()
    const linkedCheckoutPath = '/kingu/workspaces/app/feature-x'
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
  ...
  it('keeps strong Kingu metadata authoritative over the scratch path match', () => {
    const repo = makeRepo()
    const settings = makeSettings()
    expect(
      classifyWorktreeOwnership({
