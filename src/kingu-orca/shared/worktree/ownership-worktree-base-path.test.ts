...
import { buildKnownKinguWorkspaceLayouts, classifyWorktreeOwnership } from './ownership'
...
describe('repo-specific worktree ownership layouts', () => {
  ...
  it('resolves the same relative base path from each repo root', () => {
    ...
    expect(buildKnownKinguWorkspaceLayouts(settings, repoA)[0]).toEqual({
      path: '/projects/a/worktrees',
      nestWorkspaces: true
    })
    expect(buildKnownKinguWorkspaceLayouts(settings, repoB)[0]).toEqual({
    ...
    expect(
      classifyWorktreeOwnership({
        ...
        knownKinguLayouts: buildKnownKinguWorkspaceLayouts(settings, repoA)
      })
    ).toBe('external')
    expect(
      classifyWorktreeOwnership({
        ...
        settings,
        worktree: makeWorktree('/projects/a/worktrees/repo/feature'),
        knownKinguLayouts: buildKnownKinguWorkspaceLayouts(settings, repoB)
      })
    ).toBe('external')
  })
  ...
  it('resolves an absolute Linux base path of a WSL repo into its distro layout (STA-4772)', () => {
    const repo = makeRepo({
      ...
      worktreeBasePath: '/home/jin/src/.kingu-worktrees'
    })
    ...
    expect(layouts[0]).toEqual({
      path: '//wsl.localhost/Ubuntu-24.04/home/jin/src/.kingu-worktrees',
      nestWorkspaces: true
    })
    expect(
      classifyWorktreeOwnership({
        ...
        worktree: makeWorktree(
          '\\\\wsl.localhost\\Ubuntu-24.04\\home\\jin\\src\\.kingu-worktrees\\repo\\feature'
        ),
  ...
  it('classifies worktrees under a dotted Linux base exactly where creation collapses it', () => {
    ...
    expect(layouts[0]).toEqual({
      path: '//wsl.localhost/Ubuntu-24.04/home/jin/.kingu-worktrees',
      nestWorkspaces: true
    })
    expect(
      classifyWorktreeOwnership({
        ...
        settings,
        worktree: makeWorktree(
          '\\\\wsl.localhost\\Ubuntu-24.04\\home\\jin\\.kingu-worktrees\\repo\\feature'
        ),
        knownKinguLayouts: layouts
      })
    ).toBe('external')
  ...
  it('includes relative global layouts for SSH repos without applying absolute desktop paths', () => {
    ...
    expect(buildKnownKinguWorkspaceLayouts(relativeSettings, repo)[0]).toEqual({
      path: '/remote/worktrees',
      nestWorkspaces: true
    })
    expect(
      buildKnownKinguWorkspaceLayouts(absoluteSettings, repo).some(
