...
import type { EphemeralVmCheckoutMode } from '../kingu-yaml-hook-types'
import type {
...
import type { KinguWorkspaceLayout } from '../global-settings-types'
...
export type WorktreeMeta = {
  ...
  /** See {@link Worktree.createdAt}. Persisted to kingu-data.json. */
  createdAt?: number
  /** See {@link Worktree.createdWithAgent}. Persisted to kingu-data.json. */
  ...
  baseRef?: string
  /** True when Kingu checked out a pre-existing local branch that delete must not prune. */
  preserveBranchOnDelete?: boolean
  /** See {@link Worktree.pushTarget}. Persisted so refreshed worktree lists keep the target. */
  pushTarget?: GitPushTarget
  /** Explicit marker stamped when Kingu creates the worktree. */
  kinguCreatedAt?: number
  kinguCreationSource?: 'desktop' | 'runtime' | 'cli' | 'ssh'
  /** Workspace layout active when Kingu created the worktree. */
  kinguCreationWorkspaceLayout?: KinguWorkspaceLayout
  ...
  /** System-owned provenance for workspaces created via `kingu worktree create`. */
