...
export type Worktree = {
  ...
  /** Set once when Kingu creates the worktree. Absent for worktrees discovered
  ...
  createdAt?: number
  /** Agent selected when Kingu originally created the worktree. Used only to
  ...
  baseRef?: string
  /** Remote/branch Kingu should publish review commits to when it created this worktree. */
...
/** Provenance for workspaces created through `kingu worktree create`. Absent on
...
export type CliWorkspaceProvenance = {
  kind: 'created-by-cli'
  createdAt: number
  /** Kingu terminal the CLI ran inside, when the caller had one — distinguishes
...
export type GitPushTarget = {
  ...
  branchName: string
  remoteUrl?: string
  /** True when Kingu added this remote while preparing a fork-PR worktree. */
...
export type WorktreeOwnership = 'kingu-managed' | 'external' | 'unknown-legacy' | 'agent-scratch'
