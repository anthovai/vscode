...
describe('ephemeral VM runtime store', () => {
  ...
  function makeUserDataPath(): string {
    const userDataPath = mkdtempSync(join(tmpdir(), 'kingu-ephemeral-vm-store-'))
  ...
  it('updates lifecycle and cleanup state', () => {
    ...
    const failed = updateEphemeralVmRuntimeStatus(userDataPath, 'kingu-instance-1', {
    ...
    const recovered = updateEphemeralVmRuntimeStatus(userDataPath, 'kingu-instance-1', {
      status: 'cleaned',
