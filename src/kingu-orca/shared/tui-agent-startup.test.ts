...
describe('tui agent startup plans', () => {
  ...
  it.each([
    ...
    expect(plan?.env?.KINGU_HERMES_STARTUP_QUERY).toBe(prompt)
    const script =
    ...
    expect(testCase.shell === 'posix' ? plan?.launchCommand : script).toContain(
      'KINGU_HERMES_STARTUP_QUERY'
    )
    expect(script).toContain("'--yolo' '--tui'")
    expect(script).toContain(
      testCase.shell === 'posix'
        ? '--query=${__kingu_hermes_startup_query}'
        : 'Remove-Item Env:KINGU_HERMES_STARTUP_QUERY'
    )
    if (testCase.shell === 'posix') {
      expect(plan?.launchCommand).toContain('unset KINGU_HERMES_STARTUP_QUERY')
    }
  })
  ...
  it('does not launch Codex with the Kingu profile when agent status hooks are enabled', () => {
    const plan = buildAgentStartupPlan({
      agent: 'codex',
      prompt: 'fix it',
      cmdOverrides: {},
  ...
  it('launches Claude without Kingu settings injection', () => {
    const plan = buildAgentStartupPlan({
      agent: 'claude',
      prompt: 'fix it',
      cmdOverrides: {},
  ...
  it('uses the Linux Kingu CLI command for Claude Agent Teams launches', () => {
    const plan = buildAgentStartupPlan({
    ...
    expect(plan?.launchCommand).toBe('kingu-ide claude-teams')
  })

  it('uses the plain kingu shim for Claude Agent Teams on Linux SSH remotes', () => {
    // Why: the SSH relay deploys the CLI shim as `kingu` (not the local-only
    // `kingu-ide` GNOME-screen-reader workaround), so a remote launch must not
    // emit `kingu-ide claude-teams` — that name is not on the remote PATH and
    ...
    expect(plan?.launchCommand).toBe('kingu claude-teams')
  })

  it('keeps the Windows kingu.cmd shim for Claude Agent Teams on SSH remotes', () => {
    // Why: the Windows remote shim is also `kingu.cmd`, matching the local
    ...
    expect(plan?.launchCommand).toBe('kingu.cmd claude-teams')
  })

  it('keeps the Linux kingu-ide wrapper for local (non-remote) Claude Agent Teams', () => {
    // Why: the `kingu-ide` rename is still required for a local Linux desktop
    // install (avoids shadowing the GNOME Kingu screen reader), so an explicit
  ...
  it('clears draft environment variables with the target shell syntax', () => {
    ...
    ).toBe('pi; Remove-Item Env:KINGU_PI_PREFILL -ErrorAction SilentlyContinue')
    ...
    ).toBe('pi & set "KINGU_PI_PREFILL="')
  })

  it('returns an OMP draft plan with KINGU_OMP_PREFILL (OMP-scoped, not Pi-shared)', () => {
    // Why: OMP owns its own managed prefill extension and env var.
    // kingu-prefill.ts reads KINGU_OMP_PREFILL for OMP launches — see
    ...
    expect(plan?.env).toEqual({ KINGU_OMP_PREFILL: 'fix the omp regression' })
    expect(plan?.expectedProcess).toBe('omp')
    expect(plan?.launchCommand).toBe(
      `omp; command test -n "$fish_pid" && set --erase -g KINGU_OMP_PREFILL; command test -z "$fish_pid" && unset KINGU_OMP_PREFILL; true`
    )
  })
  ...
  it('excludes transient draft prompt env from launch config', () => {
    const plan = buildAgentDraftLaunchPlan({
      ...
      draft: 'prefill text',
      cmdOverrides: {},
      agentEnv: { KINGU_AGENT_MODE: 'managed' },
      platform: 'linux'
    })

    expect(plan?.env).toEqual({ KINGU_AGENT_MODE: 'managed', KINGU_PI_PREFILL: 'prefill text' })
    expect(plan?.launchConfig).toEqual({
      agentCommand: 'pi',
      agentArgs: '',
      agentEnv: { KINGU_AGENT_MODE: 'managed' }
