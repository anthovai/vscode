...
/**
 * Plugin manifest v1 (`kingu-plugin.json` at the plugin root). The
 * `contributes` key names deliberately mirror common Electron-ecosystem
 * manifest conventions so future adapters stay cheap.
 *
 * Lives in `shared` so the desktop app, the headless `kingu serve` runtime,
...
const kinguEngineRangeSchema = z
  .string()
  .max(64)
...
export const pluginManifestSchema = z
  .object({
    ...
    engines: z.object({ kingu: kinguEngineRangeSchema }),
...
export const PLUGIN_MANIFEST_FILENAME = 'kingu-plugin.json'
...
export function satisfiesKinguEngineRange(hostVersion: string, range: string): boolean {
