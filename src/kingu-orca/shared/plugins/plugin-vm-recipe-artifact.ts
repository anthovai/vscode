import { z } from 'zod'
import { KINGU_VM_RECIPE_ID_PATTERN, KINGU_VM_RECIPE_ID_RULE } from '../kingu-yaml'
...
const pluginVmRecipeArtifactSchema = z
  .object({
    ...
    id: z.string().regex(KINGU_VM_RECIPE_ID_PATTERN, KINGU_VM_RECIPE_ID_RULE),
    name: z.string().trim().min(1).max(128),
    description: z.string().trim().min(1).max(1024).optional(),
    checkoutMode: z.enum(['kingu-worktree', 'provisioned-root']).optional(),
...
export function parsePluginVmRecipeArtifact(raw: string): KinguVmRecipe {
...
export function listPluginVmRecipeCommands(recipe: KinguVmRecipe): PluginVmRecipeCommand[] {
  return [
