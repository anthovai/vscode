...
export function normalizeBrowserNavigationUrl(
  ...
  if (trimmed.length === 0 || trimmed === 'about:blank' || trimmed === KINGU_BROWSER_BLANK_URL) {
    return KINGU_BROWSER_BLANK_URL
  }
...
export function normalizeExternalBrowserUrl(rawUrl: string): string | null {
  ...
  if (normalized === null || normalized === KINGU_BROWSER_BLANK_URL) {
    return null
  }
