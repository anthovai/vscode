...
describe('getAgentRowConversationName', () => {
  ...
  it('rejects spinner+cwd titles instead of surfacing paths as names', () => {
    expect(
      getAgentRowConversationName(makeTab({ title: '⠋ ~/kingu/workspaces' }), 'codex', false)
    ).toBeNull()
    expect(
      getAgentRowConversationName(makeTab({ title: '/Users/dev/repo' }), 'codex', false)
    ).toBeNull()
    expect(
      getAgentRowConversationName(makeTab({ title: 'C:\\repos\\kingu' }), 'codex', false)
    ).toBeNull()
    expect(
      getAgentRowConversationName(makeTab({ title: 'kingu/workspaces' }), 'codex', false)
    ).toBeNull()
    expect(
      getAgentRowConversationName(
        makeTab({ title: '\\\\wsl.localhost\\Ubuntu\\home\\dev\\kingu' }),
        'codex',
        false
      )
    ).toBeNull()
    expect(
      getAgentRowConversationName(makeTab({ title: 'repos\\kingu' }), 'codex', false)
    ).toBeNull()
  })
  ...
  it('rejects identity-echo, management, and placeholder titles', () => {
    ...
    expect(
      getAgentRowConversationName(makeTab({ title: '◇ Ready (kingu)' }), 'gemini', false)
    ).toBeNull()
    expect(
