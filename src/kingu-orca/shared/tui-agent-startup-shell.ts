...
/**
 * `'posix'` covers every Unix shell Kingu can type into, fish included — not
 ...
 * is deliberately no fish member: a dialect Kingu has to detect is a dialect it
...
export function tokenizeStartupCommand(
  ...
  // Why one Unix parse: the input is a string the user typed into an Kingu
...
/**
 * Quotes one argument so the SAME text is literal in every Unix shell Kingu can
 ...
 * of them, and Kingu never has to know which shell will read the line.
 *
 ...
 *
 * Why not a helper function defined by Kingu's shell wrappers: Kingu only wraps
 ...
 * shells Kingu never spawned — in all of those a helper would be `command not
 ...
 * clipboard command, which a user may run with no Kingu-injected value at all.
