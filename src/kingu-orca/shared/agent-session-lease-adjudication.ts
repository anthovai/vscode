...
export type AgentSessionOwnerProbe =
  /** Kingu watched this exact process exit. */
...
export function evaluateAgentSessionAcquisition(args: {
  ...
  if (lease.ownerProcess !== null) {
    if (!isProvenDeadProbe(probe)) {
      // Why: a lapsed deadline means Kingu stopped hearing from the owner, not that the child
...
/**
 * A process carrying an Kingu spawn token with no matching lease is an orphan: stop it, never
