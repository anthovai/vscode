...
describe('execution host registry', () => {
  ...
  it('hides runtime-owned (ephemeral VM) SSH targets from repo-derived hosts', () => {
    const hosts = buildExecutionHostRegistry({
      ...
      repos: [
        {
          connectionId: 'runtime-ssh-kingu-instance-1',
          executionHostId: 'ssh:runtime-ssh-kingu-instance-1'
        },
        { connectionId: 'repo-ssh' }
      ],
      settings: { activeRuntimeEnvironmentId: null },
      // Even if a stale label leaked in, it must still be filtered out.
      sshTargetLabels: new Map([['runtime-ssh-kingu-instance-1', 'Hidden VM']])
    })

    expect(hosts.some((h) => h.id.includes('runtime-ssh-kingu-instance-1'))).toBe(false)
          ...
          'dev-box',
          {
            ...
            status: {
              ...
              remoteControl: {
                ...
                lastError: 'Remote Kingu runtime closed the connection.'
              }
            }
          }
        ]
