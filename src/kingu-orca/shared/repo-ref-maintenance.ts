...
export class RepoRefMaintenance {
   ...
   * pack, or yielding to work Kingu asked us to yield to, does not: both end on
  ...
  private async packIfNeeded(
    ...
    // Kingu sessions a branch moving mid-pack is the normal case, and Git's
  ...
  /** Record an aborted attempt: retry soon if Kingu yielded, back off if it stalled. */
  private yieldTo(
    key: string,
    tracked: TrackedRepo,
    span: RefMaintenanceSpan,
