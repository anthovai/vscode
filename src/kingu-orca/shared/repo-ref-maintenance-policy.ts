/**
 * Idle-time loose-ref packing for repositories Kingu itself degrades.
 *
 * Kingu strips git's auto-maintenance off its own frequent fetches
 * (`GIT_FETCH_SKIP_AUTO_MAINTENANCE_CONFIG_ARGS`) and never compensated, so an
 * Kingu-driven checkout accumulates loose refs forever and every ref
...
export type RepoRefMaintenanceTarget = {
  ...
  /** A user who told Git not to auto-maintain this repo has told Kingu too. */
...
/** Marks an abort Kingu asked for, so the attempt is retried rather than blamed on the repo. */
