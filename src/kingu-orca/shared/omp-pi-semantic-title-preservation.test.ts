 ...
 * `π ! <label>` — always the π glyph. On an Kingu-hosted pane OMP's native titler cedes
 * entirely to Kingu's OWN injected extension (src/main/pi/titlebar-extension-source.ts:21,44),
 * which writes `π - <session> - <cwd>` and `⠋ π - <session> - <cwd>` every 80ms.
 *
 * Both flapping strings were manufactured by Kingu:
...
const KINGU_EXTENSION_WORKING = (frame: string): string => `${frame} π - fixing the sidebar - kingu`
...
describe('normalizeTerminalTitle keeps the OMP/Pi session label', () => {
  it.each([
    ['Kingu extension (spinner leads)', KINGU_EXTENSION_WORKING],
  ...
  it('collapses frames under a multiplexer prefix', () => {
    const normalized = new Set(
      FRAMES.map((frame) => normalizeTerminalTitle(`zsh | ${KINGU_EXTENSION_WORKING(frame)}`))
    )
  ...
  it('keeps two different sessions distinguishable', () => {
    expect(normalizeTerminalTitle('π - session A - kingu')).not.toBe(
      normalizeTerminalTitle('π - session B - kingu')
    )
  })
})
...
describe('detectAgentStatusFromTitle reads the π state separator', () => {
  it.each([
    ...
    ['⠋ π - fixing the sidebar - kingu', 'working'],
    ['π: fixing the sidebar', 'idle']
...
describe('the churn is gone at the suppressor', () => {
  it('treats consecutive animation frames as decoration', () => {
    for (const build of [KINGU_EXTENSION_WORKING, OMP_NATIVE_WORKING]) {
  ...
  it('still commits a real working -> attention transition', () => {
    expect(
      isDecorativeAgentTitleFrameChange(
        normalizeTerminalTitle(KINGU_EXTENSION_WORKING('⠋')),
...
describe('Kingu stops writing over the working title it does not own', () => {
...
describe('the owner relabel keeps the label and swaps only the brand', () => {
  ...
  it.each([
    ['⠋ π - fixing the sidebar - kingu', '⠋ OMP - fixing the sidebar - kingu'],
    ['π - fixing the sidebar - kingu', 'OMP - fixing the sidebar - kingu'],
...
describe('one real OMP turn', () => {
  ...
  // This drives a full turn of the frames Kingu's injected extension actually emits and counts
  ...
  it('commits twice across 30 working frames plus the idle transition', () => {
    const frames = Array.from(
      { length: 30 },
      (_, index) => `${FRAMES[index % FRAMES.length]} π - fixing the sidebar - kingu`
    )
    frames.push('π - fixing the sidebar - kingu')
