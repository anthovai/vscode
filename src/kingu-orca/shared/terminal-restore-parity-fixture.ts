...
import { activateKinguTerminalUnicodeProvider } from './terminal-unicode-provider'
 ...
 *  (pane-dom-creation.ts) and the Kingu ZWJ provider (pane-lifecycle.ts).
...
export function createRendererParityTerminal(dims: { cols: number; rows: number }): ParityTerminal {
  ...
  activateKinguTerminalUnicodeProvider(terminal)
 ...
 *    while Kingu's WebGL glyph renderer skips decorations on null cells.
 ...
 *  KinguRuntime.serializeHeadlessTerminalBuffer (normal buffer separated from
