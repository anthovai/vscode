...
function isUserFacingExternalWorktree(worktree: DetectedWorktree): boolean {
  ...
  return (
    ...
    worktree.ownership !== 'kingu-managed' &&
...
function isImportableExternalWorktree(worktree: DetectedWorktree): boolean {
  return !worktree.selectedCheckout && worktree.ownership !== 'kingu-managed'
}
...
export function getVisibleNonKinguWorktrees(
  detected: DetectedWorktreeListResult | undefined
): DetectedWorktree[] {
  if (detected?.authoritative !== true) {
    return []
  ...
  return detected.worktrees.filter(
    (worktree) =>
      worktree.visible && !worktree.selectedCheckout && worktree.ownership !== 'kingu-managed'
  )
}
