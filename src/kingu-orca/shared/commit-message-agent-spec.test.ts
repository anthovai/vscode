...
describe('model discovery parsers', () => {
  it('parses Claude list_models output into commit-message models', () => {
    const stdout = `${JSON.stringify({
      ...
      response: {
        subtype: 'success',
        request_id: 'kingu-model-discovery',
        response: {
          models: [
            {
              value: 'default',
  ...
  it('returns no Claude models when the CLI lacks list_models so the seed stays', () => {
    expect(
      parseClaudeModels(
        '{"type":"control_response","response":{"subtype":"error","request_id":"kingu-model-discovery","error":"Unsupported control request subtype: list_models"}}\n'
      )
    ).toEqual([])
  })
