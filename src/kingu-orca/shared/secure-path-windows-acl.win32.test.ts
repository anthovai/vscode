...
describeOnWindows('restrictWindowsPathSync against a real filesystem', () => {
  ...
  beforeAll(() => {
    resetSecureFileWindowsUserSidForTests()
    root = mkdtempSync(join(tmpdir(), 'kingu-acl-win32-'))
  ...
  it.each([
    ...
    // so Kingu cannot write into the directory it just cached as hardened.
   ...
   * presents as an Kingu crash rather than as the hardening problem it is.
   */
