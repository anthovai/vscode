...
async function fixture(
  ...
  const root = await mkdtemp(join(tmpdir(), 'kingu-relay-skill-test-'))
  roots.push(root)
...
describe('SkillInstallHandler', () => {
  ...
  it('recovers a committed install before the first managed-install listing', async () => {
    ...
    const { archive, call, state } = await fixture(async (input) => {
      ...
      const extractionPath = join(input.home, '.agents', 'skills', '.kingu-skill-extract-crashed')
      const journal: SkillInstallJournalV1 = {
        ...
        stagingPath: join(
          ...
          '.agents',
          'skills',
          `.${input.archive.manifest.name}.kingu-staging-crashed`
        ),
        backupPath: join(
          input.home,
          '.agents',
          'skills',
          `.${input.archive.manifest.name}.kingu-backup-crashed`
        ),
        backupDigest: null,
