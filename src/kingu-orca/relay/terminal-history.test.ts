...
const historyDir = join(homedir(), '.kingu-remote', 'terminal-history')
...
describe('relay shell history', () => {
  ...
  ])('replaces a %s HISTFILE inherited from a parent Kingu', (_kind, inherited) => {
    // HISTFILE is exported, so a relay (or the client that spawned it) started
    // from an Kingu pane would otherwise scope every remote pane to that one
