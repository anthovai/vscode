// Why: relay-side equivalent of Kingu's local agent integration installers.
// OpenCode still needs a config overlay, while Pi/OMP now get Kingu-managed
...
// independently from Kingu and the plugin source changes frequently as new
...
// directly: those modules import `electron` and ride on Kingu's userData
// path. The relay's electron-free constraint forces a thin parallel
// implementation rooted at $HOME/.kingu-relay/ for OpenCode and at the remote
...
const RELAY_HOOKS_DIR = '.kingu-relay'
const OPENCODE_OVERLAY_SUBDIR = 'opencode-overlays'
const PI_OVERLAY_SUBDIR_BY_KIND: Record<LegacyOverlayAgentKind, string> = {
  pi: 'pi-overlays',
  omp: 'omp-overlays'
}
const OPENCODE_PLUGIN_FILE = 'kingu-opencode-status.js'
const PI_EXTENSION_FILE = 'kingu-agent-status.ts'
const PI_AGENT_SUBDIR = 'agent'
// Why: bare-shell OMP still needs KINGU_OMP_STATUS_EXTENSION without mkdir ~/.omp.
...
const KINGU_MANAGED_EXTENSION_MARKER = '@kingu-managed-pi-extension'

function withKinguManagedPiExtensionMarker(source: string): string {
  return source.includes(KINGU_MANAGED_EXTENSION_MARKER)
    ? source
    : `// ${KINGU_MANAGED_EXTENSION_MARKER}\n${source}`
}
...
function safeDirName(input: string): string {
  ...
  // filesystem-unsafe characters in some Kingu builds. Hash to a fixed-width
...
export type PluginSources = {
  /** Source body of `kingu-opencode-status.js` to drop into <overlay>/plugins/. */
  opencodePluginSource?: string
  /** Source body of Pi's `kingu-agent-status.ts` to drop into <overlay>/extensions/. */
  piExtensionSource?: string
  /** Source body of OMP's `kingu-agent-status.ts` to drop into <overlay>/extensions/. */
  ompExtensionSource?: string
  /** Source body of Prime Agent's `kingu-agent-status.ts` to install in its real agent dir. */
...
export type MaterializePiResult = {
  ...
  /** Absolute path to kingu-agent-status.ts (real home or relay-managed fallback). */
...
export class PluginOverlayManager {
  ...
  /** Replace the cached source bodies. Called from relay.ts when Kingu sends
   *  `agent_hook.installPlugins`. The first install enables the augmenter
   *  output; subsequent installs (e.g. Kingu version upgrade in flight) refresh
  ...
  setSources(sources: PluginSources): void {
    ...
    if (typeof sources.piExtensionSource === 'string') {
      this.piExtensionSources.pi = withKinguManagedPiExtensionMarker(sources.piExtensionSource)
    }
    if (typeof sources.ompExtensionSource === 'string') {
      this.piExtensionSources.omp = withKinguManagedPiExtensionMarker(sources.ompExtensionSource)
    }
    if (typeof sources.primeAgentExtensionSource === 'string') {
      this.piExtensionSources['prime-agent'] = withKinguManagedPiExtensionMarker(
  ...
  materializeOpenCode(id: string, existingConfigDir?: string): string | null {
    ...
    try {
      ...
      if (existingConfigDir) {
        ...
        // remote root into the overlay before adding Kingu's plugin so status
  ...
  private canOverwritePiExtension(path: string): boolean {
    try {
      return readFileSync(path, 'utf8').includes(KINGU_MANAGED_EXTENSION_MARKER)
    } catch {
      return true
    }
  }
   ...
   *  export KINGU_OMP_STATUS_EXTENSION without KINGU_OMP_SOURCE_AGENT_DIR. */
  materializePi(
    id: string,
