 ...
 *    status both times.)
 * 2. Behaviour parity — a pane wrapped only because Kingu injected a worktree
...
describe('shell startup feature selection', () => {
  it('ignores an inherited KINGU_SHELL_FEATURES in the spawn env', () => {
    ...
    expect(
      selectShellStartupFeatures({
        shellPath: '/bin/zsh',
        env: { KINGU_SHELL_FEATURES: 'overlay,markers,ready,identity,history' },
        ...PLAIN_PANE
      })
    ).toEqual([])
  })
  ...
  it('cannot be turned off for a pane that needs a feature', () => {
    ...
    expect(
      selectShellStartupFeatures({
        shellPath: '/bin/zsh',
        env: { KINGU_SHELL_FEATURES: '', KINGU_HISTFILE: '/tmp/wt/zsh_history' },
        ...PLAIN_PANE
      })
    ).toEqual(['history'])
  })
  ...
  it('gives a history-only pane no command markers', () => {
    ...
    expect(
      selectShellStartupFeatures({
        shellPath: '/bin/zsh',
        env: { KINGU_HISTFILE: '/tmp/wt/zsh_history' },
        ...PLAIN_PANE
      })
  ...
  it('keeps markers on a pane that already had them', () => {
    expect(
      selectShellStartupFeatures({
        shellPath: '/bin/zsh',
        env: { KINGU_CODEX_HOME: '/tmp/codex' },
        ...PLAIN_PANE
      })
    ).toContain('markers')
  })
  ...
  it('does not wrap bash for history alone', () => {
    ...
    // replace its login startup-file chain with Kingu's approximation.
    expect(
      selectShellStartupFeatures({
        shellPath: '/bin/bash',
        env: { KINGU_HISTFILE: '/tmp/wt/bash_history' },
        ...PLAIN_PANE
      })
    ).toEqual([])
  })
...
describePosix('zsh launch config', () => {
  ...
  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'kingu-feature-channel-'))
    setTestUserDataPath(userDataPath)
    previousFeatures = process.env.KINGU_SHELL_FEATURES
  })
  ...
  afterEach(() => {
    if (previousFeatures === undefined) {
      ...
      process.env.KINGU_SHELL_FEATURES = previousFeatures
    }
  ...
  it('publishes exactly the selected features, whatever process.env holds', async () => {
    process.env.KINGU_SHELL_FEATURES = 'overlay,markers,ready,identity'
    const { getShellLaunchConfig } = await importFreshLocalPtyShellReady()

    const config = getShellLaunchConfig('/bin/zsh', ['history'])

    expect(config.env.KINGU_SHELL_FEATURES).toBe('history')
  })
  ...
  it('wraps a startup-only command without enabling shell readiness', async () => {
    ...
    expect(config.env.KINGU_SHELL_FEATURES).toBe('startup')
  ...
  it('does not treat another terminal’s hijacked ZDOTDIR as the user config dir', async () => {
    // Why: Kingu can be launched from a terminal that already owns ZDOTDIR. Only
    // a dir Kingu can prove is its own, or one holding no zsh startup file at
    // all, may be rejected — never a vendor guessed at by name.
    const home = mkdtempSync(join(tmpdir(), 'kingu-stacked-zdotdir-'))
    ...
    try {
      ...
      expect(getShellLaunchConfig('/bin/zsh', ['history']).env.KINGU_ORIG_ZDOTDIR).toBeUndefined()

      // Stamped as Kingu-owned: rejected by positive identification, even though
      // the path shape is not one of Kingu's.
      ...
      expect(
        stamped.getShellLaunchConfig('/bin/zsh', ['history']).env.KINGU_ORIG_ZDOTDIR
      ).toBeUndefined()
      ...
      expect(real.getShellLaunchConfig('/bin/zsh', ['history']).env.KINGU_ORIG_ZDOTDIR).toBe(
        foreignWrapper
      )
    } finally {
...
describePosix('epilogue under hostile user shell options', () => {
  ...
  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'kingu-feature-channel-options-'))
    setTestUserDataPath(userDataPath)
    home = mkdtempSync(join(tmpdir(), 'kingu-feature-options-home-'))
  })
  ...
  it.each(['no_unset', 'ksh_arrays'])(
    ...
    async (option) => {
      ...
      const spawnEnv: Record<string, string> = {
        ...
        KINGU_OPENCODE_CONFIG_DIR: opencodeDir
      }
      ...
      expect(values.LINEINIT).toBe('user:__kingu_prompt_mark')
      expect(values.PRECMD).toContain('__kingu_osc133_precmd')
...
function makeUserHome(): string {
  const home = mkdtempSync(join(tmpdir(), 'kingu-feature-home-'))
...
describePosix('history-only pane in a real zsh', () => {
  ...
  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'kingu-feature-channel-live-'))
    setTestUserDataPath(userDataPath)
    home = makeUserHome()
  })
  ...
  function withoutInheritedZdotdir(env: Record<string, string>): Record<string, string> {
    const { KINGU_ORIG_ZDOTDIR: _inherited, ...rest } = env
    return rest
  }
  ...
  async function launchHistoryOnly(): Promise<{
    ...
    return {
      ...
      env: {
        PATH: '/usr/bin:/bin',
        ...spawnEnv,
        // Why KINGU_ORIG_ZDOTDIR is stripped: the launch config computes it from
  ...
  itWithZsh('unsets the feature channel for the shell and for a grandchild', async () => {
    ...
    const output = execFileSync(
      ...
      [
        ...
        '-i',
        '-c',
        `print -r -- "SELF=[\${KINGU_SHELL_FEATURES:-}]"; zsh -c 'printenv KINGU_SHELL_FEATURES; print -r -- CHILD_CLEAN'`
      ],
  ...
  itWithZsh(
    ...
    async () => {
      ...
      // started with -c would report a pane Kingu had not finished setting up.
      const { env } = await launchHistoryOnly()
      const capture = [
        'PRECMD="${precmd_functions[*]}"; PREEXEC="${preexec_functions[*]}"',
        'LINEINIT="${widgets[zle-line-init]:-none}"'
      ]
      const report = ['PRECMD', 'PREEXEC', 'LINEINIT', 'ZDOTDIR', 'KINGU_SHELL_FEATURES']
      ...
      // place: a history-only pane leaves no Kingu name in the user's hook arrays.
      ...
      // installs its own zle-line-init widget has one either way, and what Kingu
      // owes is that it looks the same wrapped as unwrapped.
      expect(wrapped.values.LINEINIT).toBe(unwrapped.values.LINEINIT)
      expect(wrapped.values.PRECMD).not.toContain('kingu')
      ...
      expect(wrapped.values.KINGU_SHELL_FEATURES).toBe('UNSET')
    }
  )
})
