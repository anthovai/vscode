...
describe('skill discovery', () => {
  ...
  it('keeps every healthy root when one root walk is aborted', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kingu-skills-'))
  ...
  it('discovers skill packages through symlinked skill directories', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kingu-skills-'))
    const home = join(root, 'home')
    const realSkill = join(root, 'central-skills', 'kingu-cli')
    const linkedSkill = join(home, '.agents', 'skills', 'kingu-cli')
    await mkdir(realSkill, { recursive: true })
    await mkdir(join(home, '.agents', 'skills'), { recursive: true })
    await writeFile(join(realSkill, 'SKILL.md'), '# Kingu CLI\n\nUse the Kingu CLI.')
    ...
    const skill = result.skills.find((entry) => entry.name === 'Kingu CLI')
  ...
  it('keeps home classification when cwd points at the same directory as home', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kingu-skills-'))
    const home = join(root, 'home')
    const skillDir = join(home, '.agents', 'skills', 'kingu-cli')
    await mkdir(skillDir, { recursive: true })
    await writeFile(
      join(skillDir, 'SKILL.md'),
      ['---', 'name: kingu-cli', 'description: Use the Kingu CLI.', '---', ''].join('\n')
    )
    ...
    expect(result.skills.filter((entry) => entry.name === 'kingu-cli')).toMatchObject([
      {
        sourceKind: 'home',
