...
const DISTRO = process.env.KINGU_REAL_WSL_SKILL_DISTRO ?? 'Ubuntu-24.04'
const RUN_REAL_WSL = process.platform === 'win32' && process.env.KINGU_REAL_WSL_SKILL_TEST === '1'
...
describe.runIf(RUN_REAL_WSL)('real WSL skill install transactions', () => {
  ...
  beforeAll(async () => {
    localRoot = await mkdtemp(join(tmpdir(), 'kingu-wsl-skill-integration-'))
    guestRoot = await runWsl('mktemp', '-d', '/tmp/kingu-skill-integration.XXXXXX')
    if (!guestRoot.startsWith('/tmp/kingu-skill-integration.')) {
  ...
  function installInput(
    ...
    return {
      ...
      kinguStateDirectory: join(localRoot, `state-${scope}`),
  ...
  it('installs, recovers, updates, conflicts, and removes on the guest filesystem', async () => {
    ...
    const removeInput = {
      ...
      homeDirectory,
      kinguStateDirectory: join(localRoot, 'state-global'),
      detectedProviders: ['codex', 'claude'],
      filesystem
    }
    ...
    expect(
      (
        await removeSharedSkill({
          ...
          homeDirectory,
          workspaceDirectory,
          kinguStateDirectory: join(localRoot, 'state-workspace'),
          detectedProviders: ['codex', 'claude'],
          filesystem
        })
      ).status
  ...
  it('installs and removes within a Windows-backed WSL workspace', async () => {
    ...
    const input = {
      ...
      workspaceDirectory: windowsWorkspace,
      kinguStateDirectory: join(localRoot, 'state-drvfs')
    }
