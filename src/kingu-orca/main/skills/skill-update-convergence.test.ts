...
describe('convergableSkillNames', () => {
  ...
  it('drops a skill whose lock records a revision the disk does not have', () => {
    const result = convergableSkillNames(
      [placement('kingu-linear', 1)],
      new Map([['kingu-linear', '091d9bcc']]),
      { 'kingu-linear': [PRE_STUB, STUB] }
    )
  ...
  it('keeps a skill whose lock matches disk even when it is outdated', () => {
    const result = convergableSkillNames(
      [placement('kingu-cli', 1)],
      new Map([['kingu-cli', 'aaaa1111']]),
      { 'kingu-cli': [revision(1, 'aaaa1111')] }
    )
    expect([...result]).toEqual(['kingu-cli'])
  })

  it('keeps a skill whose disk content matches no known revision', () => {
    const result = convergableSkillNames(
      [placement('kingu-cli', null)],
      new Map([['kingu-cli', 'aaaa1111']]),
      { 'kingu-cli': [revision(1, 'bbbb2222')] }
    )
    expect([...result]).toEqual(['kingu-cli'])
  })
  ...
  it('keeps a skill with no observable placement', () => {
    const result = convergableSkillNames(
      [placement('kingu-cli', null, 'canonical-copy', null)],
      new Map([['kingu-cli', 'aaaa1111']]),
      { 'kingu-cli': [revision(1, 'aaaa1111')] }
    )
    expect([...result]).toEqual(['kingu-cli'])
  ...
  it('drops a stale skill whose folder holds files no revision lists', () => {
    const result = convergableSkillNames(
      [placement('kingu-linear', 1, 'canonical-copy', 'digest-with-sidecar')],
      new Map([['kingu-linear', '091d9bcc']]),
      { 'kingu-linear': [PRE_STUB, STUB] }
    )
    expect([...result]).toEqual([])
  ...
  it('keeps a skill when any placement still matches the lock', () => {
    const result = convergableSkillNames(
      [placement('kingu-cli', 1), placement('kingu-cli', 2)],
      new Map([['kingu-cli', 'aaaa1111']]),
      { 'kingu-cli': [revision(1, 'aaaa1111'), revision(2, 'f3727995')] }
    )
    expect([...result]).toEqual(['kingu-cli'])
  })
  ...
  it('keeps a skill whose lock names no revision we know', () => {
    const result = convergableSkillNames(
      [placement('kingu-cli', 1)],
      new Map([['kingu-cli', 'not-a-known-tree']]),
      { 'kingu-cli': [revision(1, 'f3727995')] }
    )
    expect([...result]).toEqual(['kingu-cli'])
  })
  ...
  it('keeps a skill when one placement is stale but another is unidentifiable', () => {
    const result = convergableSkillNames(
      [placement('kingu-cli', 1), placement('kingu-cli', null)],
      new Map([['kingu-cli', '091d9bcc']]),
      { 'kingu-cli': [PRE_STUB, STUB] }
    )
    expect([...result]).toEqual(['kingu-cli'])
  })
  ...
  it('ignores an unidentifiable plugin-cache copy when judging the canonical', () => {
    const result = convergableSkillNames(
      [placement('kingu-linear', 1), placement('kingu-linear', null, 'plugin-cache')],
      new Map([['kingu-linear', '091d9bcc']]),
      { 'kingu-linear': [PRE_STUB, STUB] }
    )
    expect([...result]).toEqual([])
  ...
  it('ignores a plugin-cache copy that matches the lock', () => {
    const result = convergableSkillNames(
      [placement('kingu-linear', 1), placement('kingu-linear', 2, 'plugin-cache')],
      new Map([['kingu-linear', '091d9bcc']]),
      { 'kingu-linear': [PRE_STUB, STUB] }
    )
    expect([...result]).toEqual([])
  ...
  it('judges each locked skill independently', () => {
    const result = convergableSkillNames(
      [placement('kingu-linear', 1), placement('kingu-cli', 1)],
      new Map([
        ['kingu-linear', '091d9bcc'],
        ['kingu-cli', 'aaaa1111']
      ]),
      {
        'kingu-linear': [PRE_STUB, STUB],
