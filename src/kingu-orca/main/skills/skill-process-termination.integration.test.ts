  ...
  const child = spawn(
    ...
    ],
    {
      ...
      env: {
        ...
        KINGU_SKILL_PROCESS_CHILD: '1',
        KINGU_SKILL_CRASH_ROOT: root,
        KINGU_SKILL_CRASH_MARKER: marker,
        KINGU_SKILL_CRASH_OPERATION: crashCase.operation,
        KINGU_SKILL_CRASH_PHASE: crashCase.phase,
        KINGU_SKILL_CRASH_BOUNDARY: crashCase.boundary
      },
...
describe.runIf(RUN_REAL_PROCESS)('skill process termination recovery', () => {
  it.each(cases)(
    ...
    async (crashCase) => {
      const root = await mkdtemp(join(tmpdir(), 'kingu-skill-process-crash-'))
      roots.push(root)
