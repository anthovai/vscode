...
describe.runIf(RUN_REAL_WINDOWS)('real Windows skill workspace installation', () => {
  ...
  beforeAll(async () => {
    root = await mkdtemp(join(tmpdir(), 'kingu-windows-skill-integration-'))
    ...
    stateDirectory = join(root, 'kingu state')
    ...
    await git(['config', 'user.name', 'Kingu Test'], repository)
    await git(['config', 'user.email', 'kingu@example.invalid'], repository)
