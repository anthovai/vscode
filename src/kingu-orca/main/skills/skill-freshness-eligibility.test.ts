...
const globallyUpdatableNames = new Set(['computer-use', 'kingu-cli', 'orchestration'])
...
describe('skill freshness name-scoped update eligibility', () => {
  ...
  it.each([
    ...
    (status, topology) => {
      ...
      expect(
        eligible([
          placement('kingu-cli'),
          placement('kingu-cli', { id: `outlier-${status}-${topology}`, status, topology })
        ])
      ).toEqual(['kingu-cli'])
    }
  )
  ...
  it.each(['unrecognized', 'inaccessible', 'newer-known'] as const)(
    ...
    (status) => {
      ...
      expect(
        eligible([
          placement('kingu-cli', { id: 'blocked-canonical', status }),
          placement('kingu-cli', {
            id: 'kingu-cli-claude',
            rootId: 'home-claude',
            topology: 'provider-alias',
    ...
    expect(
      eligible([
        ...
        placement('kingu-cli', {
          id: 'kingu-cli-gemini',
          rootId: 'home-gemini',
          unresolvedPath: '/home/.gemini/skills/kingu-cli',
          resolvedPath: '/home/.gemini/skills/kingu-cli',
  ...
  it('does not offer an official canonical copy missing from the updater lock (#10791)', () => {
    expect(eligibleSkillUpdateNames([placement('kingu-cli')], new Set())).toEqual([])
  })

  it('builds only an explicit, deterministic global command', () => {
    expect(buildTargetedSkillUpdateCommand(['orchestration', 'kingu-cli', 'kingu-cli'])).toBe(
      'npx skills update kingu-cli orchestration --global'
    )
    expect(buildTargetedSkillUpdateCommand([])).toBeNull()
    expect(buildTargetedSkillUpdateCommand(['kingu-cli;echo unsafe'])).toBeNull()
  })
})
