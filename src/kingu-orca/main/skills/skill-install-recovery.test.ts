...
async function interruptedUpdate(phase: SkillInstallJournalV1['phase']): Promise<{
  ...
  const root = await mkdtemp(join(tmpdir(), 'kingu-skill-recovery-test-'))
  roots.push(root)
  ...
  const extractionPath = join(destinationRoot, '.kingu-skill-extract-recovery')
  const stagingPath = join(destinationRoot, '.recovery-skill.kingu-staging-recovery')
  const backupPath = join(destinationRoot, '.recovery-skill.kingu-backup-recovery')
