...
describe('skillUpdateFailedNames', () => {
  it('treats a convergent copy that is now current as landed', () => {
    expect(
      skillUpdateFailedNames(['kingu-cli'], [placement('kingu-cli', 'current')], noLocks)
    ).toEqual([])
  })

  it('reports a copy the run left outdated', () => {
    expect(
      skillUpdateFailedNames(['kingu-cli'], [placement('kingu-cli', 'outdated')], noLocks)
    ).toEqual(['kingu-cli'])
  })
  ...
  it('reports a half-written bundle instead of reading it as success', () => {
    ...
    expect(
      skillUpdateFailedNames(['kingu-cli'], [placement('kingu-cli', 'unrecognized')], noLocks)
    ).toEqual(['kingu-cli'])
  })

  it('reports an unreadable copy', () => {
    expect(
      skillUpdateFailedNames(['kingu-cli'], [placement('kingu-cli', 'inaccessible')], noLocks)
    ).toEqual(['kingu-cli'])
  })

  it('reports a skill the run removed outright', () => {
    expect(skillUpdateFailedNames(['kingu-cli'], [], noLocks)).toEqual(['kingu-cli'])
  })
  ...
  it('accepts a revision newer than this build ships', () => {
    ...
    expect(
      skillUpdateFailedNames(['kingu-cli'], [placement('kingu-cli', 'newer-known')], noLocks)
    ).toEqual([])
  })
  ...
  it('ignores placements the update command never writes to', () => {
    expect(
      skillUpdateFailedNames(
        ['kingu-cli'],
        [placement('kingu-cli', 'current'), placement('kingu-cli', 'outdated', 'plugin-cache')],
        noLocks
      )
    ).toEqual([])
  })
  ...
  it('fails the name when any convergent alias was left behind', () => {
    expect(
      skillUpdateFailedNames(
        ['kingu-cli'],
        [placement('kingu-cli', 'current'), placement('kingu-cli', 'outdated', 'provider-alias')],
        noLocks
      )
    ).toEqual(['kingu-cli'])
  })
  ...
  it('judges each requested name independently', () => {
    expect(
      skillUpdateFailedNames(
        ['kingu-cli', 'orchestration'],
        [placement('kingu-cli', 'current'), placement('orchestration', 'outdated')],
        noLocks
      )
  ...
  it('treats unrecognized content whose tree sha matches the lock as landed', () => {
    ...
    expect(
      skillUpdateFailedNames(
        ['kingu-cli'],
        [placement('kingu-cli', 'unrecognized', 'canonical-copy', 'ahead-of-bundle')],
        new Map([['kingu-cli', 'ahead-of-bundle']])
      )
    ).toEqual([])
  })
  ...
  it('still reports unrecognized content whose bytes do not match the lock', () => {
    expect(
      skillUpdateFailedNames(
        ['kingu-cli'],
        [placement('kingu-cli', 'unrecognized', 'canonical-copy', 'half-written-bytes')],
        new Map([['kingu-cli', 'ahead-of-bundle']])
      )
    ).toEqual(['kingu-cli'])
  })
  ...
  it('never forgives an outdated copy, even at the lock hash', () => {
    ...
    expect(
      skillUpdateFailedNames(
        ['kingu-cli'],
        [placement('kingu-cli', 'outdated', 'canonical-copy', 'locked-revision')],
        new Map([['kingu-cli', 'locked-revision']])
      )
    ).toEqual(['kingu-cli'])
  })
  ...
  it('does not let a lock-matching canonical copy excuse a degraded alias', () => {
    expect(
      skillUpdateFailedNames(
        ...
        [
          placement('kingu-cli', 'unrecognized', 'canonical-copy', 'ahead-of-bundle'),
          placement('kingu-cli', 'inaccessible', 'provider-alias')
        ],
        new Map([['kingu-cli', 'ahead-of-bundle']])
      )
    ).toEqual(['kingu-cli'])
...
describe('skillUpdateFailedNames over a real inventory', () => {
  ...
  async function postCutFixture(): Promise<{ homeDir: string; installedTreeSha: string }> {
    const root = await mkdtemp(join(tmpdir(), 'kingu-skill-outcome-'))
    temporaryDirectories.push(root)
    const homeDir = join(root, 'home')
    const skillDir = join(homeDir, '.agents', 'skills', 'kingu-cli')
    ...
    const current = await readFile(join(repoRoot, 'skills', 'kingu-cli', 'SKILL.md'))
    await writeFile(
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        skills: {
          'kingu-cli': {
            skillFolderHash,
            skillPath: 'skills/kingu-cli',
            source: 'github.com/anthovai/kingu-intelligence'
          }
        }
      })
    )
  ...
  it('accepts post-cut source content when the lock records exactly those bytes', async () => {
    ...
    const canonical = inventory.installations.filter(
      (entry) => entry.name === 'kingu-cli' && entry.topology === 'canonical-copy'
    )
    expect(canonical).toHaveLength(1)
    expect(canonical[0].status).toBe('newer-known')
    expect(canonical[0].installedReleaseRevision).toBeNull()

    expect(skillUpdateFailedNames(['kingu-cli'], inventory.installations, locks)).toEqual([])
  })
