...
function makeTempDir(): string {
  const root = mkdtempSync(join(tmpdir(), 'kingu shell preflight '))
  tempDirs.push(root)
  return root
}
...
function writeFailingPreflight(root: string): string {
  const executable = join(root, 'kingu preflight.exe')
  ...
  writeFileSync(
    ...
    [
      ...
      "writeFileSync(process.env.KINGU_PREFLIGHT_MARKER, 'ran')",
      'process.exit(7)'
    ].join('\n')
  )
  return executable
...
describeWindows('Windows Codex shell preflight runtime', () => {
  it('runs a spaced executable through cmd.exe and continues after failure', async () => {
    ...
    await runPty({
      ...
      env: {
        ...process.env,
        KINGU_CODEX_LAUNCH_PREFLIGHT: preflight,
        [KINGU_CODEX_LAUNCH_PREFLIGHT_CMD_QUOTE_ENV]: '"',
        KINGU_PREFLIGHT_MARKER: preflightMarker
      }
    })
  ...
  it('runs the typed-Codex wrapper through Git Bash without MSYS switch rewriting', async () => {
    ...
    const previousUserDataPath = process.env.KINGU_USER_DATA_PATH
    process.env.KINGU_USER_DATA_PATH = join(root, 'user data')
    ...
    try {
      ...
      await runPty({
        ...
        env: {
          ...
          KINGU_CODEX_LAUNCH_PREFLIGHT: preflight,
          KINGU_PREFLIGHT_MARKER: preflightMarker,
          KINGU_CODEX_MARKER: codexMarker,
          TERM: 'xterm-256color'
        },
        input:
          "type -P codex > git-bash-codex-path\ncodex -e \"require('node:fs').writeFileSync(process.env.KINGU_CODEX_MARKER,'ran')\"\nexit\n",
