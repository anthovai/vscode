  ...
  describe('spawn', () => {
    it('does not pass a Windows Codex home into WSL terminals', async () => {
      ...
      provider.configure({
        buildSpawnEnv: (_id, env) => {
          ...
          env.KINGU_CODEX_HOME = 'C:\\Users\\jin\\.codex'
          return env
        }
      })
      ...
      expect(spawnCall[2].env.KINGU_CODEX_HOME).toBeUndefined()
    })
    ...
    it('does not pass a WSL managed Codex home into Windows terminals', async () => {
      ...
      provider.configure({
        buildSpawnEnv: (_id, env) => {
          env.CODEX_HOME =
            '\\\\wsl.localhost\\Ubuntu\\home\\jin\\.local\\share\\kingu\\codex-accounts\\a\\home'
          env.KINGU_CODEX_HOME =
            '\\\\wsl.localhost\\Ubuntu\\home\\jin\\.local\\share\\kingu\\codex-accounts\\a\\home'
          return env
        }
      })
    ...
    it('translates a WSL managed Codex home before launching a WSL terminal', async () => {
      ...
      const spawnCall = spawnMock.mock.calls.at(-1)!
      expect(spawnCall[0]).toBe('wsl.exe')
      expect(spawnCall[2].env.CODEX_HOME).toBe('/home/jin/.local/share/kingu/codex-accounts/a/home')
      expect(spawnCall[2].env.KINGU_CODEX_HOME).toBe(
        '/home/jin/.local/share/kingu/codex-accounts/a/home'
      )
      expect(spawnCall[2].env.WSLENV).toContain('CODEX_HOME')
      expect(spawnCall[2].env.WSLENV).toContain('KINGU_CODEX_HOME')
    })
    ...
    it('uses the preferred WSL distro for Windows cwd WSL terminals', async () => {
      ...
      expect(spawnCall[1][5]).toContain('exec "$_kingu_wsl_shell" -l')
    ...
    it('marks Kingu terminal handle for WSL import when buildSpawnEnv opts in', async () => {
      Object.defineProperty(process, 'platform', { configurable: true, value: 'win32' })
      ...
      provider.configure({
        buildSpawnEnv: (_id, env, ctx) => {
          env.KINGU_TERMINAL_HANDLE = 'term_wsl'
          if (ctx?.isWsl) {
            env.WSLENV = 'KINGU_TERMINAL_HANDLE/u'
          }
          return env
        }
      })
      ...
      try {
        await provider.spawn({
          ...
          rows: 24,
          cwd: '\\\\wsl.localhost\\Ubuntu\\home\\jin\\repo',
          env: { KINGU_HERMES_STARTUP_QUERY: 'line one\nline two' }
        })
      } finally {
      ...
      const spawnCall = spawnMock.mock.calls.at(-1)!
      expect(spawnCall[0]).toBe('wsl.exe')
      expect(spawnCall[2].env.KINGU_TERMINAL_HANDLE).toBe('term_wsl')
    ...
    it('resolves the Git Bash default shell and preserves the requested cwd', async () => {
      ...
      provider.configure({
        ...
        buildSpawnEnv: (_id, env) => ({
          ...env,
          KINGU_CODEX_LAUNCH_PREFLIGHT: CODEX_LAUNCH_PREFLIGHT
        })
      })
    ...
    it('runs the Codex preflight once in the cmd.exe startup chain', async () => {
      ...
      expect(spawnMock).toHaveBeenCalledWith(
        ...
        [
          '/K',
          'chcp 65001 > nul & if defined KINGU_CODEX_LAUNCH_PREFLIGHT call %KINGU_CODEX_LAUNCH_PREFLIGHT_CMD_QUOTE%%KINGU_CODEX_LAUNCH_PREFLIGHT%%KINGU_CODEX_LAUNCH_PREFLIGHT_CMD_QUOTE% agent hooks prepare-codex > nul 2>&1'
        ],
        expect.objectContaining({
          env: expect.objectContaining({
            KINGU_CODEX_LAUNCH_PREFLIGHT: CODEX_LAUNCH_PREFLIGHT,
            KINGU_CODEX_LAUNCH_PREFLIGHT_CMD_QUOTE: '"'
          })
        })
      )
    })
