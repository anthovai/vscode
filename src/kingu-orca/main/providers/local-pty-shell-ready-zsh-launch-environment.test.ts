...
describePosix('live zsh subprocess tests', () => {
  describeIfZsh('terminal emulator edge cases', () => {
    ...
    beforeEach(async () => {
      testHome = mkdtempSync(join(tmpdir(), 'kingu-term-'))
      userDataPath = mkdtempSync(join(tmpdir(), 'kingu-term-userdata-'))
    ...
    it('discovers ZDOTDIR when launched inside tmux', async () => {
      ...
      delete cleanEnv.KINGU_ORIG_ZDOTDIR
    ...
    it('re-discovers ZDOTDIR despite stale KINGU_ORIG_ZDOTDIR from previous session', async () => {
      ...
      const previousKinguZdotdir = process.env.KINGU_ORIG_ZDOTDIR
      process.env.KINGU_ORIG_ZDOTDIR = '/opt/kingu-old/shell-ready/zsh' // stale wrapper path
      ...
      try {
        ...
        const cleanEnv: Record<string, string | undefined> = {
          ...process.env,
          HOME: testHome,
          KINGU_ORIG_ZDOTDIR: '/opt/kingu-old/shell-ready/zsh'
        }
        delete cleanEnv.ZDOTDIR
        cleanEnv.ZDOTDIR = config.env.ZDOTDIR
        ...
        if (previousKinguZdotdir === undefined) {
          delete process.env.KINGU_ORIG_ZDOTDIR
        } else {
          process.env.KINGU_ORIG_ZDOTDIR = previousKinguZdotdir
        }
      }
    })

    it('prioritizes fresh discovery over inherited KINGU_ORIG_ZDOTDIR', async () => {
      ...
      process.env.KINGU_ORIG_ZDOTDIR = oldZdotdir
    ...
    it('sources launch-time ZDOTDIR .zshenv when it is explicitly inherited', async () => {
      ...
      try {
        const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
        const config = getShellReadyLaunchConfig('/bin/zsh')
        // One channel now does what KINGU_ORIG_ZDOTDIR and KINGU_ZSHENV_SOURCE_DIR
        // split between them: the wrapper hands this back as ZDOTDIR, and zsh
        // reads .zshenv through it.
        expect(config.env.KINGU_ORIG_ZDOTDIR).toBe(inheritedZdotdir)
