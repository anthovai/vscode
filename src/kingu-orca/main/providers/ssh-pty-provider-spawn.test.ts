...
describe('spawn', () => {
  ...
  it('preserves explicit TERM and forwards final env deletions to the relay', async () => {
    ...
    const envToDelete = ['TERM_PROGRAM', 'KINGU_STALE_TEST_ENV']
    ...
    await provider.spawn({
      ...
      env: {
        ...
        KINGU_STALE_TEST_ENV: '/tmp/stale-env'
      },
      envToDelete
    })
    ...
    expect(spawnCall?.[1]?.env).not.toHaveProperty('KINGU_STALE_TEST_ENV')
  })
  ...
  it('injects the relay-backed Kingu CLI bridge into remote PTY env', async () => {
    mux.request.mockResolvedValue({ id: 'pty-bridge' })
    provider = new SshPtyProvider('conn-1', mux as never, {
      binDir: '/home/user/.kingu-relay/bin',
      relayDir: '/home/user/.kingu-relay/relay-v1',
      nodePath: '/usr/bin/node',
      sockPath: '/home/user/.kingu-relay/relay.sock'
    })

    await provider.spawn({
      cols: 120,
      rows: 40,
      env: { PATH: '/usr/bin', KINGU_TERMINAL_HANDLE: 'term_ssh' }
    })
    ...
    expectRequest(mux.request, 'pty.spawn', {
      ...
      cwd: undefined,
      env: {
        PATH: '/home/user/.kingu-relay/bin:/usr/bin',
        KINGU_TERMINAL_HANDLE: 'term_ssh',
        [POWERLEVEL10K_WIZARD_DISABLE_ENV]: 'true',
        KINGU_REMOTE_CLI_BIN_DIR: '/home/user/.kingu-relay/bin',
        KINGU_RELAY_DIR: '/home/user/.kingu-relay/relay-v1',
        KINGU_RELAY_NODE_PATH: '/usr/bin/node',
        KINGU_RELAY_SOCKET_PATH: '/home/user/.kingu-relay/relay.sock'
      }
    })
  })
  ...
  it('uses Windows PATH delimiters for native Windows SSH bridge env', async () => {
    mux.request.mockResolvedValue({ id: 'pty-bridge' })
    provider = new SshPtyProvider('conn-1', mux as never, {
      binDir: 'C:/Users/me/.kingu-relay/bin',
      relayDir: 'C:/Users/me/.kingu-remote/relay-v1',
    ...
    expectRequest(mux.request, 'pty.spawn', {
      ...
      cwd: undefined,
      env: {
        Path: 'C:/Users/me/.kingu-relay/bin;C:/Windows/System32;C:/Tools',
        [POWERLEVEL10K_WIZARD_DISABLE_ENV]: 'true',
        KINGU_REMOTE_CLI_BIN_DIR: 'C:/Users/me/.kingu-relay/bin',
        KINGU_RELAY_DIR: 'C:/Users/me/.kingu-remote/relay-v1',
        KINGU_RELAY_NODE_PATH: 'C:/Program Files/nodejs/node.exe',
        KINGU_RELAY_SOCKET_PATH: '\\\\.\\pipe\\kingu-relay-123'
      }
    })
  })
