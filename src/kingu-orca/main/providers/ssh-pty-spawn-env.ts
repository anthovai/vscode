...
export function buildSshPtySpawnEnv(args: {
  ...
  if (args.remoteCliBridgeEnv) {
    ...
    merged.KINGU_REMOTE_CLI_BIN_DIR = args.remoteCliBridgeEnv.binDir
    merged.KINGU_RELAY_DIR = args.remoteCliBridgeEnv.relayDir
    merged.KINGU_RELAY_NODE_PATH = args.remoteCliBridgeEnv.nodePath
    merged.KINGU_RELAY_SOCKET_PATH = args.remoteCliBridgeEnv.sockPath
    if (args.remoteCliBridgeEnv.credentialFile) {
      merged.KINGU_RELAY_CREDENTIAL_FILE = args.remoteCliBridgeEnv.credentialFile
    }
  }
