...
import type {
  ...
  KinguHooks,
  SetupRunPolicy
} from '../shared/kingu-yaml-hook-types'
...
export function getEffectiveHooksFromConfig(
  ...
  // Why: committed `kingu.yaml` and local Settings can coexist; the source policy decides which is authoritative.
  return {
    scripts: {
      ...(setup ? { setup } : {}),
      ...(archive ? { archive } : {})
...
export function getDefaultTabCommandTrustContent(hooks: KinguHooks | null): string {
...
export function getDefaultTabsLaunch(
  ...
  // Why: local-only repos may use shared tab titles/colors but must not run the committed kingu.yaml commands.
