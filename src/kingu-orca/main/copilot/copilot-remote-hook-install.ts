...
export async function installCopilotHooksRemote(
  ...
  const remoteConfigPath = `${home}/.copilot/hooks/kingu.json`
  const remoteScriptPath = `${home}/.kingu/agent-hooks/copilot-hook.sh`
  ...
  try {
    ...
    if (!config) {
      return {
        ...
        detail: 'Could not parse remote Copilot hooks/kingu.json'
      }
    }
    ...
    for (const eventName of COPILOT_EVENTS) {
      ...
      nextHooks[eventName] = [
        ...
        getRemoteManagedHookDefinition(
          wrapPosixHookCommand(remoteScriptPath, { KINGU_COPILOT_HOOK_EVENT: eventName })
        )
      ]
    }
    ...
    // Why: SSH remotes use POSIX scripts regardless of Kingu's local OS. Write
    // the script before hooks/kingu.json so a partial install cannot point
