...
describe('BrowserSessionRegistry retired identity data', () => {
  ...
  it('ignores legacy persisted UAs, valid or broken, and applies the engine UA', async () => {
    const importedPartition = 'persist:kingu-browser-session-11111111-1111-4111-8111-111111111111'
    const brokenUa =
    ...
    seedMeta(fsState, {
      ...
      userAgentByPartition: {
        'persist:kingu-browser': brokenUa,
      ...
      profiles: [
        {
          id: profileId,
          scope: 'isolated',
          partition: `persist:kingu-browser-session-${profileId}`,
          label: 'Existing',
          source: null,
          userAgentMode: 'native'
        }
  ...
  it.each([
    ...
    const profileId = '11111111-1111-4111-8111-111111111111'
    const partition = `persist:kingu-browser-session-${profileId}`
    const fsState = createFsState()
    seedMeta(fsState, {
      defaultSource: null,
      profiles: [
  ...
  it('hydrates a retired native profile under the process identity', async () => {
    const importedPartition = 'persist:kingu-browser-session-12121212-1212-4121-8121-121212121212'
    const fsState = createFsState()
    seedMeta(fsState, {
      defaultSource: null,
      userAgent: null,
