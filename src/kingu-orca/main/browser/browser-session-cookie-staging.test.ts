...
const defaultPartition = 'persist:kingu-browser-default'
const routePartition = `persist:kingu-browser-v1-${'c'.repeat(64)}`
let userData = ''
...
beforeEach(() => {
  ...
  userData = realpathSync(mkdtempSync(join(tmpdir(), 'kingu-cookie-staging-')))
...
describe('supportsPendingBrowserCookieImportReplay', () => {
  it('refuses a client-hosted route partition and allows profile partitions', () => {
    ...
    expect(supportsPendingBrowserCookieImportReplay('persist:kingu-browser-profile-a')).toBe(true)
  })
})
...
describe('applyPendingBrowserCookieImports', () => {
  it('unlinks the plaintext staged database when the partition is not replayable', () => {
    ...
    applyPendingBrowserCookieImports({
      ...
      defaultPartition,
      activeKinguProfileId: 'profile-a'
    })
