...
import {
  activeBrowserRoutePartitionKinguProfileId,
...
const AUTHORITY_CONNECTION_IDENTITY_TAG = 'kingu-local-ssh-browser'
 ...
 * per-boot value: the same Kingu profile reaching the same SSH target must keep
 * reusing one partition, or cookies die on every restart.
 */
export function localSshBrowserAuthorityConnectionIdentity(kinguProfileId: string): string {
...
async function prepareFresh(input: {
  ...
  const kinguProfileId = activeBrowserRoutePartitionKinguProfileId()
  if (!kinguProfileId) {
  ...
  const derived = deriveBrowserRoutePartition({
    ...
    authorityConnectionIdentity: localSshBrowserAuthorityConnectionIdentity(kinguProfileId),
  ...
  if (persistedFingerprint === null) {
    const evicted = bindings.set(
      ...
      deriveLocalSshBrowserRoutePartitionStorageScope({ kinguProfileId, targetId: input.targetId })
    )
    if (evicted.length > 0) {
