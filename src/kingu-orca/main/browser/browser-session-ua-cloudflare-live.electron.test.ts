...
const enabled = process.env.KINGU_UA_CLOUDFLARE_LIVE === '1'
let liveTargetUrl = 'https://dash.cloudflare.com/login'
const repetitions = Number(process.env.KINGU_UA_CLOUDFLARE_REPETITIONS ?? 5)
...
describe.skipIf(!enabled)('Cloudflare live user-agent compatibility', () => {
  it('interleaves origin/main and branch with isolated profiles', async () => {
    ...
    console.info(`KINGU_UA_CLOUDFLARE_REPORT=${JSON.stringify(report)}`)
    const reportPath = process.env.KINGU_UA_CLOUDFLARE_REPORT_PATH
    if (reportPath) {
  ...
  it.skip('compares the Google auth document and cross-host resources', async () => {
    ...
    console.info(`KINGU_UA_GOOGLE_AUTH_REPORT=${JSON.stringify(report)}`)
    const reportPath = process.env.KINGU_UA_GOOGLE_AUTH_REPORT_PATH
    if (reportPath) {
      writeFileSync(reportPath, `${JSON.stringify(report, null, 2)}\n`)
    }
...
async function runLiveProbe(arm: LiveArm, repetition: number, site: LiveSite): Promise<LiveRun> {
  const root = mkdtempSync(join(tmpdir(), `kingu-cloudflare-${arm}-${repetition}-`))
...
app.setName('KinguCloudflareLiveProbe')
...
function launchFixture(fixturePath: string, root: string, cdpPort: number): ChildProcess {
  ...
  return spawn(
    ...
    { env: { ...env, KINGU_BACKGROUND_LAUNCH: '1' }, stdio: ['ignore', 'pipe', 'pipe'] }
  )
}
