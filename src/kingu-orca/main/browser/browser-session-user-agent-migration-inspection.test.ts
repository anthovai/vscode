...
function profileWithMode(mode: unknown): Record<string, unknown> {
  return {
    id: PROFILE_ID,
    scope: 'isolated',
    partition: getKinguProfileBrowserSessionPartition(KINGU_PROFILE_ID, PROFILE_ID),
    label: 'Existing',
    source: null,
    userAgentMode: mode
  }
...
function unhydratableProfile(extra: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    id: 'not-a-uuid',
    scope: 'isolated',
    partition: 'persist:kingu-browser-session-not-a-uuid',
    label: 'Unhydratable',
    source: null,
    ...extra
  }
...
describe('retired browser profile identity inspection', () => {
  it('detects an inspectable old choice without removing its bytes', () => {
    ...
    expect(inspectRetiredBrowserSessionProfileUserAgentModes([profile], KINGU_PROFILE_ID)).toEqual({
      noticePending: true,
      degraded: false
    })
  ...
  it.each([
    ...
    expect(inspectRetiredBrowserSessionProfileUserAgentModes([entry], KINGU_PROFILE_ID)).toEqual({
      noticePending: false,
      degraded: false
    })
  })
  ...
  it.each([
    ...
    expect(() =>
      inspectRetiredBrowserSessionProfileUserAgentModes([entry], KINGU_PROFILE_ID)
    ).not.toThrow()
    expect(inspectRetiredBrowserSessionProfileUserAgentModes([entry], KINGU_PROFILE_ID)).toEqual({
      noticePending: true,
      degraded: true
