...
const HIDE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  var g = window.__kinguGrab;
  if (g && g.host) g.host.style.display = 'none';
  document.querySelectorAll('[data-kingu-browser-annotation-overlay]').forEach(function(el) {
    el.setAttribute('data-kingu-previous-display', el.style.display || '');
...
const RESTORE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  ...
  document.querySelectorAll('[data-kingu-browser-annotation-overlay]').forEach(function(el) {
    el.style.display = el.getAttribute('data-kingu-previous-display') || '';
    el.removeAttribute('data-kingu-previous-display');
  });
})()`
