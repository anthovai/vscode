...
const {
  ...
  resolveDiagnosticKinguChannelMock,
  spanEndMock,
  startSpanMock,
  submitFeedbackMock
} = vi.hoisted(() => {
  ...
  return {
    ...
    resolveDiagnosticKinguChannelMock: vi.fn(),
    spanEndMock,
    startSpanMock: vi.fn(() => ({
      traceId: 'trace-id',
      spanId: 'span-id',
...
vi.mock('../observability/diagnostic-upload-endpoint', () => ({
  resolveDiagnosticKinguChannel: resolveDiagnosticKinguChannelMock
}))
...
describe('registerCrashReportingHandlers', () => {
  beforeEach(() => {
    ...
    resolveDiagnosticKinguChannelMock.mockReset()
    resolveDiagnosticKinguChannelMock.mockReturnValue('stable')
  ...
  it('copies sanitized submission and diagnostic omission failures for a captured report', async () => {
    ...
    const result = await handlers.get('crashReports:copyLatestDiagnostics')?.(null, {
      ...
      notes: 'current notes',
      submissionFailure: {
        error: 'fallback failed at C:\\Users\\alice\\Kingu',
        diagnosticContext: {
          status: 'not_uploaded',
  ...
  it('submits an uncaptured Help menu crash report with an attached diagnostic bundle', async () => {
    ...
    expect(collectDiagnosticBundleMock).toHaveBeenCalledWith(
      expect.objectContaining({ lookbackMinutes: 3 * 24 * 60, kinguChannel: 'stable' })
    )
    expect(submitFeedbackMock).toHaveBeenCalledWith({
