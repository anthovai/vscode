...
describe('diagnostics IPC handlers', () => {
  beforeEach(() => {
    ...
    delete (globalThis as { KINGU_BUILD_IDENTITY?: unknown }).KINGU_BUILD_IDENTITY
    delete (globalThis as { KINGU_DIAGNOSTICS_TOKEN_URL?: unknown }).KINGU_DIAGNOSTICS_TOKEN_URL
    process.env.KINGU_DIAGNOSTICS_TOKEN_URL = 'https://diagnostics.example.com/diagnostics/token'
  ...
  it('pins official builds to the compile-time diagnostics endpoint', async () => {
    ...
    const globalOverrides = globalThis as {
      KINGU_BUILD_IDENTITY?: 'stable'
      KINGU_DIAGNOSTICS_TOKEN_URL?: string
    }
    globalOverrides.KINGU_BUILD_IDENTITY = 'stable'
    globalOverrides.KINGU_DIAGNOSTICS_TOKEN_URL = 'https://official.example.com/diagnostics/token'
    process.env.KINGU_DIAGNOSTICS_TOKEN_URL = 'https://attacker.example.com/diagnostics/token'
    collectDiagnosticBundleMock.mockReturnValue(bundle)
    readFileSyncMock.mockReturnValue(bundle.payload)
    const collect = handlers.get('diagnostics:collectBundle')!
    const openPreview = handlers.get('diagnostics:openBundlePreview')!
