...
const { getAuthenticatedViewer: getAuthenticatedViewerMock, starKingu: starKinguMock } =
  mocks.client
...
describe('registerGitHubHandlers', () => {
  ...
  it('emits app_starred_kingu once after a successful star with cohort context', async () => {
    starKinguMock.mockResolvedValue(true)
    getCohortAtEmitMock.mockReturnValue({ nth_repo_added: 3 })

    registerGitHubHandlers(store as never, stats as never)

    await expect(handlers['gh:starKingu'](null, 'settings')).resolves.toBe(true)

    expect(starKinguMock).toHaveBeenCalledTimes(1)
    expect(getCohortAtEmitMock).toHaveBeenCalledTimes(1)
    expect(trackMock).toHaveBeenCalledTimes(1)
    expect(trackMock).toHaveBeenCalledWith('app_starred_kingu', {
      source: 'settings',
      nth_repo_added: 3
    })
  })
  ...
  it('accepts every app star source for success telemetry', async () => {
    ...
    for (const source of [
      ...
      await expect(handlers['gh:starKingu'](null, source)).resolves.toBe(true)
    }
  ...
  it('does not emit app_starred_kingu when the star action returns false', async () => {
    starKinguMock.mockResolvedValue(false)

    registerGitHubHandlers(store as never, stats as never)

    await expect(handlers['gh:starKingu'](null, 'landing')).resolves.toBe(false)
  ...
  it('does not emit app_starred_kingu when the star action throws', async () => {
    starKinguMock.mockRejectedValue(new Error('gh failed'))

    registerGitHubHandlers(store as never, stats as never)

    await expect(handlers['gh:starKingu'](null, 'star_nag')).rejects.toThrow('gh failed')
  ...
  it('preserves star result but skips telemetry for an invalid IPC source', async () => {
    ...
    await expect(handlers['gh:starKingu'](null, 'github_website')).resolves.toBe(true)
