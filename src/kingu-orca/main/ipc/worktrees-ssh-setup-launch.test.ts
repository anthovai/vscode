...
describe('registerWorktreeHandlers', () => {
  ...
  it('reads remote kingu.yaml and returns a setup launch payload during SSH create', async () => {
    const repo = {
      id: 'repo-ssh',
      path: '/remote/repo',
      displayName: 'ssh',
    ...
    const provider = {
      exec: vi.fn().mockImplementation(async (args: string[]) => {
        ...
        if (args[0] === 'rev-parse' && args[1] === '--git-path') {
          return {
            stdout: '/remote/repo/.git/worktrees/repo-improve-dashboard/kingu/setup-runner.sh\n',
            stderr: ''
          }
        }
        if (args[0] === 'rev-parse') {
    ...
    parseKinguYamlMock.mockReturnValue({
    ...
    expect(fsProvider.readFile).toHaveBeenCalledWith('/remote/repo-improve-dashboard/kingu.yaml')
    ...
    expect(fsProvider.createDir).toHaveBeenCalledWith(
      '/remote/repo/.git/worktrees/repo-improve-dashboard/kingu'
    )
    expect(fsProvider.writeFile).toHaveBeenCalledWith(
      '/remote/repo/.git/worktrees/repo-improve-dashboard/kingu/setup-runner.sh',
      '#!/usr/bin/env bash\nset -e\npnpm install\n'
    )
      ...
      expect.objectContaining({
        setup: {
          ...
          envVars: expect.objectContaining({
            KINGU_ROOT_PATH: '/remote/repo',
            KINGU_WORKTREE_PATH: '/remote/repo-improve-dashboard'
          }),
  ...
  it('keeps Windows SSH setup runners independent from the local Git Bash setting', async () => {
    ...
    const provider = {
      exec: vi.fn().mockImplementation(async (args: string[]) => {
        ...
        if (args[0] === 'rev-parse' && args[1] === '--git-path') {
          return {
            stdout:
              'C:\\remote\\repo\\.git\\worktrees\\improve-dashboard\\kingu\\setup-runner.cmd\n',
            stderr: ''
          }
        }
        if (args[0] === 'rev-parse') {
    ...
    store.setWorktreeMeta.mockImplementation((_worktreeId, meta) => meta)
    parseKinguYamlMock.mockReturnValue({ scripts: { setup: 'pnpm install' } })
    getEffectiveHooksFromConfigMock.mockReturnValue({ scripts: { setup: 'pnpm install' } })
    shouldRunSetupForCreateMock.mockReturnValue(true)
    ...
    expect(provider.exec).toHaveBeenCalledWith(
      ['rev-parse', '--git-path', 'kingu/setup-runner.cmd'],
      ...
      expect.objectContaining({
        setup: {
          runnerScriptPath:
            'C:\\remote\\repo\\.git\\worktrees\\improve-dashboard\\kingu\\setup-runner.cmd',
          envVars: expect.objectContaining({
            KINGU_ROOT_PATH: 'C:\\remote\\repo',
            KINGU_WORKTREE_PATH: 'C:\\remote\\improve-dashboard'
          })
        }
      })
    )
