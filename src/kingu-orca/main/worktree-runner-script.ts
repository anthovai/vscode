...
function createWorktreeRunnerScript(args: {
  ...
  // `#!` line opts a script into bash, so the same kingu.yaml runs identically for every Windows
  ...
  const gitRelPath = `kingu/${runnerBaseName}.${runnerExtension}`
  ...
  if (wslWorktree) {
    ...
    // and PWD the same way, so leaving KINGU_* in C:\ form would make them the lone exception.
...
export function resolveSetupRunnerShell(
  ...
  // Why: existing Windows setup scripts were authored for Kingu's cmd runner;
