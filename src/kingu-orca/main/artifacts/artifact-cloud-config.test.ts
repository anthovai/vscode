...
describe('resolveArtifactCloudApiUrl', () => {
  it('uses the first-party production origin by default', () => {
    expect(resolveArtifactCloudApiUrl(undefined, {}, true)).toBe('https://share.onkingu.dev')
  })
  ...
  it('allows loopback HTTP only in development', () => {
    expect(
      resolveArtifactCloudApiUrl(
        undefined,
        { KINGU_ARTIFACTS_API_URL: 'http://127.0.0.1:45961' },
        false
      )
  ...
  it('rejects origins that could receive an Kingu access token', () => {
    expect(() => resolveArtifactCloudApiUrl('https://example.com', {}, false)).toThrow(
      /onkingu\.dev/
    )
    expect(() => resolveArtifactCloudApiUrl('https://share.onkingu.dev/path', {}, false)).toThrow(
      /origin/
    )
  })
