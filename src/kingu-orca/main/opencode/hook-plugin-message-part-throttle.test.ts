 ...
 * coalesced and capped before POSTing to Kingu's agent-hook server. The
 * un-throttled plugin re-posted the full accumulated reply per streamed
 * append — O(n²) bytes per turn — which saturated Kingu's main + renderer
...
const ENV_KEYS = ['KINGU_PANE_KEY', 'KINGU_AGENT_HOOK_PORT', 'KINGU_AGENT_HOOK_TOKEN'] as const
...
describe('OpenCode plugin MessagePart throttling', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'kingu-opencode-plugin-test-'))
    posts = []
    savedEnv = {}
  ...
  async function loadPluginEventHandler(): Promise<PluginEventHandler> {
    ...
    const module = (await import(pathToFileURL(pluginPath).href)) as {
      KinguOpenCodeStatusPlugin: (ctx: unknown) => Promise<{ event: PluginEventHandler }>
    }
    const client = {
      session: {
    ...
    const hooks = await module.KinguOpenCodeStatusPlugin({ client })
    return hooks.event
  }
