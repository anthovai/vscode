...
describe('createPtySubprocess', () => {
    ...
    try {
      await expect(
        createPtySubprocess({
          ...
          cols: 80,
          rows: 24,
          cwd: 'C:\\definitely-missing-kingu-wsl-cwd',
          shellOverride: 'wsl.exe'
        })
      ).rejects.toThrow(/Working directory "C:\\definitely-missing-kingu-wsl-cwd" does not exist/)
    } finally {
      if (platform) {
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.not.objectContaining({
          ...
          KINGU_CODEX_HOME: expect.anything()
        })
      })
    )
  })
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.objectContaining({
          CODEX_HOME: '/home/jin/.local/share/kingu/codex-accounts/a/home',
          KINGU_CODEX_HOME: '/home/jin/.local/share/kingu/codex-accounts/a/home',
  ...
  it('marks Kingu terminal handles for WSL env import in daemon WSL terminals', async () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const platform = Object.getOwnPropertyDescriptor(process, 'platform')
    ...
    try {
      await createPtySubprocess({
        ...
        env: {
          ...
          KINGU_HERMES_STARTUP_QUERY: 'line one\nline two',
          WSLENV: 'FOO/u'
        }
      })
    } finally {
