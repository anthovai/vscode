...
const COMMAND_OUTPUT = 'KINGU_STARTUP_COMMAND_RAN'
...
const READ_STARTED_FILE = '.kingu-read-started'
...
const FIXTURES: ShellFixture[] = [
  {
    ...
    command: `printf 'KINGU_STARTUP_%s:PF=%s\\n' COMMAND_RAN "\${(j:,:)precmd_functions}"\r`,
  ...
  },
  {
    ...
    command: `printf 'KINGU_STARTUP_%s:PC=%s\\n' COMMAND_RAN "$PROMPT_COMMAND"\r`,
...
async function startFixture(
  ...
  const tempHome = mkdtempSync(join(tmpdir(), 'kingu-shell-ready-exec-'))
  ...
  try {
    ...
    subprocess = await createPtySubprocess({
      ...
      envToDelete: ['KINGU_EXEC_REPRO_DONE', 'KINGU_ORIG_ZDOTDIR', 'ZDOTDIR']
    })
    session = new Session({
      sessionId: `repro-13767-${fixture.startupFile}`,
      cols: 80,
...
function restoreEnvironment(
  ...
  setEnvironmentValue('KINGU_ORIG_ZDOTDIR', originalZdotdir)
}
...
async function runExecOracle(fixture: ShellFixture): Promise<void> {
  const running = await startFixture(
    fixture,
    `if [[ -z "\${KINGU_EXEC_REPRO_DONE:-}" ]]; then
  export KINGU_EXEC_REPRO_DONE=1
  ${fixture.replacement}
fi
`
  )
  try {
    ...
    expect(running.output()).not.toContain('kingu-shell-start')
  } finally {
...
`,
  ...
  const alternateBashProfile = `if [[ -z "\${KINGU_EXEC_REPRO_DONE:-}" ]]; then
  export KINGU_EXEC_REPRO_DONE=1
