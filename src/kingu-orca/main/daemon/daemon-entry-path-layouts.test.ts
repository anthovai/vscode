 ...
 *
 * The layout that matters here is kingud's: a packaged host with NO asar, whose bundle root
 * holds `kingud.js` and `daemon-entry.js` side by side (config/scripts/build-kingud.mjs emits
...
const KINGUD_ROOT = join('/opt', 'kingud')
const KINGUD_ADJACENT_ENTRY = join(KINGUD_ROOT, 'daemon-entry.js')
...
describe('daemon entry path per deployment layout', () => {
  ...
  it('forks the entry beside kingud.js on a packaged host with no asar', async () => {
    // kingud answers isPackaged() true; the question the resolver must ask is whether the app
    // root is an asar archive, not whether the build is packaged.
    expect(
      await forkedDaemonEntryPath({
        appPath: KINGUD_ROOT,
        isPackaged: true,
        existingEntry: KINGUD_ADJACENT_ENTRY
      })
    ).toBe(KINGUD_ADJACENT_ENTRY)
  })
})
