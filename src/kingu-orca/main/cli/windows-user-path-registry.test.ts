...
describe('WindowsUserPathRegistryReader', () => {
  it('reads Unicode REG_EXPAND_SZ values without expanding the stored text', async () => {
    const registry = registryModule('%LOCALAPPDATA%\\Kingu;C:\\工具\\bin;C:\\Développement')
    ...
    await expect(reader.read()).resolves.toEqual({
      state: 'success',
      value: '%LOCALAPPDATA%\\Kingu;C:\\工具\\bin;C:\\Développement',
      expandable: true
    })
  ...
  it('coalesces concurrent reads, caches status reads, and bypasses the cache when fresh', async () => {
    ...
    resolveRegistry(registryModule('C:\\Tools;C:\\Kingu'))
    await expect(refreshed).resolves.toEqual({
      state: 'success',
      value: 'C:\\Tools;C:\\Kingu',
      expandable: true
    })
  })
  ...
  it.each([
    ...
    const reader = new WindowsUserPathRegistryReader({
      platform: 'win32',
      registryLoader: async () => registryModule('%LOCALAPPDATA%\\Kingu', type)
    })

    await expect(reader.read()).resolves.toEqual({
      state: 'success',
      value: '%LOCALAPPDATA%\\Kingu',
      expandable
    })
  })
