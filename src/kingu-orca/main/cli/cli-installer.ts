...
export class CliInstaller extends CliPathRegistration {
  ...
  async getStatus(): Promise<CliInstallStatus> {
    ...
    if (!launcherPath) {
      const detail = this.hasUnverifiedAppImageRuntime
        ? 'Kingu could not verify the inherited AppImage runtime identity, so CLI registration is unavailable.'
    ...
    const baseStatus =
      ...
      !isAppImageInstalledLauncherCurrent(extractionOptions)
        ? {
            ...
            detail: `${spec.commandPath} does not point to the current Kingu AppImage payload.`
          }
        : inspectedStatus
  ...
  private async installUnlocked(): Promise<CliInstallStatus> {
    ...
    if (initialStatus.state === 'conflict') {
      throw new Error(
        `Refusing to replace non-Kingu command at ${initialStatus.commandPath}. Remove it and register again if it is no longer needed.`
      )
    }
    ...
    if (status.state === 'conflict') {
      throw new Error(
        `Refusing to replace non-Kingu command at ${status.commandPath}. Remove it and register again if it is no longer needed.`
      )
    }
    ...
    if (status.installMethod === 'symlink') {
      ...
      // Why: packaged Windows already ships resources/bin/kingu.exe; registration only owns the PATH entry.
    } else {
  ...
  private async removeUnlocked(): Promise<CliInstallStatus> {
    ...
    if (status.state === 'conflict') {
      throw new Error(`Refusing to remove non-Kingu command at ${status.commandPath}.`)
    }
    if (status.state === 'stale' && status.installMethod !== 'symlink') {
      throw new Error(`Refusing to remove a command not owned by Kingu at ${status.commandPath}.`)
    }
