...
async function makeFixture(): Promise<{ userDataPath: string; resourcesPath: string }> {
  const root = await mkdtemp(join(tmpdir(), 'kingu-terminal-cli-shim-'))
  created.push(root)
  const resourcesPath = join(root, 'resources')
  // The bundled kingu-ide launcher must exist for the shim to be written.
  mkdirSync(join(resourcesPath, 'bin'), { recursive: true })
  writeFileSync(join(resourcesPath, 'bin', 'kingu-ide'), '#!/usr/bin/env bash\n', 'utf8')
...
describe('ensureLinuxTerminalKinguCliShimDir', () => {
  it('uses the mounted bundled launcher when only APPDIR is inherited', async () => {
    ...
    const shimDir = ensureLinuxTerminalKinguCliShimDir({ userDataPath, resourcesPath })

    expect(shimDir).toBe(join(userDataPath, 'linux-kingu-cli-shim'))
    expect(readFileSync(join(shimDir!, 'kingu'), 'utf8')).toContain(
      `exec '${join(resourcesPath, 'bin', 'kingu-ide')}' "$@"`
    )
  })

  it('writes an executable bare-kingu shim that execs the bundled kingu-ide launcher', async () => {
    const { userDataPath, resourcesPath } = await makeFixture()

    const shimDir = ensureLinuxTerminalKinguCliShimDir({
      userDataPath,
      resourcesPath,
      appImagePath: null
    })
    ...
    expect(shimDir).toBe(join(userDataPath, 'linux-kingu-cli-shim'))
    const content = readFileSync(join(shimDir!, 'kingu'), 'utf8')
    // Single-quoted so a resources path with shell metacharacters can't break out.
    expect(content).toContain(`exec '${join(resourcesPath, 'bin', 'kingu-ide')}' "$@"`)
    const mode = statSync(join(shimDir!, 'kingu')).mode & 0o777
  ...
  it('reuses the shim path and re-asserts its exec bit', async () => {
    ...
    const first = ensureLinuxTerminalKinguCliShimDir(options)
    expect(first).not.toBeNull()
    const shimPath = join(first!, 'kingu')
    chmodSync(shimPath, 0o644)

    const second = ensureLinuxTerminalKinguCliShimDir(options)
    expect(second).toBe(first)
    expect(statSync(shimPath).mode & 0o111).not.toBe(0)

    const root = await mkdtemp(join(tmpdir(), 'kingu-terminal-cli-shim-2-'))
    created.push(root)
    const otherUserData = join(root, 'user-data')
    mkdirSync(join(otherUserData, 'linux-kingu-cli-shim'), { recursive: true })
    writeFileSync(join(otherUserData, 'linux-kingu-cli-shim', 'kingu'), 'stale contents', 'utf8')
    chmodSync(join(otherUserData, 'linux-kingu-cli-shim', 'kingu'), 0o644)

    const healed = ensureLinuxTerminalKinguCliShimDir({
      userDataPath: otherUserData,
      resourcesPath,
      appImagePath: null
    })
    expect(healed).not.toBeNull()
    const healedPath = join(healed!, 'kingu')
    expect(readFileSync(healedPath, 'utf8')).toContain('kingu-ide')
  ...
  it.skipIf(!canFenceAppImageRuntime)(
    ...
    async () => {
      const { userDataPath, resourcesPath } = await makeFixture()
      const appImagePath = join(userDataPath, 'Kingu.AppImage')
      await mkdir(userDataPath, { recursive: true })
      await writeFile(appImagePath, '#!/usr/bin/env bash\n', { encoding: 'utf8', mode: 0o755 })
      const cacheRootPath = join(userDataPath, 'cache')
      const liveLauncherPath = join(resourcesPath, 'bin', 'kingu-ide')
      ...
      const shimPath = join(shimDir!, 'kingu')
  ...
  it.skipIf(!canFenceAppImageRuntime)(
    ...
    async () => {
      ...
      await writeFile(appImagePath, '#!/usr/bin/env bash\n', { encoding: 'utf8', mode: 0o755 })
      const cacheRootPath = join(userDataPath, 'cache')
      const firstLauncher = join(resourcesPath, 'bin', 'kingu-ide')
      ...
      const shimDir = ensureLinuxTerminalKinguCliShimDir(options)
      const shimPath = join(shimDir!, 'kingu')
      const originalShim = readFileSync(shimPath, 'utf8')

      const nextResourcesPath = join(userDataPath, 'next-mount', 'resources')
      const nextLauncher = join(nextResourcesPath, 'bin', 'kingu-ide')
      ...
      expect(
        ensureLinuxTerminalKinguCliShimDir({ ...options, resourcesPath: nextResourcesPath })
      ).toBe(shimDir)
  ...
  it.skipIf(!canFenceAppImageRuntime)(
    ...
    async () => {
      ...
      await mkdir(userDataPath, { recursive: true })
      await writeFile(appImagePath, '#!/usr/bin/env bash\n', { mode: 0o755 })
      const liveLauncher = join(resourcesPath, 'bin', 'kingu-ide')
  ...
  it('returns null (and does not memoize) when the bundled launcher is missing', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kingu-terminal-cli-shim-missing-'))
    created.push(root)
    const userDataPath = join(root, 'user-data')

    const missing = ensureLinuxTerminalKinguCliShimDir({
      userDataPath,
    ...
    mkdirSync(join(resourcesPath, 'bin'), { recursive: true })
    writeFileSync(join(resourcesPath, 'bin', 'kingu-ide'), '#!/usr/bin/env bash\n', 'utf8')
    const recovered = ensureLinuxTerminalKinguCliShimDir({
      userDataPath,
      resourcesPath,
      appImagePath: null
    })
    expect(recovered).toBe(join(userDataPath, 'linux-kingu-cli-shim'))
  })
})
