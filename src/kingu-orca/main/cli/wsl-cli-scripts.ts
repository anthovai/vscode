const MANAGED_MARKER = '# Kingu managed WSL CLI launcher'
const BRIDGE_MANAGED_MARKER = '# Kingu managed WSL CLI PowerShell bridge'

export function buildWslLauncher(
  windowsLauncherPath: string,
  bridgePath = '${XDG_DATA_HOME:-$HOME/.local/share}/kingu/kingu-wsl-bridge.ps1'
): string {
  const encodedTarget = Buffer.from(windowsLauncherPath, 'utf8').toString('base64')
  return `#!/usr/bin/env bash
set -euo pipefail
${MANAGED_MARKER}
# KINGU_WIN_LAUNCHER_B64=${encodedTarget}
KINGU_WIN_LAUNCHER=${quoteShell(windowsLauncherPath)}
KINGU_BRIDGE_PS1=${quoteShell(bridgePath)}
...
KINGU_WSL_CWD=$(pwd -P 2>/dev/null) || {
  KINGU_WSL_CWD=/
  cd /
}
KINGU_BRIDGE_PS1_WIN=$(wslpath -w "$KINGU_BRIDGE_PS1")
KINGU_WSL_CWD_WIN=$(wslpath -w "$KINGU_WSL_CWD")
exec "$KINGU_POWERSHELL" -NoProfile -ExecutionPolicy Bypass -File "$KINGU_BRIDGE_PS1_WIN" "$KINGU_WIN_LAUNCHER" -WslCwd "$KINGU_WSL_CWD_WIN" "$@"
`
}
...
try {
  ...
  if ($args.Count -lt 1) {
    throw 'Invalid Kingu WSL CLI bridge invocation.'
  }
  [string]$KinguLauncher = $args[0]
  [string]$WslCwd = ''
  ...
  if ([string]::IsNullOrEmpty($WslCwd)) {
    Remove-Item Env:KINGU_CLI_CWD -ErrorAction SilentlyContinue
  } else {
    $env:KINGU_CLI_CWD = $WslCwd
  }
  $LauncherDirectory = Split-Path -Parent $KinguLauncher
  ...
  $StartInfo.FileName = $KinguLauncher
  ...
  if ($null -eq $Process) {
    throw 'Unable to start the Kingu Windows CLI launcher.'
  }
  $Process.WaitForExit()
...
export function getBridgePathFromCommandPath(commandPath: string): string {
  ...
  // share one WSL bridge under ~/.local/share/kingu.
  return `${commandPath.replace(/\/\.local\/bin\/(?:kingu|kingu-ide)$/, '/.local/share/kingu')}/kingu-wsl-bridge.ps1`
}
...
export function buildRegistrationLockPrelude(commandPath: string): string {
  ...
  // Why: the per-distro queue only serializes one Kingu process; flock covers
  ...
  return [
    ...
    `  exec 9>${quoteShell(`${lockDir}/.kingu-wsl-cli.lock`)}`,
    '  flock -x -w 30 9',
    'fi'
  ].join('\n')
}
...
export function buildManagedLegacyRemoveCommand(quotedLegacyCommandPath: string): string {
  // Why: remove only the Kingu-managed pre-rename wrapper; user-owned `kingu`
...
export function buildSafeRemoveCommand(commandPath: string, legacyCommandPath?: string): string {
  ...
  return [
    ...
    // Why: leaving a managed legacy `kingu` behind lets startup reconciliation
...
export function parseManagedLauncherTarget(content: string): string | null {
  const encoded = content.match(/^# KINGU_WIN_LAUNCHER_B64=([A-Za-z0-9+/=]+)$/m)?.[1]
  if (encoded) {
    try {
  ...
  const legacyTarget = content.match(/^KINGU_WIN_LAUNCHER='((?:[^']|'"'"')*)'$/m)?.[1]
