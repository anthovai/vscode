...
describe('CliInstaller', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    'falls back to ~/.local/bin/kingu on macOS when /usr/local/bin does not exist',
    async () => {
      ...
      const absentUsrLocalBin = join(fixture.root, 'usr', 'local', 'bin', 'kingu')
      const installer = new CliInstaller({
        platform: 'darwin',
        isPackaged: true,
        resourcesPath,
        userDataPath: fixture.userDataPath,
        execPath: '/Applications/Kingu.app/Contents/MacOS/Kingu',
      ...
      expect(status.commandPath).toBe(join(homePath, '.local', 'bin', 'kingu'))
      expect(status.state).toBe('not_installed')
      expect(status.supported).toBe(true)

      const installed = await installer.install()
      expect(installed.state).toBe('installed')
      expect(installed.commandPath).toBe(join(homePath, '.local', 'bin', 'kingu'))
  ...
  it.skipIf(process.platform === 'win32')(
    'uses /usr/local/bin/kingu on macOS when /usr/local/bin exists',
    async () => {
      const fixture = await makeFixture()
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const installPath = join(usrLocalBin, 'kingu')
      const installer = new CliInstaller({
        platform: 'darwin',
        isPackaged: true,
        resourcesPath,
  ...
  // Why: users can have a managed Kingu command in ~/.local/bin even when
  // /usr/local/bin exists; Settings must follow the shell-visible command.
  it.skipIf(process.platform === 'win32')(
    'uses an existing managed macOS kingu command from the shell PATH before /usr/local/bin',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const defaultInstallPath = join(usrLocalBin, 'kingu')
      const userInstallPath = join(userLocalBin, 'kingu')
  ...
  it.skipIf(process.platform === 'win32')(
    'skips a broken managed macOS kingu symlink before /usr/local/bin',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const userInstallPath = join(userLocalBin, 'kingu')
      const launcherPath = join(resourcesPath, 'bin', 'kingu')
      const oldLauncherPath = join(fixture.root, 'Old.app', 'Contents', 'Resources', 'bin', 'kingu')
      await mkdir(usrLocalBin, { recursive: true })
      await mkdir(userLocalBin, { recursive: true })
  ...
  // ~/.local/bin/kingu must not steal status from /usr/local/bin/kingu.
  it.skipIf(process.platform === 'win32')(
    'keeps the default macOS command when a managed kingu appears later on PATH',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  // is still the shell-visible Kingu command until the default path is installed.
  it.skipIf(process.platform === 'win32')(
    'uses a later managed macOS kingu command when the default command is missing',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  // Why: bash/zsh skip non-executable PATH entries even at Kingu's configured
  // install slot, then keep looking for a runnable command later in PATH.
  it.skipIf(process.platform === 'win32')(
    'uses a later managed macOS kingu command when the default command is not executable',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const installed = await installer.install()
      expect(installed.commandPath).toBe(userInstallPath)
      await expect(readFile(defaultInstallPath, 'utf8')).resolves.toContain('other-kingu')
    }
  )

  // Why: a non-Kingu command after an empty default install slot can be shadowed
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      expect(installed.state).toBe('installed')
      await expect(readlink(defaultInstallPath)).resolves.toBe(launcherPath)
      await expect(readFile(userInstallPath, 'utf8')).resolves.toContain('other-kingu')
    }
  )

  // Why: an off-PATH ~/.local/bin/kingu must not hijack CLI registration and
  // leave the shell-visible /usr/local/bin command missing.
  it.skipIf(process.platform === 'win32')(
    'ignores managed macOS kingu commands that are not visible on the shell PATH',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  it.skipIf(process.platform === 'win32')(
    'reports a conflict for an unmanaged macOS kingu that shadows the install path',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  it.skipIf(process.platform === 'win32')(
    'skips a non-executable unmanaged macOS kingu before the install path',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  // Why: when macCommandPath falls back to ~/.local/bin/kingu on arm64, commandName
  // must still be 'kingu' (not 'kingu-ide' which is Linux-only).
  it.skipIf(process.platform === 'win32')(
    'reports commandName as kingu (not kingu-ide) when falling back to ~/.local/bin on macOS',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const status = await installer.getStatus()
      expect(status.commandName).toBe('kingu')
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      expect(s1.commandPath).toBe(join(homePath, '.local', 'bin', 'kingu'))
    }
  )

  // Why: the arm64 fallback must apply for packaged builds, not just dev launchers.
  it.skipIf(process.platform === 'win32')(
    'resolves to ~/.local/bin/kingu on arm64 even when isPackaged is true',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const absentUsrLocalBin = join(fixture.root, 'usr', 'local', 'bin', 'kingu')
      const resourcesPath = join(fixture.root, 'resources')
      const bundledLauncher = join(resourcesPath, 'bin', 'kingu')
