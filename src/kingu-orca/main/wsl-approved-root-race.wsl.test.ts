...
const DISTRO = process.env.KINGU_WSL_TEST_DISTRO ?? 'Ubuntu-24.04'
const runRealWsl = process.platform === 'win32' && process.env.KINGU_REAL_WSL_DELETE_TEST === '1'
...
async function wsl(command: string, ...args: string[]): Promise<string> {
  const result = await execFileAsync(
    'wsl.exe',
    ['-d', DISTRO, '--exec', 'sh', '-c', command, 'kingu-wsl-test', ...args],
...
describe.skipIf(!runRealWsl)('WSL approved-root traversal race', () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await wsl("mktemp -d -p /tmp 'kingu-wsl-root-race.XXXXXX'")
    const statHook = String.raw`#!/bin/sh
for argument do last=$argument; done
if [ "$PWD" = "$KINGU_RACE_PARENT" ] && [ "$(/usr/bin/basename "$last")" = vault ]; then
  inspected=$(/usr/bin/stat "$@") || exit $?
  /usr/bin/mv -- "$KINGU_RACE_PARENT/vault" "$KINGU_RACE_PARENT/vault-original" || exit $?
  /usr/bin/ln -s -- "$KINGU_RACE_OUTSIDE" "$KINGU_RACE_PARENT/vault" || exit $?
  printf '%s\n' "$inspected"
  exit 0
fi
exec /usr/bin/stat "$@"
`
  ...
  afterAll(async () => {
    if (/^\/tmp\/kingu-wsl-root-race\.[A-Za-z0-9]+$/u.test(fixtureRoot)) {
    ...
    await expect(
      execFileAsync(
        ...
        [
          ...
          `KINGU_RACE_PARENT=${fixtureRoot}/race`,
          `KINGU_RACE_OUTSIDE=${fixtureRoot}/outside/root-target`,
          ...(command ?? [])
        ],
        { encoding: 'utf-8', timeout: 30000 }
      )
    ).rejects.toMatchObject({ stderr: expect.stringContaining('KINGU_WSL_DELETE_REJECT:race') })
