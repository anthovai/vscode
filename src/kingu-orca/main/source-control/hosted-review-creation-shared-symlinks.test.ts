...
// cannot commit — it is a symlink Kingu created.
describe('createHostedReview with shared symlinks', () => {
  ...
  beforeEach(() => {
    worktree = mkdtempSync(join(tmpdir(), 'kingu-hosted-shared-'))
  ...
  // Why: only an *untracked* record can be Kingu's artifact. A tracked change at a
