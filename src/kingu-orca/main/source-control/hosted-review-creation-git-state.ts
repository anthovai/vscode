...
/** True when any record is real user work rather than a shared symlink Kingu put
 *  in the worktree.
 *
 *  Fails closed on purpose: anything not positively identified as an Kingu-owned
...
async function anyRecordIsUserDirt(
  ...
  // fail-closed check would block review creation over Kingu's own symlink.
