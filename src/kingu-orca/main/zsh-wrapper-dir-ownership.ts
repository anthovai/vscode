/**
 * Decides which inherited ZDOTDIR Kingu may treat as the user's real zsh config
 * dir, on the Node side of the launch.
 *
 * Why: Kingu used to recognise only its OWN wrapper path shape (a path ending in
 * shell-ready/zsh). Launched from any other terminal that had already
 * hijacked ZDOTDIR, Kingu captured that dir as "the user's config" and re-sourced
 * it. Ownership is now established positively — a stamped marker file, or Kingu's
...
export function isKinguOwnedZshWrapperDir(dir: string): boolean {
...
function usableInheritedZdotdir(value: string | undefined): string | null {
  ...
  if (!normalized || isKinguOwnedZshWrapperDir(normalized)) {
    return null
  }
 ...
 * one Kingu invented. `ZDOTDIR=$HOME` and an unset ZDOTDIR look identical to zsh
...
export function resolveInheritedZdotdir(env: EnvLike): string | null {
  return (
    usableInheritedZdotdir(env.ZDOTDIR) ?? usableInheritedZdotdir(env.KINGU_ORIG_ZDOTDIR) ?? null
  )
}
...
export function inheritedZdotdirEnv(inherited: string | null): Record<string, string> {
  return inherited ? { KINGU_ORIG_ZDOTDIR: inherited } : {}
}
