...
describe('resolveConsent', () => {
  ...
  it('returns kingu_disabled when KINGU_TELEMETRY_DISABLED=1', () => {
    process.env.KINGU_TELEMETRY_DISABLED = '1'
    expect(
      resolveConsent(
        settingsWithTelemetry({
          optedIn: true,
      ...
      effective: 'disabled',
      reason: 'kingu_disabled'
    })
  })

  it('prefers do_not_track over kingu_disabled when both are set', () => {
    process.env.DO_NOT_TRACK = '1'
    process.env.KINGU_TELEMETRY_DISABLED = '1'
    expect(
      resolveConsent(
  ...
  it('env-var override wins over a stored pending_banner state', () => {
    ...
    ).toEqual({ effective: 'disabled', reason: 'kingu_disabled' })
  })
})
