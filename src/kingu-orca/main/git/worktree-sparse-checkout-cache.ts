...
// already skips rows where that's set), but Kingu's compatibility baseline is Git 2.25, which predates it —
...
//  - Kingu-driven remove/move: explicit calls below (worktree-removal.ts, worktree-move.ts).
...
/** Drop one worktree's cached state; call when Kingu itself removes or moves a worktree path. */
