/**
 * The single `.zshenv` Kingu writes for every transport: local PTY, daemon/SSH,
 * and relay.
 *
 * Kingu needs to run code AFTER the user's own zsh startup files. The old shape
 * bought that by keeping ZDOTDIR pointed at Kingu's wrapper dir for the whole of
 ...
 * unconditionally, so history landed inside Kingu's own dir (#11044); zsh's
 ...
 * Kingu's work to a `precmd` hook that runs at the first prompt — after
...
export type ZshWrapperRestoreSpec = {
  /** Kingu's agent-teams shim dir back onto PATH. */
  agentTeamsPath: boolean
  /** Remote CLI bin dir onto PATH — relay hosts only. */
  remoteCliBinDir: boolean
  /** Kingu's runtime CODEX_HOME. */
  codexHome: boolean
  /** The `codex()` wrapper that runs Kingu's launch preflight. */
  codexLaunchPreflight: boolean
}

export type ZshStartupHookSpec = {
  /** First line of the generated file, e.g. `# Kingu zsh shell-ready wrapper`. */
  headerLabel: string
...
const AGENT_TEAMS_PATH_RESTORE_BLOCK = `__kingu_restore_agent_teams_path() {
...
const OPENCODE_CONFIG_DIR_RESTORE = `[[ -n "\${KINGU_OPENCODE_CONFIG_DIR:-}" ]] && export OPENCODE_CONFIG_DIR="\${KINGU_OPENCODE_CONFIG_DIR}"`
const MIMOCODE_HOME_RESTORE = `[[ -n "\${KINGU_MIMOCODE_HOME:-}" ]] && export MIMOCODE_HOME="\${KINGU_MIMOCODE_HOME}"`
const REMOTE_CLI_BIN_DIR_RESTORE = `[[ -n "\${KINGU_REMOTE_CLI_BIN_DIR:-}" ]] && case ":$PATH:" in *:"\${KINGU_REMOTE_CLI_BIN_DIR}":*) ;; *) export PATH="\${KINGU_REMOTE_CLI_BIN_DIR}:$PATH" ;; esac`
const CODEX_HOME_RESTORE = `# Why: Codex must keep using Kingu's runtime CODEX_HOME after rc files.
 ...
 */
const ZSH_OSC133_FUNCTION_BLOCK = `__kingu_osc133_precmd() {
  local exit_code=$?
  if [[ -n "\${__kingu_in_command:-}" ]]; then
    builtin printf "\\033]133;D;%s\\007" "$exit_code"
    builtin unset __kingu_in_command
  fi
...
__kingu_osc133_preexec() {
  ...
  builtin typeset -g __kingu_in_command=1
}`
...
/**
 * Everything Kingu owns that must run after the user's config, in one function
...
function buildDeferredInit(spec: ZshStartupHookSpec): string {
  ...
  // the unwrapped pane it was — no stray Kingu name in `precmd_functions`.
  ...
  const permanentPrecmd = spec.osc133CommandMarkers
    ? `  if __kingu_has_feature markers; then
    precmd_functions=(\${precmd_functions:/__kingu_deferred_init/__kingu_osc133_precmd})
    preexec_functions=(__kingu_osc133_preexec \${preexec_functions[@]})
  else
    precmd_functions=(\${precmd_functions:#__kingu_deferred_init})
  fi`
    : `  precmd_functions=(\${precmd_functions:#__kingu_deferred_init})`
  const lineInitRegistration = spec.startupCommandDelivery
    ? `  if __kingu_has_feature ready || __kingu_has_feature startup; then
    __kingu_emit_ready_marker=""
    __kingu_has_feature ready && __kingu_emit_ready_marker=1
${indentBlock(getZshShellReadyMarkerRegistrationBlock(spec.readyMarkerEscaped, true), '    ')}
  ...
  (( $+_kingu_deferred_init_done )) && return 0
  builtin typeset -g _kingu_deferred_init_done=1
...
${joinBlocks([
  ...
  `  if [[ -n "\${_kingu_histfile:-}" ]]; then
    HISTFILE="$_kingu_histfile"
  fi`,
...
${
  ...
  __kingu_has_feature markers && __kingu_osc133_precmd\n`
    : ''
}  builtin unset _kingu_shell_features _kingu_histfile
  builtin unfunction __kingu_deferred_init __kingu_has_feature
}`
}
