...
const NO_WINDOW = 'No Kingu window was available to launch the automation.'
...
async function seedUnresolvableAutomation(): Promise<{ store: Store; automation: Automation }> {
  ...
  const file = join(testState.dir, 'kingu-data.json')
...
describe('repeated skipped_unavailable coalescing', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'kingu-skip-coalescing-test-'))
    vi.useFakeTimers()
  })
