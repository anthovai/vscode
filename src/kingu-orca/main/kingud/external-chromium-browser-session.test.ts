...
const BASE = {
  executablePath: '/opt/kingu/chromium',
  profilePath: '/state/browser-chromium',
  sessionName: 'kingu-kingud-0123456789abcdef'
}
...
describe('kingud external-chromium agent-browser environment', () => {
  beforeEach(() => {
  ...
  // Why (#16367): kingud is the backend a remote user's Chromium hangs off, and this session
  // never passes --cdp, so a daemon an earlier kingud left behind is not bound to the dead
  ...
  // that is the killed-kingud case the stable session name exists to recover.
