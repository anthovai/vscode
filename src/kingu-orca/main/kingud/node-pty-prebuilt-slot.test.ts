...
const dirs: string[] = []
const temp = (): string => {
  const dir = mkdtempSync(join(tmpdir(), 'kingud-slot-'))
  dirs.push(dir)
  return dir
}
afterEach(() => {
  ...
  delete process.env.KINGU_KINGUD_PREBUILDS_DIR
})
...
describe('resolveKingudPrebuildsDir', () => {
  it('looks beside the running bundle', () => {
    expect(resolveKingudPrebuildsDir('/opt/kingud/kingud.js')).toBe(
      join('/opt/kingud', 'prebuilds')
    )
  })

  it('honours an explicit override', () => {
    process.env.KINGU_KINGUD_PREBUILDS_DIR = '/custom/prebuilds'
    expect(resolveKingudPrebuildsDir('/opt/kingud/kingud.js')).toBe('/custom/prebuilds')
  })
})
...
describe('installPrebuiltSlot', () => {
  ...
  it('installs a Linux slot without claiming a spawn-helper it never execs', () => {
    ...
    // macOS is what made every Linux kingud boot degraded on spawn_helper_missing (#17844).
    const prebuilds = temp()
    const nodePtyDir = temp()
