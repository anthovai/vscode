 ...
 * lazy initializer because `kingud-entry` reaches it through `await import('../ipc/pty')`,
 ...
 *
 * Why this builds the bundle instead of skipping without one: no CI job builds kingud and
 * runs vitest. `smoke:kingud-terminal` builds it in the static-analysis job, which never
 * runs vitest; the `kingud_browser` job runs vitest but deliberately does not build kingud.
 * A `runIf(existsSync(...))` guard therefore skips in every shard, forever — the same way
 * an unset KINGU_BROWSER_EXECUTABLE kept the browser provider uncovered.
 *
...
const BUNDLE = join(REPO_ROOT, 'out', 'kingud', 'kingud.js')
const BUILD_SCRIPT = join(REPO_ROOT, 'config', 'scripts', 'build-kingud.mjs')
 ...
 */
function ensureKingudBundle(): void {
...
describe('kingud bundle native load order', () => {
  const dirs: string[] = []
  afterEach(() => {
  ...
  it('does not require node-pty in-process before the entry rejects its argv', () => {
    ensureKingudBundle()
    const dir = mkdtempSync(join(tmpdir(), 'kingud-load-order-'))
    dirs.push(dir)
    ...
    writeFileSync(
      ...
      [
        ...
        '}',
        "process.argv.push('--kingud-load-order-check')",
    ...
    expect(output).toContain('Unknown argument: --kingud-load-order-check')
