...
} from '../runtime/kingu-runtime-browser'
...
import { sendKingudSidecarRequest } from './kingud-sidecar-runtime-client'
import {
...
export class ElectronServeBrowserProcess {
  ...
  async start(): Promise<void> {
    ...
    const userDataPath = await mkdtemp(join(temporaryRoot, 'kingud-browser-'))
      ...
      if (metadata) {
        try {
          const status = RuntimeStatusResult.parse(
            await sendKingudSidecarRequest(metadata, 'status.get', undefined, 5_000)
          )
  ...
  private async invoke(
    ...
    if (method === 'browserScreencast') {
      throw new BrowserError(
        ...
        'The kingud Electron provider does not proxy screencast.'
      )
    }
    ...
    let result: unknown
    try {
      result = await sendKingudSidecarRequest(metadata, rpcMethod, params)
    } catch (error) {
      if (
        targetPage &&
