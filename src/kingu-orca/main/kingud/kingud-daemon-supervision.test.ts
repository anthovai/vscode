...
vi.mock('../daemon/daemon-init', () => ({
  ...
  // kingud shutdown is what would make an kingud restart destructive again.
...
describe('startKingudDaemon', () => {
  it('reports live with the daemon pid once the provider is installed', async () => {
    await expect(startKingudDaemon()).resolves.toEqual({ state: 'live', pid: 4242 })
  })

  it('does not arm the macOS login-session death watch', async () => {
    await startKingudDaemon()
    // That watch retires the daemon when the spawning GUI login session dies. An kingud
  ...
  it('reports degraded when fresh terminals would fall back to the local provider', async () => {
    ...
    const result = await startKingudDaemon()
  ...
  it('fails open when the daemon cannot start at all', async () => {
    ...
    await expect(startKingudDaemon()).resolves.toEqual({
      state: 'unavailable',
...
describe('stopKingudDaemon', () => {
  it('disconnects and never shuts the daemon down', async () => {
    ...
    // The whole point of item 4: an kingud restart is non-destructive only if the daemon
