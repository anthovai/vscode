/**
 * The address `kingud` binds its RPC listener to.
 *
 * Why loopback by default: the desktop stays on loopback until the user pairs, and the
 * shipping design reaches a remote kingud over an SSH local port-forward — so the wide
 * bind kingud had was both a departure from the desktop's posture and unnecessary for the
 * deploy model. Exposure is now something an operator asks for by name.
 */
import { isIP } from 'node:net'

export const KINGUD_LOOPBACK_BIND_HOST = '127.0.0.1'
const ALL_INTERFACES_V4 = '0.0.0.0'
const ALL_INTERFACES_V6 = '::'

export class KingudBindAddressError extends Error {
  readonly code = 'kingud_invalid_bind_address'
}
 ...
 */
export function resolveKingudBindHost(raw?: string): string {
  if (raw === undefined) {
    return KINGUD_LOOPBACK_BIND_HOST
  }
  const value = raw.trim()
  if (value === '') {
    throw new KingudBindAddressError('--bind expects an address')
  }
  if (value === 'localhost') {
    return KINGUD_LOOPBACK_BIND_HOST
  }
  if (isIP(value) === 0) {
    throw new KingudBindAddressError(
...
export function describeKingudBindExposure(host: string): string {
  return bindHostIsNetworkExposed(host)
    ? `kingud is bound to ${host} and is reachable from the network. Anything that can reach ` +
        'this port can attempt pairing.'
    : `kingud is bound to ${host} (local only). Reach it from another machine with an SSH ` +
