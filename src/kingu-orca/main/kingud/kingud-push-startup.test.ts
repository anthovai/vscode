...
vi.mock('./kingud-app-paths', () => ({
  resolveKingudInstallRoot: () => state.root,
...
vi.mock('./kingud-daemon-supervision', () => ({
  ...
  stopKingudDaemon: async () => {}
}))
vi.mock('./kingud-health', () => ({ collectKingudHealth: async () => ({}) }))
...
vi.mock('../runtime/kingu-runtime', () => ({
  KinguRuntimeService: class {
    getRuntimeId() {
...
it('starts push after RPC identity is available and stops dispatch on shutdown', async () => {
  ...
  const host = await startKingud({ noPairing: true, json: true })
  try {
