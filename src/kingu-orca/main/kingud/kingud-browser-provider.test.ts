...
import {
  ...
  resolveKingudBrowserProvider
} from './kingud-browser-provider'
import { kingudAgentBrowserNativeName } from './kingud-agent-browser-binary'
import {
...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), 'kingud-browser-provider-'))
...
describe('ExternalChromiumBrowserProcess', () => {
  it('navigates, evaluates, and screenshots through the operator browser', async () => {
    ...
    const commands = processHandle.createCommands({
      ...
      // KinguRuntimeService, which owns the client-hosted registries.
...
describe('cross-platform browser provider paths', () => {
  it('resolves installed Electron launchers on macOS, Linux, and Windows', () => {
    expect(installedElectronCandidates('darwin', '/Users/test', {})).toContain(
      '/Users/test/Applications/Kingu.app/Contents/MacOS/Kingu'
    )
    expect(installedElectronCandidates('linux', '/home/test', {})).toContain(
      '/home/test/.local/bin/kingu-ide'
    )
    expect(
      installedElectronCandidates('win32', 'C:\\Users\\test', {
        LOCALAPPDATA: 'C:\\Users\\test\\AppData\\Local'
      })
    ).toContain('C:\\Users\\test\\AppData\\Local\\Programs\\Kingu\\Kingu.exe')
  })

  it('uses platform-specific bundled agent-browser names', () => {
    expect(kingudAgentBrowserNativeName('darwin', 'arm64')).toBe('agent-browser-darwin-arm64')
    expect(kingudAgentBrowserNativeName('linux', 'x64')).toBe('agent-browser-linux-x64')
    expect(kingudAgentBrowserNativeName('win32', 'x64')).toBe('agent-browser-win32-x64.exe')
  })
})

describe('resolveKingudBrowserProvider', () => {
  it('uses KINGU_BROWSER_EXECUTABLE when Electron is absent', async () => {
    ...
    const provider = await resolveKingudBrowserProvider({
      userDataPath: root,
      environment: { KINGU_BROWSER_EXECUTABLE: executable },
  ...
  it('reports an KINGU_BROWSER_EXECUTABLE path that does not exist', async () => {
    const missing = join(root, 'absent-chromium')

    await expect(
      resolveKingudBrowserProvider({
        userDataPath: root,
        environment: { KINGU_BROWSER_EXECUTABLE: missing },
        resolveInstalledElectronExecutable: async () => null,
        resolveAgentBrowserBinary: () => '/agent-browser'
      })
    ).resolves.toBeNull()
  ...
  it.skipIf(process.platform === 'win32')(
    'separates a non-executable KINGU_BROWSER_EXECUTABLE from a missing one',
    async () => {
  ...
  it('reports a failed Electron start instead of staying invisible to the client', async () => {
    ...
    await expect(
      resolveKingudBrowserProvider({
        userDataPath: root,
        environment: {},
        resolveInstalledElectronExecutable: async () => '/opt/Kingu/kingu-ide',
        resolveAgentBrowserBinary: () => '/agent-browser'
      })
    ).resolves.toBeNull()
    expect(runtimeBrowserUnavailableCause()).toEqual({
  ...
  it('prefers the configured Chromium fault over an earlier Electron failure', async () => {
    ...
    await expect(
      resolveKingudBrowserProvider({
        userDataPath: root,
        environment: { KINGU_BROWSER_EXECUTABLE: join(root, 'absent-chromium') },
        resolveInstalledElectronExecutable: async () => '/opt/Kingu/kingu-ide',
        resolveAgentBrowserBinary: () => null
      })
    ).resolves.toBeNull()
