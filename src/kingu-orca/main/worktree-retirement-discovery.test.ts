...
describe('extractBucketLeafCandidates', () => {
  it('takes everything past the encoded parent as the leaf', () => {
    expect(extractBucketLeafCandidates(`-w-kingu-${FIRST}`, ['-w-kingu'])).toEqual([FIRST])
  })

  it('does not treat the parent directory as a leaf when the workspace name is numeric', () => {
    // Real data: `-Users-x-kingu-workspaces-kingu-7474` must not retire `kingu`, which is in the pool.
    expect(
      extractBucketLeafCandidates('-w-workspaces-kingu-7474', ['-w-workspaces-kingu'])
    ).toEqual(['7474'])
  })

  it('offers the first segment too, so an agent run in a subdirectory still retires the leaf', () => {
    expect(extractBucketLeafCandidates(`-w-kingu-${FIRST}-packages-api`, ['-w-kingu'])).toEqual([
      `${FIRST}-packages-api`,
      FIRST
    ])
  })
  ...
  it('rejects a sibling directory that shares the parent prefix', () => {
    expect(extractBucketLeafCandidates(`-w-kingudyne-${FIRST}`, ['-w-kingu'])).toEqual([])
    expect(extractBucketLeafCandidates(`-w-kingu-secret-${FIRST}`, ['-w-kingu-fix'])).toEqual([])
  })

  it('yields nothing for the parent bucket itself', () => {
    expect(extractBucketLeafCandidates('-w-kingu', ['-w-kingu'])).toEqual([])
  })
})
...
describe('discoverRetiredWorktreeNames', () => {
  ...
  async function withFakeHome(
    ...
    const home = await mkdtemp(join(tmpdir(), 'kingu-retirement-home-'))
    try {
  ...
  it('calls a machine with no agent state a complete answer, not a hole to rescan forever', async () => {
    ...
    const retired = await discoverRetiredWorktreeNames({
      workspaceRoots: [join(tmpdir(), 'kingu-retirement-absent-root')],
      home: join(tmpdir(), 'kingu-retirement-absent-home'),
      env: {}
    })
  ...
  it('matches a plain POSIX workspace root', async () => {
    await withFakeHome([`-Users-ada-kingu-workspaces-kingu-${FIRST}`], async (home) => {
      const retired = await discoverRetiredWorktreeNames({
        workspaceRoots: ['/Users/ada/kingu/workspaces/kingu'],
        home,
        env: {}
      })
  ...
  it('matches a dot-directory root, where the separator run encodes to two dashes', async () => {
    await withFakeHome([`-Users-ada--kingu-worktrees-${FIRST}`], async (home) => {
      const retired = await discoverRetiredWorktreeNames({
        workspaceRoots: ['/Users/ada/.kingu/worktrees'],
        home,
        env: {}
      })
      expect(retired.names).toEqual(new Set([FIRST]))
  ...
  it('matches a Windows drive root', async () => {
    // `getDefaultWorkspaceDir` returns `C:\Users\<user>\kingu\workspaces` on Windows, so an encoder
    // that collapsed `:\` rejected every bucket on that platform by default.
    await withFakeHome([`C--Users-ada-kingu-workspaces-${FIRST}`], async (home) => {
      const retired = await discoverRetiredWorktreeNames({
        workspaceRoots: ['C:\\Users\\ada\\kingu\\workspaces'],
        home,
        env: {}
      })
      expect(retired.names).toEqual(new Set([FIRST]))
  ...
  it('matches a WSL UNC root', async () => {
    await withFakeHome([`--wsl--Ubuntu-home-ada-kingu-workspaces-${FIRST}`], async (home) => {
      const retired = await discoverRetiredWorktreeNames({
        workspaceRoots: ['\\\\wsl$\\Ubuntu\\home\\ada\\kingu\\workspaces'],
        home,
        env: {},
  ...
  it('ignores buckets belonging to a sibling root with the same prefix', async () => {
    await withFakeHome(
      [
        `-Users-ada-kingu-workspaces-kingudyne-${FIRST}`,
        `-Users-ada-kingu-workspaces-kingu-${SECOND}`
      ],
      async (home) => {
        const retired = await discoverRetiredWorktreeNames({
          workspaceRoots: ['/Users/ada/kingu/workspaces/kingu'],
  ...
  it('reads buckets from CLAUDE_CONFIG_DIR when it is set', async () => {
    const configDir = await mkdtemp(join(tmpdir(), 'kingu-retirement-config-'))
  ...
  it("matches a WSL workspace's bucket inside the distro, keyed by its Linux cwd", async () => {
    ...
    const distroHome = await mkdtemp(join(tmpdir(), 'kingu-retirement-distro-'))
    await withFakeHome([], async (home) => {
      try {
        await mkdir(
          join(distroHome, '.claude', 'projects', `-home-ada-kingu-workspaces-${FIRST}`),
          {
            recursive: true
          }
        )
        const retired = await discoverRetiredWorktreeNames({
          workspaceRoots: ['\\\\wsl.localhost\\Ubuntu\\home\\ada\\kingu\\workspaces'],
          home,
          env: {},
  ...
  it('still reads the Windows-side home for a WSL root when the distro home cannot be resolved', async () => {
    ...
    await withFakeHome(
      [`--wsl-localhost-Ubuntu-home-ada-kingu-workspaces-${FIRST}`],
      async (home) => {
        const retired = await discoverRetiredWorktreeNames({
          workspaceRoots: ['\\\\wsl.localhost\\Ubuntu\\home\\ada\\kingu\\workspaces'],
          home,
  ...
  it('retires live workspace directories alongside surviving buckets', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kingu-retirement-roots-'))
    await withFakeHome([], async (home) => {
      try {
