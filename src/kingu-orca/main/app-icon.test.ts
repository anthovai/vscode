...
vi.mock('../../resources/app-icons/kingu-watercolor.png?asset', () => ({
  default: 'watercolor-icon'
}))

vi.mock('../../resources/app-icons/kingu-watercolor.png?asset&asarUnpack', () => ({
  default: 'watercolor-icon-unpacked'
}))

vi.mock('../../resources/app-icons/kingu-blue.png?asset', () => ({
  default: 'blue-icon'
}))

vi.mock('../../resources/app-icons/kingu-blue.png?asset&asarUnpack', () => ({
...
describe('app icon selection', () => {
  ...
  it('persists a custom macOS dock icon to the app bundle for inactive Dock pins', async () => {
    ...
    persistMacDockIcon('watercolor', {
      appBundlePath: '/Applications/Kingu.app',
      execFile,
      isDevApp: false,
      platform: 'darwin'
    })
    ...
    expect(execFile).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.objectContaining({
          KINGU_APP_BUNDLE_PATH: '/Applications/Kingu.app',
          KINGU_APP_ICON_PATH: 'watercolor-icon-unpacked'
        })
      }),
      expect.any(Function)
    )
  ...
  it('clears the AppKit icon and Finder metadata when switching macOS back to classic', async () => {
    ...
    expect(execFile).toHaveBeenCalledWith(
      '/usr/bin/xattr',
      ['-d', 'com.apple.FinderInfo', '/Applications/Kingu.app'],
      expect.objectContaining({
        timeout: 10_000
      }),
      expect.any(Function)
    ...
    expect(execFile).toHaveBeenCalledWith(
      '/usr/bin/xattr',
      ['-d', 'com.apple.ResourceFork', '/Applications/Kingu.app'],
      expect.objectContaining({
        timeout: 10_000
      }),
      expect.any(Function)
  ...
  it('serializes rapid macOS dock icon persistence so the last icon request wins', async () => {
    ...
    expect(execFile).not.toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.objectContaining({
          KINGU_APP_ICON_PATH: 'blue-icon-unpacked'
        })
      }),
      expect.any(Function)
    )
