...
import { checkKinguStarred } from '../github/client'
...
export class StarNagService {
  ...
  registerIpcHandlers(): void {
    ...
    ipcMain.handle('star-nag:starKingu', () => this.starKinguFromNag())
  ...
  private async maybeShow(
    ...
    try {
      // Why: checkKinguStarred lets us skip users who already starred outside
      ...
      // button.
      const starred = await checkKinguStarred()
  ...
  private async starKinguFromNag(): Promise<boolean> {
    const session = this.promptSession
    if (!session) {
      return false
    }
    ...
    const attempt = this.runStarKinguAttempt(session)
  ...
  private async runStarKinguAttempt(session: StarNagPromptSession): Promise<boolean> {
