...
function stashWebBuild(): () => void {
  ...
  const tempDir = mkdtempSync(join(dirname(outWebPath), '.kingu-dev-web-stash-'))
    ...
    try {
      const wrapper = spawn(process.execPath, [wrapperPath, '--remote-debugging-port=9446'], {
        ...
        env: {
          ...
          KINGU_DEV_WRAPPER_TEST_VITE_FILE: viteFile
        },
