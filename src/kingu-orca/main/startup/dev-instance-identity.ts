...
const BASE_APP_NAME = 'Kingu'
const BASE_APP_USER_MODEL_ID = 'com.anthovai.kingu'
...
export type DevInstanceIdentity = AppIdentity & {
  ...
  // creating a new one per branch and re-prompting. Distinct from prod's 'Kingu'.
  appName: string
}
 ...
 * CFBundleName, which downstream forks ship differently ("Kingu ALab Edition").
...
export function getDevInstanceIdentity(
  ...
  const repoRoot = cleanEnvValue(env.KINGU_DEV_REPO_ROOT)
  const branch = cleanEnvValue(env.KINGU_DEV_BRANCH)
  const worktreeName =
    cleanEnvValue(env.KINGU_DEV_WORKTREE_NAME) ??
    cleanEnvValue(path.basename(repoRoot ?? process.cwd()))
  const devLabel = cleanEnvValue(env.KINGU_DEV_INSTANCE_LABEL) ?? formatLabel(branch, worktreeName)
  const dockTitle =
    cleanEnvValue(env.KINGU_DEV_DOCK_TITLE) ?? `${BASE_APP_NAME}: ${branch ?? devLabel ?? 'dev'}`

  return {
    name: dockTitle,
    // Why: one stable Keychain key ('Kingu Dev Safe Storage') for all dev
