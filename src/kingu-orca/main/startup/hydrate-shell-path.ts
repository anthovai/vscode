...
// `which` probe even though they work fine from Terminal (see anthovai/kingu-intelligence#829).
//
// Probe the profile-loading shell once instead of hard-coding every tool's install path.

const DELIMITER = '__KINGU_SHELL_PATH__'
...
const PROBE_MARKER_ENV_VAR = 'KINGU_SHELL_PATH_PROBE'
...
/** Run a synchronous launcher without passing Kingu's seeded PATH to its child. */
...
function spawnShellAndReadPath(shell: string): Promise<HydrationResult> {
  return new Promise((resolve) => {
    ...
    const child = spawn(shell, probe.args, {
      ...
      // and we don't want that in Kingu's console.
      env: shellProbeEnv(),
