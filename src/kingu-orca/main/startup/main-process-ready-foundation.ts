...
export async function initializeReadyFoundation(): Promise<void> {
  ...
  const profile = ensureActiveKinguProfile()
  state.activeKinguProfile = profile
  ...
  scheduleSecretProtectionGapReport({
    dataFile: profile.dataFile,
    force: process.env.KINGU_ALWAYS_REPORT_SECRET_PROTECTION === '1',
  ...
  // Why: pre-`ready` startup reads this flag from a marker so it never has to parse kingu-data.json.
  ...
  initializeBrowserSessionsForApp({
    kinguProfileId: profile.profile.id,
