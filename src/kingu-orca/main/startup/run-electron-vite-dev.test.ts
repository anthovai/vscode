...
describe('run-electron-vite-dev', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), 'kingu-dev-wrapper-'))
      ...
      const wrapper = spawn(process.execPath, [wrapperPath], {
        ...
        env: devWrapperTestEnv({
          KINGU_ELECTRON_VITE_CLI: fakeCliPath,
          KINGU_SKIP_DEV_CLI_PREPARE: '1',
          KINGU_SKIP_DEV_ELECTRON_APP_PREPARE: '1',
          KINGU_SKIP_DEV_WEB_PREPARE: '1',
          KINGU_DEV_WRAPPER_TEST_PID_FILE: pidFile
        }),
        stdio: 'ignore'
      })
  ...
  it('forwards dev instance identity to electron-vite', async () => {
    ...
    const wrapper = spawn(process.execPath, [wrapperPath, '--remote-debugging-port=9444'], {
      ...
      env: devWrapperTestEnv({
        ...
        KINGU_SKIP_DEV_WEB_PREPARE: '1',
        KINGU_DEV_WRAPPER_TEST_PID_FILE: pidFile,
        KINGU_DEV_WRAPPER_TEST_ENV_FILE: envFile,
    ...
    expect(envSnapshot.dockTitle).toBe('Kingu: feature/billing-shell')
  ...
  it.skipIf(process.platform === 'win32')(
    'prepares userData kingu and kingu-dev wrappers for dev terminals',
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), 'kingu-dev-wrapper-'))
      ...
      const trackedPids = trackPidFile(pidFile)
      const devWrapper = readFileSync(join(userDataPath, 'cli', 'bin', 'kingu-dev'), 'utf8')
      const publicAliasWrapper = readFileSync(join(userDataPath, 'cli', 'bin', 'kingu'), 'utf8')
      expect(publicAliasWrapper).toBe(devWrapper)
      expect(publicAliasWrapper).toContain('KINGU_USER_DATA_PATH')
    ...
    const wrapper = spawn(
      ...
      [wrapperPath, '--stable-name', '--remote-debugging-port=9445'],
      {
        ...
        env: devWrapperTestEnv({
          ...
          KINGU_DEV_WRAPPER_TEST_PID_FILE: pidFile,
          KINGU_DEV_WRAPPER_TEST_ENV_FILE: envFile,
          KINGU_DEV_BRANCH: 'feature/stable-name',
          KINGU_DEV_WORKTREE_NAME: 'stable-ui'
        }),
        stdio: 'ignore'
      }
    )
  ...
  it.skipIf(process.platform !== 'darwin')(
    ...
    async () => {
      ...
      const baseEnv = devWrapperTestEnv({
        ...
        KINGU_SKIP_DEV_CLI_PREPARE: '1',
        KINGU_SKIP_DEV_WEB_PREPARE: '1',
        KINGU_DEV_BRANCH: 'feature/rebuild-electron-app',
        KINGU_DEV_WORKTREE_NAME: 'electron-app-rebuild'
      })
    ...
    async () => {
      ...
      const { wrapper, readOutput } = spawnDevWrapper(
        ...
        devWrapperTestEnv({
          ...
          KINGU_DEV_WRAPPER_TEST_PID_FILE: pidFile,
          KINGU_DEV_WRAPPER_TEST_ENV_FILE: envFile,
          KINGU_DEV_BRANCH: 'feature/framework-symlinks',
          KINGU_DEV_WORKTREE_NAME: 'symlink-ui'
        })
      )
