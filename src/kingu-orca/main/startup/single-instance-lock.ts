...
export const SINGLE_INSTANCE_LOCK_FAILURE_MESSAGE =
  '[single-instance] Another Kingu instance is already running for this userData profile; exiting this launch after requesting the existing window. If no Kingu process is running, this may be an Electron/macOS single-instance lock failure.'
export const SINGLE_INSTANCE_LOCK_BYPASS_ENV = 'KINGU_BYPASS_SINGLE_INSTANCE_LOCK'
export const SINGLE_INSTANCE_LOCK_E2E_ENFORCE_ENV = 'KINGU_E2E_ENFORCE_SINGLE_INSTANCE_LOCK'
export const SINGLE_INSTANCE_LOCK_BYPASS_MESSAGE =
  '[single-instance] KINGU_BYPASS_SINGLE_INSTANCE_LOCK=1 is set; bypassing the packaged macOS single-instance lock for diagnostics. Do not use this with another Kingu instance running for the same profile.'
...
// Why: a duplicate `kingu serve` is a supervisor artifact, not a user asking for a window; fail open when argv is unavailable.
...
/**
 * Why: Kingu writes two canonical discovery files into `<userData>/`:
 * `kingu-runtime.json` (RPC endpoint + authToken for the bundled CLI) and
 ...
 * instance quits, metadata points at a dead pid and `kingu status` reports
 ...
 * way dev (`kingu-dev` userData) and packaged (`kingu` userData) runs lock in
