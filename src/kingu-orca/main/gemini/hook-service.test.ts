...
describe('GeminiHookService', () => {
  ...
  beforeAll(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'kingu-gemini-home-'))
    userDataDir = mkdtempSync(join(tmpdir(), 'kingu-gemini-userdata-'))
  ...
  it('removes stale PreToolUse hooks when reinstalling managed Gemini hooks', () => {
    ...
    const staleManagedHookPath =
      process.platform === 'win32'
        ? `C:\\Users\\ramzi\\.kingu\\agent-hooks\\${managedHookFileName}`
        : `/Users/ramzi/.kingu/agent-hooks/${managedHookFileName}`
    const staleManagedCommand =
      process.platform === 'win32'
        ? staleManagedHookPath
        : `if [ -x '${staleManagedHookPath}' ]; then /bin/sh '${staleManagedHookPath}'; fi`
    const managedHookPath = join(homeDir, '.kingu', 'agent-hooks', managedHookFileName)
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'kingu gemini home with spaces')
