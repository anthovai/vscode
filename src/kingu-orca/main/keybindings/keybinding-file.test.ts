...
describe('keybinding-file', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'kingu-keybindings-'))
  ...
  it('resolves the user-facing keybindings path under ~/.kingu', () => {
    expect(getUserKeybindingsPath('/home/test')).toBe(
      join('/home/test', '.kingu', 'keybindings.json')
    )
  })
  ...
  it('preserves valid plugin overrides while rejecting malformed plugin action IDs', () => {
    writeFileSync(
      ...
      JSON.stringify({
        keybindings: {
          'plugin:kingu-samples.tasks/open': 'Mod+Shift+T',
    ...
    writeKeybindingOverride(filePath, 'linux', 'plugin:kingu-samples.tasks/open', [])
    expect(readKeybindingFile(filePath, 'linux').overrides).toEqual({
      'plugin:kingu-samples.tasks/open': []
    })
  })
