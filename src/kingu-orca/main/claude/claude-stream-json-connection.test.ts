...
function scriptScenario(
  ...
  return {
    ...
    env: {
      ...
      KINGU_SDK_CONTRACT_SCENARIO_PATH: scenarioPath,
      KINGU_SDK_CONTRACT_REPORT_PATH: reportPath
    },
...
/** The derived child environment, captured where Kingu actually hands it to the OS. */
...
describe('Claude stream-json connection', () => {
  ...
  it('hands the child a derived environment, the resolved CLI path, and keeps the pid', async () => {
    ...
    const connection = await open(
      launchFor(scenario, {
        ...
        KINGU_AGENT_SESSION_SPAWN_TOKEN: 'spawn-9',
    ...
    expect(env.KINGU_AGENT_SESSION_SPAWN_TOKEN).toBe('spawn-9')
    expect(env.KINGU_CONNECTION_MARKER).toBe('inherited')
    ...
    // itself. Either way the resolved path is what Kingu's spawner is asked to execute.
  ...
  it('drives Kingu control methods onto the SDK and times out with the init proof message', async () => {
    const scenario = scriptScenario([HOLD_OPEN], {
  ...
  it('reports an unauthenticated start through the init deadline instead of hanging', async () => {
    ...
    const connection = await open({
      ...launchFor(scenario),
      env: { ...launchFor(scenario).env, KINGU_SDK_CONTRACT_IGNORE_CONTROL_REQUESTS: '1' }
    })
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      ...
      const connection = await open(
        {
          ...launchFor(scenario),
          env: { ...launchFor(scenario).env, KINGU_SDK_CONTRACT_DESCENDANT: '1' }
        },
  ...
  it('proves the exit of a child that ignores a graceful shutdown', async () => {
    ...
    const connection = await open({
      ...launchFor(scenario),
      env: { ...launchFor(scenario).env, KINGU_SDK_CONTRACT_IGNORE_SIGTERM: '1' }
    })
