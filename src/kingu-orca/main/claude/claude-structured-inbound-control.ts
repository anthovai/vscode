...
/**
 * The blocking control requests Kingu answers, each mapped to the SDK consumer callback that
 ...
 * blocking control Kingu must answer means adding its callback here, and the catalog test
...
function denySafeResult(toolUseId: string | undefined): PermissionResult {
  return {
    behavior: 'deny',
    message: 'Kingu could not decode this permission request.',
 ...
 * by `answerClaudePrompt`. `onUserDialog` is deny-safe; the CLI only emits dialog kinds Kingu
