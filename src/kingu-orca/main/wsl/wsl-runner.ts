...
/**
 * The single place Kingu runs a program inside WSL.
 *
...
function withGuestCwd(cwd: string | undefined, argv: readonly string[]): string[] {
  ...
  return ['sh', '-c', 'cd "$1" || exit 1; shift; exec "$@"', 'kingu-wsl', cwd, ...argv]
}
...
export async function runWslProcess(spec: WslSpec): Promise<WslResult> {
  ...
  // `kingu.yaml` hook is the one unbounded script Kingu runs, so past the cap the
  ...
  const result = await runProcess({
    ...
    // is later deleted (the worktree Kingu launched from) fails every later spawn
