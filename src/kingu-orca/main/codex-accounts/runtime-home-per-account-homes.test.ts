...
describe('CodexRuntimeHomeService', () => {
  ...
  it('preserves a managed selection whose auth.json is temporarily missing', async () => {
    ...
    writeFileSync(join(brokenHome, '.kingu-managed-home'), 'account-1\n', 'utf-8')
    const settings = createSettings({
      shellStartupEnvProbeSupported: true,
      codexManagedAccounts: [
        {
  ...
  it('includes WSL account homes in session discovery', async () => {
    const wslHome =
      '\\\\wsl.localhost\\Ubuntu\\home\\me\\.local\\share\\kingu\\codex-accounts\\account-1\\home'
    const store = createStore(
      createSettings({
        codexManagedAccounts: [
          {
            ...
            wslDistro: 'Ubuntu',
            wslLinuxHomePath: '/home/me/.local/share/kingu/codex-accounts/account-1/home',
            providerAccountId: null,
            workspaceLabel: null,
            workspaceAccountId: null,
            createdAt: 1,
