...
beforeEach(() => {
  vi.resetModules()
  testState.userData = mkdtempSync(join(tmpdir(), 'kingu-codex-e-migration-'))
  testState.home = mkdtempSync(join(tmpdir(), 'kingu-codex-e-home-'))
  for (const key of [
    'KINGU_USER_DATA_PATH',
    'KINGU_DISABLE_CODEX_TRUST_RPC',
    'CODEX_HOME',
    'KINGU_CODEX_HOME'
  ]) {
...
describe('CodexRuntimeHomeService per-account takeover composition', () => {
  ...
  it('does not expose an untrusted persisted home through rollout discovery', async () => {
    ...
    writeFileSync(join(outsideHome, '.kingu-managed-home'), 'account-1\n', 'utf-8')
    writeFileSync(
