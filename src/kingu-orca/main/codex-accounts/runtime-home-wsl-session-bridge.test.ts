...
describe('CodexRuntimeHomeService', () => {
  ...
  it('keeps WSL in-Codex setting changes in the direct system home', async () => {
    ...
    try {
      ...
      const baselinePath = join(wslRuntimeHomePath, '.kingu-config-settings-baseline.json')
      ...
      return {
        ...
        parseWslUncPath: (candidate: string) =>
          candidate.includes('codex-accounts/debian-account/home')
            ? {
                distro: 'Debian',
                linuxPath: '/home/alice/.local/share/kingu/codex-accounts/debian-account/home'
              }
            : null
      }
    })
        ...
        codexManagedAccounts: [
          {
            ...
            wslDistro: 'Debian',
            wslLinuxHomePath: '/home/alice/.local/share/kingu/codex-accounts/debian/home',
  ...
  it('does not rescan retired sessions after the launch drain bridges them', async () => {
    ...
    const managedHomePath = join(wslHome, '.local', 'share', 'kingu', 'codex-accounts', 'a', 'home')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const retiredHome = join(guestHome, '.local', 'share', 'kingu', 'codex-runtime-home', 'home')
