...
export class CodexManagedHomePath {
  ...
  assert(candidatePath: string, expectedAccountId?: string): string {
    ...
    if (
      !wslInfo.linuxPath.includes('/.local/share/kingu/codex-accounts/') ||
      !wslInfo.linuxPath.endsWith('/home')
    ) {
      throw new Error('Managed WSL Codex home is outside Kingu account storage.')
    }
    if (
      expectedAccountId !== undefined &&
      !wslInfo.linuxPath.endsWith(`/.local/share/kingu/codex-accounts/${expectedAccountId}/home`)
    ) {
  ...
  private recreateExpectedHostHome(account: CodexManagedAccount, originalError: unknown): string {
    ...
    // Why: re-auth may recreate a lost empty home, but only at the exact Kingu-owned path persisted for this account.
    mkdirSync(expectedPath, { recursive: true })
    writeFileSync(join(expectedPath, '.kingu-managed-home'), `${account.id}\n`, 'utf-8')
  ...
  private async ensureExpectedWslHome(
    ...
    if (
      ...
      !wslInfo.linuxPath.endsWith(`/.local/share/kingu/codex-accounts/${account.id}/home`)
    ) {
      return
    }
    const result = await runWslProcess({
      ...
      script: [
        ...
        'marker="$candidate/.kingu-managed-home"',
    ...
    try {
      const canonicalLinuxPath = this.validateWslPath(
        ...
        [
          'set -euo pipefail',
          `candidate=${quotePosixShell(wslInfo.linuxPath)}`,
          'managed_root="${HOME%/}/.local/share/kingu/codex-accounts"',
          'candidate_real=$(readlink -f -- "$candidate")',
          'managed_root_real=$(readlink -f -- "$managed_root")',
          'test -f "$candidate_real/.kingu-managed-home"',
          ...(expectedAccountId === undefined
            ? [
            ...
            : [
                ...
                'test "$(cat "$candidate_real/.kingu-managed-home")" = "$expected_marker"',
                'printf "%s\\n" "$candidate_real"'
              ])
        ].join('\n')
      ).trim()
  ...
  private assertMountedWslPath(
    ...
    const markerPath = join(candidatePath, '.kingu-managed-home')
    if (!existsSync(markerPath)) {
      throw new Error('Managed Codex home is missing Kingu ownership marker.')
    }
    if (
      expectedAccountId !== undefined &&
