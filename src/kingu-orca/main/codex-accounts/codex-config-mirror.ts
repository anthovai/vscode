...
export class CodexConfigMirror {
  ...
  readForManagedHome(managedHomePath: string): CanonicalCodexConfig | null {
    ...
    const managedRootMarker = '/.local/share/kingu/codex-accounts/'
  ...
  assertOAuthAccountAddAllowed(canonicalConfig: CanonicalCodexConfig | null): void {
    ...
    throw new Error(
      `Kingu cannot add a Codex OAuth account while ~/.codex/config.toml pins the custom provider ${JSON.stringify(modelProvider)}. Keep using the system-default account for this provider, or remove model_provider (or set it to "openai") before adding an OAuth account. Kingu left your config unchanged.`
    )
  }
