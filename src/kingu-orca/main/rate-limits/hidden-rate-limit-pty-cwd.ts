...
const WSL_RATE_LIMIT_PTY_CWD_DIR = 'kingu-rate-limit-pty-cwd'
...
function resolveUserDataRoot(userDataPath?: string | null): string {
  const root = userDataPath?.trim() || process.env.KINGU_USER_DATA_PATH?.trim()
  if (root && !isRootLikePath(root)) {
    return root
  }
  return join(tmpdir(), 'kingu-rate-limit-pty')
}
...
export function getHiddenRateLimitWslCwdSetupCommands(): string[] {
  return [
    `kingu_rate_limit_cwd="\${TMPDIR:-/tmp}/${WSL_RATE_LIMIT_PTY_CWD_DIR}"`,
    'mkdir -p "$kingu_rate_limit_cwd"',
    'cd "$kingu_rate_limit_cwd"'
  ]
}
