...
export function getGrokManagedCommand(scriptPath: string): string {
  ...
  // managed script because Kingu injects KINGU_PANE_KEY only into panes it owns.
  return process.platform === 'win32'
    ? wrapWindowsCmdHookCommand(scriptPath)
    : wrapPosixHookCommand(scriptPath, {}, { requiredEnvVar: 'KINGU_PANE_KEY' })
}
...
export function getGrokManagedScript(target: 'local' | 'posix' = 'local'): string {
  ...
  return [
    ...
    '  grok_home=$GROK_HOME',
    'fi',
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${KINGU_AGENT_HOOK_PORT}/hook/grok" \\',
