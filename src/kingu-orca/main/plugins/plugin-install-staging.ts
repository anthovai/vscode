...
async function readInstallManifest(
  ...
  if (!satisfiesKinguEngineRange(hostVersion, parsed.manifest.engines.kingu)) {
    return {
      ok: false,
      error: `plugin requires Kingu ${parsed.manifest.engines.kingu} (this is ${hostVersion})`
    }
  }
...
export async function installStagedPluginTree(input: {
  ...
  if (blockedReason) {
    return { ok: false, error: `plugin is blocked by Kingu's safety list: ${blockedReason}` }
  }
