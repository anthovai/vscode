...
/** API surface handed to a plugin's `activate(kingu)` export. Everything is
 *  EXPERIMENTAL until pluginApi v1 freezes. */
export type PluginWorkerKinguApi = {
...
export function createPluginWorkerRuntime(
  ...
  async function handleInit(input: {
    ...
    const kingu: PluginWorkerKinguApi = {
      commands: {
