...
import { assessKingudRollback, planKingudUpdate } from './kingud-update-plan'
import {
...
describe('planKingudUpdate', () => {
  ...
  it('defers rather than restarting a host with live terminals', () => {
    ...
    expect(plan).toMatchObject({ action: 'defer', code: 'kingud_update_terminals_running' })
...
describe('assessKingudRollback', () => {
  ...
  it('refuses when the snapshot the record names is gone from the host', () => {
    ...
    expect(safety).toMatchObject({ safety: 'unsafe', code: 'kingud_rollback_snapshot_missing' })
  ...
  it('refuses when the post-activation session count is unverifiable', () => {
    ...
    expect(safety).toMatchObject({ safety: 'unsafe', code: 'kingud_rollback_census_unavailable' })
  })
  ...
  it('refuses when there is no previous version to go back to', () => {
    ...
    expect(safety).toMatchObject({ safety: 'unsafe', code: 'kingud_rollback_no_target' })
  })
})
