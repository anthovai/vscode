...
const LINEAR_HELP = `kingu linear

Usage: kingu linear <command> [options]
...
Run \`kingu linear <command> --help\` for command-specific usage.`

const LINEAR_ISSUE_HELP = `kingu linear issue

Usage: kingu linear issue [<id>] [--current] [--comments] [--children] [--depth <n>] [--attachments] [--relations] [--activity] [--full] [--workspace <id>] [--json]
...
Options:
  ...
  --pairing-code
  --environment
  --current              Use the current Kingu worktree linked Linear issue
...
Examples:
  $ kingu linear search "auth bug"
  $ kingu linear search ENG --workspace all --json`

const LINEAR_TEAM_LIST_HELP = `kingu linear team list

Usage: kingu linear team list [--workspace <id>|all] [--json]

List connected Linear teams`

const LINEAR_TEAM_MEMBERS_HELP = `kingu linear team members

Usage: kingu linear team members --team <key|id> [--workspace <id>] [--json]

List Linear team members`

const LINEAR_TEAM_STATES_HELP = `kingu linear team states

Usage: kingu linear team states --team <key|id> [--workspace <id>] [--json]

List Linear team workflow states`

const LINEAR_TEAM_LABELS_HELP = `kingu linear team labels

Usage: kingu linear team labels --team <key|id> [--workspace <id>] [--json]

List Linear team labels`

const LINEAR_PROJECT_LIST_HELP = `kingu linear project list

Usage: kingu linear project list [--query <text>] [--limit <n>] [--workspace <id>|all] [--json]

List connected Linear projects`

const LINEAR_LIST_HELP = `kingu linear list

Usage: kingu linear list [--filter assigned|created|all|completed|open] [--team <key|id>] [--limit <n>] [--workspace <id>|all] [--json]
