...
// (`.kingu-remote/relay-<fullVersion>`); a random marker inside the shared install
...
/**
 * The model-parameterized form. `relay-<v>` and `kingud-<v>` are permanent siblings under
 * one `.kingu-remote/` (see remote-install-model.ts), so the prefix is an argument.
 */
