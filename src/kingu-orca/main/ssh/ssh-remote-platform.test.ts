...
describe('joinRemotePath', () => {
  it('joins POSIX remote paths', () => {
    expect(joinRemotePath(getRemoteHostPlatform('linux-x64'), '/home/me', '.kingu-remote')).toBe(
      '/home/me/.kingu-remote'
    )
  })

  it('normalizes and joins Windows remote paths with forward slashes for SFTP and Node', () => {
    expect(
      joinRemotePath(getRemoteHostPlatform('win32-x64'), 'C:\\Users\\me', '.kingu-remote', 'relay')
    ).toBe('C:/Users/me/.kingu-remote/relay')
  })
})
...
describe('detectRemoteHostPlatform', () => {
  it('uses uname when the remote is POSIX', async () => {
    vi.mocked(execCommand).mockResolvedValueOnce('__KINGU_REMOTE_PLATFORM__ Darwin arm64')
  ...
  it('falls back to PowerShell when uname is unavailable on Windows', async () => {
    ...
    expect(script).toContain('Write-Output ("`n__KINGU_REMOTE_PLATFORM__ Windows " + $arch)')
  })
})
