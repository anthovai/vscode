...
export type HostKeyDecision = {
  ...
  disagreeingSource?: 'kingu-store' | 'known-hosts'
...
export function decideHostKey(input: HostKeyDecisionInput): HostKeyDecision {
  ...
  if (
    ...
    return {
      ...
      disagreeingSource: fromKnownHosts ? 'known-hosts' : 'kingu-store',
      reason: rejection(
        displayHost,
  ...
  // escape hatch that is an environment variable, unreachable when Kingu is launched from the Dock.
