 ...
 *
 * The tree now lives at `~/.kingu-remote/native/<relayPlatform>-<depsHash>/node_modules` and each
...
/** Sibling of `relay-<version>` and `kingud-<version>`; owned by neither model's version GC. */
 ...
 * reading `~/.kingu-remote/native/` can tell what an entry is for.
 */
...
/** `~/.kingu-remote` — the parent both relay dirs and the cache sit under. */
...
/** `~/.kingu-remote/native/<key>/node_modules` — the symlink target, and the reference identity. */
