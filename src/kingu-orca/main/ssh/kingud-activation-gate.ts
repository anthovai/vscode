/**
 * Whether a freshly launched kingud has earned the right to become the active one.
 *
 * The failure this exists to prevent is the one `docs/design/shipping-kingud.html` names
 * throughout: a deployment that reports success because a port opened. kingud answers RPC
...
export type KingudActivationRejectCode =
  | 'kingud_activation_no_readiness'
  | 'kingud_activation_no_health'
  | 'kingud_activation_build_mismatch'
  | 'kingud_activation_not_listening'
  | 'kingud_activation_daemon_absent'
  | 'kingud_activation_daemon_degraded'
  | 'kingud_activation_pty_self_test_failed'
  | 'kingud_activation_no_persistent_terminals'

export type KingudActivationVerdict =
  | {
      decision: 'activate'
      /**
  ...
  | { decision: 'reject'; code: KingudActivationRejectCode; reason: string }

export type KingudActivationExpectation = {
  /** sha256(kingud.js).slice(0,16) computed from the bytes this client just uploaded. */
  buildHash: string
 ...
 */
export function evaluateKingudActivation(
  readiness: ServeReadiness | null,
  expected: KingudActivationExpectation
): KingudActivationVerdict {
  if (!readiness) {
    return {
      decision: 'reject',
      code: 'kingud_activation_no_readiness',
      reason:
        'The candidate kingud never published an `kingu_server_ready` line. It may have exited, ' +
  ...
  if (!health) {
    return {
      decision: 'reject',
      code: 'kingud_activation_no_health',
      reason:
  ...
  // Why identity first: a stale kingud already holding the port would answer readiness and
  ...
  if (health.buildHash !== expected.buildHash) {
    return {
      decision: 'reject',
      code: 'kingud_activation_build_mismatch',
      reason:
  ...
  if (!readiness.boundEndpoint) {
    return {
      decision: 'reject',
      code: 'kingud_activation_not_listening',
      reason:
  ...
  if (daemon.state === 'absent') {
    return {
      decision: 'reject',
      code: 'kingud_activation_daemon_absent',
      reason:
        'The candidate has no terminal daemon. Every terminal on this host would run in the ' +
        'kingud process and die with it, which is the exact regression the daemon exists to ' +
  ...
  if (daemon.state === 'degraded') {
    return {
      decision: 'reject',
      code: 'kingud_activation_daemon_degraded',
      reason:
  ...
  if (!daemon.selfTest.ok) {
    return {
      decision: 'reject',
      code: 'kingud_activation_pty_self_test_failed',
      reason:
  ...
  if (!daemon.ownsFreshSessions) {
    return {
      decision: 'reject',
      code: 'kingud_activation_no_persistent_terminals',
      reason:
  ...
  if (health.buildVersion !== expected.fullVersion) {
    // Not a rejection: the hash already proved identity, and KINGU_VERSION is whatever the
    ...
    warnings.push(
      `The candidate reports version ${health.buildVersion} but was installed as ` +
        `${expected.fullVersion}; check KINGU_VERSION in the launch command.`
    )
  }
