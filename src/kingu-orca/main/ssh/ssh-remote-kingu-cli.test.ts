...
describe('runRemoteKinguCli', () => {
  ...
  // Why: `kingu terminal create --shell` gates on these; without them an SSH pane was told the
  ...
  it('uses the remote KINGU_TERMINAL_HANDLE as orchestration sender identity', async () => {
    ...
    const result = await runRemoteKinguCli(
      runtime,
      {
        ...
        cwd: '/home/alice/repo',
        env: { KINGU_TERMINAL_HANDLE: 'term_ssh' }
      },
      LEGACY_FALLBACK_OPTIONS
    )
    ...
    const result = await runRemoteKinguCli(
      runtime,
      {
        ...
        env: {
          ...
          KINGU_PANE_KEY: 'tab_ssh:leaf_ssh'
        }
      },
      LEGACY_FALLBACK_OPTIONS
    )
    ...
    try {
      const result = await runRemoteKinguCli(
        runtime,
        {
          ...
          cwd: '/home/alice/repo',
          env: { KINGU_PANE_KEY: 'tab_foreign:leaf_foreign' }
        },
        LEGACY_FALLBACK_OPTIONS
      )
    ...
    try {
      const result = await runRemoteKinguCli(
        runtime,
        {
          ...
          env: {
            KINGU_TERMINAL_HANDLE: 'term_ssh',
            KINGU_PANE_KEY: 'tab_owner:leaf_owner'
          }
        },
        LEGACY_FALLBACK_OPTIONS
      )
  ...
  it('uses the remote KINGU_TERMINAL_HANDLE as orchestration check identity', async () => {
    const { runtime, db } = createRuntime()
    db.insertMessage({
      from: 'term_windows',
      to: 'term_ssh',
  ...
  it('carries the remote pane key for an implicit orchestration check', async () => {
    ...
    const result = await runRemoteKinguCli(
      runtime,
      {
        ...
        cwd: '/home/alice/repo',
        env: {
          KINGU_TERMINAL_HANDLE: 'term_stale_ssh',
          KINGU_PANE_KEY: 'tab_ssh:leaf_ssh'
        }
      },
      LEGACY_FALLBACK_OPTIONS
  ...
  it('routes previously-unsupported commands through the full host CLI', async () => {
    ...
    const resultPromise = runRemoteKinguCli(
      runtime,
      {
        argv: ['worktree', 'create', '--repo', 'kingu', '--branch', 'fix/x', '--json'],
        cwd: '/home/alice/repo',
        env: { KINGU_TERMINAL_HANDLE: 'term_ssh' }
      },
      {
  ...
  it('rejects host-interactive commands with a targeted error instead of bridging them', async () => {
    ...
    expect(result.exitCode).toBe(1)
    expect(result.stderr).toContain('kingu serve')
  ...
  it('rejects interactive account add but still bridges account list', async () => {
    ...
    const addResult = await runRemoteKinguCli(
      runtime,
    ...
    const listPromise = runRemoteKinguCli(
      runtime,
  ...
  it('bridges account add help because it does not start an interactive login', async () => {
    ...
    await Promise.resolve()
    child.stdout.emit('data', Buffer.from('Usage: kingu account add\n'))
    child.emit('close', 0)

    await expect(resultPromise).resolves.toEqual({
      stdout: 'Usage: kingu account add\n',
      stderr: '',
      exitCode: 0
    })
    expect(spawn).toHaveBeenCalledOnce()
  ...
  it('explains the root cause when falling back and the command is not in the legacy switch', async () => {
    ...
    expect(result.exitCode).toBe(1)
    expect(result.stderr).toContain('Unsupported SSH Kingu CLI command: worktree list')
    expect(result.stderr).toContain('full Kingu CLI bridge unavailable')
  })
  ...
  it('does not parse Android --activity values as Linear boolean flags', async () => {
    ...
    expect(result.stderr).toContain(
      'Unsupported SSH Kingu CLI command: emulator launch com.acme.app'
    )
