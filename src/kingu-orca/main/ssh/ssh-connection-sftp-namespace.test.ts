...
vi.mock('./ssh-system-fallback', () => ({
  getKinguControlSocketPath: vi.fn().mockReturnValue(null),
...
const RELAY_DIR = '.kingu-remote/relay-0.1.0+hash'
...
describe('SshConnection SFTP namespace resolution', () => {
  ...
  beforeEach(() => {
    ...
    localDir = realpathSync(mkdtempSync(join(tmpdir(), 'kingu-relay-')))
