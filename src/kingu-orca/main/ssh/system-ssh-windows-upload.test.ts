...
const remoteRoot = 'C:/Users/dev/.kingu-remote'
...
beforeEach(() => {
  ...
  localDir = mkdtempSync(join(tmpdir(), 'kingu-win-upload-'))
  process.env.KINGU_SYSTEM_SFTP_PATH = '/usr/bin/sftp'
...
afterEach(async () => {
  delete process.env.KINGU_SYSTEM_SFTP_PATH
...
describe('Windows upload over sftp', () => {
  ...
  it('creates the parent chain and sends the payload in one round trip', async () => {
    ...
    expect(sftpBatches[0]!.script.split('\n').filter(Boolean)).toEqual([
      '-mkdir "/C:/Users"',
      '-mkdir "/C:/Users/dev"',
      '-mkdir "/C:/Users/dev/.kingu-remote"',
      '-mkdir "/C:/Users/dev/.kingu-remote/a"',
      '-mkdir "/C:/Users/dev/.kingu-remote/a/b"',
  ...
  it('addresses the destination in the drive-rooted namespace sftp exposes', async () => {
    ...
    expect(putDestination(putLines()[0]!)).toMatch(
      /^\/C:\/Users\/dev\/\.kingu-remote\/relay\.js\.kingu-partial-[0-9a-f]{12}$/
    )
  })
  ...
  it('creates upload directories over sftp rather than a PowerShell stdin batch', async () => {
    ...
    expect(sftpBatches[0]!.script).toContain('-mkdir "/C:/Users/dev/.kingu-remote"')
  })
  ...
  it('keeps using sftp for the next file after one path it could not spell', async () => {
    ...
    expect(putLines()).toHaveLength(1)
    expect(putDestination(putLines()[0]!)).toContain('/C:/Users/dev/.kingu-remote/b.js')
  })
