 ...
 *
 * `SIGKILL` is absent on purpose. kingud's own escalation contract (`kingud-entry.ts`) is
 ...
 * `kingud_data_root_shared`, so the update turns into an outage for no gain.
 */
 ...
 *
 * `escalate` sends the second SIGTERM kingud reads as "exit immediately". Callers use it only
