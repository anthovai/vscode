 ...
 *
 * `docs/design/shipping-kingud.html` §04's state-schema row asks for "backward-readable
 * migrations or a pre-activation snapshot". Only the second is available here, and not as a
 * preference: Kingu's persisted state carries **no schema version**. Migrations are cohort
 ...
 * daemon's socket, PID record and auth token, and that daemon outlives every kingud restart
...
import { assertPosixKingudHost as assertPosixHost } from './kingud-remote-host-support'
 ...
 */
export const KINGUD_SNAPSHOT_MEMBERS = [
  'kingu-profile-index.json',
...
export const KINGUD_SNAPSHOT_EXCLUDED = ['daemon', 'logs'] as const

/**
 * The member names go into the command unquoted (see `captureKingudStateSnapshotCommand`), so
...
function assertPlainMemberName(member: string): string {
  if (!/^[A-Za-z0-9._-]+$/.test(member)) {
    throw new Error(`Unsafe kingud snapshot member name: ${JSON.stringify(member)}`)
  }
  return member
}

export function kingudSnapshotDirName(fullVersion: string, takenAtMs: number): string {
 ...
 */
export function captureKingudStateSnapshotCommand(
  ...
  const memberTests = KINGUD_SNAPSHOT_MEMBERS.map(
...
export type KingudSnapshotCapture = 'captured' | 'empty' | 'failed'

export function parseKingudSnapshotCapture(output: string): KingudSnapshotCapture {
...
export function probeKingudStateSnapshotCommand(
  host: RemoteHostPlatform,
  snapshotDir: string
): string {
  assertPosixHost(host)
 ...
 *
 * The caller must have stopped kingud first. This does not check — it cannot, from a shell —
 * so `kingud-remote-deploy.ts` owns that ordering.
 */
export function restoreKingudStateSnapshotCommand(
  host: RemoteHostPlatform,
  userDataDir: string,
  snapshotDir: string
): string {
  ...
  const root = shellEscape(userDataDir)
  const archive = shellEscape(joinRemotePath(host, snapshotDir, 'state.tar'))
  const removals = KINGUD_SNAPSHOT_MEMBERS.map(
...
export type KingudSnapshotRestore = 'restored' | 'missing' | 'failed'

export function parseKingudSnapshotRestore(output: string): KingudSnapshotRestore {
  const value = output.trim().split('\n').pop()?.trim()
  if (value === 'RESTORED') {
    return 'restored'
  }
 ...
 * caller compares; an `UNKNOWN` becomes `null`, which `assessKingudRollback` treats as "yes,
 * assume writes".
 */
export function newestStateMtimeCommand(host: RemoteHostPlatform, userDataDir: string): string {
  assertPosixHost(host)
  const root = shellEscape(userDataDir)
  const paths = KINGUD_SNAPSHOT_MEMBERS.map((member) => `${root}/${shellEscape(member)}`).join(' ')
  return [
