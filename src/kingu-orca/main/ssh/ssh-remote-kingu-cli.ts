...
import {
  ...
  runHostKinguCliPassthrough,
  type HostCliPassthroughOptions,
  type RemoteKinguCliRequest,
  type RemoteKinguCliResult
...
export type { RemoteKinguCliRequest, RemoteKinguCliResult } from './ssh-remote-cli-host-passthrough'
...
const HOST_INTERACTIVE_COMMANDS: Record<string, string> = {
  serve:
    'kingu serve starts a foreground headless Kingu server and cannot run through the SSH relay bridge. Run it directly on the machine that should host Kingu.',
  'claude-teams':
    'kingu claude-teams starts an interactive Claude Code session and cannot run through the SSH relay bridge. Run it in a terminal on the Kingu host machine.',
  'agent-teams-tmux':
    'kingu agent-teams-tmux is a tmux pane shim for the Kingu host machine and cannot run through the SSH relay bridge.',
  'account add':
    'kingu account add runs an interactive agent login and cannot run through the buffered SSH relay bridge. Run it directly in a terminal on the Kingu host machine.'
}

export async function runRemoteKinguCli(
  runtime: KinguRuntimeService,
  request: RemoteKinguCliRequest,
  passthroughOptions?: HostCliPassthroughOptions
): Promise<RemoteKinguCliResult> {
  ...
  if (command === 'orchestration check' || command === 'orchestration ask') {
    ...
    return await runLegacyRemoteKinguCli(
      runtime,
      request,
      parsed,
      json,
  ...
  try {
    return await runHostKinguCliPassthrough(request, passthroughOptions)
  } catch (err) {
  ...
  return await runLegacyRemoteKinguCli(runtime, request, parsed, json, passthroughFailure)
}

async function runLegacyRemoteKinguCli(
  runtime: KinguRuntimeService,
  request: RemoteKinguCliRequest,
  ...
  switch (command) {
    case 'status': {
      ...
      const cliStatus: CliStatusResult = {
        ...
        // Why: this answers for the Kingu host the caller reached over SSH, not for the caller's
    ...
    case 'orchestration send': {
      ...
      return await call(
        ...
        'orchestration.send',
        {
          ...
          senderPaneKey: env.KINGU_PANE_KEY || undefined
        },
        {
    ...
    default:
      ...
      throw new Error(
        `Unsupported SSH Kingu CLI command: ${command} (full Kingu CLI bridge unavailable: ${passthroughFailureReason})`
      )
  }
}
