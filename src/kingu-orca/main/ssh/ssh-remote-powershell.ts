...
export function powerShellCommand(
  ...
  if (compressed.length > WINDOWS_REMOTE_COMMAND_LINE_BUDGET_CHARS) {
    throw new Error(
      `Remote Windows command needs ${compressed.length} characters; Kingu budgets ${WINDOWS_REMOTE_COMMAND_LINE_BUDGET_CHARS} for a line sshd hands to cmd.exe, which itself refuses more than ${CMD_EXE_COMMAND_LINE_MAX_CHARS}.`
    )
  }
  return compressed
}
...
/** Kingu-prefixed names so the payload can never shadow the bootstrap's own state. */
function selfExtractingPowerShellScript(script: string): string {
  ...
  return [
    `$KinguScriptBytes = [Convert]::FromBase64String('${payload}')`,
    '$KinguScriptMemory = New-Object System.IO.MemoryStream -ArgumentList (,$KinguScriptBytes)',
    '$KinguScriptGzip = New-Object System.IO.Compression.GZipStream -ArgumentList $KinguScriptMemory, ([System.IO.Compression.CompressionMode]::Decompress)',
    '$KinguScriptReader = New-Object System.IO.StreamReader -ArgumentList $KinguScriptGzip, ([System.Text.Encoding]::UTF8)',
    '$KinguScriptText = $KinguScriptReader.ReadToEnd()',
    '$KinguScriptReader.Dispose()',
    'Invoke-Expression $KinguScriptText'
  ].join('\n')
}
...
export function decodeRemotePowerShellScript(command: string): string {
  ...
  const payload = script.match(
    /^\$KinguScriptBytes = \[Convert\]::FromBase64String\('([A-Za-z0-9+/=]+)'\)/u
  )?.[1]
