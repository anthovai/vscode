...
function connectWithKinguConfig(
  target: SshTarget,
...
describe('multi-stage SSH authentication', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'kingu-mfa-'))
  ...
  it('answers a keyboard-interactive stage that follows a password partial success', async () => {
    ...
    try {
      const { ready, prompts } = connectWithKinguConfig(makeTarget(server.port), null, PASSWORD, [
        PASSCODE
      ])
  ...
  it('answers a second keyboard-interactive stage after the first partially succeeds', async () => {
    ...
    try {
      const { ready, prompts } = connectWithKinguConfig(makeTarget(server.port), null, undefined, [
        PASSCODE,
        PASSCODE
      ])
  ...
  it('reaches the MFA stage without burning the host auth-try budget on rejected keys', async () => {
    ...
    try {
      ...
      const { ready } = connectWithKinguConfig(
        target,
