/**
 * Which installed kingud is the live one, and which one a rollback goes back to.
 *
 ...
 *
 * It lives beside the version dirs (`~/.kingu-remote/kingud-active.json`), not inside one,
 * because it has to outlive whichever version GC removes.
 */
import { remoteInstallDirName, KINGUD_INSTALL_MODEL } from './remote-install-model'

export const KINGUD_ACTIVATION_FILENAME = 'kingud-active.json'
export const KINGUD_ACTIVATION_SCHEMA_VERSION = 1

/** Where a pre-activation copy of the shared data root lives, relative to `.kingu-remote/`. */
export const KINGUD_STATE_SNAPSHOT_DIR = 'kingud-state-snapshots'

export type KingudStateSnapshot = {
  /** Directory name under `KINGUD_STATE_SNAPSHOT_DIR`. */
  dirName: string
...
export type KingudActivationRecord = {
  schemaVersion: typeof KINGUD_ACTIVATION_SCHEMA_VERSION
...
export function emptyKingudActivationRecord(): KingudActivationRecord {
  return {
    schemaVersion: KINGUD_ACTIVATION_SCHEMA_VERSION,
    active: null,
    previous: null,
    activatedAt: null,
    snapshot: null
 ...
 * install. Callers distinguish the two through `KingudActivationReadResult`.
 */
export type KingudActivationReadResult =
  | { state: 'absent' }
  | { state: 'ok'; record: KingudActivationRecord }
  | { state: 'unreadable'; reason: string }

export function parseKingudActivationRecord(raw: string | null): KingudActivationReadResult {
  ...
  const record = parsed as Partial<KingudActivationRecord>
  if (record.schemaVersion !== KINGUD_ACTIVATION_SCHEMA_VERSION) {
    return {
      state: 'unreadable',
      reason:
        `activation record schemaVersion ${String(record.schemaVersion)} is not ` +
        `${KINGUD_ACTIVATION_SCHEMA_VERSION}; this client cannot safely interpret it`
    }
  }
  return {
    state: 'ok',
...
function parseSnapshot(value: unknown): KingudStateSnapshot | null {
  if (typeof value !== 'object' || value === null) {
    return null
  }
  const snapshot = value as Partial<KingudStateSnapshot>
...
export function serializeKingudActivationRecord(record: KingudActivationRecord): string {
 ...
 * OUTGOING bundle (see kingud-update-plan.ts) — deleting the tree under a running process is
 * how a later respawn finds no entry point.
 */
export function kingudGcPinnedDirNames(
  record: KingudActivationRecord,
  ...
  return [...new Set(versions)].map((version) =>
    remoteInstallDirName(KINGUD_INSTALL_MODEL, version)
  )
}
