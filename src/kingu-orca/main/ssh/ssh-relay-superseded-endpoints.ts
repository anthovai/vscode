 ...
 *
 * Every relay build installs to `~/.kingu-remote/relay-<fullVersion>/` and binds its socket
...
export function supersededRelayEndpointListCommand(options: {
  ...
  return [
    ...
    // under `/tmp/.kingu-relay-<uid>/relay-<versionHash>/` instead (relay-socket-path-limit.ts).
...
export function removeStaleRelayEndpointCommand(sockPath: string): string {
  ...
  // `gcOldRelayVersions` only walks `$HOME/.kingu-remote`, so nothing else would ever
