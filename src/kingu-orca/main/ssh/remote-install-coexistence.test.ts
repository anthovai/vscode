...
const BOTH_INSTALLED = ['relay-0.1.0+aa01', 'kingud-0.2.0+bb01', 'kingud-0.1.0+aa01']
...
describe('what a client does when it finds both models installed', () => {
  it('uses the registered model and leaves the other install alone', () => {
    const selection = selectRemoteInstallModel({
      registration: 'kingud-peer',
      installedDirNames: BOTH_INSTALLED
    })
    expect(selection).toMatchObject({ outcome: 'use', model: 'kingud' })
  ...
  it('picks the registered model even when only the other one is installed', () => {
    // On-disk presence is diagnostic, never a vote: an kingud-registered host with only relay
    // dirs is a first kingud deploy, not a reason to fall back to the relay.
    const selection = selectRemoteInstallModel({
      registration: 'kingud-peer',
  ...
  it('says nothing when there is nothing coexisting', () => {
    const selection = selectRemoteInstallModel({
      registration: 'kingud-peer',
      installedDirNames: ['kingud-0.2.0+bb01']
    })
    expect(selection).toMatchObject({ outcome: 'use', model: 'kingud', note: null })
  })
})
