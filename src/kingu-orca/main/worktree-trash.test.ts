...
beforeEach(async () => {
  scratchDir = await mkdtemp(join(tmpdir(), 'kingu-worktree-trash-'))
})
...
describe('sweepStaleWorktreeTrash', () => {
  it('removes leftover entries from both workspace layouts and nothing else', async () => {
    ...
    // Not generated by Kingu: a stray directory and file inside the trash root must survive.
...
describe('collectWorktreeTrashSweepRoots', () => {
  const settings = { workspaceDir: '/home/dev/kingu/workspaces', nestWorkspaces: true }

  function repo(overrides: Partial<Repo>): Repo {
    return { id: 'repo-1', path: '/home/dev/code/kingu', ...overrides } as unknown as Repo
  }
  ...
  it('collects one root per local git repo', () => {
    ...
    ).toEqual(['/home/dev/kingu/workspaces'])
  })
  ...
  it('skips WSL repos that cannot create host trash', () => {
    ...
    try {
      expect(
        collectWorktreeTrashSweepRoots(
          [
            repo({ path: '\\\\wsl.localhost\\Ubuntu\\home\\dev\\kingu' }),
            repo({
              id: 'repo-2',
              path: 'C:\\code\\kingu',
