...
function writeLegacyLinearToken(token: string | Buffer, viewer: Record<string, unknown>): void {
  const kinguDir = join(tempHome, '.kingu')
  mkdirSync(kinguDir, { recursive: true })
  writeFileSync(join(kinguDir, 'linear-token.enc'), token)
  writeFileSync(join(kinguDir, 'linear-viewer.json'), JSON.stringify(viewer), {
    encoding: 'utf-8'
  })
}
...
function writeMultiWorkspaceFiles(
  ...
  const kinguDir = join(tempHome, '.kingu')
  mkdirSync(join(kinguDir, 'linear-tokens'), { recursive: true })
  writeFileSync(
    join(kinguDir, 'linear-workspaces.json'),
    JSON.stringify({
      version: 1,
...
beforeEach(() => {
  tempHome = mkdtempLike('kingu-linear-client-')
  fixtures = new Map([
    [
      'token-alpha',
      {
...
describe('Linear client workspace storage', () => {
  ...
  it('migrates legacy token storage to a real workspace id when explicitly tested', async () => {
    ...
    expect(existsSync(join(tempHome, '.kingu', 'linear-token.enc'))).toBe(false)
    expect(readFileSync(join(tempHome, '.kingu', 'linear-workspaces.json'), 'utf-8')).toContain(
      'org-alpha'
    )
  })
  ...
  it('does not pass encrypted safeStorage bytes to the Linear SDK when encryption is unavailable', async () => {
    const tokenPath = join(tempHome, '.kingu', 'linear-token.enc')
