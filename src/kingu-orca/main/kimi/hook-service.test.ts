...
// homedir()/.kingu, and getKimiHome() honors KIMI_CODE_HOME. Point both at a
// temp dir so the local install/remove cycle never touches the real ~/.kingu or
...
beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), 'kingu-kimi-hook-'))
...
const scriptPath = (): string => join(home, '.kingu', 'agent-hooks', 'kimi-hook.sh')
