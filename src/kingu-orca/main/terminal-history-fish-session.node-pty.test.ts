...
const PROMPT_MARK = 'KINGUHIST> '
const WORKTREE_HASH = 'deadbeefdeadbeef'
const MARKER = 'echo kingu-worktree-scoped-history'
...
describe('fish keeps per-worktree history under the session Kingu names', () => {
  ...
  itWithFish(
    ...
    async () => {
      ...
      home = mkdtempSync(path.join(tmpdir(), 'kingu-fish-history-'))
      ...
      term.write(`${MARKER}\r`)
      expect(await waitUntil(() => rendered.includes('kingu-worktree-scoped-history'), 5_000)).toBe(
        true
      )
