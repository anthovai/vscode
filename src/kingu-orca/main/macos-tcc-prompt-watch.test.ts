...
describe('parseTccPromptEvent', () => {
  it('parses a real captured AUTHREQ_PROMPTING line', () => {
    expect(parseTccPromptEvent(REAL_PROMPT_LINE)).toEqual({
      ...
      accessingIdentifier: 'com.kingu.tccprobe.shapecapture',
      responsibleIdentifier: 'com.kingu.tccprobe.shapecapture',
  ...
  it('separates the accessing binary from the responsible app', () => {
    const event = parseTccPromptEvent(KINGU_APPDATA_LINE)
    // The whole point of #9756: the dialog says Kingu, but node did the access.
    expect(event?.responsibleIdentifier).toBe('com.anthovai.kingu')
...
describe('isKinguAttributedPrompt', () => {
  it('accepts the app and detached terminal helper across Kingu build identities', () => {
    for (const id of [
      'com.anthovai.kingu',
      'com.anthovai.kingu.helper',
      'com.anthovai.kingu.dev',
      'com.anthovai.kingu.dev.helper',
      'com.anthovai.kingu.local',
      'com.anthovai.kingu.local.helper'
    ]) {
      expect(
        isKinguAttributedPrompt({
  ...
  it('rejects unrelated services even when Kingu is responsible', () => {
    expect(
      isKinguAttributedPrompt({
        service: 'kTCCServiceMicrophone',
        accessingIdentifier: 'kingu',
        responsibleIdentifier: 'com.anthovai.kingu'
      })
    ).toBe(false)
  })
})
...
describe('MacosTccPromptWatch', () => {
  ...
  it('reports only Kingu-attributed dialogs from a live stream', async () => {
    ...
    stdout.write(`${KINGU_APPDATA_LINE}\n`)
