...
// macOS convention is `~/Library/Application Support/Kingu/logs/`; Windows and
...
// and that path has to resolve under plain Node (kingud) as well as the desktop.
...
function getUserDataDir(): string {
  ...
  const home = homedir()
  if (platform() === 'darwin') {
    return join(home, 'Library', 'Application Support', 'Kingu')
  }
  if (platform() === 'win32') {
    return join(process.env.APPDATA ?? home, 'Kingu')
  }
  return join(home, '.config', 'Kingu')
}
