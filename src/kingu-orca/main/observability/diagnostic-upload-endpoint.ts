...
export function resolveDiagnosticBuildTokenEndpoint(): string | null {
  const endpoint =
    typeof KINGU_DIAGNOSTICS_TOKEN_URL !== 'undefined'
      ? KINGU_DIAGNOSTICS_TOKEN_URL
      : ((globalThis as { KINGU_DIAGNOSTICS_TOKEN_URL?: string | null })
          .KINGU_DIAGNOSTICS_TOKEN_URL ?? null)
  return typeof endpoint === 'string' && endpoint.length > 0 ? endpoint : null
}

export function resolveDiagnosticBuildIdentity(): 'stable' | 'rc' | null {
  const ident =
    typeof KINGU_BUILD_IDENTITY !== 'undefined'
      ? KINGU_BUILD_IDENTITY
      : ((globalThis as { KINGU_BUILD_IDENTITY?: 'stable' | 'rc' | null }).KINGU_BUILD_IDENTITY ??
        null)
...
export function resolveDiagnosticTokenEndpoint(): string | null {
  ...
  // cannot redirect uploads that the UI labels as going to Kingu support.
  if (resolveDiagnosticBuildIdentity()) {
    return buildEndpoint
  }
  const fromEnv = process.env.KINGU_DIAGNOSTICS_TOKEN_URL
...
export function resolveDiagnosticKinguChannel(): 'stable' | 'rc' | 'dev' {
