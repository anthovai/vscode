...
/** Basename of the file every Kingu-generated zsh wrapper dir is stamped with. */
export const ZSH_WRAPPER_DIR_MARKER_FILE = '.kingu-shell-wrapper'

export const ZSH_WRAPPER_DIR_MARKER_CONTENT = `# Kingu-generated zsh startup wrapper directory.
# Its presence is how Kingu recognises its own wrapper dir instead of treating it
 ...
 *
 * Why destroy it here: `_kingu_shell_features` is a plain (non-exported) shell
 ...
 * Kingu's feature selection.
 */
export const ZSH_FEATURE_CHANNEL_BLOCK = `builtin typeset -ga _kingu_shell_features
_kingu_shell_features=(\${(s:,:)\${${SHELL_STARTUP_FEATURE_ENV}:-}})
builtin unset ${SHELL_STARTUP_FEATURE_ENV}
# Why KINGU_HISTFILE is consumed HERE and not in the deferred hook: a user config
...
# including a nested Kingu (#11146). Captured non-exported, it cannot escape.
builtin typeset -g _kingu_histfile="\${KINGU_HISTFILE:-}"
builtin unset KINGU_HISTFILE
__kingu_has_feature() { (( \${_kingu_shell_features[(Ie)$1]} )) }`

/** The bash rcfile equivalent of ZSH_FEATURE_CHANNEL_BLOCK. */
export const BASH_FEATURE_CHANNEL_BLOCK = `_kingu_shell_features=",\${${SHELL_STARTUP_FEATURE_ENV}:-},"
builtin unset ${SHELL_STARTUP_FEATURE_ENV}
__kingu_has_feature() { [[ "$_kingu_shell_features" == *",$1,"* ]]; }`

// Why one line usable by both languages: __kingu_has_feature is defined with the
// same name and semantics in the zsh and bash channel blocks above.
export const SHELL_STARTUP_IDENTITY_MARKER_BLOCK = `__kingu_has_feature identity && printf "\\033]777;kingu-shell-start:%s\\007" "$$"`
 ...
 * instead of one inside Kingu's wrapper dir, so #11044 cannot happen rather than
 * having to be repaired afterwards.
 *
 * KINGU_ORIG_ZDOTDIR is consumed: it has done its job, and leaving it exported
 ...
 * environment, so a stale KINGU_ORIG_ZDOTDIR written by an older build can arrive
 * on its own. Handing that back would point ZDOTDIR at an Kingu wrapper dir — the
 ...
 * stamped marker file, or Kingu's own path shape for wrappers older builds wrote.
 */
export const ZSH_ZDOTDIR_HANDBACK_BLOCK = `__kingu_usable_zdotdir() {
  [[ -n "\${1:-}" ]] || return 1
  # Kingu's own dir, by marker file or by the shape older builds wrote.
  ...
  local _kingu_startup_file
  for _kingu_startup_file in .zshenv .zshrc .zprofile .zlogin; do
    [[ -r "$1/$_kingu_startup_file" ]] && return 0
  done
  return 1
}
if __kingu_usable_zdotdir "\${KINGU_ORIG_ZDOTDIR:-}"; then
  builtin export ZDOTDIR="$KINGU_ORIG_ZDOTDIR"
else
  builtin unset ZDOTDIR
fi
builtin unset KINGU_ORIG_ZDOTDIR KINGU_ZSHENV_SOURCE_DIR
builtin unfunction __kingu_usable_zdotdir`
...
export const ZSH_USER_ZSHENV_SOURCE_BLOCK = `{
  builtin typeset _kingu_user_zshenv="\${ZDOTDIR-$HOME}/.zshenv"
  [[ ! -r "$_kingu_user_zshenv" ]] || builtin source -- "$_kingu_user_zshenv"
} always {
  builtin unset _kingu_user_zshenv
  builtin typeset -ag precmd_functions
  (( \${precmd_functions[(Ie)__kingu_deferred_init]} )) || precmd_functions+=(__kingu_deferred_init)
}`
 ...
 * check-before-set, and it runs before any wrapper file Kingu controls — so the
 * injected value is already gone, and because ZDOTDIR still points at Kingu's
 ...
 *
 * `builtin unset KINGU_HISTFILE` is the root-cause fix for #11146: the variable
 ...
 */
export const BASH_HISTFILE_RESTORE_BLOCK = `if [[ -n "\${KINGU_HISTFILE:-}" ]]; then
  HISTFILE="$KINGU_HISTFILE"
  builtin unset KINGU_HISTFILE
elif [[ "\${HISTFILE:-}" == "$ZDOTDIR/.zsh_history" ]]; then
  # Why also when Kingu injected nothing: /etc/zshrc derived this from Kingu's
  ...
  HISTFILE="\${KINGU_ORIG_ZDOTDIR:-$HOME}/.zsh_history"
fi`
...
export function getZshShellReadyMarkerRegistrationBlock(
  ...
  const markerBlock = supportsStartupCommand
    ? `  if [[ "\${__kingu_emit_ready_marker-1}" == 1 ]]; then
...
if [[ "\${widgets[zle-line-init]:-}" == "user:__kingu_prompt_mark" ]]; then
  :
elif (( \${+widgets[zle-line-init]} )) && [[ "\${widgets[zle-line-init]}" == user:* ]]; then
  __kingu_prev_line_init_fn="\${widgets[zle-line-init]#user:}"
else
  __kingu_prev_line_init_fn=""
fi
__kingu_prompt_mark() {
${markerBlock}
  ...
  if [[ -n "\${__kingu_prev_line_init_fn:-}" ]]; then
    "\${__kingu_prev_line_init_fn}" "$@"
  fi${startupCommandBlock}
}
zle -N zle-line-init __kingu_prompt_mark`.replace(/\n\n}\n$/, '\n}\n')
}
...
export function getFishShellReadyInitCommand(
  ...
  const startupCommandBlock = supportsStartupCommand
    ...
    set -l __kingu_command "\$${POSIX_SHELL_STARTUP_COMMAND_ENV}"
    ...
    : ''
  return `function __kingu_shell_ready_marker --on-event fish_prompt
${readyMarkerBlock}  functions -e __kingu_shell_ready_marker
