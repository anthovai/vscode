...
beforeEach(() => {
  vi.stubEnv('KINGU_BACKGROUND_LAUNCH', undefined)
  vi.stubEnv('KINGU_E2E_HEADLESS', undefined)
  vi.stubEnv('KINGU_E2E_HEADFUL', undefined)
})
...
describe('createMainWindow', () => {
  ...
  it('keeps the headless E2E window hidden when the Windows fallback fires', () => {
    vi.useFakeTimers()
    const previousHeadless = process.env.KINGU_E2E_HEADLESS
    process.env.KINGU_E2E_HEADLESS = '1'
    const { browserWindowInstance } = createStartupRevealWindowFixture()
    ...
    try {
      ...
      if (previousHeadless === undefined) {
        delete process.env.KINGU_E2E_HEADLESS
      } else {
        process.env.KINGU_E2E_HEADLESS = previousHeadless
      }
    }
  })
