...
describe('createMainWindow', () => {
  ...
  it('enables renderer sandboxing and opens external links safely', () => {
    ...
    const webContents = {
      getURL: vi.fn(() => 'file:///opt/kingu/renderer/index.html'),
      isDestroyed: vi.fn(() => false),
      mainFrame: {},
    ...
    const allowBlankPrefs = { partition: 'persist:kingu-browser' }
    fire(
      'will-attach-webview',
    ...
    routePartitionAllowedMock.mockImplementation(
      (partition) => partition === 'persist:kingu-browser-v1-route-partition'
    )
    const allowRouteEvent = { preventDefault: vi.fn() }
    const allowRoutePrefs = { partition: 'persist:kingu-browser-v1-route-partition' }
    fire(
      'will-attach-webview',
    ...
    expect(allowRoutePrefs).toMatchObject({
      partition: 'persist:kingu-browser-v1-route-partition',
      sandbox: true
    })
    ...
    fire(
      ...
      { partition: 'persist:kingu-browser-v1-route-partition' } as never,
    ...
    fire(
      ...
      { partition: 'persist:kingu-browser' } as never,
