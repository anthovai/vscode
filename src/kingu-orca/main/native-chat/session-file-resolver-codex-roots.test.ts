...
const MANAGED_HOME = '/tmp/kingu-user-data/codex-runtime-home/home'
...
vi.mock('../codex/codex-home-paths', () => ({
  getKinguManagedCodexHomePath: vi.fn(() => MANAGED_HOME),
  resolveKinguManagedCodexHomePath: vi.fn(() => MANAGED_HOME)
}))
...
beforeEach(() => {
  ...
  vi.mocked(getKinguManagedCodexHomePath).mockClear()
  scanned.dirs = []
})
...
describe('codex sessions roots', () => {
  it('scans the managed home without materializing it', async () => {
    ...
    expect(vi.mocked(getKinguManagedCodexHomePath)).not.toHaveBeenCalled()
  })
})
