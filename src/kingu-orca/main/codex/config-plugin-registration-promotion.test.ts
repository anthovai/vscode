...
beforeEach(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'kingu-codex-registration-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'kingu-codex-registration-user-data-'))
...
function baselinePath(homePath = runtimeHomeDir()): string {
  return join(homePath, '.kingu-config-settings-baseline.json')
}
...
describe('codex plugin registration survives the managed-home mirror', () => {
  ...
  it('honors a canonical removal instead of resurrecting the registration', () => {
    ...
    // The user edits ~/.codex outside Kingu and deletes both registrations.
    writeSystemConfig('model = "gpt-5"\n')
    mirrorTwice()
