...
beforeEach(() => {
  ...
  fakeHomeDir = mkdtempSync(join(tmpdir(), 'kingu-codex-resource-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'kingu-codex-resource-user-data-'))
...
describe('syncSystemCodexResourcesIntoManagedHome', () => {
  it('uses KINGU_USER_DATA_PATH when Electron cannot be required', async () => {
    vi.resetModules()
    vi.doMock('electron', () => {
    ...
    process.env.KINGU_USER_DATA_PATH = userDataDir
    try {
      const { getKinguManagedCodexHomePath: getCliSafeManagedPath } =
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      mkdirSync(join(managedHomePath, '.kingu-resource-copies', 'AGENTS.md.json'), {
        recursive: true
      })
  ...
  it('removes an unowned copy when recording copy ownership fails', () => {
    ...
    mkdirSync(managedHomePath, { recursive: true })
    writeFileSync(join(managedHomePath, '.kingu-resource-copies'), 'blocks marker directory\n')
