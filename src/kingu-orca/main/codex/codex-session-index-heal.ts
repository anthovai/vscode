...
// stamped `complete` on first app-server startup), so rollouts that Kingu's
...
// Codex's DB-driven surfaces. Kingu never writes Codex's sqlite schema itself.
...
function buildNativeHealInvocation(
  ...
  return {
    ...
    args: spawnArgs,
    cliPath: command,
    // Why: pin the real home explicitly — nested Kingu launches can inherit a
