...
async function createTemporaryRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'kingu-codex-backfill-recovery-'))
...
describe.skipIf(process.platform === 'win32')('Codex backfill supervisor owner lock', () => {
  ...
  it('does not interfere with a live supervisor from another Kingu instance', async () => {
    const userData = await createTemporaryRoot()
    vi.stubEnv('KINGU_USER_DATA_PATH', userData)
    const home = join(userData, 'managed-home')
