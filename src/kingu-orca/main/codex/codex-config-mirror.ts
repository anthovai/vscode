...
export function syncSystemConfigIntoManagedCodexHome(
  ...
  // user made inside Kingu-launched Codex (/model, /approvals) must be written
  ...
  // unpromoted runtime change as Kingu-written would strand it forever.
  snapshotCodexRuntimeSettingsBaseline(homes.runtimeHomePath, {
    conflicts: new Map(
...
function mergeSystemCodexConfigIntoRuntime(runtimeConfig: string, systemConfig: string): string {
  ...
  // trust and project trust are written under Kingu's managed CODEX_HOME and
