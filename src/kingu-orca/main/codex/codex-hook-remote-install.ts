...
export async function installCodexHooksRemote(
  ...
  const remoteScriptPath = redirectedCodexHome
    ? `${redirectedCodexHome}/.kingu/agent-hooks/codex-hook.sh`
    : `${remoteHome.replace(/\/$/, '')}/.kingu/agent-hooks/codex-hook.sh`
  try {
    ...
    // Why: SSH remotes use POSIX `.sh` paths even when Kingu runs on Windows; never derive remote script syntax from local OS.
    await writeManagedScriptRemote(sftp, remoteScriptPath, getManagedScript('posix'))
    // Why: SSH edits the user's remote ~/.codex/hooks.json directly, so preserve non-Kingu top-level metadata while replacing the hooks tree.
