...
/** Kingu-side state dir; nothing extra is ever written into the user's ~/.codex. */
...
export function assertHooksJsonGeneration(
  ...
  if (currentRaw !== expectedRaw || resolveHooksJsonWritePath(hooksJsonPath) !== hooksWritePath) {
    ...
    throw new Error('Codex hooks.json changed while Kingu prepared its trust repair')
  }
}

/** One-time pristine copy of the user's file, kept under Kingu's userData. */
export function backupRealHomeHooksJsonOnce(
  ...
  const backupPath = join(backupDir, 'hooks.json.pre-kingu')
...
/** Places Kingu's managed hook in `definitions`, reusing its existing slot when
...
export function reconcileManagedHookDefinition(
  ...
  if (!hasManagedDirectCommand && nestedLocations.length === 1) {
    ...
    if (definition.matcher === undefined && !hasDirectCommand) {
      ...
      // Why: users can append groups or handlers after Kingu's first install.
