...
export function syncSystemCodexSessionsIntoManagedHome(sourceCodexHomePath?: string): void {
  ...
  const managedSessionsRoot = join(getKinguManagedCodexHomePath(), 'sessions')
...
function replaceSymlinkSessionBridgeWithHardlink(
  ...
  try {
    ...
    replacementPath = `${targetPath}.kingu-link-${process.pid}-${Date.now()}`
...
function getLegacySessionCopyMarkerPath(relativePath: string): string {
  return join(getKinguManagedCodexHomePath(), '.kingu-session-copies', `${relativePath}.json`)
}
