...
function resolveAccountManagedHome(candidate: string, userDataPath: string): string | null {
  ...
  try {
    return readFileSync(join(candidate, '.kingu-managed-home'), 'utf-8').trim() === accountId
      ? expectedHome
      : null
  } catch {
    return null
...
export function resolveManagedCodexShellPreflightHome(
  ...
  if (!codexHome || !kinguCodexHome || !pathsEqual(codexHome, kinguCodexHome)) {
    return null
  }
...
export async function prepareManagedCodexHomeBeforeShellLaunch(args: {
  ...
  const env = args.env ?? {
    ...
    KINGU_CODEX_HOME: process.env.KINGU_CODEX_HOME
  }
