...
import { getKinguProfileListState } from './profile-index-store'
...
function assertKnownProfiles(args: TransferKinguProfileProjectArgs, userDataPath: string): void {
  const profiles = getKinguProfileListState(userDataPath).profiles
  const ids = new Set(profiles.map((profile) => profile.id))
  if (!ids.has(args.sourceProfileId)) {
    throw new Error('unknown_source_kingu_profile')
  }
  if (!ids.has(args.targetProfileId)) {
    throw new Error('unknown_target_kingu_profile')
  }
  if (args.sourceProfileId === args.targetProfileId) {
    throw new Error('matching_kingu_profile_transfer')
  }
}

export function transferKinguProfileProject(
