...
function profile(
  id: string,
  name: string,
  kind: KinguProfileKind = 'local'
): KinguProfileIndex['profiles'][number] {
  return {
    id,
    name,
...
function writeProfileState(profileId: string, repos: Repo[]): void {
  ...
  const dataFile = join(testState.dir, 'profiles', profileId, 'kingu-data.json')
...
describe('profile project presence', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'kingu-profile-presence-'))
    writeIndex()
  })
  ...
  it('finds matching projects in other profiles while excluding the active profile', async () => {
    writeProfileState('personal', [
      makeRepo({ id: 'personal-repo', path: 'C:\\Code\\Kingu', displayName: 'Personal Kingu' })
    ])
    writeProfileState('work', [
      makeRepo({ id: 'work-repo', path: 'C:\\Code\\Kingu', displayName: 'Work Kingu' })
    ])

    const { findKinguProfileProjectsByPath } = await loadPresenceModule()
    const result = findKinguProfileProjectsByPath(
      {
        path: 'c:/code/kingu/',
  ...
  it('keeps SSH projects separate from local projects with the same path', async () => {
    writeProfileState('personal', [
      makeRepo({ id: 'local-repo', path: '/srv/kingu', displayName: 'Local Kingu' })
    ])
    writeProfileState('work', [
      makeRepo({
        id: 'ssh-repo',
        path: '/srv/kingu',
        displayName: 'SSH Kingu',
