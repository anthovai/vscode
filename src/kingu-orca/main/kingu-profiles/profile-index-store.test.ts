...
describe('profile index store', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'kingu-profile-test-'))
  ...
  it('creates the default local profile and copies legacy state without deleting it', async () => {
    ...
    writeFileSync(join(testState.dir, 'kingu-data.json'), JSON.stringify(legacyState), 'utf-8')
    writeFileSync(
      join(testState.dir, 'kingu-data.json.bak.0'),
    ...
    const { ensureActiveKinguProfile, getKinguProfileIndexPath } = await loadProfileIndexStore()
    const activeProfile = ensureActiveKinguProfile()

    expect(activeProfile.profile.id).toBe(DEFAULT_LOCAL_KINGU_PROFILE_ID)
    expect(activeProfile.profile.name).toBe(DEFAULT_LOCAL_KINGU_PROFILE_NAME)
    expect(activeProfile.dataFile).toBe(
      join(testState.dir, 'profiles', DEFAULT_LOCAL_KINGU_PROFILE_ID, 'kingu-data.json')
    )
    ...
    expect(
      readJson(
        join(testState.dir, 'profiles', DEFAULT_LOCAL_KINGU_PROFILE_ID, 'browser-session-meta.json')
      )
    ).toEqual(legacyBrowserSessionMeta)
    expect(existsSync(join(testState.dir, 'kingu-data.json'))).toBe(true)

    expect(readJson(getKinguProfileIndexPath())).toMatchObject({
      schemaVersion: KINGU_PROFILE_INDEX_SCHEMA_VERSION,
      activeProfileId: DEFAULT_LOCAL_KINGU_PROFILE_ID,
      profiles: [expect.objectContaining({ id: DEFAULT_LOCAL_KINGU_PROFILE_ID, kind: 'local' })]
    })
  })
  ...
  it('uses an existing active profile data file without overwriting it from legacy state', async () => {
    ...
    writeFileSync(join(profileDirectory, 'kingu-data.json'), JSON.stringify(profileData), 'utf-8')
    writeFileSync(
      join(testState.dir, 'kingu-data.json'),
      JSON.stringify({ schemaVersion: 1, repos: [{ id: 'legacy-repo' }] }),
      'utf-8'
    )
    const index: KinguProfileIndex = {
      schemaVersion: KINGU_PROFILE_INDEX_SCHEMA_VERSION,
    ...
    writeFileSync(join(testState.dir, 'kingu-profile-index.json'), JSON.stringify(index), 'utf-8')

    const { ensureActiveKinguProfile } = await loadProfileIndexStore()
    const activeProfile = ensureActiveKinguProfile()

    expect(activeProfile.profile.id).toBe(profileId)
    expect(activeProfile.dataFile).toBe(join(profileDirectory, 'kingu-data.json'))
  ...
  it('creates an empty local profile without copying legacy state into it', async () => {
    ...
    const { createLocalKinguProfile, getKinguProfileDataFile, getKinguProfileListState } =
      await loadProfileIndexStore()
    const created = createLocalKinguProfile({ name: ' Work ' })

    expect(created.profile.name).toBe('Work')
    expect(created.profile.id).toMatch(/^local-/)
    expect(created.activeProfileId).toBe(DEFAULT_LOCAL_KINGU_PROFILE_ID)
    expect(created.profiles.map((profile) => profile.id)).toContain(created.profile.id)
    expect(existsSync(getKinguProfileDataFile(created.profile.id))).toBe(false)
    expect(getKinguProfileListState().profiles.map((profile) => profile.id)).toContain(
      created.profile.id
    )
  })

  it('switches the active profile and updates last-opened metadata', async () => {
    const { createLocalKinguProfile, setActiveKinguProfile } = await loadProfileIndexStore()
    const created = createLocalKinguProfile({ name: 'Work' })

    const switched = setActiveKinguProfile(created.profile.id)
  ...
  it('rejects switching to an unknown profile', async () => {
    const { setActiveKinguProfile } = await loadProfileIndexStore()

    expect(() => setActiveKinguProfile('missing-profile')).toThrow('unknown_kingu_profile')
  })
  ...
  posixIt('writes a fresh profile index when umask removes owner-write permission', async () => {
    ...
    const indexPath = store.getKinguProfileIndexPath()
    const profile = createDefaultLocalKinguProfile(1)
    const index: KinguProfileIndex = {
      schemaVersion: KINGU_PROFILE_INDEX_SCHEMA_VERSION,
  ...
  it('recovers a corrupted profile index from the backup copy', async () => {
    const store = await loadProfileIndexStore()
    store.ensureActiveKinguProfile()
    const created = store.createLocalKinguProfile({ name: 'Work' })
    // Trigger one more write so the backup captures the two-profile index.
    store.setActiveKinguProfile(created.profile.id)

    const indexPath = store.getKinguProfileIndexPath()
    expect(existsSync(`${indexPath}.bak`)).toBe(true)
    writeFileSync(indexPath, '{ not json', 'utf-8')

    const recovered = store.getKinguProfileListState()
  ...
  it('rejects profile ids that are not safe path segments', async () => {
    ...
    const state = store.ensureActiveKinguProfile()
    expect(state.profile.id).toBe(DEFAULT_LOCAL_KINGU_PROFILE_ID)
  })
})
