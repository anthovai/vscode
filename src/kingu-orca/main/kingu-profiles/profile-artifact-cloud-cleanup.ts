...
import type { KinguProfileCloudSummary, KinguProfileSummary } from '../../shared/kingu-profiles'
...
function cleanupMarkerPath(profileId: string, userDataPath: string): string {
  return join(getKinguProfileDirectory(profileId, userDataPath), 'artifact-cloud-cleanup.json')
}

export function artifactCloudIdentity(cloud: KinguProfileCloudSummary | undefined): string {
  return cloud
...
export function prepareArtifactCloudCleanup(
  profileId: string,
  userDataPath: string,
  targetCloud: KinguProfileCloudSummary | undefined
): void {
...
export function completeArtifactCloudCleanupIfCommitted(
  profileId: string,
  userDataPath: string,
  currentCloud: KinguProfileCloudSummary | undefined
): void {
  const marker = readCleanupMarker(profileId, userDataPath)
  if (
  ...
  bestEffortFsyncDirectorySync(getKinguProfileDirectory(profileId, userDataPath))
}
...
function assertArtifactCloudCleanupReady(
  ...
  if (
    ...
    throw new Error('The Kingu profile transition must be retried before publishing artifacts.')
  }
}

export function prepareArtifactCloudUse(
  profile: Pick<KinguProfileSummary, 'id' | 'cloud'>,
  userDataPath: string
): void {
