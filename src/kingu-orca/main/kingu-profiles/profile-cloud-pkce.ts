...
import {
  KINGU_CLOUD_CALLBACK_RESPONSE_HEADERS,
  KINGU_CLOUD_CALLBACK_SUCCESS_PAGE
} from './profile-cloud-callback-page'

export type KinguCloudAuthorizationCode = {
  code: string
  codeVerifier: string
  nonce: string
  redirectUri: string
...
export function beginKinguCloudPkceFlow(
  config: KinguCloudAuthConfig,
  localProfileId: string
): Promise<KinguCloudAuthorizationCode> {
  ...
  return new Promise((resolve, reject) => {
    ...
    function writeInvalidCallback(response: ServerResponse): void {
      ...
      response.end('Invalid Kingu sign-in response.')
    }
    ...
    const server = createServer((request, response) => {
      try {
        ...
        if (url.searchParams.has('error')) {
          ...
          response.end(
            cancelled
              ? 'Kingu sign-in was cancelled.'
              : 'Kingu sign-in failed. Return to Kingu and try again.'
          )
          rejectFlow(
            new Error(cancelled ? 'kingu_cloud_auth_denied' : 'kingu_cloud_auth_callback_failed')
          )
          return
        }
        if (!code) {
        ...
        response.writeHead(200, KINGU_CLOUD_CALLBACK_RESPONSE_HEADERS)
        response.end(KINGU_CLOUD_CALLBACK_SUCCESS_PAGE)
        resolveFlow(code)
      } catch (error) {
        rejectFlow(error instanceof Error ? error : new Error('kingu_cloud_auth_callback_failed'))
      }
    })

    const timeout = setTimeout(() => {
      rejectFlow(new Error('kingu_cloud_auth_timeout'))
    }, AUTH_TIMEOUT_MS)
    ...
    server.listen(0, '127.0.0.1', () => {
      ...
      if (!address || typeof address === 'string') {
        rejectFlow(new Error('kingu_cloud_auth_loopback_unavailable'))
        return
      }
      ...
      void shell.openExternal(authorizeUrl.toString()).catch((error) => {
        rejectFlow(
          error instanceof Error ? error : new Error('kingu_cloud_auth_browser_open_failed')
        )
      })
    })
  })
