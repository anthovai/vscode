...
import type { KinguProfileCloudSummary } from '../../shared/kingu-profiles'
...
import { linkKinguProfileToCloud, unlinkKinguProfileFromCloud } from './profile-cloud-index'
import {
...
describe('profile artifact cloud cleanup', () => {
  it('preserves recovery state when the profile write fails', async () => {
    ...
    expect(() => linkKinguProfileToCloud(profileId, cloud('org-b'), userDataPath)).toThrow(
      'profile write failed'
    )
    expect(getArtifactCreateIntent(profileId, userDataPath, '/report.md', scope)).not.toBeNull()

    linkKinguProfileToCloud(profileId, cloud('org-b'), userDataPath)
  ...
  it('reconciles an interrupted transition before linking again', async () => {
    ...
    expect(() => linkKinguProfileToCloud(profileId, cloud('org-c'), userDataPath)).toThrow(
      'cleanup failed'
    )
    ...
    linkKinguProfileToCloud(profileId, cloud('org-c'), userDataPath)
  ...
  it('reconciles an interrupted transition before unlinking', async () => {
    ...
    expect(() => unlinkKinguProfileFromCloud(profileId, userDataPath)).toThrow('cleanup failed')
    expect(writeProfileIndex).not.toHaveBeenCalled()
    expect(currentCloud(userDataPath)?.activeOrgId).toBe('org-b')
    expect(getArtifactCreateIntent(profileId, userDataPath, '/report.md', scope)).not.toBeNull()

    unlinkKinguProfileFromCloud(profileId, userDataPath)
  ...
  it('preserves an orphaned local marker for an unknown profile', async () => {
    ...
    const scope = shareScope('org-a')
    mkdirSync(getKinguProfileDirectory(orphanProfileId, userDataPath), { recursive: true })
    createIntent(userDataPath, scope, orphanProfileId)
    prepareArtifactCloudCleanup(orphanProfileId, userDataPath, undefined)

    const transitions = [
      () => linkKinguProfileToCloud(orphanProfileId, cloud('org-b'), userDataPath),
      () => unlinkKinguProfileFromCloud(orphanProfileId, userDataPath)
    ]
    for (const transition of transitions) {
      expect(transition).toThrow('unknown_kingu_profile')
...
function cloud(activeOrgId: string): KinguProfileCloudSummary {
  return {
...
function shareScope(cloudOrganizationId: string): ArtifactShareScope {
  return {
    ...
    cloudProfileId: 'cloud-profile-a',
    cloudOrganizationId,
    apiOrigin: 'https://share.onkingu.dev'
  }
}

async function createLinkedProfile(cloudSummary: KinguProfileCloudSummary): Promise<string> {
  const userDataPath = await mkdtemp(join(tmpdir(), 'kingu-profile-artifact-cleanup-'))
  createdPaths.push(userDataPath)
  loadOrCreateProfileIndex(userDataPath)
  linkKinguProfileToCloud(profileId, cloudSummary, userDataPath)
  vi.clearAllMocks()
  return userDataPath
}
...
function currentCloud(userDataPath: string): KinguProfileCloudSummary | undefined {
  return readProfileIndex(getKinguProfileIndexPath(userDataPath))?.profiles.find(
