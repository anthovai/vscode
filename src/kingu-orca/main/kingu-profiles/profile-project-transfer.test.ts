...
function profile(id: string, name: string): KinguProfileIndex['profiles'][number] {
  return {
    id,
    name,
...
function profileDataPath(profileId: string): string {
  return join(testState.dir, 'profiles', profileId, 'kingu-data.json')
}
...
describe('profile project transfer', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'kingu-profile-transfer-'))
    writeIndex()
  })
  ...
  it('copies a project into another profile with a new repo id and re-keyed metadata', async () => {
    const sourceWorktreeId = 'repo-1::/workspace/kingu-feature'
    writeProfileState(
      'personal',
      makeState({
        repos: [makeRepo()],
        ...
        retiredWorktreeNamesByNamespace: {
          'ssh:ssh-1:/workspace/kingu-nautilus': { exhaustedTiers: 0, names: ['seahorse'] }
        },
        worktreeMeta: {
    ...
    const { transferKinguProfileProject } = await loadTransferModule()
    const result = transferKinguProfileProject(
      {
    ...
    const targetWorktreeId = `${targetRepoId}::/workspace/kingu-feature`
    expect(target.repos).toEqual([
      expect.objectContaining({ id: targetRepoId, path: '/workspace/kingu' })
    ])
  ...
  it('moves a project, preserving SSH identity and restorable workspace session state', async () => {
    const sourceWorktreeId = 'repo-ssh::/srv/kingu-feature'
    ...
    writeProfileState(
      ...
      makeState({
        ...
        retiredWorktreeNamesByNamespace: {
          'ssh:ssh-1:posix:/srv/kingu-kingu-retirement-probe': {
            exhaustedTiers: 0,
            names: ['seahorse']
          }
        },
            ...
            [sourceWorktreeId]: [
              {
                ...
                sessionPartition: 'persist:kingu-profile-personal-deadbeef-browser-default',
    ...
    expect(target.retiredWorktreeNamesByNamespace).toEqual({
      'ssh:builder.example.com|22|dev:posix:/srv/kingu-kingu-retirement-probe': {
        exhaustedTiers: 0,
        names: ['seahorse']
      }
    })
  ...
  it('rejects a duplicate physical project inside the target profile', async () => {
    writeProfileState(
      'personal',
      makeState({
        repos: [makeRepo({ path: 'C:\\Work\\Kingu\\' })]
      })
    )
    writeProfileState(
      'work',
      makeState({
        repos: [makeRepo({ id: 'repo-existing', path: 'c:/work/kingu' })]
      })
    )
