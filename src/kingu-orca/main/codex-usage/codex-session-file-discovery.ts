...
import { getKinguManagedCodexHomePath, getSystemCodexHomePath } from '../codex/codex-home-paths'
...
export function getCodexSessionsDirectory(): string {
  // Why: Kingu-launched Codex processes receive an Kingu-owned CODEX_HOME, so
  // callers that need the primary runtime path should not consult ambient
  // shell CODEX_HOME.
  return join(getKinguManagedCodexHomePath(), 'sessions')
}
...
function hasLegacyCopiedSessionBridgeMarkers(): boolean {
  return existsSync(join(getKinguManagedCodexHomePath(), '.kingu-session-copies'))
}
