...
// Kingu's script while the presence gate skips install() forever. These tests pin the
...
beforeEach(() => {
  ...
  isolatedUserDataDir = mkdtempSync(join(tmpdir(), 'kingu-hook-refresh-user-data-'))
...
const STALE_WINDOWS_HOOK = [
  '@echo off',
  'setlocal',
  'if "%KINGU_AGENT_HOOK_PORT%"=="" goto :kingu_agent_hook_drain_stdin',
...
describe('refreshManagedScriptIfPresent', () => {
  it('rewrites an existing script and refuses to create a missing one', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'kingu-hook-refresh-unit-'))
    try {
...
describe('managed hook script refresh', () => {
  it('brings a stale leaking script current without touching agent config', async () => {
    const home = mkdtempSync(join(tmpdir(), 'kingu-hook-refresh-'))
    homedirMock.mockReturnValue(home)
    try {
      // Why: the bug population has a script (from a past install) but no reachable
      // CLI — and possibly no config dir Kingu may create. Seed only the script.
      ...
      expect(refreshed).toContain('if "%KINGU_AGENT_HOOK_PORT%"=="" exit /b 0')
      expect(refreshed).not.toContain('if "%KINGU_AGENT_HOOK_PORT%"=="" goto')
  ...
  it('covers every shared launcher script with a refresher', async () => {
    const home = mkdtempSync(join(tmpdir(), 'kingu-hook-refresh-coverage-'))
    homedirMock.mockReturnValue(home)
    ...
    try {
      ...
      for (const file of files) {
        expect(
          ...
          `${file} is written to ~/.kingu/agent-hooks but no refresher owns it`
        ).toBe(true)
      }
  ...
  it('creates nothing anywhere when no managed scripts exist', async () => {
    const home = mkdtempSync(join(tmpdir(), 'kingu-hook-refresh-empty-'))
    homedirMock.mockReturnValue(home)
    try {
