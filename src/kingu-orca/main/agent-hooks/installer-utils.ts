...
// Why: prod/dev/parallel Kingu instances must write the same managed entry, not race between per-userData script paths.
export function getSharedManagedScriptPath(scriptFileName: string): string {
  return join(homedir(), '.kingu', 'agent-hooks', scriptFileName)
}
...
export function buildWindowsHookPowerShellCommand(
  ...
  // env guard, and only then own stdin — outside an Kingu pane the caller may abandon the
...
export function buildWindowsAgentHookPostCommand(
  ...
  return [
    `"%SystemRoot%\\System32\\curl.exe" -sS -X POST "http://127.0.0.1:%KINGU_AGENT_HOOK_PORT%/hook/${source}" ^`,
...
export function buildWindowsAgentHookCurlPostCommand(source: AgentHookSource): string {
  return [
    ...
    `"http://127.0.0.1:%KINGU_AGENT_HOOK_PORT%/hook/${source}"`,
    '--connect-timeout 0.5 --max-time 1.5',
    '-H "Content-Type: application/x-www-form-urlencoded"',
    '-H "X-Kingu-Agent-Hook-Token: %KINGU_AGENT_HOOK_TOKEN%"',
    '--data-urlencode "paneKey=%KINGU_PANE_KEY%"',
