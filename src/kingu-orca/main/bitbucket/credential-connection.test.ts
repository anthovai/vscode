...
beforeEach(() => {
  process.env = { ...OLD_ENV }
  for (const key of [
    'KINGU_BITBUCKET_ACCESS_TOKEN',
    'KINGU_BITBUCKET_EMAIL',
    'KINGU_BITBUCKET_API_TOKEN',
    'KINGU_BITBUCKET_API_BASE_URL'
  ]) {
    delete process.env[key]
  }
  tempHome = mkdtempSync(join(tmpdir(), 'kingu-bitbucket-conn-'))
})
...
describe('Bitbucket credential connection', () => {
  ...
  it('reports environment variables as the source and takes precedence over stored creds', async () => {
    ...
    process.env.KINGU_BITBUCKET_ACCESS_TOKEN = 'env-token'
  ...
  it('lets an env API base URL override the stored one without env credentials', async () => {
    ...
    process.env.KINGU_BITBUCKET_API_BASE_URL = 'https://env.example.com/2.0'
