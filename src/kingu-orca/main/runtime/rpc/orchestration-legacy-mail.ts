            ...
            formatMessageBanner(message, {
              ...
              supportedActionHints: readOnly
                ...
                : supportedLegacyHints(
                    message,
                    principal,
                    params.compatibilityCliCommand ?? 'kingu'
                  )
            })
          )
          .join('\n\n')
  ...
  const currentDelivery =
    principal.role === 'coordinator' && db.hasPendingCurrentDelivery(principal.run_id)
      ? {
          ...
          checkCommand: `${params.compatibilityCliCommand ?? 'kingu'} orchestration check --run ${principal.run_id}`,
          ackCommand: `${params.compatibilityCliCommand ?? 'kingu'} orchestration check --run ${principal.run_id} --ack <delivery-id>`
        }
      : undefined
  return {
    runId: principal.run_id,
