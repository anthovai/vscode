...
export function releaseStructuredWorkerSession(
  dispatchId: string,
  runtime?: Pick<KinguRuntimeService, 'forgetStructuredSessionMail'>
): void {
...
export async function createStructuredWorkerSession(args: {
  ...
  // ships a worker with no KINGU_TERMINAL_HANDLE, whose bare `kingu orchestration check` then
...
export async function discardStructuredWorkerSession(
  sessionId: string,
  runtime: Pick<KinguRuntimeService, 'retireStructuredAgentSessionTabFromSnapshot'>
): Promise<void> {
