...
const LINUX_HOME = '/home/jin/.local/share/kingu/codex-runtime-home/home'
const RUNTIME_HOME =
...
describe('agent hook RPC methods', () => {
  ...
  it.each([
    ...
    const method = prepareMethod()
    const params = method.params!.parse({
      codexHome: '/home/jin/.local/share/kingu/codex-runtime-home/home',
      kinguCodexHome: '/home/jin/.local/share/kingu/codex-runtime-home/home',
      wslDistro: 'Ubuntu-24.04'
    })
  ...
  it.each(['runtime', 'mobile'] as const)('rejects non-local %s callers', async (clientKind) => {
    ...
    ).rejects.toThrow(/only available to the local Kingu CLI/)
    expect(installForRuntimeHomeSerializedMock).not.toHaveBeenCalled()
  })
