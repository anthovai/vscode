...
import { TEST_WORKTREE_ID, TEST_WORKTREE_PATH, store } from '../kingu-runtime-test-fixtures.spec'
import { wireHeadlessServeRuntime } from '../kingu-runtime-test-scenario-builders.spec'
...
describe('KinguRuntimeService', () => {
  it('restores captured native Claude Agent Teams mode with fresh service env', async () => {
    ...
    await runtime.createTerminal(`path:${TEST_WORKTREE_PATH}`, {
      ...
      env: {
        ...
        KINGU_AGENT_TEAMS_SHIM_BIN: '/opt/kingu/bin/kingu-ide',
        KINGU_AGENT_TEAMS_TEAM_ID: 'stale-team',
        KINGU_AGENT_TEAMS_TOKEN: 'stale-token',
        TMUX: '/tmp/kingu-claude-agent-teams/stale-team,0,1'
      },
      launchAgent: 'claude',
      launchConfig: {
        agentCommand: 'claude',
    ...
    expect(spawnCall?.env?.KINGU_AGENT_TEAMS_TEAM_ID).toMatch(/^team-/)
    expect(spawnCall?.env?.KINGU_AGENT_TEAMS_TEAM_ID).not.toBe('stale-team')
    expect(spawnCall?.env?.KINGU_AGENT_TEAMS_TOKEN).not.toBe('stale-token')
    expect(spawnCall?.env?.TMUX).not.toBe('/tmp/kingu-claude-agent-teams/stale-team,0,1')
    ...
    expect(revealedLaunchConfig?.agentEnv.KINGU_AGENT_TEAMS_TEAM_ID).not.toBe('stale-team')
    expect(revealedLaunchConfig?.agentEnv.KINGU_AGENT_TEAMS_TOKEN).not.toBe('stale-token')
  })
  ...
  it('adopts renderer pane identity for remote runtime terminal creates', async () => {
    ...
    expect(spawnedEnv.KINGU_TAB_ID).toBe(tabId)
    expect(spawnedEnv.KINGU_PANE_KEY).toBe(`${tabId}:${leafId}`)
  })
  ...
  it('does not adopt web mirror ids as host terminal ids', async () => {
    ...
    const spawnedEnv =
      (spawn.mock.calls[0]?.[0] as { env?: Record<string, string> } | undefined)?.env ?? {}
    expect(spawnedEnv.KINGU_TAB_ID).not.toBe(tabId)
    expect(spawnedEnv.KINGU_TAB_ID).not.toMatch(/^web-terminal-/)
    expect(spawnedEnv.KINGU_PANE_KEY).toMatch(`${spawnedEnv.KINGU_TAB_ID}:`)
  })
  ...
  it('spawns focus-requested CLI terminal creates in background on headless serve', async () => {
    ...
    // `kingu terminal create --worktree <wt> --command "echo test" --focus`
    await expect(
      runtime.createTerminal(`path:${TEST_WORKTREE_PATH}`, {
        command: 'echo test',
        focus: true
