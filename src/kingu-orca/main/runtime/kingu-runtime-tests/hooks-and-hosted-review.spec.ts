...
import { TEST_REPO_ID, TEST_REPO_PATH, store } from '../kingu-runtime-test-fixtures.spec'
...
describe('KinguRuntimeService', () => {
  describe('checkRepoHooks status', () => {
    ...
    it('reports ok for a missing remote kingu.yaml and error for any other read failure', async () => {
    ...
    it('reports ok for a local repo hook check', async () => {
      const runtime = new KinguRuntimeService(store as never)

      await expect(runtime.checkRepoHooks('id:repo-1')).resolves.toMatchObject({ status: 'ok' })
    })
  })

  it('resolves SSH issue commands from shared kingu.yaml and deletes empty overrides', async () => {
    const remoteStore = {
      ...store,
      getRepos: () => [
        {
    ...
    vi.mocked(parseKinguYaml).mockReturnValue({
      scripts: {},
      issueCommand: 'claude -p "Fix #{{issue}}"'
    })
    const fsProvider = {
      readFile: vi.fn(async (filePath: string) => {
        if (filePath.endsWith('.kingu/issue-command')) {
          throw Object.assign(new Error('missing'), { code: 'ENOENT' })
        }
        if (filePath.endsWith('kingu.yaml')) {
    ...
    try {
      await expect(runtime.readRepoIssueCommand('id:repo-1')).resolves.toMatchObject({
        ...
        localFilePath: '/remote/repo/.kingu/issue-command',
        source: 'shared'
      })
    ...
    expect(fsProvider.readFile).toHaveBeenCalledWith('/remote/repo/kingu.yaml')
    expect(fsProvider.deletePath).toHaveBeenCalledWith('/remote/repo/.kingu/issue-command', false)
    expect(fsProvider.writeFile).not.toHaveBeenCalledWith(
      '/remote/repo/.kingu/issue-command',
      expect.anything()
    )
  })

  it('allows host integration slug helpers for SSH repos through provider-aware GitHub clients', async () => {
    const prRepo = { owner: 'acme', repo: 'kingu', host: 'github.acme.test' }
  ...
  it('routes runtime GitHub repo identity helpers through the selected WSL project runtime', async () => {
    ...
    getRepoSlugMock.mockResolvedValueOnce({ owner: 'acme', repo: 'kingu' })
  ...
  it('routes runtime GitHub issue and work-item actions through the selected WSL project runtime', async () => {
    ...
    createGitHubIssueMock.mockResolvedValueOnce({
      ok: true,
      number: 12,
      url: 'https://github.com/acme/kingu/issues/12'
    })
  ...
  it('pins explicit origin preference on runtime open-by-number work item lookups', async () => {
    ...
    const prRepo = { owner: 'acme', repo: 'kingu' }
