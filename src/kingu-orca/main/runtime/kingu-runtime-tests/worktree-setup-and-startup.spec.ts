...
import { UUID_RE, expectStablePaneKeyEnv, store } from '../kingu-runtime-test-fixtures.spec'
...
describe('KinguRuntimeService', () => {
  it('returns a setup launch payload for CLI-created worktrees when hooks are explicitly enabled', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        ...
        KINGU_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-test'
      }
    })
  ...
  it('passes setup payloads through when explicitly activating CLI-created worktrees', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        KINGU_ROOT_PATH: '/tmp/repo',
        KINGU_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-activate'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValueOnce([
      {
  ...
  it('passes the selected Windows setup shell into runtime runner generation', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      shell: { family: 'posix' },
      envVars: {
        KINGU_ROOT_PATH: 'C:\\repo',
        KINGU_WORKTREE_PATH: 'C:\\workspaces\\runtime-hook-activate'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValueOnce([
      {
  ...
  it('follows normal setup policy for CLI-created worktrees without activating them', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        KINGU_ROOT_PATH: '/tmp/repo',
        KINGU_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-skip'
      }
    })
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        cwd: '/tmp/workspaces/runtime-hook-skip',
        command: 'bash /tmp/repo/.git/kingu/setup-runner.sh',
        // Why: createTerminal stamps KINGU_PANE_KEY/TAB_ID/WORKTREE_ID so hook-based agent status can attribute events to a stable pane.
        env: expect.objectContaining({
          KINGU_ROOT_PATH: '/tmp/repo',
          KINGU_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-skip',
          KINGU_TAB_ID: expect.stringMatching(UUID_RE),
    ...
    const setupLeafId = setupSpawnEnv.KINGU_PANE_KEY.slice(`${setupSpawnEnv.KINGU_TAB_ID}:`.length)
    ...
    expect(revealTerminalSession).toHaveBeenLastCalledWith(result.worktree.id, {
      ...
      activate: false,
      surfaceOwner: false,
      tabId: setupSpawnEnv.KINGU_TAB_ID,
      leafId: setupLeafId
    })
  })
  ...
  it('uses returned WSL setup shell metadata when runtime spawns setup', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        KINGU_ROOT_PATH: 'C:\\repo',
        KINGU_WORKTREE_PATH: 'C:\\workspaces\\runtime-hook-wsl'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({
        command: 'bash /mnt/c/repo/.git/kingu/setup-runner.sh',
        env: expect.objectContaining({
          KINGU_ROOT_PATH: 'C:\\repo',
          KINGU_WORKTREE_PATH: 'C:\\workspaces\\runtime-hook-wsl',
          KINGU_TAB_ID: expect.stringMatching(UUID_RE),
  ...
  it('uses the shell-aware setup runner for windowless creates without a startup command', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        KINGU_ROOT_PATH: 'C:\\repo',
        KINGU_WORKTREE_PATH: 'C:\\workspaces\\runtime-hook-windowless'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({
        command: 'bash /c/repo/.git/kingu/setup-runner.sh',
        worktreeId: result.worktree.id
      })
    )
  })
