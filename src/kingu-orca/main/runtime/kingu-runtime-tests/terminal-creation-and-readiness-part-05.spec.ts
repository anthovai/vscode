...
describe('KinguRuntimeService', () => {
  ...
  it('splits visible pty-backed terminal sessions through the parent renderer tab', async () => {
    ...
    const sourceLeafId = sourceEnv.KINGU_PANE_KEY.slice(`${sourceEnv.KINGU_TAB_ID}:`.length)

    const split = await runtime.splitTerminal(handle, { direction: 'vertical' })
    expect(split).toMatchObject({
      handle: expect.stringMatching(/^term_/),
      tabId: sourceEnv.KINGU_TAB_ID,
      paneRuntimeId: -1
    })

    const splitEnv =
      (spawn.mock.calls[1]?.[0] as { env?: Record<string, string> } | undefined)?.env ?? {}
    const splitLeafId = splitEnv.KINGU_PANE_KEY.slice(`${sourceEnv.KINGU_TAB_ID}:`.length)
    expect(split.leafId).toBe(splitLeafId)
    expect(splitTerminal).not.toHaveBeenCalled()
    expect(splitEnv.KINGU_TAB_ID).toBe(sourceEnv.KINGU_TAB_ID)
    expect(splitEnv.KINGU_WORKTREE_ID).toBe(TEST_WORKTREE_ID)
    ...
    const siblingSurfaces = publishedTabs.filter(
      (tab): tab is Extract<typeof tab, { type: 'terminal' }> =>
        tab.type === 'terminal' && tab.parentTabId === sourceEnv.KINGU_TAB_ID
    )
  ...
  it('splits folder workspace pty-backed terminal sessions with folder cwd and env', async () => {
    const folderPath = await mkdtemp(join(tmpdir(), 'kingu-runtime-folder-split-'))
    const spawn = vi
      .fn()
    ...
    expect(splitEnv.KINGU_TAB_ID).toBe(sourceEnv.KINGU_TAB_ID)
    expect(splitEnv.KINGU_WORKSPACE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
    expect(splitEnv.KINGU_PROJECT_GROUP_ID).toBe(TEST_FOLDER_PROJECT_GROUP_ID)
    expect(splitEnv.KINGU_WORKSPACE_ROOT).toBe(folderPath)
    expect(splitEnv.KINGU_WORKTREE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
  ...
  it('returns an actionable discoverability warning when default adoption fails after spawn', async () => {
    ...
    try {
      ...
      expect(created.warning).toContain(`kingu terminal focus --terminal ${created.handle}`)
