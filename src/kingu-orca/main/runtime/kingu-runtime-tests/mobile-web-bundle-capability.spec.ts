...
import { createRuntime } from '../kingu-runtime-test-fixtures.spec'
...
describe('KinguRuntimeService mobile web bundle capability', () => {
  afterEach(() => {
    resetBundledMobileWebBundleCacheForTests()
  })

  // The mixed-version guarantee: a dev tree or an `kingu serve` install that never built
  ...
  it('omits the mobile web bundle capability where the install carries no bundle', () => {
    withInstallRoot('kingu-mobile-web-absent-', (install) => {
  ...
  it('advertises the mobile web bundle capability where the install carries one', () => {
    withInstallRoot('kingu-mobile-web-present-', (install) => {
  ...
  it('omits the capability where the bundle manifest does not parse', () => {
    withInstallRoot('kingu-mobile-web-unparseable-', (install) => {
