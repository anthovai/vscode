...
describe('KinguRuntimeService', () => {
  ...
  it('matches canonical bare agent commands when a command override is configured', async () => {
    ...
    const runtime = new KinguRuntimeService(runtimeStore)
    runtime.setPtyController({
      spawn,
      write: () => true,
      kill: () => true,
  ...
  it('keeps non-bare agent command terminal creates unchanged', async () => {
    ...
    expect(spawnCall?.env?.KINGU_AGENT_LAUNCH_TOKEN).toBeUndefined()
  })
  ...
  it('injects runtime hook receiver env into terminal sessions', async () => {
    ...
    const runtime = new KinguRuntimeService(store, undefined, {
      buildAgentHookPtyEnv: () => ({
        KINGU_AGENT_HOOK_PORT: '5678',
        KINGU_AGENT_HOOK_TOKEN: 'agent-token',
        KINGU_AGENT_HOOK_ENV: 'remote',
    ...
    await runtime.createTerminal(`path:${TEST_WORKTREE_PATH}`, {
      ...
      env: {
        ...
        KINGU_AGENT_HOOK_TRANSPORT: 'stale-transport',
    ...
    expect(spawnCall?.env).toEqual(
      expect.objectContaining({
        ...
        KINGU_AGENT_HOOK_ENV: 'remote',
        KINGU_AGENT_HOOK_VERSION: '1',
        KINGU_PANE_KEY: expect.any(String),
        KINGU_TAB_ID: expect.any(String),
        KINGU_WORKTREE_ID: TEST_WORKTREE_ID
      })
    )
    expect(spawnCall?.env?.KINGU_AGENT_HOOK_ENDPOINT).toBeUndefined()
    expect(spawnCall?.env?.KINGU_AGENT_HOOK_TRANSPORT).toBeUndefined()
    expect(spawnCall?.envToDelete).toEqual(['CODEX_HOME', 'KINGU_CODEX_HOME'])
  })
  ...
  it.each([
    ...
    const folderPath = await mkdtemp(join(tmpdir(), 'kingu-runtime-folder-workspace-'))
    ...
    expect(spawnedEnv.KINGU_WORKSPACE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
    expect(spawnedEnv.KINGU_PROJECT_GROUP_ID).toBe(TEST_FOLDER_PROJECT_GROUP_ID)
    expect(spawnedEnv.KINGU_WORKSPACE_ROOT).toBe(folderPath)
    expect(spawnedEnv.KINGU_WORKTREE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
  })
  ...
  it.each([
    ...
    expect(spawnCall?.env?.KINGU_WORKTREE_ID).toBe(FLOATING_TERMINAL_WORKTREE_ID)
    expect(spawnCall?.env?.KINGU_WORKSPACE_ID).toBeUndefined()
    expect(spawnCall?.env?.KINGU_PROJECT_GROUP_ID).toBeUndefined()
    expect(spawnCall?.env?.KINGU_WORKSPACE_ROOT).toBeUndefined()
  })

  it('rejects folder workspace terminal creation when the backing path is missing', async () => {
    const missingPath = join(tmpdir(), `kingu-missing-folder-workspace-${randomUUID()}`)
    const spawn = vi.fn().mockResolvedValue({ id: 'pty-folder' })
  ...
  it('rejects folder workspace folderPath updates when the new path is missing', async () => {
    const missingPath = join(tmpdir(), `kingu-missing-folder-update-${randomUUID()}`)
    const folderWorkspace = makeFolderWorkspace()
    const runtimeStore = {
      ...createFolderWorkspaceRuntimeStore(folderWorkspace),
      updateFolderWorkspace: vi.fn()
    }
    const runtime = new KinguRuntimeService(runtimeStore as never)
  ...
  it('reveals Claude Agent Teams launches with the rewritten launch config', async () => {
    ...
    const spawnedEnv = spawnCall?.env ?? {}
    const spawnedLeafId = spawnedEnv.KINGU_PANE_KEY.slice(`${spawnedEnv.KINGU_TAB_ID}:`.length)
    expect(revealTerminalSession).toHaveBeenCalledWith(TEST_WORKTREE_ID, {
      ...
      launchToken: spawnedEnv.KINGU_AGENT_LAUNCH_TOKEN,
      launchAgent: 'claude',
      activate: false,
      tabId: spawnedEnv.KINGU_TAB_ID,
