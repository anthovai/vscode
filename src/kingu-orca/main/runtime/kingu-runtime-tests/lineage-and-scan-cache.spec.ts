...
describe('KinguRuntimeService', () => {
  ...
  it('rejects SSH lineage updates when Kingu worktree identity is missing', async () => {
    const remoteRepo = {
      id: 'remote-repo',
      path: '/home/user/repo',
      displayName: 'remote',
  ...
  it('rejects local lineage updates when Kingu worktree identity is missing', async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'kingu-runtime-lineage-'))
    ...
    try {
      ...
      await expect(lstat(join(childPath, '.kingu'))).rejects.toThrow()
      await expect(lstat(join(parentPath, '.kingu'))).rejects.toThrow()
      expect(setWorktreeLineage).not.toHaveBeenCalled()
    } finally {
