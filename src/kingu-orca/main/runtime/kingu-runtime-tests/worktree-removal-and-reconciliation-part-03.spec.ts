...
import { createWorktreeRemovalRuntime } from '../kingu-runtime-test-scenario-builders.spec'
...
describe('KinguRuntimeService', () => {
  ...
  it('treats forced runtime deletion of an already-missing unregistered worktree as cleanup', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'kingu-runtime-remove-'))
  ...
  it('routes already-missing SSH runtime history cleanup through the PTY owner', async () => {
    ...
    const metaById: Record<string, WorktreeMeta> = {
      [worktreeId]: makeWorktreeMeta({ hostId: 'ssh:ssh-1', kinguCreationSource: 'ssh' })
    }
    ...
    const runtime = new KinguRuntimeService(runtimeStore as never, undefined, {
  ...
  it('force-removes a legacy Kingu-created runtime orphaned worktree directory after Git tracking is gone', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'kingu-runtime-orphan-'))
  ...
  it('prompts then force-removes an Kingu-created runtime unregistered leftover directory with no git marker', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'kingu-runtime-leftover-'))
    const repoPath = join(parentDir, 'repo')
  ...
  it('rejects an Kingu-created runtime unregistered local directory with a git directory', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'kingu-runtime-standalone-'))
    const repoPath = join(parentDir, 'repo')
