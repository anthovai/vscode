...
import { KinguRuntimeService } from '../kingu-runtime-test-mocks.spec'
 ...
 * row the user dismissed on the desktop stayed in `kingu worktree ps` and on the phone until the
...
function wiredRuntime(incarnationId?: string): {
  ...
  const runtime = new KinguRuntimeService(store, undefined, statusWiring.deps)
...
function emitWorkingStatus(runtime: KinguRuntimeService, sequence: number): void {
  runtime.onPtyData(
    'dismiss-pty',
