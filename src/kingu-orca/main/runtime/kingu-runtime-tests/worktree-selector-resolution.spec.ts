...
describe('KinguRuntimeService', () => {
    ...
    vi.mocked(listWorktrees).mockResolvedValueOnce([
      {
        ...
        head: 'abc',
        branch: 'refs/heads/wolfiesch/kingu-skill-smoke',
        isBare: false,
        isMainWorktree: false
      }
    ])
  ...
  it.each([
    ...
    const folderPath = await mkdtemp(join(tmpdir(), 'kingu-runtime-folder-files-'))
  ...
  it('guides registered SSH repo ids without probing the provider', async () => {
    ...
    const runtime = new KinguRuntimeService(remoteStore)
  ...
  it('retires a generated name before a post-create listing failure', async () => {
    const addRetiredWorktreeName = vi.fn()
    const runtime = new KinguRuntimeService({ ...store, addRetiredWorktreeName })
