...
> = {
  permission_denied: {
    ...
    fallback:
      "Kingu couldn't write its runtime file. Check permissions on Kingu's data folder, then restart."
  },
  storage_unavailable: {
    key: 'runtimeRpc.startupFailure.guidance.storageUnavailable',
    fallback: 'Your disk may be full or read-only. Free up space, then restart Kingu.'
  },
  invalid_path: {
    key: 'runtimeRpc.startupFailure.guidance.invalidPath',
    fallback:
      "Kingu's data folder may be missing, moved, or at a path that is too long. Restore it or use a shorter path, then restart Kingu."
  },
  address_in_use: {
    key: 'runtimeRpc.startupFailure.guidance.addressInUse',
    fallback: 'Another process may be holding the port. Restart Kingu to try again.'
  },
  unknown: {
    key: 'runtimeRpc.startupFailure.guidance.unknown',
    fallback: 'Restart Kingu to try again.'
  }
}
...
function createRuntimeRpcStartupFailureDialogOptions(error: unknown): MessageBoxOptions {
  ...
  return {
    ...
    cancelId: 0,
    noLink: true,
    title: translateMain('runtimeRpc.startupFailure.title', 'Kingu CLI unavailable'),
    message: translateMain(
      'runtimeRpc.startupFailure.message',
      "Kingu couldn't start its local command transport."
    ),
