...
describe('claude agent teams shim env', () => {
  it('writes a private tmux shim that calls the Kingu shim command', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kingu-agent-teams-shim-'))
    roots.push(root)
  ...
  it('builds native shim env only for direct Claude commands', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kingu-agent-teams-cli-'))
    roots.push(root)
    const cliName = process.platform === 'win32' ? 'kingu-dev.cmd' : 'kingu-dev'
  ...
  it('refuses to resolve a CLI through relative PATH entries or a bare override', async () => {
    const root = await mkdtemp(join(tmpdir(), 'kingu-agent-teams-cli-'))
    roots.push(root)
    for (const name of ['kingu', 'kingu-ide', 'kingu.cmd']) {
    ...
    expect(resolveClaudeAgentTeamsShimBin({ PATH: '' })).toBeNull()
    expect(
      resolveClaudeAgentTeamsShimBin({ PATH: '.', KINGU_AGENT_TEAMS_SHIM_BIN: 'kingu' })
    ).toBeNull()
    // Why: a bare override is still honored when it maps to a real absolute PATH entry.
    expect(
      resolveClaudeAgentTeamsShimBin({ PATH: root, KINGU_AGENT_TEAMS_SHIM_BIN: 'kingu' })
    ).toBe(join(root, 'kingu'))
  })
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'kingu-agent-teams-cli-'))
      roots.push(root)
      const cliPath = join(root, 'kingu.cmd')
  ...
  it.skipIf(process.platform === 'win32')(
    'never runs a cwd-resolved kingu when the shim bin is unqualified',
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'kingu-agent-teams-shim-'))
      roots.push(root)
      await ensureClaudeAgentTeamsShimDir(root)
      const cwd = await mkdtemp(join(tmpdir(), 'kingu-agent-teams-cwd-'))
      roots.push(cwd)
      const marker = join(cwd, 'hijacked')
      for (const name of ['kingu', 'kingu-ide']) {
  ...
  it('writes a Windows shim that rejects an unqualified shim bin', () => {
    ...
    expect(script).not.toMatch(/^set "KINGU_AGENT_TEAMS_SHIM_BIN=kingu/m)
    expect(script).toContain('if "%KINGU_SHIM_BIN:~1,1%"==":" goto :run')
