...
import { KinguRuntimeWithBuildHeadlessMobileSessionBrowserTabs } from './kingu-runtime-build-headless-mobile-session-browser-tabs'
...
export class KinguRuntimeWithPerformMobileSessionPtyRecordsRefresh extends KinguRuntimeWithBuildHeadlessMobileSessionBrowserTabs {
    ...
    if (tab.type === 'terminal') {
      ...
      if (shouldMaterializePendingTerminal) {
        ...
        let agentStartup: Awaited<
          ReturnType<KinguRuntimeService['resolveMobileSessionTerminalCommand']>
        > = {}
        if (tab.launchAgent) {
          try {
