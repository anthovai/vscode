...
import { classifyRuntimeLongPoll, KinguRuntimeRpcServer } from './runtime-rpc'
...
describe('runtime RPC browser-host admission', () => {
  ...
  it('reserves wait capacity and releases host admission on socket close', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'kingu-browser-host-admission-'))
    const aborted = vi.fn()
  ...
  it('reserves browser-host capacity for a second paired device', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'kingu-browser-host-fairness-'))
  ...
  it('keeps a wait slot when asks and browser hosts fill their shared budget', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'kingu-browser-host-wait-reserve-'))
