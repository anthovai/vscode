...
export class RuntimeRepositoryIssueCommand {
  ...
  async write(repoSelector: string, content: string): Promise<{ ok: true }> {
    ...
    if (!(await isIssueCommandIgnoredByGit(repo.path, repo.connectionId))) {
      await ensureRemoteKinguDirIgnored(fsProvider, repo.path)
    }
...
async function readRemoteShared(
  ...
  try {
    const result = await fsProvider.readFile(joinWorktreeRelativePath(repoPath, 'kingu.yaml'))
    return result.isBinary ? null : parseKinguYaml(result.content)?.issueCommand?.trim() || null
  } catch {
    return null
  }
}

async function ensureRemoteKinguDirIgnored(
  fsProvider: IFilesystemProvider,
  repoPath: string
): Promise<void> {
  ...
  try {
    ...
    if (!isENOENT(error)) {
      console.warn('[runtime] Could not inspect remote .gitignore for .kingu', error)
      return
    }
    try {
      await fsProvider.writeFile(gitignorePath, '.kingu\n')
    } catch (writeError) {
      console.warn('[runtime] Could not update remote .gitignore to exclude .kingu', writeError)
    }
    return
  }
