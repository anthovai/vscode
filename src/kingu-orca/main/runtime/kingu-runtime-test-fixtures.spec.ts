...
import { InMemoryOrchestrationMessages } from './kingu-runtime-test-orchestration-messages.spec'
import type { OrchestrationDb } from './orchestration/db'
import type { PtyProcessInspection } from '../providers/pty-process-inspection'

type RuntimeService = InstanceType<typeof KinguRuntimeService>
...
function expectStablePaneKeyEnv(env: Record<string, string>): string {
  expect(env.KINGU_TAB_ID).toMatch(UUID_RE)
  const leafId = env.KINGU_PANE_KEY?.slice(`${env.KINGU_TAB_ID}:`.length)
  expect(leafId).toMatch(UUID_RE)
  expect(env.KINGU_PANE_KEY).toBe(`${env.KINGU_TAB_ID}:${leafId}`)
  return env.KINGU_PANE_KEY
}

function createRuntime(): RuntimeService {
  return new KinguRuntimeService(store)
}
