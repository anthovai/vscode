 ...
 * the data root came from somewhere else — a relocated `KINGU_USER_DATA_PATH`, a share, a roaming
...
function canDenyReads(): boolean {
  ...
  const probeRoot = mkdtempSync(join(tmpdir(), 'kingu-deny-probe-'))
...
describeOnWindows('a secure store that exists but cannot be read', () => {
  ...
  beforeAll(() => {
    root = mkdtempSync(join(tmpdir(), 'kingu-unreadable-'))
  })
   ...
   * session used to fall past the `status === 'found'` guard into `clearKinguCloudSession`.
   */
  it('does not delete the account session it could not read', async () => {
    ...
    const { readKinguCloudSession, getKinguCloudSessionPath } =
      await import('./../kingu-profiles/profile-cloud-session-store')
    const dir = join(root, 'profiles')
    mkdirSync(dir, { recursive: true })
    const filePath = getKinguCloudSessionPath('profile-1', dir)
    mkdirSync(join(filePath, '..'), { recursive: true })
    ...
    expect(readKinguCloudSession('profile-1', dir).status).toBe('unreadable')
