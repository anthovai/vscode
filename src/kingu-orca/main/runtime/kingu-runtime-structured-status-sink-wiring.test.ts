...
type KinguRuntimeDeps = NonNullable<ConstructorParameters<typeof KinguRuntimeService>[2]>
...
] as const satisfies readonly (keyof KinguRuntimeDeps)[]

const MAIN_ROOT = join(import.meta.dirname, '..')

/** The text of the `new KinguRuntimeService(...)` call in one entry point. */
function runtimeConstruction(relativePath: string): string {
  const source = readFileSync(join(MAIN_ROOT, relativePath), 'utf8')
  const start = source.indexOf('new KinguRuntimeService(')
  ...
  throw new Error(`unbalanced KinguRuntimeService construction in ${relativePath}`)
}
 ...
 *  constructs a runtime without these lists no agents at all — and `kingud` serves `worktree.ps`
 *  and `agentSession.*` exactly like the desktop does. */
describe('every host that constructs a runtime wires the agent-status store', () => {
  it.each([['kingud/kingud-entry.ts'], ['startup/main-process-runtime-service.ts']])(
...
describe('structured status sink wiring', () => {
  it('hands the host the sink the runtime was constructed with', async () => {
    ...
    const runtime = new KinguRuntimeService(null, undefined, { structuredAgentStatusSink: sink })
