...
describe('orchestration compatibility runtime authority', () => {
  ...
  it('uses the hydrated hook commitment for a restored exact PTY', () => {
    ...
    const uncommitted = new KinguRuntimeService()
  ...
  it('attests current coordinators immediately without upgrading restored legacy callers', () => {
    ...
    const createIntegratedRuntime = (launchTokenHash: string | null): KinguRuntimeService => {
      const runtime = new KinguRuntimeService(null, undefined, {
    ...
    const evidenceFor = (runtime: KinguRuntimeService, launchToken = TOKEN) => ({
      terminalHandle: 'term-1',
      paneKey: PANE_KEY,
      launchToken,
