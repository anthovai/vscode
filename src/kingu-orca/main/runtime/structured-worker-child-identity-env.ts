 ...
 * to speak for itself.
 *
 * Without `KINGU_TERMINAL_HANDLE` the worker's Bash tool has nothing to pass as `--from`, and
 ...
 *
 * `KINGU_CLI_COMMAND: 'kingu'` is honest ONLY because of the PATH prepend below. Kingu's Linux CLI
 * installs as `kingu-ide` so it never claims GNOME Kingu's /usr/bin/kingu (anthovai/kingu-intelligence#7904), and
 ...
 *
 * Deliberately NOT `KINGU_PANE_KEY`. Claude structured sessions run hooks, and a pane key in their
 ...
 *
 * A session that is not a dispatched worker gets ONE variable, `KINGU_STRUCTURED_SESSION`, and it
 ...
 * — which is why it is a different decision from withholding `KINGU_PANE_KEY`, not a reversal of it.
...
export function structuredWorkerChildIdentityEnv(
  ...
  if (!identity) {
    return { ...childEnv, [KINGU_STRUCTURED_SESSION_ENV]: '1' }
  }
  const env: Record<string, string> = {
    ...childEnv,
    KINGU_TERMINAL_HANDLE: identity.handle,
    KINGU_CLI_COMMAND: 'kingu'
  }
  applyKinguCliPath(env)
  return env
}
 ...
 */
function applyKinguCliPath(env: Record<string, string>): void {
