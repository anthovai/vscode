// #11761: on a headless `kingu serve` host the HTTP agent hook is the only carrier of
...
async function projectAgentStatus(
  rows: AgentStatusIpcPayload[],
  preparePane?: (runtime: KinguRuntimeService) => void
...
function observePaneTitle(runtime: KinguRuntimeService, title: string): void {
  runtime.onPtyData(PTY_ID, `\x1b]0;${title}\x07`, Date.now())
}

function lastOscTitleEpochMs(runtime: KinguRuntimeService): number {
  const pty = (
