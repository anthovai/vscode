...
const { browserSessionRegistryMock } = vi.hoisted(() => ({
  browserSessionRegistryMock: {
    getDefaultProfile: () => ({ id: 'default', partition: 'persist:kingu-browser' }),
    getProfile: () => ({ id: 'default', partition: 'persist:kingu-browser' }),
    resolveKnownPartition: () => 'persist:kingu-browser'
  }
}))
 ...
 */
async function seedSelections(runtime: KinguRuntimeService): Promise<void> {
