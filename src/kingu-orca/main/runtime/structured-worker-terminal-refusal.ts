...
export function structuredWorkerTerminalRefusal(
  ...
  const error = new Error(
    `${handle} is an agent session, not a terminal, so terminal commands cannot address it. ` +
      'Read its output with `kingu terminal read` or `kingu orchestration worker-read --source transcript`, ' +
      'send it work with `kingu orchestration send`, and open it from its chat tab.'
  )
