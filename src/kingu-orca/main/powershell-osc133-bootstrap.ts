 ...
 */
const POWERSHELL_OSC133_BOOTSTRAP = `# Kingu OSC 133 shell integration for PowerShell.
...
if ($env:KINGU_OPENCODE_CONFIG_DIR) { $env:OPENCODE_CONFIG_DIR = $env:KINGU_OPENCODE_CONFIG_DIR }
if ($env:KINGU_MIMOCODE_HOME) { $env:MIMOCODE_HOME = $env:KINGU_MIMOCODE_HOME }
if ($env:KINGU_CODEX_HOME) { $env:CODEX_HOME = $env:KINGU_CODEX_HOME }

if ($ExecutionContext.SessionState.LanguageMode -eq "FullLanguage" -and
    ((-not (Test-Path variable:global:__KinguOsc133State)) -or
     $null -eq $Global:__KinguOsc133State.OriginalPrompt)) {
...
${getPowerShellCodexShellLaunchPreflight()}
    ...
    function Global:prompt {
        ...
        if ($Global:__KinguOsc133State.HasSeenPrompt) {
            $result += "$($Global:__KinguOsc133State.Esc)]133;D;$fakeExitCode$($Global:__KinguOsc133State.Bel)"
        }
        $Global:__KinguOsc133State.HasSeenPrompt = $true

        $result += "$($Global:__KinguOsc133State.Esc)]133;A$($Global:__KinguOsc133State.Bel)"
        ...
        $result += $Global:__KinguOsc133State.OriginalPrompt.Invoke()
        $result += "$($Global:__KinguOsc133State.Esc)]133;B$($Global:__KinguOsc133State.Bel)"
        $result
    }

    if ($Global:__KinguOsc133State.HasPSReadLine -and
        $null -ne $Global:__KinguOsc133State.OriginalReadLine) {
        function Global:PSConsoleHostReadLine {
            $commandLine = $Global:__KinguOsc133State.OriginalReadLine.Invoke()
            [Console]::Write("$($Global:__KinguOsc133State.Esc)]133;C$($Global:__KinguOsc133State.Bel)")
            return $commandLine
        }
    }
}
