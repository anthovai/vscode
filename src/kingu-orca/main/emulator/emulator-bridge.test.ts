...
import { RuntimeEmulatorCommands } from '../runtime/kingu-runtime-emulator'
...
describe('EmulatorBridge helper ownership', () => {
  ...
  it('stops the previous Kingu-managed helper when a worktree switches devices', async () => {
  ...
  it('shuts down the previous Kingu-managed device when requested', async () => {
    const bridge = new EmulatorBridge()
    bridge.registerActiveEmulator('wt-1', session('device-old'), { managed: true })
  ...
  it('only kills Kingu-managed helpers during app shutdown cleanup', async () => {
    const bridge = new EmulatorBridge()
