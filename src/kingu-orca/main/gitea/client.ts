...
function getAuthConfig(): GiteaAuthConfig {
  const apiBaseUrl = envValue('KINGU_GITEA_API_BASE_URL')
  return {
    apiBaseUrl: apiBaseUrl ? normalizeGiteaApiBaseUrl(apiBaseUrl) : null,
    token: envValue('KINGU_GITEA_TOKEN')
  }
}
...
/** Invalidate the shared /pulls scan after Kingu itself creates a PR so the
