...
describe('GitHub GraphQL rate-limit guard', () => {
  ...
  it('hydrates GitHub-registered stack metadata for exact linked PRs', async () => {
    getOwnerRepoMock.mockResolvedValue({ owner: 'anthovai', repo: 'kingu', host: 'github.com' })
    ghExecFileAsyncMock
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          title: 'Stack API',
          state: 'OPEN',
          url: 'https://github.com/anthovai/kingu-intelligence/pull/202',
          statusCheckRollup: [],
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          title: 'Stack API',
          state: 'open',
          html_url: 'https://github.com/anthovai/kingu-intelligence/pull/202',
                    ...
                    nodes: [
                      {
                        ...
                        pullRequest: {
                          number: 201,
                          title: 'Stack models',
                          url: 'https://github.com/anthovai/kingu-intelligence/pull/201',
                          state: 'OPEN',
                          isDraft: false,
                          mergeable: 'MERGEABLE',
  ...
  it.each([
    ...
    const mergeCall = ghExecFileAsyncMock.mock.calls.find(([args]) =>
      args.includes('repos/anthovai/kingu-intelligence/pulls/202/merge-async')
    )
    expect(mergeCall?.[0]).toEqual(
      expect.arrayContaining([
        'PUT',
        'repos/anthovai/kingu-intelligence/pulls/202/merge-async',
  ...
  it.each([
    ...
    expect
      ...
      .toEqual([['api', 'repos/anthovai/kingu-intelligence/pulls/202']])
    ...
    expect
      ...
      .toHaveBeenCalledWith(
        'mergePR stack metadata probe failed for anthovai/kingu-intelligence#202:',
  ...
  it.each([
    ...
    ghExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          title: 'Fail closed on unavailable stack metadata',
          state: 'OPEN',
          url: 'https://github.com/anthovai/kingu-intelligence/pull/13866',
          statusCheckRollup: [],
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      3,
      ['pr', 'merge', '13866', '--squash', '--repo', 'anthovai/kingu-intelligence'],
  ...
  it('keeps legacy merge for unregistered dependent PR chains', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      3,
      ['pr', 'merge', '202', '--squash', '--repo', 'anthovai/kingu-intelligence'],
      expect.objectContaining({ env: expect.objectContaining({ GH_PROMPT_DISABLED: '1' }) })
    )
  })
})
