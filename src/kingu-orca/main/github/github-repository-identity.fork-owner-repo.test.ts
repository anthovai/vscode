...
const REMOTE_URLS_BY_REPO: Record<string, Record<string, string>> = {
  [FORK_PATH]: {
    origin: 'https://github.com/fsdwen/kingu.git',
    upstream: 'https://github.com/anthovai/kingu-intelligence.git'
  },
  [NON_FORK_PATH]: {
    origin: 'https://github.com/anthovai/kingu-intelligence.git'
  },
  [SSH_FORK_PATH]: {
    origin: 'git@github.com:fsdwen/kingu.git',
...
describe('issue #7331: fork PR owner/repo resolution', () => {
  it('getOwnerRepo prefers the upstream parent on a fork checkout', async () => {
    ...
    // getIssueOwnerRepo).
    expect(prRepo).toEqual({ owner: 'anthovai', repo: 'kingu' })
  })
  ...
  it('getOwnerRepoForRemote(origin) still resolves the fork itself', async () => {
    ...
    expect(origin).toEqual({ owner: 'fsdwen', repo: 'kingu' })
  })
  ...
  it('getRepoUpstream still resolves the fork parent offline (regression)', async () => {
    ...
    expect(upstream).toEqual({ owner: 'anthovai', repo: 'kingu', host: 'github.com' })
