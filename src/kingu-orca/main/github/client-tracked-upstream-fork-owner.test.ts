...
describe('getPRForBranch', () => {
  ...
  it('uses the tracked upstream remote owner for fork branch lookup', async () => {
    ...
    ghExecFileAsyncMock
        ...
        stdout: JSON.stringify([
          {
            ...
            state: 'open',
            html_url: 'https://github.com/anthovai/kingu-intelligence/pull/78',
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          state: 'OPEN',
          url: 'https://github.com/anthovai/kingu-intelligence/pull/78',
          statusCheckRollup: [],
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        'api',
        'repos/anthovai/kingu-intelligence/pulls?head=fork-owner%3Acontributor%2Foriginal&state=all&per_page=1'
      ],
      { cwd: '/repo-root' }
    )
    expect(pr).toMatchObject({
      ...
      headRepo: { owner: 'fork-owner', repo: 'kingu' }
    })
  })
  ...
  it('uses the tracked upstream remote owner when the fork branch name matches locally', async () => {
    ...
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'brennanb2025', repo: 'kingu' })
    ghExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: JSON.stringify([]) })
      .mockResolvedValueOnce({ stdout: JSON.stringify([]) })
      .mockResolvedValueOnce({
        stdout: JSON.stringify([
          {
            ...
            state: 'open',
            html_url: 'https://github.com/anthovai/kingu-intelligence/pull/6433',
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          title: 'Recover Windows worktree deletes from long paths',
          state: 'OPEN',
          url: 'https://github.com/anthovai/kingu-intelligence/pull/6433',
          statusCheckRollup: [],
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        'api',
        'repos/anthovai/kingu-intelligence/pulls?head=brennanb2025%3Abrennanb2025%2Fworktree-remove-fix&state=all&per_page=1'
      ],
      { cwd: '/repo-root' }
    )
    expect(pr).toMatchObject({
      number: 6433,
      prRepo: { owner: 'anthovai', repo: 'kingu' },
      headRepo: { owner: 'brennanb2025', repo: 'kingu' }
    })
  })
  ...
  it('uses the same-name tracked upstream fork owner through the SSH git provider', async () => {
        ...
        stdout: JSON.stringify([
          {
            ...
            state: 'open',
            html_url: 'https://github.com/anthovai/kingu-intelligence/pull/79',
            updated_at: '2026-03-28T00:00:00Z',
            draft: false,
            mergeable: true,
            base: { ref: 'main', sha: 'base-oid' },
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        'api',
        'repos/anthovai/kingu-intelligence/pulls?head=fork-owner%3Acontributor%2Ffix&state=all&per_page=1'
      ],
      {}
    )
    expect(pr).toMatchObject({
