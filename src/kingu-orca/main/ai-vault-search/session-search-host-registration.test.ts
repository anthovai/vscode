...
it.each([
  ...
  ['kingud', 'src/main/kingud/kingud-session-search.ts', 'installInProcessSessionSearchService'],
  [
    'the relay daemon',
...
it('kingud resolves no roots while disabled and discovers late roots when enabled', async () => {
  const { installKingudSessionSearchService } = await import('../kingud/kingud-session-search')
  installed = await installKingudSessionSearchService({
...
// kingud reaches the index through the deps hook the runtime RPC calls; the wiring is
// what no unit of either module can show.
it('wires kingud consent from the runtime hook to the installed service', () => {
  const source = readFileSync(join(ROOT, 'src/main/kingud/kingud-entry.ts'), 'utf8')
