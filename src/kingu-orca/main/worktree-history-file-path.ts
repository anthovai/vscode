/**
 * Recognises a `HISTFILE` value Kingu itself minted for a worktree.
 *
 * Why: HISTFILE is EXPORTED into the pane, so an Kingu launched from an Kingu
 ...
 * only a value Kingu can prove it minted is dropped, so a HISTFILE the user set
 ...
 *
 * Why the shape is enough without an Kingu-specific token: two of the three
 * shapes carry one already (`.kingu-remote`, `terminal-history-wsl`), and the
 ...
 * is dropped from ONE spawn's env, so the pane gets Kingu's own worktree history
...
// shape is the user's, not Kingu's.
const KINGU_MINTED_HISTFILE = new RegExp(
  '/(?:' +
    ...
    '|' +
    // Relay: ~/.kingu-remote/terminal-history/<hash>-<file>
    `\\.kingu-remote/terminal-history/${WORKTREE_HASH}-${HISTORY_FILE}` +
    ')$'
)

export function isKinguMintedHistFile(value: string | undefined): boolean {
  return typeof value === 'string' && KINGU_MINTED_HISTFILE.test(value.replace(/\\/g, '/'))
}

/** Drop a `HISTFILE` this process inherited from an outer Kingu pane. */
export function dropInheritedKinguHistFile(env: Record<string, string | undefined>): void {
  if (isKinguMintedHistFile(env.HISTFILE)) {
    delete env.HISTFILE
  }
}
