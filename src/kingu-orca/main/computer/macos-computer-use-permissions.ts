...
const DEFAULT_COMPUTER_USE_BUNDLE_ID = 'com.anthovai.kingu.computer-use'
...
async function openComputerUsePermissionsAsync(
  ...
  if (!helperAppPath) {
    throw new RuntimeClientError('accessibility_error', 'Kingu Computer Use.app was not found')
  }
...
function closeExistingPermissionHelpers(): void {
  ...
  const setupHelperPatterns = [
    'kingu-computer-use-macos[[:space:]]+--permission([[:space:]]|$)',
    'kingu-computer-use-macos[[:space:]]+--permissions([[:space:]]|$)'
  ]
...
function nextPermissionStep(
  ...
  return `Grant ${missing.id === 'accessibility' ? 'Accessibility' : 'Screen Recording'} to Kingu Computer Use, then retry get-app-state.`
}
