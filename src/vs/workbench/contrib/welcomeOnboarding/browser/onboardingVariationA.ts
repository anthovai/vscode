...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	show(): void {
		...
		this.overlay.setAttribute('aria-label', localize('onboarding.a.aria', "Welcome to Kingu"));
		...
		if (this.footerLeft) {
			if (this._isLastStep()) {
				...
				if (!this._footerSignInBtn && !this._userSignedIn) {
					...
					this._footerSignInBtn.textContent = localize('onboarding.sessions.signInNudge', "Sign in to use Arkai");
	...
	private _renderSignInStep(container: HTMLElement): void {
		...
		title.textContent = localize('onboarding.signIn.heroTitle', "Welcome to Kingu");

		const subtitle = append(contentMain, $('p.onboarding-a-signin-subtitle'));
		subtitle.textContent = localize('onboarding.signIn.heroSubtitle', "Sign in to use Arkai.");
		...
		copilotDisclaimer.append(localize('onboarding.signIn.disclaimer.copilotPrefix', ". {0} Arkai may show ", defaultChat.provider.default.name));
	...
	private _renderPersonalizeSubtitle(container: HTMLElement): void {
		...
		container.append(
			...
			localize('onboarding.personalize.tip.suffix', " to access all Kingu commands."),
		);
	}
