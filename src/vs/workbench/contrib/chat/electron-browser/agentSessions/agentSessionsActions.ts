...
export class AgentsHandoffInputTipContribution extends Disposable implements IWorkbenchContribution {
	...
	private _update(): void {
		...
		const message = this._titleTreatment ?? (useEmptyWorkspaceCopy
			? localize('chat.agentsHandoff.tip.emptyWorkspace.message', "Arkai isn't available without an open folder")
		...
		const description = this._descriptionTreatment ?? (useEmptyWorkspaceCopy
			? localize('chat.agentsHandoff.tip.emptyWorkspace.description', "Open the Agents Window to start an Arkai session.")
			: mode === AgentsHandoffTipMode.Custom
				? localize('chat.agentsHandoff.tip.description.copilot', "Free with your Arkai plan — get a dedicated, multi-pane view alongside your workspace.")
