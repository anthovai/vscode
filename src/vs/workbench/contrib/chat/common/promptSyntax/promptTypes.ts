...
export function getSourceDescription(source: PromptFileSource): string | undefined {
	switch (source) {
		...
		case PromptFileSource.GitHubWorkspace:
			return localize('source.githubWorkspace', "Workspace (only used by Arkai agents)");
		case PromptFileSource.CopilotPersonal:
			return localize('source.copilotPersonal', "Global (only used by Arkai agents)");
		...
		case PromptFileSource.UserData:
			return localize('source.userData', "Global (roams with Settings Sync, only used by Kingu)");
