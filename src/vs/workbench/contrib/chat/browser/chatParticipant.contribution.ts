...
export class ChatCompatibilityNotifier extends Disposable implements IWorkbenchContribution {
	...
	private registerWelcomeView(chatExtension: IExtension) {
		...
		const mainMessage = localize('chatFailErrorMessage', "Chat failed to load because the installed version of the Arkai Chat extension is not compatible with this version of {0}. Please ensure that the Arkai Chat extension is up to date.", this.productService.nameLong);
