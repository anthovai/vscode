...
export function getMcpCompatibilityDetail(reason: AgentHostMcpSupportReason): string {
	switch (reason) {
		...
		case AgentHostMcpSupportReason.SandboxConfigurationIgnored:
			return localize('mcpCompatibilitySandboxConfigurationIgnored', "Per-server sandbox settings are not supported by the Arkai harness.\nTo migrate this server, remove the server sandbox setting.");
		case AgentHostMcpSupportReason.DevelopmentModeIgnored:
			return localize('mcpCompatibilityDevelopmentModeIgnored', "MCP development mode is not supported by the Arkai harness.\nTo migrate this server, remove the development mode setting and restart the server manually after configuration changes.");
		case AgentHostMcpSupportReason.OAuthClientConfigurationIgnored:
			return localize('mcpCompatibilityOAuthClientConfigurationIgnored', "Custom OAuth client configuration is not supported by the Arkai harness.\nTo migrate this server, remove the custom OAuth client configuration and sign in when the Arkai harness prompts for authentication.");
