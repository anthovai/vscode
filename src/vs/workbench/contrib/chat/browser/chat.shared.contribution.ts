...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		[ChatConfiguration.PermissionsSandboxToggleEnabled]: {
			type: 'boolean',
			default: true,
			markdownDescription: nls.localize('chat.experimental.permissionsSandboxToggle.enabled', "Controls whether the permissions picker shows a \"Sandboxing for terminal\" toggle. Local sessions show it on the Default permissions option. Local and remote Arkai Agent Host sessions show it as a session-specific setting that applies to every permission mode; changing it saves the choice only for that session. New Arkai Agent Host sessions initially follow `#chat.agent.sandbox.enabled#` or `#chat.agent.sandbox.enabledWindows#` based on the host's operating system, for both the SDK's built-in shell tool and the Agent Host terminal tool."),
			tags: ['experimental'],
			experiment: {
				mode: 'auto'
			},
		...
		[CopilotTgrepEnabledSettingId]: {
			type: 'boolean',
			markdownDescription: nls.localize('chat.copilot.tgrep.enabled', "When enabled, local Arkai Agent Host sessions force-enable tgrep indexed search, bypassing the repository-size threshold. Requires a local Git repository on a non-virtual filesystem. Changing this setting restarts the Arkai SDK client after active turns finish."),
			default: false,
			experiment: { mode: 'auto' },
			tags: ['experimental', 'advanced'],
			scope: ConfigurationScope.APPLICATION,
		...
		[AgentHostSdkSandboxEnabledSettingId]: {
			...
			markdownDescription: nls.localize('chat.agentHost.sdkSandbox.enabled.description', "Deprecated sandbox mode for the Arkai SDK's built-in shell tool on macOS and Linux."),
		...
		[AgentHostSdkSandboxWindowsEnabledSettingId]: {
			type: 'string',
			enum: [AgentSandboxEnabledValue.Off, AgentSandboxEnabledValue.On],
			markdownDescription: nls.localize('chat.agentHost.sdkSandbox.enabledWindows.description', "Deprecated sandbox mode for the Arkai SDK's built-in shell tool on Windows."),
