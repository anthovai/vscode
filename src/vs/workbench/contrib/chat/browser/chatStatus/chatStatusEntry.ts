...
export class ChatStatusBarEntry extends Disposable implements IWorkbenchContribution {
	...
	private getEntryProps(): IStatusbarEntry {
		let text = '$(copilot)';
		let ariaLabel = localize('chatStatusAria', "Arkai status");
		...
		if (isNewUser(this.chatEntitlementService)) {
			...
			if (this.chatEntitlementService.sentiment.disabled || this.chatEntitlementService.sentiment.untrusted) {
				...
				ariaLabel = localize('copilotDisabledStatus', "Arkai disabled");
			}
			...
			// Copilot Resumed (limit reset after the user was previously blocked)
				...
				const resumedLabel = localize('chatResumedStatus', "Arkai Resumed");
		...
		const baseResult = {
			name: localize('chatStatus', "Arkai Status"),
			text,
			ariaLabel,
	...
	private getSetupEntryProps(): IStatusbarEntry {
		...
		return {
			...
			ariaLabel: showSignInLabel ? signInLabel : localize('chatStatusAria', "Arkai status"),
