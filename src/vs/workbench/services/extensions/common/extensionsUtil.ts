...
export function dedupExtensions(system: IExtensionDescription[], user: IExtensionDescription[], workspace: IExtensionDescription[], development: IExtensionDescription[], logService: ILogService): IExtensionDescription[] {
	...
	system.forEach((systemExtension) => {
		...
		if (extension) {
			logService.warn(localize('overwritingExtension', "Overwriting snap {0} with {1}.", extension.extensionLocation.fsPath, systemExtension.extensionLocation.fsPath));
		}
	...
	user.forEach((userExtension) => {
		...
		if (extension) {
			if (extension.isBuiltin) {
				...
				logService.warn(localize('overwritingExtension', "Overwriting snap {0} with {1}.", extension.extensionLocation.fsPath, userExtension.extensionLocation.fsPath));
			}
	...
	workspace.forEach(workspaceExtension => {
		...
		if (extension) {
			logService.warn(localize('overwritingWithWorkspaceExtension', "Overwriting {0} with Workspace Snap {1}.", extension.extensionLocation.fsPath, workspaceExtension.extensionLocation.fsPath));
		}
		result.set(workspaceExtension.identifier, workspaceExtension);
	});
	development.forEach(developedExtension => {
		logService.info(localize('extensionUnderDevelopment', "Loading development snap at {0}", developedExtension.extensionLocation.fsPath));
