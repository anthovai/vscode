...
export const copilotCliConfigSchema = createSchema({
	...
	[CopilotCliConfigKey.Tgrep]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.tgrep.description', "When enabled, Arkai SDK sessions force-enable tgrep indexed search, bypassing the repository-size threshold. Requires a local Git repository on a non-virtual filesystem."),
		default: false,
	}),
	...
	[CopilotCliConfigKey.ToolSearchEnabled]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.toolSearchEnabled.description', "When enabled, Arkai SDK sessions defer MCP and non-core Kingu tools behind a tool-search tool so the model discovers them on demand instead of loading every tool definition up front."),
		default: true,
	}),
		...
		additionalProperties: {
			...
			properties: {
				...
				availableTools: {
					...
					description: localize('agentHost.config.modelCapabilityOverrides.availableTools.description', "When set, only matching tools are available to sessions on this model. Patterns: bare tool names, `builtin:*` or `builtin:<name>` (Arkai runtime tools), `mcp:*` or `mcp:<name>` (MCP server tools), and `custom:*` or `custom:<name>` (every tool Kingu registers with the SDK, including the agent host's own terminal tools); a bare `*` expands to all three sources. Applied when the session launches or resumes."),
				},
				excludedTools: {
					type: 'array',
