...
public static class KinguDesktopWin32 {
...
function Assert-KinguProcessAllowed($Process) {
...
function Test-KinguBrowserProcess($Process) {
...
function Get-KinguRootElement($Process) {
...
function Get-KinguWindowFrame($Process, $RootElement) {
    $rect = New-Object KinguDesktopWin32+RECT
    if ([KinguDesktopWin32]::GetWindowRect([IntPtr]$Process.MainWindowHandle, [ref]$rect)) {
        return New-KinguFrame $rect.Left $rect.Top ($rect.Right - $rect.Left) ($rect.Bottom - $rect.Top)
    }

    try {
        $bounds = $RootElement.Current.BoundingRectangle
        if (-not $bounds.IsEmpty) {
            return New-KinguFrame $bounds.X $bounds.Y $bounds.Width $bounds.Height
        }
    } catch {}
    $null
}

function Get-KinguWindowId($Process) {
    [int64]$Process.MainWindowHandle
}

function Get-KinguAppName($Process) {
...
function New-KinguAppRecord($Process) {
    [pscustomobject]@{
        name = Get-KinguAppName $Process
...
function Assert-KinguWindowTarget($Process, $WindowId, $WindowIndex) {
    if ($null -ne $WindowIndex -and [int]$WindowIndex -ne 0) {
        throw "windowNotFound(`"$WindowIndex`")"
    }
    if ($null -ne $WindowId -and [int64]$WindowId -ne (Get-KinguWindowId $Process)) {
        throw "windowNotFound(`"$WindowId`")"
    }
}

function Restore-KinguWindow($Process) {
    if ($Process.MainWindowHandle -eq 0) { return }
    [void][KinguDesktopWin32]::ShowWindow([IntPtr]$Process.MainWindowHandle, 9)
    [void][KinguDesktopWin32]::SetForegroundWindow([IntPtr]$Process.MainWindowHandle)
}

function Test-KinguWindowFocused([IntPtr]$WindowHandle) {
    [KinguDesktopWin32]::GetForegroundWindow() -eq $WindowHandle
}

function Wait-KinguWindowFocused([IntPtr]$WindowHandle, [int]$TimeoutMilliseconds) {
    $stopwatch = [Diagnostics.Stopwatch]::StartNew()
    while ($stopwatch.ElapsedMilliseconds -lt $TimeoutMilliseconds) {
        if (Test-KinguWindowFocused $WindowHandle) { return $true }
        Start-Sleep -Milliseconds 50
    }
    Test-KinguWindowFocused $WindowHandle
}

function Assert-KinguKeyboardFocus([IntPtr]$WindowHandle, $Operation) {
    if (Test-KinguWindowFocused $WindowHandle) { return }
    if ([bool]$Operation.restoreWindow) {
        if (Wait-KinguWindowFocused $WindowHandle 500) { return }
...
function Get-KinguElementFrame($Element, $WindowFrame) {
    try {
        ...
        New-KinguFrame ($bounds.X - $WindowFrame.x) ($bounds.Y - $WindowFrame.y) $bounds.Width $bounds.Height
    } catch {
        $null
    }
}

function Get-KinguProperty($Element, [string]$Name) {
    try { [string]$Element.Current.$Name } catch { "" }
}

function Get-KinguRuntimeId($Element) {
    try { @($Element.GetRuntimeId()) } catch { @() }
}

function Test-KinguSensitiveElement($Element) {
    try {
        if ($Element.Current.IsPassword) { return $true }
    } catch {}
    $controlType = try { [string]$Element.Current.ControlType.ProgrammaticName } catch { "" }
    $parts = @(
        (Get-KinguProperty $Element "LocalizedControlType"),
        $controlType,
        (Get-KinguProperty $Element "Name"),
        (Get-KinguProperty $Element "AutomationId"),
        (Get-KinguProperty $Element "ClassName")
    )
...
function Get-KinguValueText($Element) {
    try {
        if (Test-KinguSensitiveElement $Element) { return "[redacted]" }
...
function Get-KinguActions($Element) {
...
function Get-KinguMeaningfulActions($Actions) {
    $noisy = @("Invoke", "ScrollToVisible", "ShowMenu")
    @($Actions | Where-Object { $noisy -notcontains $_ })
}

function Format-KinguSnapshotText([string]$Text) {
    if ([string]::IsNullOrWhiteSpace($Text)) { return "" }
    (($Text -replace "\s+", " ").Trim())
}

function Format-KinguValueSegment([string]$RoleKey, [string]$Title, [string]$Value) {
    $clean = Format-KinguSnapshotText $Value
...
function Test-KinguSuppressChildren([string]$RoleKey, [string]$Title, [string]$Value, [string]$Summary) {
    $hasCompactLabel = -not [string]::IsNullOrWhiteSpace($Title) -or -not [string]::IsNullOrWhiteSpace((Format-KinguSnapshotText $Value)) -or -not [string]::IsNullOrWhiteSpace((Format-KinguSnapshotText $Summary))
...
function Get-KinguTextSnippets($Element, [int]$Limit = 6, [int]$MaxDepth = 3) {
    $values = New-Object System.Collections.Generic.List[string]
    $seen = New-Object System.Collections.Generic.HashSet[string]

    function Visit-KinguText($Node, [int]$Depth) {
        ...
        if ($role -match "text|link|label") {
            foreach ($raw in @((Get-KinguProperty $Node "Name"), (Get-KinguValueText $Node))) {
        ...
        try {
            ...
            for ($i = 0; $i -lt $children.Count; $i++) {
                Visit-KinguText $children.Item($i) ($Depth + 1)
                if ($values.Count -ge $Limit) { return }
            }
        } catch {}
    }

    Visit-KinguText $Element 0
    @($values.ToArray())
}

function Test-KinguPlainTextSubtree($Element, [int]$MaxDepth = 4) {
    $script:sawKinguText = $false
    $allowed = @("pane", "group", "custom", "unknown", "text", "link", "image")

    function Visit-KinguPlainText($Node, [int]$Depth) {
        if ($Depth -gt $MaxDepth) { return $false }
        $role = try { [string]$Node.Current.LocalizedControlType } catch { "" }
        $roleKey = $role.ToLowerInvariant()
        if ($allowed -notcontains $roleKey) { return $false }
        if ($roleKey -match "text|link") { $script:sawKinguText = $true }
        if (@(Get-KinguMeaningfulActions @(Get-KinguActions $Node)).Count -gt 0) { return $false }
        try {
            $children = $Node.FindAll([Windows.Automation.TreeScope]::Children, [Windows.Automation.Condition]::TrueCondition)
            for ($i = 0; $i -lt $children.Count; $i++) {
                if (-not (Visit-KinguPlainText $children.Item($i) ($Depth + 1))) { return $false }
            }
        } catch {}
        return $true
    }

    (Visit-KinguPlainText $Element 0) -and $script:sawKinguText
}

function New-KinguElementRecord($Element, [int]$Index, $WindowFrame) {
    $controlType = try { [string]$Element.Current.ControlType.ProgrammaticName } catch { "" }
    $nativeWindowHandle = try { [int64]$Element.Current.NativeWindowHandle } catch { 0 }
    [pscustomobject]@{
        index = $Index
        runtimeId = @(Get-KinguRuntimeId $Element)
        automationId = Get-KinguProperty $Element "AutomationId"
        name = Get-KinguProperty $Element "Name"
        controlType = $controlType
        localizedControlType = Get-KinguProperty $Element "LocalizedControlType"
        className = Get-KinguProperty $Element "ClassName"
        value = Get-KinguValueText $Element
        isSelected = Test-KinguElementSelected $Element
        nativeWindowHandle = $nativeWindowHandle
        frame = Get-KinguElementFrame $Element $WindowFrame
        actions = @(Get-KinguActions $Element)
    }
}

function Test-KinguElementSelected($Element) {
    try {
...
function Render-KinguTree($RootElement, $WindowFrame, [bool]$CompactBrowserTabs = $false) {
    ...
    function Visit-KinguNode($Node, [int]$Depth) {
        ...
        $record = New-KinguElementRecord $Node $records.Count $WindowFrame
        $children = @()
        try {
            $children = @($Node.FindAll([Windows.Automation.TreeScope]::Children, [Windows.Automation.Condition]::TrueCondition))
        } catch {}
        $meaningfulActions = @(Get-KinguMeaningfulActions $record.actions)
        ...
        if (($roleKey -in @("pane", "group", "custom", "unknown")) -and [string]::IsNullOrWhiteSpace($title) -and [string]::IsNullOrWhiteSpace($record.value)) {
            $snippets = @(Get-KinguTextSnippets $Node 8 4)
            if ($snippets.Count -ge 2 -and (Test-KinguPlainTextSubtree $Node)) {
        ...
        if (($roleKey -in @("pane", "group", "custom", "unknown")) -and [string]::IsNullOrWhiteSpace($title) -and [string]::IsNullOrWhiteSpace($record.value) -and $meaningfulActions.Count -eq 0 -and $null -eq $genericSummary -and $children.Count -le 1) {
            for ($i = 0; $i -lt $children.Count; $i++) {
                Visit-KinguNode $children.Item($i) $Depth
            }
            return
        }
        ...
        $line = "$($record.index) $role $(Format-KinguSnapshotText $title)".TrimEnd()
        $line += Format-KinguValueSegment $roleKey $title $record.value
        if (-not [string]::IsNullOrWhiteSpace($genericSummary) -and $genericSummary -ne $title) {
            $line += ", Text: " + (Format-KinguSnapshotText $genericSummary)
        } elseif ($roleKey -in @("row", "data item", "list item")) {
            $rowSummary = @((Get-KinguTextSnippets $Node 6 3)) -join " "
            if (-not [string]::IsNullOrWhiteSpace($rowSummary) -and $rowSummary -ne $title) {
                $line += ", Text: " + (Format-KinguSnapshotText $rowSummary)
            }
        }
        ...
        if (-not [string]::IsNullOrWhiteSpace($genericSummary) -or (Test-KinguSuppressChildren $roleKey $title $record.value $genericSummary)) { return }
        $childLineStart = $lines.Count
        for ($i = 0; $i -lt $children.Count; $i++) {
            Visit-KinguNode $children.Item($i) ($Depth + 1)
        }
        if ($CompactBrowserTabs) {
            Compress-KinguRenderedBrowserTabs $records $lines $childLineStart ($Depth + 1)
        }
    }

    Visit-KinguNode $RootElement 0
    [pscustomobject]@{ elements = @($records.ToArray()); lines = @($lines.ToArray()); truncation = $truncation }
}

function Compress-KinguRenderedBrowserTabs($Records, $Lines, [int]$StartLine, [int]$Depth) {
    $tabLineIndexes = New-Object System.Collections.Generic.List[int]
    for ($lineIndex = $StartLine; $lineIndex -lt $Lines.Count; $lineIndex++) {
        if (Test-KinguDirectRenderedBrowserTabLine ([string]$Lines[$lineIndex]) $Depth) {
    ...
    foreach ($lineIndex in $tabLineIndexes) {
        if (Test-KinguActiveRenderedBrowserTabLine ([string]$Lines[$lineIndex]) $Depth $recordsByIndex) {
    ...
    for ($i = $tabLineIndexes.Count - 1; $i -ge 0; $i--) {
        ...
        $recordIndex = Get-KinguRenderedElementIndex ([string]$Lines[$lineIndex]) $Depth
...
function Test-KinguDirectRenderedBrowserTabLine([string]$Line, [int]$Depth) {
    $indent = "`t" * $Depth
...
function Test-KinguActiveRenderedBrowserTabLine([string]$Line, [int]$Depth, $RecordsByIndex) {
    if ($Line.Contains("(selected")) { return $true }
    $recordIndex = Get-KinguRenderedElementIndex $Line $Depth
    ...
    [bool]$record.isSelected -or (Format-KinguSnapshotText $record.value) -eq "1"
}

function Get-KinguRenderedElementIndex([string]$Line, [int]$Depth) {
    $text = $Line.Substring(("`t" * $Depth).Length)
    if ($text -match "^(\d+)") { return [int]$Matches[1] }
    $null
}

function ConvertTo-KinguPngBytes([System.Drawing.Image]$Image) {
    $stream = $null
    try {
...
function New-KinguScreenshotPayload([byte[]]$Bytes, [int]$Width, [int]$Height, [double]$Scale) {
    [pscustomobject]@{
...
function Resize-KinguBitmap([System.Drawing.Bitmap]$Source, [int]$Width, [int]$Height) {
    $resized = $null
    $graphics = $null
    try {
...
function Get-KinguBoundedScreenshotPayload([System.Drawing.Bitmap]$Bitmap) {
    $originalWidth = [int][Math]::Max(1, $Bitmap.Width)
    $originalHeight = [int][Math]::Max(1, $Bitmap.Height)
    $pngBytes = ConvertTo-KinguPngBytes $Bitmap
    if ($pngBytes.Length -le $MaxScreenshotPngBytes) {
        return New-KinguScreenshotPayload $pngBytes $originalWidth $originalHeight 1.0
    }
    ...
    while ($scale -ge $MinScreenshotScale) {
        ...
        $resized = $null
        try {
            $resized = Resize-KinguBitmap $Bitmap $width $height
            $candidateBytes = ConvertTo-KinguPngBytes $resized
            if ($candidateBytes.Length -le $MaxScreenshotPngBytes) {
                return New-KinguScreenshotPayload $candidateBytes $width $height ($width / [double]$originalWidth)
            }
        } finally {
            if ($null -ne $resized) { $resized.Dispose() }
        }
...
function Get-KinguScreenshot([bool]$IncludeScreenshot, $WindowFrame) {
    ...
    try {
        ...
        Get-KinguBoundedScreenshotPayload $bitmap
    } catch {
        $null
    } finally {
        if ($null -ne $graphics) { $graphics.Dispose() }
...
function New-KinguSnapshot([string]$Query, [bool]$IncludeScreenshot, $WindowId = $null, $WindowIndex = $null, [bool]$RestoreWindow = $false) {
    $process = Find-KinguProcess $Query
    if ($RestoreWindow) { Restore-KinguWindow $process }
    Assert-KinguWindowTarget $process $WindowId $WindowIndex
    $root = Get-KinguRootElement $process
    $windowFrame = Get-KinguWindowFrame $process $root
    $tree = Render-KinguTree $root $windowFrame (Test-KinguBrowserProcess $process)
    $screenshot = Get-KinguScreenshot $IncludeScreenshot $windowFrame

    [pscustomobject]@{
        snapshotId = [guid]::NewGuid().ToString()
        app = New-KinguAppRecord $process
        windowTitle = $process.MainWindowTitle
        windowId = Get-KinguWindowId $process
...
function Get-KinguAppList {
    @(Get-KinguWindowProcesses | ForEach-Object {
        New-KinguAppRecord $_
    })
}

function Get-KinguWindowList([string]$Query) {
    $process = Find-KinguProcess $Query
    $root = Get-KinguRootElement $process
    $windowFrame = Get-KinguWindowFrame $process $root
    $x = $null
    ...
    [pscustomobject]@{
        ...
        windows = @([pscustomobject]@{
            ...
            isOffscreen = $false
            screenIndex = $null
            platform = [pscustomobject]@{ backend = "uia"; nativeWindowHandle = Get-KinguWindowId $process }
        })
    }
}

function Get-KinguHandshake {
    [pscustomobject]@{
        platform = "win32"
        provider = "kingu-computer-use-windows"
...
function Test-KinguSameRuntimeId($Left, $Right) {
...
function Find-KinguElement($RootElement, $Record) {
    ...
    try {
        ...
        for ($i = 0; $i -lt $descendants.Count; $i++) {
            ...
            if (Test-KinguSameRuntimeId @($candidate.GetRuntimeId()) @($Record.runtimeId)) {
                return $candidate
            }
        }
    } catch {}
...
function Invoke-KinguPrimaryAction($Element) {
    foreach ($pattern in @(
...
function Invoke-KinguNamedAction($Element, [string]$Action) {
    $wanted = ""
...
function Set-KinguElementValue($Element, [string]$Value) {
    try {
        $pattern = $Element.GetCurrentPattern([Windows.Automation.ValuePattern]::Pattern)
...
function Get-KinguRequiredNumber($Value, [string]$Name) {
...
function Get-KinguPositiveInteger($Value, [string]$Name) {
...
function Get-KinguPositiveNumber($Value, [string]$Name) {
    if ($null -eq $Value) { $Value = 1 }
    $number = Get-KinguRequiredNumber $Value $Name
    if ($number -le 0) { throw "$Name must be a positive number" }
    $number
}

function Get-KinguRequiredString($Value, [string]$Name) {
    if ($null -eq $Value) { throw "$Name is required" }
    $text = [string]$Value
...
function Get-KinguScreenPoint($Operation, $WindowFrame) {
    if ($null -ne $Operation.element) {
        throw "stale element frame; run get-app-state again and use a fresh element index"
    }
    $x = Get-KinguRequiredNumber $Operation.x "x"
    $y = Get-KinguRequiredNumber $Operation.y "y"
    @{
...
function Get-KinguElementScreenPoint($Element) {
...
function Send-KinguMouseClick([IntPtr]$WindowHandle, [int]$ScreenX, [int]$ScreenY, [string]$Button, [int]$Count, [string]$Modifiers) {
    [void][KinguDesktopWin32]::SetForegroundWindow($WindowHandle)
    [void][KinguDesktopWin32]::SetCursorPos($ScreenX, $ScreenY)
    ...
    $modifierKeys = @(Get-KinguClickModifierVirtualKeys $Modifiers)
    $clickCount = Get-KinguPositiveInteger $Count "click_count"
    if ($modifierKeys.Count -eq 0) {
        for ($i = 0; $i -lt $clickCount; $i++) {
            [KinguDesktopWin32]::mouse_event($down, 0, 0, 0, [UIntPtr]::Zero)
            Start-Sleep -Milliseconds 35
            [KinguDesktopWin32]::mouse_event($up, 0, 0, 0, [UIntPtr]::Zero)
        }
        return
    }
    for ($i = 0; $i -lt $clickCount; $i++) {
        [KinguDesktopWin32]::SendModifiedClick(
            [byte[]]$modifierKeys,
            [uint32]$down,
            [uint32]$up
        )
...
function Send-KinguDrag([IntPtr]$WindowHandle, $From, $To) {
    [void][KinguDesktopWin32]::SetForegroundWindow($WindowHandle)
    $startX = [int]$From.x
    $startY = [int]$From.y
    $endX = [int]$To.x
    $endY = [int]$To.y
    [void][KinguDesktopWin32]::SetCursorPos($startX, $startY)
    [KinguDesktopWin32]::mouse_event($MouseEvents.LeftDown, 0, 0, 0, [UIntPtr]::Zero)
    for ($step = 1; $step -le 12; $step++) {
        $x = [int][Math]::Round($startX + (($endX - $startX) * $step / 12))
        $y = [int][Math]::Round($startY + (($endY - $startY) * $step / 12))
        [void][KinguDesktopWin32]::SetCursorPos($x, $y)
        Start-Sleep -Milliseconds 20
    }
    [KinguDesktopWin32]::mouse_event($MouseEvents.LeftUp, 0, 0, 0, [UIntPtr]::Zero)
}

function Send-KinguText([IntPtr]$WindowHandle, [string]$Text) {
    [void][KinguDesktopWin32]::SetForegroundWindow($WindowHandle)
    $hasNonAscii = $false
    ...
    if ($hasNonAscii) {
        foreach ($character in $Text.ToCharArray()) {
            [void][KinguDesktopWin32]::PostMessage($WindowHandle, $WindowsMessages.Char, [IntPtr][int][char]$character, [IntPtr]::Zero)
            Start-Sleep -Milliseconds 8
        }
        return
    }
    [System.Windows.Forms.SendKeys]::SendWait((ConvertTo-KinguSendKeysText $Text))
}

function Get-KinguVirtualKey([string]$Key) {
...
function Send-KinguKey([IntPtr]$WindowHandle, [string]$Key) {
    [void][KinguDesktopWin32]::SetForegroundWindow($WindowHandle)
    [System.Windows.Forms.SendKeys]::SendWait((ConvertTo-KinguSendKeysKey $Key))
}

function Get-KinguModifierVirtualKey([string]$Modifier) {
...
function Get-KinguClickModifierVirtualKeys([string]$Modifiers) {
    ...
    @($parts | ForEach-Object { Get-KinguModifierVirtualKey $_ })
}

function Send-KinguHotkey([IntPtr]$WindowHandle, [string]$KeySpec) {
    ...
    if ($parts.Count -gt 1) {
        foreach ($modifier in $parts[0..($parts.Count - 2)]) {
            $prefix += ConvertTo-KinguSendKeysModifier $modifier
        }
    }
    [void][KinguDesktopWin32]::SetForegroundWindow($WindowHandle)
    [System.Windows.Forms.SendKeys]::SendWait($prefix + (ConvertTo-KinguSendKeysKey $key))
}

function ConvertTo-KinguSendKeysText([string]$Text) {
...
function ConvertTo-KinguSendKeysKey([string]$Key) {
    switch ($Key.ToLowerInvariant()) {
        ...
        default {
            if ($Key.Length -eq 1) { return (ConvertTo-KinguSendKeysText $Key) }
            throw "Unsupported key: $Key"
        }
    }
}

function ConvertTo-KinguSendKeysModifier([string]$Modifier) {
    switch ($Modifier.ToLowerInvariant()) {
...
function Send-KinguPasteText([IntPtr]$WindowHandle, [string]$Text) {
    $previous = $null
    $hadPrevious = $false
    ...
    try {
        ...
        Send-KinguHotkey $WindowHandle "Ctrl+v"
    } finally {
        if ($hadPrevious) {
...
function Invoke-KinguOperation($Operation) {
    $includeScreenshot = -not [bool]$Operation.noScreenshot
    if ($Operation.tool -eq "handshake") {
        return [pscustomobject]@{ ok = $true; capabilities = Get-KinguHandshake }
    }
    if ($Operation.tool -eq "list_apps") {
        return [pscustomobject]@{ ok = $true; apps = @(Get-KinguAppList) }
    }
    if ($Operation.tool -eq "list_windows") {
        $list = Get-KinguWindowList $Operation.app
        return [pscustomobject]@{ ok = $true; app = $list.app; windows = @($list.windows) }
    }
    if ($Operation.tool -eq "get_app_state") {
        return [pscustomobject]@{ ok = $true; snapshot = New-KinguSnapshot $Operation.app $includeScreenshot $Operation.windowId $Operation.windowIndex ([bool]$Operation.restoreWindow) }
    }

    $process = Find-KinguProcess $Operation.app
    if ([bool]$Operation.restoreWindow) { Restore-KinguWindow $process }
    Assert-KinguWindowTarget $process $Operation.windowId $Operation.windowIndex
    $root = Get-KinguRootElement $process
    $windowFrame = if ($null -ne $Operation.windowBounds) { $Operation.windowBounds } else { Get-KinguWindowFrame $process $root }
    $element = Find-KinguElement $root $Operation.element
    $fromElement = Find-KinguElement $root $Operation.fromElement
    $toElement = Find-KinguElement $root $Operation.toElement
    $handle = [IntPtr]$process.MainWindowHandle
    if ($Operation.tool -in @("type_text", "press_key", "hotkey", "paste_text")) {
        Assert-KinguKeyboardFocus $handle $Operation
    }
    $action = $null
    ...
    switch ($Operation.tool) {
        "click" {
            ...
            Restore-KinguWindow $process
            $handledByPattern = $false
            $clickCount = Get-KinguPositiveInteger $Operation.click_count "click_count"
            ...
            if (-not $hasModifiers -and $null -ne $element -and $Operation.mouse_button -ne "right" -and $Operation.mouse_button -ne "middle" -and $clickCount -le 1) {
                $handledByPattern = Invoke-KinguPrimaryAction $element
            }
            if (-not $handledByPattern) {
                $point = Get-KinguElementScreenPoint $element
                if ($null -eq $point) { $point = Get-KinguScreenPoint $Operation $windowFrame }
                Send-KinguMouseClick $handle $point.x $point.y $Operation.mouse_button $clickCount $Operation.modifiers
        ...
        "perform_secondary_action" {
            ...
            if (-not (Invoke-KinguNamedAction $element $Operation.action)) {
        ...
        "scroll" {
            $delta = 120 * [int][Math]::Ceiling((Get-KinguPositiveNumber $Operation.pages "pages"))
            ...
            $point = Get-KinguElementScreenPoint $element
            if ($null -eq $point) { $point = Get-KinguScreenPoint $Operation $windowFrame }
            [void][KinguDesktopWin32]::SetForegroundWindow($handle)
            [void][KinguDesktopWin32]::SetCursorPos([int]$point.x, [int]$point.y)
        ...
        "drag" {
            $from = Get-KinguElementScreenPoint $fromElement
            ...
            if ($null -eq $from) {
                $from = @{
                    x = $windowFrame.x + (Get-KinguRequiredNumber $Operation.from_x "from_x")
                    y = $windowFrame.y + (Get-KinguRequiredNumber $Operation.from_y "from_y")
                }
            }
            $to = Get-KinguElementScreenPoint $toElement
            ...
            if ($null -eq $to) {
                $to = @{
                    x = $windowFrame.x + (Get-KinguRequiredNumber $Operation.to_x "to_x")
                    y = $windowFrame.y + (Get-KinguRequiredNumber $Operation.to_y "to_y")
                }
            }
            Send-KinguDrag $handle $from $to
            $action = [pscustomobject]@{ path = "synthetic"; actionName = "drag"; fallbackReason = $null }
        }
        "type_text" {
            Send-KinguText $handle (Get-KinguRequiredString $Operation.text "text")
        ...
        "press_key" {
            Send-KinguKey $handle (Get-KinguRequiredString $Operation.key "key")
        ...
        "hotkey" {
            Send-KinguHotkey $handle (Get-KinguRequiredString $Operation.key "key")
        ...
        "paste_text" {
            Send-KinguPasteText $handle (Get-KinguRequiredString $Operation.text "text")
        ...
        "set_value" {
            if ($null -eq $element -or -not (Set-KinguElementValue $element ([string]$Operation.value))) {
    ...
    try {
        $snapshot = New-KinguSnapshot $Operation.app $includeScreenshot $Operation.windowId $Operation.windowIndex
    } catch {
        ...
        $snapshot = New-KinguSnapshot $Operation.app $includeScreenshot $null $null
    }
    [pscustomobject]@{ ok = $true; action = $action; snapshot = $snapshot }
}

function Invoke-KinguServeLoop {
    ...
    while ($true) {
        ...
        try {
            ...
            $response = Invoke-KinguOperation $operation
        } catch {
...
if ($Serve) {
    Invoke-KinguServeLoop
} elseif ([string]::IsNullOrWhiteSpace($OperationPath)) {
    Write-KinguJson ([pscustomobject]@{ ok = $false; error = "runtime.ps1 requires an operation path or -Serve" })
} else {
    try {
        $operation = Read-KinguOperation $OperationPath
        Write-KinguJson (Invoke-KinguOperation $operation)
    } catch {
        Write-KinguJson ([pscustomobject]@{ ok = $false; error = [string]$_.Exception.Message })
    }
}
