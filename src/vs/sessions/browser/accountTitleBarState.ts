...
function getCopilotPresentation(
	...
	if (sentiment.disabled || sentiment.untrusted) {
		return {
			...
			kind: 'warning',
			icon: Codicon.account,
			label: localize('copilotUnavailable', "Arkai Unavailable"),
			ariaLabel: sentiment.untrusted
				? localize('copilotUnavailableUntrustedAria', "Arkai is unavailable in untrusted workspaces")
				: localize('copilotUnavailableDisabledAria', "Arkai is disabled"),
		};
	}
	...
	if (entitlement === ChatEntitlement.Free && typeof remainingPercent === 'number' && remainingPercent <= 25) {
		return {
			...
			ariaLabel: localize('copilotTokensRemainingAria', "{0}% Arkai tokens remaining", remainingPercent),
		};
	}
...
function getQuotaReachedAriaLabel(chatQuotaExceeded: boolean, completionsQuotaExceeded: boolean): string {
	if (chatQuotaExceeded && completionsQuotaExceeded) {
		return localize('copilotAllQuotaReachedAria', "Arkai chat and inline suggestion quota reached");
	}

	if (chatQuotaExceeded) {
		return localize('copilotChatQuotaReachedAria', "Arkai chat quota reached");
	}

	return localize('copilotCompletionsQuotaReachedAria', "Arkai inline suggestion quota reached");
}
