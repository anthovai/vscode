...
- `computer` (34 files) — computer use. The reading half is now here; see
  *Reading the desktop*. The acting half is not, deliberately.
...
That script can also click, type, paste and press keys. This host cannot ask it
to. `KINGU_COMPUTER_READ_TOOLS` is four names, checked in TypeScript before
anything is written to the process, and the test that asserts the acting tools
are refused is there to keep it that way. Shipping a capable script behind a
closed door is deliberate: trimming the script would be the more fragile of the
two ways to be safe.
